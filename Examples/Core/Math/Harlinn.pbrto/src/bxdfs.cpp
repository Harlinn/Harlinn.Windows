...
#include <pbrto/bssrdf.h>
#include <pbrto/interaction.h>
#include <pbrto/media.h>
...
#undef ASSERT
    ...
    // BxDF Method Definitions
#ifdef PBRT_USES_HCCMATH_SQRT
    PBRT_CPU_GPU SampledSpectrum BxDF::rho( const Vector3f& wo, const pstd::span<const Float>& uc, const pstd::span<const Point2f>& u2 ) const
#else
    PBRT_CPU_GPU SampledSpectrum BxDF::rho( Vector3f wo, pstd::span<const Float> uc, pstd::span<const Point2f> u2 ) const
#endif
    {
        if ( wo.z == 0 )
            return {};
...
#ifdef PBRT_USES_HCCMATH_SQRT
    SampledSpectrum BxDF::rho( const pstd::span<const Point2f>& u1, const pstd::span<const Float>& uc, const pstd::span<const Point2f>& u2 ) const
#else
