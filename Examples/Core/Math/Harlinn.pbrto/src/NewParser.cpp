...
namespace pbrto
{
    ...
    {
        NCHECK( ints.empty( ) && strings.empty( ) && bools.empty( ) );
        floats.push_back( v );
    }

    void ParsedParameter::AddInt( int i )
    {
        NCHECK( floats.empty( ) && strings.empty( ) && bools.empty( ) );
        ints.push_back( i );
    }

    void ParsedParameter::AddString( std::string_view str )
    {
        NCHECK( floats.empty( ) && ints.empty( ) && bools.empty( ) );
        strings.push_back( { str.begin( ), str.end( ) } );
    }

    void ParsedParameter::AddBool( bool v )
    {
        NCHECK( floats.empty( ) && ints.empty( ) && strings.empty( ) );
        bools.push_back( v );
    }
    ...
    pstdo::optional<Token> Tokenizer::Next( )
    {
        while ( true )
        {
    ...
    static double parseFloat( const Token& t )
    {
        ...
        NCHECK_RARE( 1e-5, t.token.size( ) + 1 >= sizeof( buf ) );
        ...
        const std::function<void( const Token& token, const char* )>& errorCallback )
    {
        ...
        while ( true )
        {
            pstdo::optional<Token> t = nextToken( TokenOptional );
            if ( !t.has_value( ) )
    ...
    void parse( ParserTarget* target, std::unique_ptr<Tokenizer> t )
    {
        ...
        std::function<pstdo::optional<Token>( int )> nextToken;
        nextToken = [ & ]( int flags ) -> pstdo::optional<Token> {
            ...
            pstdo::optional<Token> tok = fileStack.back( )->Next( );
            ...
            switch ( tok->token[ 0 ] )
            {
                ...
                case 'M':
                    ...
                    else if ( tok->token == "MediumInterface" )
                    {
                        ...
                        pstdo::optional<Token> second = nextToken( TokenOptional );
                        if ( second.has_value( ) )
                        {
