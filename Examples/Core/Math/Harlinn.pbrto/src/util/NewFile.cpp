...
namespace pbrto
{
    ...
    PBRTO_EXPORT std::vector<std::string> MatchingFilenames( std::string filenameBase )
    {
        ...
        NLOG_FATAL( "Need Windows implementation of MatchingFilenames()" );
#else
        ...
        while ( true )
        {
            ...
            switch ( result )
            {
                ...
                case LIBDEFLATE_SHORT_OUTPUT:
                    ...
                    NLOG_FATAL( "Unexpected return value from libdeflate" );
            }
        }
    }
        ...
        while ( ( c = getc( f ) ) != EOF )
        {
            ...
            if ( inNumber )
            {
                if ( curNumberPos >= ( int )sizeof( curNumber ) )
                    NLOG_FATAL( "Overflowed buffer for parsing number in file: %s at "
                        "line %d",
                        filename, lineNumber );
                ...
                if ( ( isdigit( c ) != 0 ) || c == '.' || c == 'e' || c == 'E' || c == '-' ||
                    c == '+' )
                {
                    NCHECK_LT( curNumberPos, sizeof( curNumber ) );
