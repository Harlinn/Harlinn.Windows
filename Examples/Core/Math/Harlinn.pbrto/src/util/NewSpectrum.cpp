...
namespace pbrto
{
    ...
    {
        NCHECK_EQ( lambdas.size( ), values.size( ) );
        for ( size_t i = 0; i < lambdas.size( ) - 1; ++i )
            NCHECK_LT( lambdas[ i ], lambdas[ i + 1 ] );
    }
    ...
    PiecewiseLinearSpectrum* PiecewiseLinearSpectrum::FromInterleaved(
        pstdo::span<const Float> samples, bool normalize, Allocator alloc )
    {
        NCHECK_EQ( 0, samples.size( ) % 2 );
        ...
        for ( size_t i = 0; i < n; ++i )
        {
            ...
            if ( i > 0 )
                NCHECK_GT( lambda.back( ), lambda[ lambda.size( ) - 2 ] );
        }
        ...
        PiecewiseLinearSpectrum* spec =
            alloc.new_object<pbrto::PiecewiseLinearSpectrum>( lambda, v, alloc );
    ...
    PBRTO_EXPORT Float SampledSpectrum::Y( const SampledSpectrum& l, const SampledWavelengths& lambda )
    {
        SampledSpectrum Ys = Spectra::Y( ).Sample( lambda );
        SampledSpectrum pdf = lambda.PDF( );
    ...
    namespace Spectra
    {
        PBRTO_EXPORT DenselySampledSpectrum D( Float temperature, Allocator alloc )
        {
            ...
            pstdo::vector<Float> values( nCIES );
