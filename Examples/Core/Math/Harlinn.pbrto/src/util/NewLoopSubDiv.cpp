    ...
    struct SDFace
    {
        ...
        int vnum( SDVertex* vert ) const
        {
            for ( int i = 0; i < 3; ++i )
                if ( v[ i ] == vert )
                    return i;
            NLOG_FATAL( "Basic logic error in SDFace::vnum()" );
            return -1;
        }
        ...
        SDVertex* otherVert( SDVertex* v0, SDVertex* v1 )
        {
            for ( int i = 0; i < 3; ++i )
                if ( v[ i ] != v0 && v[ i ] != v1 )
                    return v[ i ];
            NLOG_FATAL( "Basic logic error in SDFace::otherVert()" );
            return nullptr;
        }
        SDVertex* v[ 3 ];
        SDFace* f[ 3 ];
        ...
        for ( SDVertex* vertex : v )
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    T = Vector3f( sinTheta * ( pRing[ 0 ] + pRing[ valence - 1 ] ) );
                    for ( int k = 1; k < valence - 1; ++k )
                    {
                        Float wt = ( 2 * cosTheta - 2 ) * Math::Sin( ( k )*theta );
