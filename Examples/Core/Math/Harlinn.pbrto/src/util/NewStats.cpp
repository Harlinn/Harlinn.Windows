...
namespace pbrto
{
        ...
        void StatsReportPixelStart( Point2i p )
    {
        if ( !pixelStatsEnabled )
            return;
        NCHECK( threadStatsState.active == false );
        ...
        void StatsReportPixelEnd( Point2i p )
    {
        ...
        NCHECK( tss.active == true && tss.p == p );
        tss.active = false;
    ...
    void StatsAccumulator::AccumulatePixelStats( const PixelStatsAccumulator& accum )
    {
        ...
        if ( stats->pixelTime.Resolution( ) == Point2i( 0, 0 ) )
            ...
            NCHECK_EQ( stats->pixelTime.Resolution( ), res );
        NCHECK_EQ( stats->pixelTime.Resolution( ), accum.stats->time.Resolution( ) );
    ...
    void StatsAccumulator::WritePixelImages( ) const
    {
        ...
        for ( size_t i = 0; i < stats->pixelCounterImages.size( ); ++i )
        {
            ...
            if ( !AllZero( stats->pixelCounterImages[ i ] ) )
                NCHECK( stats->pixelCounterImages[ i ].Write( n ) );
        }
        ...
        for ( size_t i = 0; i < stats->pixelRatioImages.size( ); ++i )
        {
            ...
            if ( !AllZero( stats->pixelRatioImages[ i ] ) )
                NCHECK( stats->pixelRatioImages[ i ].Write( n ) );
        }
    }
