...
namespace pbrto
{
    ...
    NSTAT_RATIO( "Geometry/Triangles per mesh", nTris, nTriMeshes );
        ...
        : nTriangles( indices.size( ) / 3 ), nVertices( p.size( ) )
    {
        ...
        if ( !uv.empty( ) )
        {
            NCHECK_EQ( nVertices, uv.size( ) );
            this->uv = point2BufferCache->LookupOrAdd( uv, alloc );
        }
        if ( !n.empty( ) )
        {
            NCHECK_EQ( nVertices, n.size( ) );
            for ( Normal3f& nn : n )
            {
        ...
        {
            NCHECK_EQ( nTriangles, faceIndices.size( ) );
        ...
        // index into things.
        NCHECK_LE( p.size( ), std::numeric_limits<int>::max( ) );
        ...
        NCHECK_LE( indices.size( ), std::numeric_limits<int>::max( ) );
    }
    ...
    bool TriangleMesh::WritePLY( std::string filename ) const
    {
        ...
        return pbrto::WritePLY(
            ...
            pstdo::span<const int>( ), pstdo::span<const Point3f>( p, nVertices ),
            pstdo::span<const Normal3f>( n, n ? nVertices : 0 ),
            pstdo::span<const Point2f>( uv, uv ? nVertices : 0 ),
            pstdo::span<const int>( faceIndices, faceIndices ? nTriangles : 0 ) );
    }

    bool WritePLY( std::string filename, pstdo::span<const int> triIndices,
        pstdo::span<const int> quadIndices, pstdo::span<const Point3f> p,
        pstdo::span<const Normal3f> n, pstdo::span<const Point2f> uv,
        pstdo::span<const int> faceIndices )
    {
        p_ply plyFile =
    ...
    NSTAT_RATIO( "Geometry/Bilinear patches per mesh", nBlps, nBilinearMeshes );
        ...
        imageDistribution( imageDist )
    {
        ...
        if ( !fIndices.empty( ) )
        {
            NCHECK_EQ( nPatches, fIndices.size( ) );
