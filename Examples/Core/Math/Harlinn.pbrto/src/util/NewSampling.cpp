...
namespace pbrto
{
        ...
        Point2f u, Float* pdf )
    {
        ...
        NCHECK_GT( LengthSquared( a ), 0 );
        NCHECK_GT( LengthSquared( b ), 0 );
        NCHECK_GT( LengthSquared( c ), 0 );
        a = Normalize( a );
        b = Normalize( b );
        c = Normalize( c );
        ...
        Float cosAp_pi;
        Float sinAp_pi;
        Math::SinCos( Ap_pi, &sinAp_pi, &cosAp_pi );
        ...
        Vector3f w )
    {
        ...
        else
        {
            ...
            NCHECK_RARE( 1e-5, LengthSquared( n_cpb ) == 0 || LengthSquared( n_acp ) == 0 );
        ...
        Float alpha, Float u, Float* fval, Float* pdf )
    {
        ...
        auto interpolate = [ & ]( pstdo::span<const Float> array, int idx ) {
            Float v = 0;
            for ( int i = 0; i < 4; ++i )
                if ( weights[ i ] != 0 )
    ...
    pstdo::vector<Float> Sample1DFunction( std::function<Float( Float )> f, int nSteps,
        ...
        Allocator alloc )
    {
        pstdo::vector<Float> values( nSteps, Float( 0 ), alloc );
    ...
    AliasTable::AliasTable( pstdo::span<const Float> weights, Allocator alloc )
    ...
    std::string AliasTable::ToString( ) const
    {
        ...
        for ( const auto& b : bins )
            s += std::format( "[ Bin q: %f p: %f alias: %d ] ", b.q, b.p, b.alias );
        return s + "] ]";
    }

    std::string SummedAreaTable::ToString( ) const
    {
        return std::format( "[ SummedAreaTable sum: %s ]", sum.ToString() );
    }
