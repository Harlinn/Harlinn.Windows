...
namespace pbrto
{
    ...
    NSTAT_MEMORY_COUNTER( "Memory/Image maps", imageMapBytes );
    ...
    */
    ...
    {
        NDCHECK( level >= 0 && level < pyramid.size( ) );
    ...
    RGB MIPMap::Texel( int level, Point2i st ) const
    {
        ...
        {
            NCHECK_EQ( 1, pyramid[ level ].NChannels( ) );
    ...
    T MIPMap::Filter( Point2f st, Vector2f dst0, Vector2f dst1 ) const
    {
        if ( options.filter != FilterFunction::EWA )
        {
            ...
            else
            {
                ...
                NDCHECK( options.filter == FilterFunction::Trilinear );
                if ( iLevel == 0 )
        ...
        Float longerVecLength = ScalarLength( dst0 ), shorterVecLength = ScalarLength( dst1 );
    ...
    RGB MIPMap::Bilerp( int level, Point2f st ) const
    {
        ...
        {
            NDCHECK_EQ( 1, pyramid[ level ].NChannels( ) );
