...
namespace pbrto
{
    ...
    std::string ToString( PixelFormat format )
    {
        switch ( format )
        {
            ...
            case PixelFormat::Float:
                return "Float";
            default:
                NLOG_FATAL( "Unhandled PixelFormat in FormatName()" );
                return "";
        }
    }
    ...
    PBRT_CPU_GPU int TexelBytes( PixelFormat format )
    {
        switch ( format )
        {
            ...
            case PixelFormat::Float:
                return 4;
            default:
                NLOG_FATAL( "Unhandled PixelFormat in TexelBytes()" );
                return 0;
        }
    }
    ...
    {
        NCHECK_LT( extent.pMin.x, extent.pMax.x );
        NCHECK_LT( extent.pMin.y, extent.pMax.y );
        ...
        else
        {
            for ( int y = extent.pMin[ 1 ]; y < extent.pMax[ 1 ]; ++y )
            {
                for ( int x = 0; x < nx; ++x )
                {
                    ...
                    NCHECK( RemapPixelCoords( &p, image.Resolution( ), wrapMode ) );
    ...
    {
        NCHECK_GE( newRes.x, resolution.x );
        NCHECK_GE( newRes.y, resolution.y );
        ...
        ParallelFor2D( Bounds2i( { 0, 0 }, newRes ), [ & ]( Bounds2i outExtent ) {
            ...
            CopyRectOut( inExtent, pstdo::span<float>( inBuf ), wrapMode );
            ...
            for ( int yOut = inExtent.pMin.y; yOut < inExtent.pMax.y; ++yOut )
            {
                for ( int xOut = outExtent.pMin.x; xOut < outExtent.pMax.x; ++xOut )
                {
                    ...
                    NDCHECK( xOut >= 0 && xOut < xWeights.size( ) );
                    ...
                    NDCHECK_GE( xIn, 0 );
                    NDCHECK_LT( xIn + 3, nxIn );
                    int yIn = yOut - inExtent.pMin.y;
                    int inOffset = NChannels( ) * ( xIn + yIn * nxIn );
                    NDCHECK_GE( inOffset, 0 );
                    NDCHECK_LT( inOffset + 3 * NChannels( ), inBuf.size( ) );
            ...
            for ( int x = 0; x < nxOut; ++x )
            {
                for ( int y = 0; y < nyOut; ++y )
                {
                    ...
                    NDCHECK( yOut >= 0 && yOut < yWeights.size( ) );
                    const ResampleWeight& rsw = yWeights[ yOut ];

                    NDCHECK_GE( rsw.firstPixel - inExtent[ 0 ][ 1 ], 0 );
                    int xBufOffset =
                        NChannels( ) * ( x + nxOut * ( rsw.firstPixel - inExtent[ 0 ][ 1 ] ) );
                    NDCHECK_GE( xBufOffset, 0 );
                    int step = NChannels( ) * nxOut;
                    NDCHECK_LT( xBufOffset + 3 * step, xBuf.size( ) );
    ...
    pstdo::vector<Image> Image::GeneratePyramid( Image image, WrapMode2D wrapMode,
        Allocator alloc )
    {
        ...
        for ( int i = 0; i < nLevels - 1; ++i )
        {
            ...
            ParallelFor( 0, nextResolution[ 1 ], [ & ]( int64_t y ) {
                ...
                pyramid[ i ].CopyRectIn(
                    ...
                    pstdo::span<const float>( image.p32.data( ) + offset, count ) );
                } );
            image = std::move( nextImage );
        }

        // Initialize top level of pyramid and return it
        NCHECK( image.resolution[ 0 ] == 1 && image.resolution[ 1 ] == 1 );
        ...
        pyramid[ nLevels - 1 ].CopyRectIn( Bounds2i( { 0, 0 }, { 1, 1 } ),
            pstdo::span<const float>( image.p32.data( ), nChannels ) );
        return pyramid;
    }
        ...
        p32( alloc )
    {
        if ( Is8Bit( format ) )
        {
            ...
            NLOG_FATAL( "Unhandled format in Image::Image()" );
    }

    ImageChannelDesc Image::GetChannelDesc(
        pstdo::span<const std::string> requestedChannels ) const
    {
    ...
    {
        NCHECK_LE( values.size( ), NChannels( ) );
        for ( size_t i = 0; i < values.size( ); ++i )
            SetChannel( p, desc.offset[ i ], values[ i ] );
    }

    Image::Image( pstdo::vector<uint8_t> p8c, Point2i resolution,
        pstdo::span<const std::string> channels, ColorEncoding encoding )
        ...
        p8( std::move( p8c ) )
    {
        NCHECK_EQ( p8.size( ), NChannels( ) * resolution[ 0 ] * resolution[ 1 ] );
    }

    Image::Image( pstdo::vector<Half> p16c, Point2i resolution,
        ...
        p16( std::move( p16c ) )
    {
        NCHECK_EQ( p16.size( ), NChannels( ) * resolution[ 0 ] * resolution[ 1 ] );
        NCHECK( Is16Bit( format ) );
    }

    Image::Image( pstdo::vector<float> p32c, Point2i resolution,
        pstdo::span<const std::string> channels )
        ...
        p32( std::move( p32c ) )
    {
        NCHECK_EQ( p32.size( ), NChannels( ) * resolution[ 0 ] * resolution[ 1 ] );
        ...
        Image* errorImage ) const
    {
        ...
        NCHECK( ( bool )refDesc );
        NCHECK_EQ( Resolution( ), ref.Resolution( ) );
    ...
    void Image::CopyRectOut( const Bounds2i& extent, pstdo::span<float> buf,
        WrapMode2D wrapMode ) const
    {
        NCHECK_GE( buf.size( ), extent.Area( ) * NChannels( ) );
    ...
    void Image::CopyRectIn( const Bounds2i& extent, pstdo::span<const float> buf )
    {
        NCHECK_GE( buf.size( ), extent.Area( ) * NChannels( ) );
    ...
    void Image::SetChannels( Point2i p, pstdo::span<const Float> values )
    {
        NCHECK_LE( values.size( ), NChannels( ) );
        for ( size_t i = 0; i < values.size( ); ++i )
    ...
    void Image::FlipY( )
    {
        for ( int y = 0; y < resolution.y / 2; ++y )
        {
            for ( int x = 0; x < resolution.x; ++x )
            {
                ...
                {
                    if ( Is8Bit( format ) )
                        pstdo::swap( p8[ o1 + c ], p8[ o2 + c ] );
                    else if ( Is16Bit( format ) )
                        pstdo::swap( p16[ o1 + c ], p16[ o2 + c ] );
                    else if ( Is32Bit( format ) )
                        pstdo::swap( p32[ o1 + c ], p32[ o2 + c ] );
                    else
                        NLOG_FATAL( "unexpected format" );
                }
            }
        }
    }
    ...
    {
        NCHECK_EQ( jointDesc.size( ), jointSigma.size( ) );
    ...
    ImageAndMetadata Image::Read( std::string name, Allocator alloc, ColorEncoding encoding )
    {
        ...
        else
        {
            ...
            if ( data )
            {
                pstdo::vector<uint8_t> pixels( data, data + x * y * n, alloc );
    ...
    bool Image::Write( std::string name, const ImageMetadata& metadata ) const
    {
        if ( metadata.pixelBounds )
            NCHECK_EQ( metadata.pixelBounds->Area( ), size_t( resolution.x ) * size_t( resolution.y ) );
        ...
        NCHECK( outImage.NChannels( ) == 1 || outImage.NChannels( ) == 3 ||
        ...
        const Imath::Box2i& dataWindow )
    {
        ...
        switch ( image.Format( ) )
        {
            ...
            case PixelFormat::Float:
                ...
                break;
            default:
                NLOG_FATAL( "Unexpected image format" );
        }
        return fb;
    }
            ...
            for ( auto iter = channels.begin( ); iter != channels.end( ); ++iter )
            {
                ...
                else
                {
                    ...
                    if ( pixelType != iter.channel( ).type )
                        NLOG_FATAL( "ReadEXR() doesn't currently support images with "
                            "multiple channel types." );
                }
                channelNames.push_back( iter.name( ) );
            }

            NCHECK( pixelType == Imf::HALF || pixelType == Imf::FLOAT );
    ...
    bool Image::WriteEXR( const std::string& name, const ImageMetadata& metadata ) const
    {
        ...
        NCHECK( Is16Bit( format ) || Is32Bit( format ) );
    ...
    {
        NCHECK_GT( bounds.Area( ), 0 );
        NCHECK( bounds.pMin.x >= 0 && bounds.pMin.y >= 0 );
    ...
    bool Image::WritePNG( const std::string& filename, const ImageMetadata& metadata ) const
    {
        ...
        switch ( NChannels( ) )
        {
            ...
            case 4:
                ...
                break;
            default:
                NLOG_FATAL( "Unexpected number of channels in WritePNG()" );
        }
...
#define BUFFER_SIZE 80
    ...
    static ImageAndMetadata ReadPFM( const std::string& filename, Allocator alloc )
    {
        pstdo::vector<float> rgb32( alloc );
        ...
        if ( hostLittleEndian ^ fileLittleEndian )
        {
            ...
            for ( unsigned int i = 0; i < nFloats; ++i )
            {
                ...
                pstdo::swap( bytes[ 0 ], bytes[ 3 ] );
                pstdo::swap( bytes[ 1 ], bytes[ 2 ] );
    ...
    static ImageAndMetadata ReadHDR( const std::string& filename, Allocator alloc )
    {
        ...
        pstdo::vector<float> pixels( data, data + size_t( x ) * size_t( y ) * size_t( n ), alloc );
        stbi_image_free( data );
    ...
    static ImageAndMetadata ReadQOI( const std::string& filename, Allocator alloc )
    {
        ...
        NCHECK( pixels != nullptr );  // qoi failure
        ...
        if ( desc.channels == 4 )
            ...
            NCHECK_EQ( 3, desc.channels );

        NCHECK( desc.colorspace == QOI_SRGB || desc.colorspace == QOI_LINEAR );
        ColorEncoding encoding =
