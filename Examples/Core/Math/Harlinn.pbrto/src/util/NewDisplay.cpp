...
namespace pbrto
{
    ...
    public:
        ...
        bool Send( pstdo::span<const uint8_t> message );
    ...
    {
        NCHECK_EQ( socketFd, INVALID_SOCKET );

        NLOG_VERBOSE( "Trying to connect to display server" );
        ...
        if ( socketFd != INVALID_SOCKET )
            NLOG_VERBOSE( "Connected to display server" );
    }
    ...
    bool IPCChannel::Send( pstdo::span<const uint8_t> message )
    {
        if ( !Connected( ) )
        {
            Connect( );
    ...
    public:
        DisplayItem(
            ...
            std::function<void( Bounds2i b, pstdo::span<pstdo::span<float>> )> getTileValues );
        ...
        int tileIndex )
    {
        ...
        if ( !ipcChannel.Send( pstdo::MakeSpan( buffer.data( ),
    ...
    bool DisplayItem::Display( IPCChannel& ipcChannel )
    {
        ...
        for ( int y = 0; y < resolution.y; y += tileSize )
            for ( int x = 0; x < resolution.x; x += tileSize, ++tileIndex )
            {
                ...
                getTileValues( b, pstdo::MakeSpan( displayValues ) );
    ...
    bool DisplayItem::SendOpenImage( IPCChannel& ipcChannel )
    {
        ...
        return ipcChannel.Send( pstdo::MakeSpan( buffer, ptr - buffer ) );
    }
    ...
    {
        auto getValues = [ = ]( Bounds2i b, pstdo::span<pstdo::span<float>> displayValues ) {
            int offset = 0;
            for ( Point2i p : b )
            {
