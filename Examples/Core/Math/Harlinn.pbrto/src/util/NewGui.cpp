...
#define GL_CHECK(call)                                                   \
    do {                                                                 \
        ...
        if (GLenum err = glGetError(); err != GL_NO_ERROR)               \
            NLOG_FATAL("GL error: %s for " #call, getGLErrorString(err)); \
    } while (0)

#define GL_CHECK_ERRORS()                                     \
    do {                                                      \
        if (GLenum err = glGetError(); err != GL_NO_ERROR)    \
            NLOG_FATAL("GL error: %s", getGLErrorString(err)); \
    } while (0)
...
namespace pbrto
{
    ...
    {
        if ( !glfwInit( ) )
            NLOG_FATAL( "Unable to initialize GLFW" );
    }
        ...
        : resolution( resolution )
    {
        ...
        {
            glfwTerminate( );
            NLOG_FATAL( "Unable to create GLFW window" );
        }
        ...
        if ( !gladLoadGLLoader( ( GLADloadproc )glfwGetProcAddress ) )
            NLOG_FATAL( "gladLoadGLLoader failed" );
