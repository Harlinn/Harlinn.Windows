...
#define VERIFY_WORLD(func)                                         \
    ...
    NSTAT_COUNTER( "Scene/Object instances created", nObjectInstancesCreated );
    NSTAT_COUNTER( "Scene/Object instances used", nObjectInstancesUsed );
    ...
    {
        graphicsState.ForActiveTransforms( []( auto t ) { return pbrto::Transform( ); } );
    }

    void BasicSceneBuilder::Translate( Float dx, Float dy, Float dz, FileLoc loc )
    {
        graphicsState.ForActiveTransforms(
            [ = ]( auto t ) { return t * pbrto::Translate( Vector3f( dx, dy, dz ) ); } );
    }
    ...
    void BasicSceneBuilder::WorldBegin( FileLoc loc )
    {
        ...
        for ( int i = 0; i < MaxTransforms; ++i )
            graphicsState.ctm[ i ] = pbrto::Transform( );
        ...
        FileLoc loc )
    {
        ...
        if ( CTMIsAnimated( ) )
        {
            ...
            const class Transform* identity = transformCache.Lookup( pbrto::Transform( ) );
    ...
    {
        graphicsState.ForActiveTransforms(
            [ = ]( auto t ) { return t * pbrto::Rotate( angle, Vector3f( dx, dy, dz ) ); } );
    }

    void BasicSceneBuilder::Scale( Float sx, Float sy, Float sz, FileLoc loc )
    {
        graphicsState.ForActiveTransforms(
            [ = ]( auto t ) { return t * pbrto::Scale( sx, sy, sz ); } );
    }
        ...
        SceneEntity integ, SceneEntity accel )
    {
        ...
        NLOG_VERBOSE( "Starting to create filter and film" );
        ...
        NLOG_VERBOSE( "Finished creating filter and film" );

        // Enqueue asynchronous job to create sampler
        samplerJob = RunAsync( [ sampler, this ]( ) {
            NLOG_VERBOSE( "Starting to create sampler" );
            Allocator alloc = threadAllocators.Get( );
        ...
        cameraJob = RunAsync( [ camera, this ]( ) {
            NLOG_VERBOSE( "Starting to create camera" );
            Allocator alloc = threadAllocators.Get( );
            ...
            NLOG_VERBOSE( "Finished creating camera" );
            return c;
            } );
    }
    ...
    std::map<std::string, Medium> BasicScene::CreateMedia( )
    {
        ...
        if ( !mediumJobs.empty( ) )
        {
            ...
            NLOG_VERBOSE( "Consume media futures start" );
            for ( auto& m : mediumJobs )
            {
                while ( mediaMap.find( m.first ) == mediaMap.end( ) )
                {
                    pstdo::optional<Medium> med = m.second->TryGetResult( &mediaMutex );
                    if ( med )
                        mediaMap[ m.first ] = *med;
                }
            }
            NLOG_VERBOSE( "Consume media futures finished" );
            mediumJobs.clear( );
        }
        mediaMutex.unlock( );
        return mediaMap;
    ...
    BasicScene::BasicScene( )
        : threadAllocators( []( ) {
        pstdo::pmr::memory_resource* baseResource = pstdo::pmr::get_default_resource( );
#ifdef PBRT_BUILD_GPU_RENDERER
        if ( Options->useGPU )
            baseResource = &CUDATrackedMemoryResource::singleton;
#endif
        pstdo::pmr::monotonic_buffer_resource* resource =
            new pstdo::pmr::monotonic_buffer_resource( 1024 * 1024, baseResource );
    ...
    void BasicScene::AddFloatTexture( std::string name, TextureSceneEntity texture )
    {
        ...
        auto create = [ = ]( TextureSceneEntity texture ) {
            ...
            pbrto::Transform renderFromTexture = texture.renderFromObject.startTransform;
    ...
    void BasicScene::AddShapes( pstdo::span<ShapeSceneEntity> s )
    {
    ...
    void BasicScene::AddInstanceUses( pstdo::span<InstanceSceneEntity> in )
    {
    ...
    {
#if 0
        // NLOG_VERBOSE messages about any unused textures..
        ...
        std::vector<pbrto::Material>* materialsOut )
    {
        ...
        {
            NCHECK( normalMaps.find( job.first ) == normalMaps.end( ) );
            normalMaps[ job.first ] = job.second->GetResult( );
        }
        normalMapJobs.clear( );
        NLOG_VERBOSE( "Finished consuming normal map futures" );
        ...
        for ( const auto& nm : namedMaterials )
        {
            ...
            if ( !fn.empty( ) )
            {
                NCHECK( normalMaps.find( fn ) != normalMaps.end( ) );
    ...
    NamedTextures BasicScene::CreateTextures( )
    {
        ...
        textureMutex.unlock( );
        NLOG_VERBOSE( "Finished consuming texture futures" );

        NLOG_VERBOSE( "Starting to create remaining textures" );
        Allocator alloc = threadAllocators.Get( );
        // Create the other SpectrumTypes for the spectrum textures.
        for ( const auto& tex : asyncSpectrumTextures )
        {
            pbrto::Transform renderFromTexture = tex.second.renderFromObject.startTransform;
        ...
        NLOG_VERBOSE( "Done creating textures" );
        return textures;
    }
        ...
        std::map<int, pstdo::vector<Light>*>* shapeIndexToAreaLights )
    {
        ...
        NLOG_VERBOSE( "Starting area lights" );
        ...
        for ( size_t i = 0; i < shapes.size( ); ++i )
        {
            ...
            {
                NCHECK_LT( sh.materialIndex, materials.size( ) );
            ...
            pstdo::vector<pbrto::Shape> shapeObjects = Shape::Create(
            ...
            pbrto::MediumInterface mi( findMedium( sh.insideMedium, &sh.loc ),
                findMedium( sh.outsideMedium, &sh.loc ) );

            pstdo::vector<Light>* shapeLights = new pstdo::vector<Light>( alloc );
            const auto& areaLightEntity = areaLights[ sh.lightIndex ];
            for ( pbrto::Shape ps : shapeObjects )
            {
        ...
        NLOG_VERBOSE( "Finished area lights" );

        NLOG_VERBOSE( "Starting to consume non-area light futures" );
        std::lock_guard<std::mutex> lock( lightMutex );
        for ( auto& job : lightJobs )
            lights.push_back( job->GetResult( ) );
        NLOG_VERBOSE( "Finished consuming non-area light futures" );
        ...
        const std::vector<pbrto::Material>& materials )
    {
        ...
        auto CreatePrimitivesForShapes =
            ...
            pstdo::vector<pstdo::vector<pbrto::Shape>> shapeVectors( shapes.size( ) );
            ...
            {
                auto& sh = shapes[ i ];
                pstdo::vector<pbrto::Shape>& shapes = shapeVectors[ i ];
                if ( shapes.empty( ) )
                    continue;
        ...
        auto CreatePrimitivesForAnimatedShapes =
            ...
            for ( auto& sh : shapes )
            {
                pstdo::vector<pbrto::Shape> shapes =
                ...
                for ( auto& s : shapes )
                {
                    if ( sh.lightIndex != -1 )
                    {
                        NCHECK( sh.renderFromObject.IsAnimated( ) );
        ...
        NLOG_VERBOSE( "Finished shapes" );

        // Instance definitions
        NLOG_VERBOSE( "Starting instances" );
        ...
        NLOG_VERBOSE( "Finished instances" );

        // Accelerator
        Primitive aggregate = nullptr;
        NLOG_VERBOSE( "Starting top-level accelerator" );
        if ( !primitives.empty( ) )
            aggregate = CreateAccelerator( accelerator.name, std::move( primitives ),
                accelerator.parameters );
        NLOG_VERBOSE( "Finished top-level accelerator" );
        return aggregate;
    }
