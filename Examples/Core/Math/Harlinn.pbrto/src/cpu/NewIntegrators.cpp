...
#include <pbrto/util/NewBluenoise.h>
...
namespace pbrto
{
    ...
    NSTAT_COUNTER( "Integrator/Camera rays traced", nCameraRays );

    // RandomWalkIntegrator Method Definitions
    std::unique_ptr<RandomWalkIntegrator> RandomWalkIntegrator::Create( const ParameterDictionary& parameters, Camera camera, Sampler sampler, Primitive aggregate, std::vector<Light> lights, const FileLoc* loc )
    {
    ...
    void ImageTileIntegrator::Render( )
    {
        ...
        if ( !Options->mseReferenceImage.empty( ) )
        {
            ...
            *referenceImage = referenceImage->Crop( cropBounds );
            NCHECK_EQ( referenceImage->Resolution( ), Point2i( pixelBounds.Diagonal( ) ) );
        ...
        NLOG_VERBOSE( "Rendering finished" );
    }
    ...
    NSTAT_COUNTER( "Intersections/Regular ray intersection tests", nIntersectionTests );
    ...
    SampledSpectrum Integrator::Tr( const Interaction& p0, const Interaction& p1, const SampledWavelengths& lambda ) const
    {
        ...
        while ( true )
        {
            ...
            if ( ray.medium )
            {
                ...
                SampledSpectrum T_maj = SampleT_maj( ray, 1.f, rng.Uniform<Float>( ), rng, lambda, 
                    [ & ]( Point3f::Simd p, MediumProperties mp, SampledSpectrum sigma_maj, SampledSpectrum T_maj )
                        {
    ...
    SampledSpectrum SimplePathIntegrator::Li( RayDifferential ray, SampledWavelengths& lambda, Sampler sampler, ScratchBuffer& scratchBuffer, VisibleSurface* ) const
    {
        ...
        while ( beta )
        {
            ...
            if ( sampleBSDF )
            {
                ...
                pstdo::optional<BSDFSample> bs = bsdf.Sample_f( wo, u, sampler.Get2D( ) );
                if ( !bs )
                    break;
                beta *= bs->f* Math::ScalarAbsDot( bs->wi, isect.shading.n ) / bs->pdf;
            ...
            else
            {
                ...
                else
                {
                    ...
                    if ( IsReflective( flags ) && ScalarDot( wo, isect.n ) * ScalarDot( wi, isect.n ) < 0 )
                        wi = -wi;
                    else if ( IsTransmissive( flags ) && ScalarDot( wo, isect.n ) * ScalarDot( wi, isect.n ) > 0 )
                        wi = -wi;
                }
                beta *= bsdf.f( wo, wi ) * ScalarAbsDot( wi, isect.shading.n ) / pdf;
                specularBounce = false;
                ray = isect.SpawnRay( wi );
            }

            NCHECK_GE( SampledSpectrum::Y( beta,lambda ), 0.f );
    ...
    LightPathIntegrator::LightPathIntegrator( int maxDepth, Camera camera, Sampler sampler, Primitive aggregate, std::vector<Light> lights )
    ...
    void LightPathIntegrator::EvaluatePixelSample( Point2i pPixel, int sampleIndex, Sampler sampler, ScratchBuffer& scratchBuffer )
    {
        // Sample wavelengths for the ray
        Float lu = sampler.Get1D( );
        if ( Options->disableWavelengthJitter )
        ...
        pstdo::optional<SampledLight> sampledLight = lightSampler.Sample( ul );
        if ( !sampledLight )
            return;
        ...
        pstdo::optional<LightLeSample> les = light.SampleLe( ul0, ul1, lambda, time );
        ...
        if ( les->intr )
        {
            ...
            if ( cs && cs->pdf != 0 )
            {
                if ( Float pdf = light.PDF_Li( cs->pLens, -cs->wi ); pdf > 0 )
                {
                    ...
                    SampledSpectrum Le = light.L( les->intr->p( ), les->intr->n, les->intr->uv, cs->wi, lambda );
                    if ( Le && Unoccluded( cs->pRef, cs->pLens ) )
                    {
                        // Compute visible light's path contribution and add to film
                        SampledSpectrum L = Le * ScalarDistanceSquared( cs->pRef.p( ), cs->pLens.p( ) ) * cs->Wi / ( p_l * pdf * cs->pdf );
        ...
        SampledSpectrum beta = les->L * les->AbsCosTheta( ray.d ) / ( p_l * les->pdfPos * les->pdfDir );
        ...
        while ( true )
        {
            ...
            pstdo::optional<CameraWiSample> cs = camera.SampleWi( isect, u, lambda );
            if ( cs && cs->pdf != 0 )
            {
                SampledSpectrum L = beta * bsdf.f( isect.wo, cs->wi, TransportMode::Importance ) * ScalarAbsDot( cs->wi, isect.shading.n ) * cs->Wi / cs->pdf;
            ...
            pstdo::optional<BSDFSample> bs = bsdf.Sample_f( isect.wo, uc, sampler.Get2D( ), TransportMode::Importance );
            if ( !bs )
                break;
            beta *= bs->f * ScalarAbsDot( bs->wi, isect.shading.n ) / bs->pdf;
    ...
    NSTAT_PERCENT( "Integrator/Zero-radiance paths", zeroRadiancePaths, totalPaths );
    NSTAT_PERCENT( "Integrator/Regularized BSDFs", regularizedBSDFs, totalBSDFs );
    NSTAT_INT_DISTRIBUTION( "Integrator/Path length", pathLength );

    // PathIntegrator Method Definitions
    PathIntegrator::PathIntegrator( int maxDepth, Camera camera, Sampler sampler, Primitive aggregate, std::vector<Light> lights, const std::string& lightSampleStrategy, bool regularize )
        : RayIntegrator( camera, sampler, aggregate, lights ),
          maxDepth( maxDepth ),
    ...
    SampledSpectrum PathIntegrator::Li( RayDifferential ray, SampledWavelengths& lambda, Sampler sampler, ScratchBuffer& scratchBuffer, VisibleSurface* visibleSurf ) const
    {
        ...
        while ( true )
        {
            ...
            if ( !si )
            {
                ...
                for ( const auto& light : infiniteLights )
                {
                    ...
                    else
                    {
                        ...
                        Float p_l = lightSampler.PMF( prevIntrCtx, light ) * light.PDF_Li( prevIntrCtx, ray.d, true );
            ...
            {
                Float q = std::max<Float>( 0, 1 - MaxComponentValue( rrBeta ) );
                if ( sampler.Get1D( ) < q )
                    break;
                beta /= 1 - q;
                NDCHECK( !IsInf( beta.y( lambda ) ) );
    ...
    SampledSpectrum PathIntegrator::SampleLd( const SurfaceInteraction& intr, const BSDF* bsdf, SampledWavelengths& lambda, Sampler sampler ) const
    {
        ...
        Vector3f::Simd wo = intr.wo, wi = ls->wi;
    ...
    std::unique_ptr<PathIntegrator> PathIntegrator::Create( const ParameterDictionary& parameters, Camera camera, Sampler sampler, Primitive aggregate, std::vector<Light> lights, const FileLoc* loc )
    {
        ...
        return std::make_unique<PathIntegrator>( maxDepth, camera, sampler, aggregate, lights, lightStrategy, regularize );
    }

    // SimpleVolPathIntegrator Method Definitions
    SimpleVolPathIntegrator::SimpleVolPathIntegrator( int maxDepth, Camera camera, Sampler sampler, Primitive aggregate, std::vector<Light> lights )
    ...
    SampledSpectrum SimpleVolPathIntegrator::Li( RayDifferential ray, SampledWavelengths& lambda, Sampler sampler, ScratchBuffer& buf, VisibleSurface* ) const
    {
    ...
    NSTAT_COUNTER( "Integrator/Volume interactions", volumeInteractions );
    NSTAT_COUNTER( "Integrator/Surface interactions", surfaceInteractions );

    // VolPathIntegrator Method Definitions
    SampledSpectrum VolPathIntegrator::Li( RayDifferential ray, SampledWavelengths& lambda, Sampler sampler, ScratchBuffer& scratchBuffer, VisibleSurface* visibleSurf ) const
    {
        ...
        while ( true )
        {
            ...
            PBRT_DBG( "%s\n", StringPrintf( "Sampled BSDF, f = %s, pdf = %f -> beta = %s",
                bs->f, bs->pdf, beta )
                .c_str( ) );
            NDCHECK( IsInf( SampledSpectrum::Y( beta, lambda ) ) == false );
            ...
            if ( bssrdf && bs->IsTransmission( ) )
            {
                ...
                pstdo::optional<BSSRDFProbeSegment> probeSeg = bssrdf.SampleSp( uc, up );
                if ( !probeSeg )
                    break;
                ...
                while ( true )
                {
                    ...
                    if ( r.d == Vector3f( 0, 0, 0 ) )
                        break;
                    pstdo::optional<ShapeIntersection> si = Intersect( r, 1 );
                    if ( !si )
                        break;
                    base = si->intr;
                ...
                Float u = sampler.Get1D( );
                pstdo::optional<BSDFSample> bs = Sw.Sample_f( pi.wo, u, sampler.Get2D( ) );
                if ( !bs )
                    break;
                beta *= bs->f * ScalarAbsDot( bs->wi, pi.shading.n ) / bs->pdf;
                r_l = r_u / bs->pdf;
    ...
    SampledSpectrum VolPathIntegrator::SampleLd( const Interaction& intr, const BSDF* bsdf, SampledWavelengths& lambda, Sampler sampler, SampledSpectrum beta, SampledSpectrum r_p ) const
    {
        ...
        if ( bsdf )
        {
            ...
            f_hat = bsdf->f( wo, wi ) * ScalarAbsDot( wi, intr.AsSurface( ).shading.n );
        ...
        else
        {
            ...
            NCHECK( intr.IsMediumInteraction( ) );
        ...
        while ( lightRay.d != Vector3f::Simd( 0, 0, 0 ) )
        {
    ...
    AOIntegrator::AOIntegrator( bool cosSample, Float maxDist, Camera camera, Sampler sampler, Primitive aggregate, std::vector<Light> lights, Spectrum illuminant )
        : RayIntegrator( camera, sampler, aggregate, lights ),
          cosSample( cosSample ),
          maxDist( maxDist ),
    ...
    SampledSpectrum AOIntegrator::Li( RayDifferential ray, SampledWavelengths& lambda, Sampler sampler, ScratchBuffer& scratchBuffer, VisibleSurface* visibleSurface ) const
    {
    ...
    retry:
        ...
        if ( si )
        {
            ...
            // geometry.
            Normal3f::Simd n = FaceForward( isect.n, -ray.d );
    ...
    int RandomWalk( const Integrator& integrator, SampledWavelengths& lambda, RayDifferential ray, Sampler sampler, Camera camera, ScratchBuffer& scratchBuffer, SampledSpectrum beta, Float pdf, int maxDepth, TransportMode mode, Vertex* path, bool regularize );

    SampledSpectrum ConnectBDPT( const Integrator& integrator, SampledWavelengths& lambda, Vertex* lightVertices, Vertex* cameraVertices, int s, int t, LightSampler lightSampler, Camera camera, Sampler sampler, pstdo::optional<Point2f>* pRaster, Float* misWeightPtr = nullptr );

    Float InfiniteLightDensity( const std::vector<Light>& infiniteLights, LightSampler lightSampler, Vector3f::Simd w );
    ...
    struct Vertex
    {
        ...
        Vertex( VertexType type, const EndpointInteraction& ei, SampledSpectrum beta )
            : type( type ), beta( beta ), ei( ei )
        {
        }

        Vertex( const SurfaceInteraction& si, const BSDF& bsdf, SampledSpectrum beta )
            : type( VertexType::Surface ), beta( beta ), si( si ), bsdf( bsdf )
        {
        }

        static inline Vertex CreateCamera( Camera camera, const Ray& ray, SampledSpectrum beta );
        static inline Vertex CreateCamera( Camera camera, const Interaction& it, SampledSpectrum beta );

        static inline Vertex CreateLight( Light light, const Ray& ray, SampledSpectrum Le, Float pdf );
        static inline Vertex CreateLight( Light light, const Interaction& intr, SampledSpectrum Le, Float pdf );
        static inline Vertex CreateLight( const EndpointInteraction& ei, SampledSpectrum beta, Float pdf );

        static inline Vertex CreateMedium( const MediumInteraction& mi, SampledSpectrum beta, Float pdf, const Vertex& prev );
        static inline Vertex CreateSurface( const SurfaceInteraction& si, const BSDF& bsdf, SampledSpectrum beta, Float pdf, const Vertex& prev );

        Vertex( const MediumInteraction& mi, SampledSpectrum beta )
        ...
        bool IsConnectible( ) const
        {
            ...
            NLOG_FATAL( "Unhandled vertex type in IsConnectible()" );
        }
        ...
        bool IsDeltaLight( ) const
        {
            return type == VertexType::Light && ei.light &&
                pbrto::IsDeltaLight( ei.light.Type( ) );
        }
        ...
        SampledSpectrum Le( const std::vector<Light>& infiniteLights, const Vertex& v, const SampledWavelengths& lambda ) const
        {
            if ( !IsLight( ) )
                return SampledSpectrum( 0.f );
        ...
        Float PDF( const Integrator& integrator, const Vertex* prev, const Vertex& next ) const
        {
            if ( type == VertexType::Light )
                return PDFLight( integrator, next );
            // Compute directions to preceding and next vertex
            Vector3f::Simd wn = next.p( ) - p( );
            ...
            if ( type == VertexType::Camera )
                ...
                NLOG_FATAL( "Vertex::PDF(): Unimplemented" );
        ...
        Float PDFLight( const Integrator& integrator, const Vertex& v ) const
        {
            ...
            else if ( IsOnSurface( ) )
            {
                ...
                if ( type == VertexType::Light )
                    NCHECK( ei.light.Is<DiffuseAreaLight>( ) );  // since that's all we've
                // got currently...
            ...
            if ( v.IsOnSurface( ) )
                pdf *= ScalarAbsDot( v.ng( ), w );
            return pdf;
        }

        Float PDFLightOrigin( const std::vector<Light>& infiniteLights, const Vertex& v, LightSampler lightSampler )
        {
            Vector3f::Simd w = v.p( ) - p( );
            if ( ScalarLengthSquared( w ) == 0 )
                return 0.;
    ...
    inline Vertex Vertex::CreateCamera( Camera camera, const Ray& ray, SampledSpectrum beta )
    {
        return Vertex( VertexType::Camera, EndpointInteraction( camera, ray ), beta );
    }

    inline Vertex Vertex::CreateCamera( Camera camera, const Interaction& it, SampledSpectrum beta )
    {
        return Vertex( VertexType::Camera, EndpointInteraction( it, camera ), beta );
    }

    inline Vertex Vertex::CreateLight( Light light, const Ray& ray, SampledSpectrum Le, Float pdf )
    {
    ...
    inline Vertex Vertex::CreateLight( Light light, const Interaction& intr, SampledSpectrum Le, Float pdf )
    {
    ...
    inline Vertex Vertex::CreateSurface( const SurfaceInteraction& si, const BSDF& bsdf, SampledSpectrum beta, Float pdf, const Vertex& prev )
    {
    ...
    inline Vertex Vertex::CreateMedium( const MediumInteraction& mi, SampledSpectrum beta, Float pdf, const Vertex& prev )
    {
        Vertex v( mi, beta );
        v.pdfFwd = prev.ConvertDensity( pdf, v );
        return v;
    ...
    inline Vertex Vertex::CreateLight( const EndpointInteraction& ei, SampledSpectrum beta, Float pdf )
    {
    ...
    int GenerateCameraSubpath( const Integrator& integrator, const RayDifferential& ray, SampledWavelengths& lambda, Sampler sampler, ScratchBuffer& scratchBuffer, int maxDepth, Camera camera, Vertex* path, bool regularize )
    {
        if ( maxDepth == 0 )
            return 0;
    ...
    int GenerateLightSubpath( const Integrator& integrator, SampledWavelengths& lambda, Sampler sampler, Camera camera, ScratchBuffer& scratchBuffer, int maxDepth, Float time, LightSampler lightSampler, Vertex* path, bool regularize )
    {
        ...
        if ( path[ 0 ].IsInfiniteLight( ) )
        {
            ...
            if ( nVertices > 0 )
            {
                ...
                if ( path[ 1 ].IsOnSurface( ) )
                    path[ 1 ].pdfFwd *= ScalarAbsDot( ray.d, path[ 1 ].ng( ) );
            }
    ...
    int RandomWalk( const Integrator& integrator, SampledWavelengths& lambda, RayDifferential ray, Sampler sampler, Camera camera, ScratchBuffer& scratchBuffer, SampledSpectrum beta, Float pdf, int maxDepth, TransportMode mode, Vertex* path, bool regularize )
    {
        ...
        while ( true )
        {
            ...
            pstdo::optional<ShapeIntersection> si = integrator.Intersect( ray );
            if ( ray.medium )
            {
                ...
                Float u = sampler.Get1D( );
                SampledSpectrum T_maj = SampleT_maj( ray, tMax, u, rng, lambda,
                    [ & ]( Point3f::Simd p, MediumProperties mp, SampledSpectrum sigma_maj, SampledSpectrum T_maj ) 
                    {
                        // Compute medium event probabilities for interaction
                        Float pAbsorb = mp.sigma_a[ 0 ] / sigma_maj[ 0 ];
                        ...
                        else if ( mode == 1 )
                        {
                            ...
                            // vertex
                            pstdo::optional<PhaseFunctionSample> ps = intr.phase.Sample_p( -ray.d, sampler.Get2D( ) );
                            if ( !ps || ps->pdf == 0 )
                            {
                                terminated = true;
                                return false;
            ...
            Vector3f::Simd wo = isect.wo;
            Float u = sampler.Get1D( );
            pstdo::optional<BSDFSample> bs = bsdf.Sample_f( wo, u, sampler.Get2D( ), mode );
            if ( !bs )
                break;
    ...
    SampledSpectrum G( const Integrator& integrator, Sampler sampler, const Vertex& v0, const Vertex& v1, const SampledWavelengths& lambda )
    {
        Vector3f::Simd d = v0.p( ) - v1.p( );
        Float g = 1 / ScalarLengthSquared( d );
        d *= Math::Sqrt( g );
        if ( v0.IsOnSurface( ) )
            g *= ScalarAbsDot( v0.ns( ), d );
        if ( v1.IsOnSurface( ) )
            g *= ScalarAbsDot( v1.ns( ), d );
        return g * integrator.Tr( v0.GetInteraction( ), v1.GetInteraction( ), lambda );
    }

    Float MISWeight( const Integrator& integrator, Camera camera, Vertex* lightVertices, Vertex* cameraVertices, Vertex& sampled, int s, int t, LightSampler lightSampler )
    {
        if ( s + t == 2 )
            return 1;
        Float sumRi = 0;
    ...
    SampledSpectrum BDPTIntegrator::Li( RayDifferential ray, SampledWavelengths& lambda, Sampler sampler, ScratchBuffer& scratchBuffer, VisibleSurface* ) const
    {
        // Trace the camera and light subpaths
        Vertex* cameraVertices = scratchBuffer.Alloc<Vertex[ ]>( maxDepth + 2 );
        int nCamera = GenerateCameraSubpath( *this, ray, lambda, sampler, scratchBuffer, maxDepth + 2, camera, cameraVertices, regularize );
        Vertex* lightVertices = scratchBuffer.Alloc<Vertex[ ]>( maxDepth + 1 );
        int nLight = GenerateLightSubpath( *this, lambda, sampler, camera, scratchBuffer, maxDepth + 1, cameraVertices[ 0 ].time( ), lightSampler, lightVertices, regularize );
        ...
        for ( int t = 1; t <= nCamera; ++t )
        {
            for ( int s = 0; s <= nLight; ++s )
            {
                ...
                Float misWeight = 0.f;
                SampledSpectrum Lpath = ConnectBDPT( *this, lambda, lightVertices, cameraVertices, s, t, lightSampler, camera, sampler, &pFilmNew, &misWeight );
                PBRT_DBG( "%s\n",
                ...
                if ( Lpath && ( visualizeStrategies || visualizeWeights ) )
                {
                    ...
                    if ( visualizeStrategies )
                        value = misWeight == 0 ? SampledSpectrum( 0.f ) : Lpath / misWeight;
                    if ( visualizeWeights )
                        value = Lpath;
                    if ( pFilmNew )
    ...
    SampledSpectrum ConnectBDPT( const Integrator& integrator, SampledWavelengths& lambda, Vertex* lightVertices, Vertex* cameraVertices, int s, int t, LightSampler lightSampler, Camera camera, Sampler sampler, pstdo::optional<Point2f>* pRaster, Float* misWeightPtr )
    {
        SampledSpectrum L( 0.f );
        ...
        else if ( s == 1 )
        {
            ...
            if ( pt.IsConnectible( ) )
            {
                ...
                if ( sampledLight )
                {
                    ...
                    pstdo::optional<LightLiSample> lightWeight = light.SampleLi( ctx, sampler.Get2D( ), lambda );
                    if ( lightWeight && lightWeight->L && lightWeight->pdf > 0 )
                    {
                        ...
                        sampled.pdfFwd = sampled.PDFLightOrigin( integrator.infiniteLights, pt, lightSampler );
            ...
            if ( qs.IsConnectible( ) && pt.IsConnectible( ) )
            {
                ...
                PBRT_DBG( "%s\n",
                    StringPrintf(
                        ...
                        ScalarDistanceSquared( qs.p( ), pt.p( ) ) )
                    .c_str( ) );
                if ( L )
    ...
    SampledSpectrum MLTIntegrator::L( ScratchBuffer& scratchBuffer, MLTSampler& sampler, int depth, Point2f* pRaster, SampledWavelengths* lambda )
    {
        if ( lights.empty( ) )
            return SampledSpectrum( 0.f );
        ...
        *pRaster = sampleBounds.Lerp2( sampler.GetPixel2D( ) );
        ...
        pstdo::optional<CameraRayDifferential> crd = camera.GenerateRayDifferential( cameraSample, *lambda );
        if ( !crd || !crd->weight )
            return SampledSpectrum( 0.f );
        Float rayDiffScale = std::max<Float>( .125, 1 / Math::Sqrt( ( Float )sampler.SamplesPerPixel( ) ) );
        crd->ray.ScaleDifferentials( rayDiffScale );

        if ( GenerateCameraSubpath( *this, crd->ray, *lambda, &sampler, scratchBuffer, t, camera, cameraVertices, regularize ) != t )
        {
            return SampledSpectrum( 0.f );
        }
        ...
        if ( GenerateLightSubpath( *this, *lambda, &sampler, camera, scratchBuffer, s, cameraVertices[ 0 ].time( ), lightSampler, lightVertices, regularize ) != s )
        {
            return SampledSpectrum( 0.f );
        }
        ...
        SampledSpectrum L = ConnectBDPT( *this, *lambda, lightVertices, cameraVertices, s, t, lightSampler, camera, &sampler, &pRasterNew ) * nStrategies;
        if ( pRasterNew )
            *pRaster = *pRasterNew;
        return L;
    }
    ...
    void MLTIntegrator::Render( )
    {
        ...
        if ( !Options->debugStart.empty( ) )
        {
            ...
            pstdo::span<const std::string> span = pstdo::MakeSpan( c );
        ...
        Float b = Float( maxDepth + 1 ) / bootstrapWeights.size( ) * std::accumulate( bootstrapWeights.begin( ), bootstrapWeights.end( ), 0. );
        ...
        if ( !Options->displayServer.empty( ) )
        {
            DisplayDynamic(
                ...
                [ & ]( Bounds2i bounds, pstdo::span<pstdo::span<float>> displayValue ) {
                    Film film = camera.GetFilm( );
        ...
        ParallelFor( 0, nChains, [ & ]( int i ) {
            ...
            SampledSpectrum LCurrent = L( scratchBuffer, sampler, depth, &pCurrent, &lambdaCurrent );
            ...
            for ( int64_t j = 0; j < nChainMutations; ++j )
            {
                ...
                SampledSpectrum LProposed = L( scratchBuffer, sampler, depth, &pProposed, &lambdaProposed );
    ...
    NSTAT_RATIO( "Stochastic Progressive Photon Mapping/Visible points checked per photon "
        "intersection",
        visiblePointsChecked, totalPhotonSurfaceInteractions );
    NSTAT_COUNTER( "Stochastic Progressive Photon Mapping/Photon paths followed", photonPaths );
    NSTAT_INT_DISTRIBUTION(
        "Stochastic Progressive Photon Mapping/Grid cells per visible point",
        gridCellsPerVisiblePoint );
    NSTAT_MEMORY_COUNTER( "Memory/SPPM Pixels", pixelMemoryBytes );
    NSTAT_MEMORY_COUNTER( "Memory/SPPM BSDF and Grid Memory", sppmMemoryArenaBytes );
    ...
    void SPPMIntegrator::Render( )
    {
        ...
        pstdo::vector<DigitPermutation>* digitPermutations(
        ...
        for ( int iter = 0; iter < nIterations; ++iter )
        {
            ...
            ParallelFor2D( pixelBounds, [ & ]( Bounds2i tileBounds ) {
                ...
                for ( Point2i pPixel : tileBounds )
                {
                    ...
                    while ( true )
                    {
                        ...
                        SampledSpectrum Ld = SampleLd( isect, bsdf, lambda, sampler, &lightSampler );
                        if ( Ld )
                        ...
                        SampledSpectrum rrBeta = beta * etaScale;
                        if ( MaxComponentValue( rrBeta ) < 1 )
                        {
                            Float q = std::max<Float>( .05f, 1 - MaxComponentValue( rrBeta ) );
                            if ( sampler.Get1D( ) < q )
                                break;
                            beta /= 1 - q;
                        }
            ...
            int gridRes[ 3 ];
            Vector3f::Simd diag = gridBounds.Diagonal( );
            ...
            ParallelFor2D( pixelBounds, [ & ]( Bounds2i tileBounds ) {
                ...
                for ( Point2i pPixel : tileBounds )
                {
                    ...
                    if ( pixel.vp.beta )
                    {
                        ...
                        Point3i pMin, pMax;
                        ToGrid( pixel.vp.p - Vector3f::Simd( r, r, r ), gridBounds, gridRes, &pMin );
                        ToGrid( pixel.vp.p + Vector3f::Simd( r, r, r ), gridBounds, gridRes, &pMax );
            ...
            ParallelFor( 0, photonsPerIteration, [ & ]( int64_t start, int64_t end ) {
                ...
                for ( int64_t photonIndex = start; photonIndex < end; ++photonIndex )
                {
                    ...
                    Float ul = Sample1D( );
                    pstdo::optional<SampledLight> sampledLight = shootLightSampler.Sample( ul );
                    if ( !sampledLight )
                        continue;
                    Light light = sampledLight->light;
                    Float p_l = sampledLight->p;
                    ...
                    SampledWavelengths lambda = passLambda;
                    pstdo::optional<LightLeSample> les =
                        light.SampleLe( uLight0, uLight1, lambda, uLightTime );
                    if ( !les || les->pdfPos == 0 || les->pdfDir == 0 || !les->L )
                        continue;
                    RayDifferential photonRay = RayDifferential( les->ray );
                    SampledSpectrum beta = ( les->AbsCosTheta( photonRay.d ) * les->L ) / ( p_l * les->pdfPos * les->pdfDir );
                    if ( !beta )
                        continue;
                    ...
                    for ( int depth = 0; depth < maxDepth; ++depth )
                    {
                        ...
                        pstdo::optional<ShapeIntersection> si = Intersect( photonRay );
                        if ( !si )
                            break;
                        SurfaceInteraction& isect = si->intr;
                        ...
                        Vector3f::Simd wo = -photonRay.d;
                        pstdo::optional<BSDFSample> bs = photonBSDF.Sample_f(
                            wo, Sample1D( ), Sample2D( ), TransportMode::Importance );
                        if ( !bs )
                            break;
                        SampledSpectrum bnew = beta * bs->f * ScalarAbsDot( bs->wi, isect.shading.n ) / bs->pdf;

                        // Possibly terminate photon path with Russian roulette
                        Float betaRatio = MaxComponentValue( bnew ) / MaxComponentValue( beta );
    ...
    SampledSpectrum SPPMIntegrator::SampleLd( const SurfaceInteraction& intr, const BSDF& b, SampledWavelengths& lambda, Sampler sampler, LightSampler lightSampler ) const
    {
        const BSDF* bsdf = &b;
        // Initialize _LightSampleContext_ for light sampling
        LightSampleContext ctx( intr );
                    ...
                    for ( Point2i pPixel : bounds )
                    {
                        ...
                        if ( isHalton )
                        {
                            int pixelIndex = ( pPixel.x - pixelBounds.pMin.x ) +
                                ( pPixel.y - pixelBounds.pMin.y ) *
                                ( pixelBounds.pMax.x - pixelBounds.pMin.x );
                            NDCHECK_GE( pixelIndex, 0 );
                            NDCHECK_LT( pixelIndex, nPixels );
                                ...
                                baseSampler.Cast<HaltonSampler>( )->GetRandomizeStrategy( ) )
                            {
                                ...
                                case RandomizeStrategy::Owen:
                                    ...
                                    break;
                                default:
                                    NLOG_FATAL( "Unhandled randomization strategy" );
                            }
                        }
                        else if ( isSobol )
                        {
