...
namespace pbrto
{
    ...
    NSTAT_MEMORY_COUNTER( "Memory/BVH", treeBytes );
    NSTAT_RATIO( "BVH/Primitives per leaf node", totalPrimitives, totalLeafNodes );
    NSTAT_COUNTER( "BVH/Interior nodes", interiorNodes );
    NSTAT_COUNTER( "BVH/Leaf nodes", leafNodes );
    NSTAT_PIXEL_COUNTER( "BVH/Nodes visited", bvhNodesVisited );
        ...
        splitMethod( splitMethod )
    {
        ...
        using Resource = pstdo::pmr::monotonic_buffer_resource;
        ...
        else
        {
            ...
            root = buildRecursive( threadAllocators, pstdo::span<BVHPrimitive>( bvhPrimitives ),
                &totalNodes, &orderedPrimsOffset, orderedPrims );
            NCHECK_EQ( orderedPrimsOffset.load( ), orderedPrims.size( ) );
        }
        ...
        NCHECK_EQ( totalNodes.load( ), offset );
    }
    ...
    {
        NDCHECK_NE( bvhPrimitives.size( ), 0 );
        ...
        std::atomic<int>* orderedPrimsOffset, int bitIndex )
    {
        ...
        else
        {
            ...
            ++splitOffset;
            NCHECK_LE( splitOffset, nPrimitives - 1 );
            NCHECK_NE( mortonPrims[ splitOffset - 1 ].mortonCode & mask,
    ...
    int BVHAggregate::flattenBVH( BVHBuildNode* node, int* offset )
    {
        ...
        {
            NCHECK( !node->children[ 0 ] && !node->children[ 1 ] );
            NCHECK_LT( node->nPrimitives, 65536 );
    ...
    pstdo::optional<ShapeIntersection> BVHAggregate::Intersect( const Ray& ray,
        Float tMax ) const
    {
        if ( !nodes )
            return {};
            ...
            if ( node->bounds.IntersectP( ray.o, ray.d, tMax, invDir, dirIsNeg ) )
            {
                if ( node->nPrimitives > 0 )
                {
                    ...
                    for ( int i = 0; i < node->nPrimitives; ++i )
                    {
                        ...
                        pstdo::optional<ShapeIntersection> primSi =
        ...
        std::atomic<int>* totalNodes ) const
    {
        ...
        NCHECK_NE( centroidBounds.pMax[ dim ], centroidBounds.pMin[ dim ] );
    ...
    {
        // KdTreeNode Methods
        void InitLeaf( pstdo::span<const int> primNums, std::vector<int>* primitiveIndices );
    ...
    NSTAT_PIXEL_COUNTER( "Kd-Tree/Nodes visited", kdNodesVisited );
        ...
        primitives( std::move( p ) )
    {
        ...
        buildTree( 0, bounds, primBounds, primNums, maxDepth, edges, pstdo::span<int>( prims0 ),
            pstdo::span<int>( prims1 ), 0 );
    }

    void KdTreeNode::InitLeaf( pstdo::span<const int> primNums,
    ...
    {
        NCHECK_EQ( nodeNum, nextFreeNode );
    ...
    retrySplit:
        ...
        NCHECK( nBelow == nPrimitives && nAbove == 0 );
    ...
    pstdo::optional<ShapeIntersection> KdTreeAggregate::Intersect( const Ray& ray,
        Float rayTMax ) const
    {
        ...
        while ( node )
        {
            ...
            else
            {
                ...
                if ( nPrimitives == 1 )
                {
                    ...
                    pstdo::optional<ShapeIntersection> primSi = p.Intersect( ray, rayTMax );
                    if ( primSi )
                    {
                        si = primSi;
                        rayTMax = si->tHit;
