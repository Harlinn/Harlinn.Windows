...
namespace pbrto
{
    ...
    NSTAT_MEMORY_COUNTER( "Memory/Film pixels", filmPixelMemory );
    ...
    Image RGBFilm::GetImage( ImageMetadata* metadata, Float splatScale )
    {
        ...
        NLOG_VERBOSE( "Converting image to RGB and computing final weighted pixel values" );
    ...
    {
        // Convert image to RGB and compute final pixel values
        NLOG_VERBOSE( "Computing final weighted pixel values" );
        PixelFormat format = writeFP16 ? PixelFormat::Half : PixelFormat::Float;
