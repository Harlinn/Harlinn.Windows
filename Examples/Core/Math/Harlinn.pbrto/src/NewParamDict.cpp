...
namespace pbrto
{
    ...
    static Spectrum readSpectrumFromFile( const std::string& filename, Allocator alloc )
    {
        ...
        pstdo::optional<Spectrum> pls = PiecewiseLinearSpectrum::Read( fn, alloc );
        if ( !pls )
            return nullptr;
            ...
            return returnArray<Spectrum>(
                ...
                [ this, spectrumType, &alloc, &param ]( const Float* v,
                    const FileLoc* loc ) -> Spectrum {
                        ...
                        {
                            NCHECK( spectrumType == SpectrumType::Illuminant );
    ...
    pstdo::optional<RGB> ParameterDictionary::GetOneRGB( const std::string& name ) const
    {
        for ( const ParsedParameter* p : params )
        {
            if ( p->name == name && p->type == "rgb" )
