#pragma once
#ifndef PBRTO_NEWSAMPLERS_H_
#define PBRTO_NEWSAMPLERS_H_
...
#include <pbrto/util/NewPmj02Tables.h>
...
namespace pbrto
{
    ...
    private:
            ...
            Float SampleDimension( int dimension ) const
        {
            ...
            {
                NDCHECK_EQ( randomize, RandomizeStrategy::Owen );
        ...
        pstdo::vector<DigitPermutation>* digitPermutations = nullptr;
    ...
    private:
        ...
        int samplesPerPixel, seed;
        int pixelTileSize;
        pstdo::vector<Point2f>* pixelSamples;
        Point2i pixel;
        int sampleIndex, dimension;
    };
    ...
    public:
            ...
            Point2f GetPixel2D( )
        {
            ...
            {
                NDCHECK_RARE( 1e-7, u[ dim ] * scale - pixel[ dim ] < 0 );
                NDCHECK_RARE( 1e-7, u[ dim ] * scale - pixel[ dim ] > 1 );
    ...
    {
    public:
        static DebugMLTSampler Create( pstdo::span<const std::string> state,
            int nSampleStreams );
        ...
        Filter filter )
    {
        ...
        {
            cs.pFilm = static_cast<Point2f>(pPixel) + Vector2f( 0.5f, 0.5f );
            cs.time = 0.5f;
