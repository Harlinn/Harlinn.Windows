...
namespace pbrt
{
    ...
    public:
        ...
        PBRTO_EXPORT BVHAggregate( std::vector<Primitive> p, int maxPrimsInNode = 1,
            SplitMethod splitMethod = SplitMethod::SAH );

        PBRTO_EXPORT static BVHAggregate* Create( std::vector<Primitive> prims,
            const ParameterDictionary& parameters );

        PBRTO_EXPORT Bounds3f Bounds( ) const;
        PBRTO_EXPORT pstd::optional<ShapeIntersection> Intersect( const Ray& ray, Float tMax ) const;
    ...
    public:
        ...
        PBRTO_EXPORT KdTreeAggregate( std::vector<Primitive> p, int isectCost = 5, int traversalCost = 1,
            Float emptyBonus = 0.5, int maxPrims = 1, int maxDepth = -1 );
        PBRTO_EXPORT static KdTreeAggregate* Create( std::vector<Primitive> prims,
            const ParameterDictionary& parameters );
        PBRTO_EXPORT pstd::optional<ShapeIntersection> Intersect( const Ray& ray, Float tMax ) const;

        PBRTO_EXPORT Bounds3f Bounds( ) const { return bounds; }
