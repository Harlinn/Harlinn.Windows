#pragma once
#ifndef PBRTO_UTIL_NEWCONTAINERS_H_
#define PBRTO_UTIL_NEWCONTAINERS_H_
...
namespace pbrto
{
    ...
    public:
        ...
        using allocator_type = pstdo::pmr::polymorphic_allocator<std::byte>;
        ...
        Array2D& operator=( Array2D&& other )
        {
            ...
            {
                pstdo::swap( extent, other.extent );
                pstdo::swap( values, other.values );
            }
    ...
    template <typename Key, typename Value, typename Hash = std::hash<Key>,
        typename Allocator =
        pstdo::pmr::polymorphic_allocator<pstdo::optional<std::pair<Key, Value>>>>
        class HashMap
    {
    public:
        // HashMap Type Definitions
        using TableEntry = pstdo::optional<std::pair<Key, Value>>;
    ...
    private:
        ...
        void Grow( )
        {
            ...
            pstdo::vector<TableEntry> newTable( std::max<size_t>( 64, 2 * currentCapacity ),
                table.get_allocator( ) );
    ...
    {
    public:
        using const_iterator = typename pstdo::vector<T>::const_iterator;
        // SampledGrid Public Methods
        SampledGrid( ) = default;
        SampledGrid( Allocator alloc ) : values( alloc ) {}
        SampledGrid( pstdo::span<const T> v, int nx, int ny, int nz, Allocator alloc )
            : values( v.begin( ), v.end( ), alloc ), nx( nx ), ny( ny ), nz( nz )
        {
            NCHECK_EQ( nx * ny * nz, values.size( ) );
        }
        ...
        PBRT_CPU_GPU auto Lookup( Point3f p, F convert ) const
        {
            ...
            Point3f pSamplesFloor = Floor( pSamples );
            Point3i pi( pSamplesFloor.x, pSamplesFloor.y, pSamplesFloor.z );
            ...
            T Lookup( Point3f p ) const
        {
            ...
            Point3f pSamples( p.x * nx - .5f, p.y * ny - .5f, p.z * nz - .5f );
            Point3f pSamplesFloor = Floor( pSamples );
            Point3i pi = ( Point3i )pSamplesFloor;
            Vector3f d = pSamples - ( Point3f )pi;
        ...
        Float MaxValue( const Bounds3f& bounds, F convert ) const
        {
            Point3f ps[ 2 ] = { Point3f( bounds.pMin.x * nx - .5f, bounds.pMin.y * ny - .5f,
                             ...
                             Point3f( bounds.pMax.x * nx - .5f, bounds.pMax.y * ny - .5f,
                                     bounds.pMax.z * nz - .5f ) };
            Point3i pi[ 2 ] = { Max( Point3i( Point3f( Floor( ps[ 0 ] ) ) ), Point3i( 0, 0, 0 ) ),
                             Min( Point3i( Point3f( Floor( ps[ 1 ] ) ) ) + Vector3i( 1, 1, 1 ),
        ...
        const T* Lookup( const T& item, F create )
        {
            ...
            while ( true )
            {
                ...
                if ( !hashTable[ offset ] )
                {
                    ...
                    {
                        pstdo::vector<const T*> newHash( 2 * hashTable.size( ),
    ...
    private:
        ...
        void Insert( const T* ptr, pstdo::vector<const T*>* table )
        {
        ...
        Allocator itemAlloc;
        size_t nEntries = 0;
        pstdo::vector<const T*> hashTable;
