#pragma once
#ifndef PBRTO_UTIL_NEWPARALLEL_H_
#define PBRTO_UTIL_NEWPARALLEL_H_
...
namespace pbrto
{
    ...
    private:
        ...
        std::vector<pstdo::optional<Entry>> hashTable;
    ...
    inline T& ThreadLocal<T>::Get( )
    {
        ...
        while ( true )
        {
            NCHECK_LT( ++tries, hashTable.size( ) );  // full hash table
    ...
    public:
        ...
        virtual ~ParallelJob( ) { NDCHECK( removed ); }
    ...
    public:
        ...
        pstdo::optional<T> TryGetResult( std::mutex* extMutex )
        {
            {
                std::lock_guard<std::mutex> lock( mutex );
                if ( result )
