...
BOOST_AUTO_TEST_CASE( Bounds3PointDistanceTest1 )
{
    ...
    using Point3f = pbrto::Point3f;
    {
        ...
        BOOST_CHECK_EQUAL( 0., Distance( Point3f( .5, .5, .5 ), b ) );
        BOOST_CHECK_EQUAL( 0., Distance( Point3f( 0, 1, 1 ), b ) );
        BOOST_CHECK_EQUAL( 0., Distance( Point3f( .25, .8, 1 ), b ) );
        BOOST_CHECK_EQUAL( 0., Distance( Point3f( 0, .25, .8 ), b ) );
        BOOST_CHECK_EQUAL( 0., Distance( Point3f( .7, 0, .8 ), b ) );

        // Aligned with the plane of one of the faces
        BOOST_CHECK_EQUAL( 5., Distance( Point3f( 6, 1, 1 ), b ) );
        BOOST_CHECK_EQUAL( 10., Distance( Point3f( 0, -10, 1 ), b ) );

        // 2 of the dimensions inside the box's extent
        BOOST_CHECK_EQUAL( 2., Distance( Point3f( 0.5, 0.5, 3 ), b ) );
        BOOST_CHECK_EQUAL( 3., Distance( Point3f( 0.5, 0.5, -3 ), b ) );
        BOOST_CHECK_EQUAL( 2., Distance( Point3f( 0.5, 3, 0.5 ), b ) );
        BOOST_CHECK_EQUAL( 3., Distance( Point3f( 0.5, -3, 0.5 ), b ) );
        BOOST_CHECK_EQUAL( 2., Distance( Point3f( 3, 0.5, 0.5 ), b ) );
        BOOST_CHECK_EQUAL( 3., Distance( Point3f( -3, 0.5, 0.5 ), b ) );

        // General points
        BOOST_CHECK_EQUAL( 3 * 3 + 7 * 7 + 10 * 10, DistanceSquared( Point3f( 4, 8, -10 ), b ) );
        BOOST_CHECK_EQUAL( 6 * 6 + 10 * 10 + 7 * 7, DistanceSquared( Point3f( -6, -10, 8 ), b ) );
    }

    {
        // A few with a more irregular box, just to be sure
        Bounds3f b( Point3f( -1, -3, 5 ), Point3f( 2, -2, 18 ) );
        BOOST_CHECK_EQUAL( 0., Distance( Point3f( -.99, -2, 5 ), b ) );
        BOOST_CHECK_EQUAL( 2 * 2 + 6 * 6 + 4 * 4, DistanceSquared( Point3f( -3, -9, 22 ), b ) );
    }
}
