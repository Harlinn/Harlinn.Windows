...
int makesky(std::vector<std::string> args) {
    ...
    pbrto::Vector3f sunDir(0., std::cos(elevation), std::sin(elevation));
    ...
    for (int i = 0; i < nLambda; ++i)
        lambda[i] = Lerp2(i / Float(nLambda - 1), 320, 720);
    ...
    ParallelFor(0, resolution, [&](int64_t start, int64_t end) {
        ...
        for (int64_t iy = start; iy < end; ++iy) {
            ...
            for (int ix = 0; ix < resolution; ++ix) {
                ...
                pbrto::Vector3f v = EqualAreaSquareToSphere({x, y});
                if (v.z <= 0)
                    // downward hemisphere
                    continue;
                ...
                // direction.
                Float gamma = pbrto::SafeACos(ScalarDot(v, sunDir));
                NDCHECK(gamma >= 0 && gamma <= Pi);

                for (int i = 0; i < lambda.size(); ++i)
                    skyv[i] = arhosekskymodel_solar_radiance(skymodel_state, theta, gamma,
                                                             lambda[i]);

                PiecewiseLinearSpectrum spec(pstdo::MakeConstSpan(lambda),
                                             pstdo::MakeConstSpan(skyv));
    ...
    NCHECK(img.Write(outfile, metadata));
...
int assemble(std::vector<std::string> args) {
    ...
    for (const std::string &file : infiles) {
        ...
        if (fullImage.Resolution() == pbrto::Point2i(0, 0)) {
            // First image read.
        ...
        for (int y = 0; y < image.Resolution().y; ++y)
            for (int x = 0; x < image.Resolution().x; ++x) {
                pbrto::Point2i fullp{x + metadata.pixelBounds->pMin.x,
                              y + metadata.pixelBounds->pMin.y};
                NCHECK(InsideExclusive(fullp, fullBounds));
...
int splitn(std::vector<std::string> args) {
    ...
    NCHECK_LE(crops.size(), sizeof(edges) / sizeof(edges[0]));
    ImageChannelDesc cropDesc = cropsImage.GetChannelDesc({"R", "G", "B"});
    NCHECK(bool(cropDesc));
    ...
    for (int c = 0; c < crops.size(); ++c) {
        ...
        for (int i = 0; i < images.size(); ++i) {
            ...
            NCHECK(bool(imageDesc));
            for (pbrto::Point2i p : crop) {
                pbrto::Point2i pOut =
                    pbrto::Point2i(x0 + borderWidth, y0 + borderWidth) + (p - crop.pMin);
...
int cat(std::vector<std::string> args) {
    ...
    for (auto iter = args.begin(); iter != args.end(); ++iter) {
        ...
        if (sort) {
            std::vector<std::pair<pbrto::Point2i, ImageChannelValues>> sorted;
            sorted.reserve(pixelBounds.Area());
            for ( pbrto::Point2i p : pixelBounds) {
            ...
            std::sort(sorted.begin(), sorted.end(),
                      [](const std::pair<pbrto::Point2i, ImageChannelValues> &a,
                         const std::pair<pbrto::Point2i, ImageChannelValues> &b) {
...
int average(std::vector<std::string> args) {
    ...
    ParallelFor(0, filenames.size(), [&](size_t i) {
        ...
        if (avg.Resolution() == pbrto::Point2i(0, 0)) {
        ...
        for (int y = 0; y < avg.Resolution().y; ++y)
            for (int x = 0; x < avg.Resolution().x; ++x)
                for (int c = 0; c < avg.NChannels(); ++c) {
                    ...
                    if (std::isnan(v))
                        NLOG_FATAL("NAN Pixel at %s in %s", pbrto::Point2f(x, y), filenames[i]);
                    if (std::isinf(v))
                        v = 0;
    ...
    avgImages.ForAll([&](const Image &im) {
        NCHECK_NE(im.Resolution(), pbrto::Point2i(0, 0));
        if (avgImage.Resolution() == pbrto::Point2i(0, 0)) {
            // First valid one
            avgImage = im;
        } else {
...
int error(std::vector<std::string> args) {
    ...
    for (auto iter = args.begin(); iter != args.end(); ++iter) {
        ...
        if (ParseArg(&iter, args.end(), "reference", &referenceFile, onError) ||
            ...
            ParseArg(&iter, args.end(), "crop", pstdo::MakeSpan(cropWindow), onError)) {
            // success
        } else if (filenameBase.empty() && (*iter)[0] != '-') {
            filenameBase = *iter;
        } else
    ...
    ParallelFor(0, filenames.size(), [&](size_t i) {
        ...
        NCHECK(imRead.metadata.samplesPerPixel.has_value());
...
int bloom(std::vector<std::string> args) {
    ...
    int nSurvivors = 0;
    pbrto::Point2i res = image.Resolution();
...
int convert(std::vector<std::string> args) {
    ...
    if (cropWindow[0] >= 0 && cropWindow[2] >= 0) {
        ...
        if (metadata.pixelBounds && !Inside(cropBounds, *metadata.pixelBounds)) {
            Error("%s: crop window bounds (%d,%d)-(%d,%d) are not inside "
                  ...
                  metadata.pixelBounds->pMax.y);
            return 1;
        } else if (!Inside(cropBounds, Bounds2i( pbrto::Point2i(0, 0), fullRes))) {
            Error("%s: crop window bounds (%d,%d)-(%d,%d) are not inside "
        ...
        if (metadata.pixelBounds) {
            cropBounds.pMin = pbrto::Point2i(cropBounds.pMin - metadata.pixelBounds->pMin);
            cropBounds.pMax = pbrto::Point2i(cropBounds.pMax - metadata.pixelBounds->pMin);
        }
    ...
    if (!colorspace.empty()) {
        ...
        pbrto::SquareMatrix<3> m = ConvertRGBColorSpace(*srcColorSpace, *dest);
        for (int y = 0; y < res.y; ++y)
            for (int x = 0; x < res.x; ++x) {
    ...
    if (repeat > 1) {
        Image scaledImage(image.Format(), pbrto::Point2i(res.x * repeat, res.y * repeat),
                          image.ChannelNames(), image.Encoding());
...
int whitebalance(std::vector<std::string> args) {
    ...
    for (auto iter = args.begin(); iter != args.end(); ++iter) {
        ...
        if (ParseArg(&iter, args.end(), "outfile", &outFile, onError) ||
            ParseArg(&iter, args.end(), "primaries", pstdo::MakeSpan(xy), onError) ||
    ...
    pbrto::Point2f srcWhite, targetWhite = colorSpace->w;
    if (!illuminant.empty()) {
        ...
        srcWhite = pbrto::Point2f(xy[0], xy[1]);

    pbrto::SquareMatrix<3> ccMatrix = colorSpace->RGBFromXYZ *
...
int makeequiarea(std::vector<std::string> args) {
    ...
    GaussianFilter filter( pbrto::Vector2f(1.5, 1.5), 2);
...
int denoise_optix(std::vector<std::string> args) {
    ...
    NCHECK(result.Write(outFilename, outMetadata));
...
int main(int argc, char *argv[]) {
    ...
    if (cmd == "average")
        ...
        for (auto iter = args.begin(); iter != args.end(); ++iter) {
            ...
            if (ParseArg(&iter, args.end(), "pixel", pstdo::MakeSpan(pixel), onError) ||
                ParseArg(&iter, args.end(), "width", &width, onError) ||
        ...
        NCHECK_GE(3 * nInstances, nPixels);  // want to be overconstrained
