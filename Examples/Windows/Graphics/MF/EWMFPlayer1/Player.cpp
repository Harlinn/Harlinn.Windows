...
//
//  Player constructor - instantiates internal objects and initializes MF
//
Player::Player(HWND videoWindow) 
...
//
HRESULT Player::Invoke(IMFAsyncResult* asyncResult) noexcept
{
...
//
bool Player::ProcessMediaEvent(const MFMediaEvent& mediaEvent)
{
    // Not closing
    bool result = true;
...
// MF components.
//
void Player::OpenURL(PCWSTR sURL)
{
    std::lock_guard lock(criticalSection_);
...
void Player::Resize(const Size& newSize)
{
    if (videoDisplay_)
    {
