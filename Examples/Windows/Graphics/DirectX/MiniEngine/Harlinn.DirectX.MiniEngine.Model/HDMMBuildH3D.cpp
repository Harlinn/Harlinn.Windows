#include "pch.h"
#include "HDMMModelH3D.h"
#include "HDMMModelLoader.h"
#include "HDMMMeshConvert.h"
...
namespace Harlinn::Windows::DirectX::MiniEngine
{
    ...
    bool ModelH3D::BuildModel( ModelData& model, const std::wstring& basePath ) const
    {
        ...
        for ( uint32_t i = 0; i < m_Header.meshCount; ++i )
        {
            ...
            auto meshBoundingBoxMin = mesh.boundingBox.GetMin( );
            auto meshBoundingBoxMax = mesh.boundingBox.GetMax( );

            memcpy( prim.minPos, &meshBoundingBoxMin, 12 );
            memcpy( prim.maxPos, &meshBoundingBoxMax, 12 );
            prim.minIndex = 0;
            prim.maxIndex = 0;
