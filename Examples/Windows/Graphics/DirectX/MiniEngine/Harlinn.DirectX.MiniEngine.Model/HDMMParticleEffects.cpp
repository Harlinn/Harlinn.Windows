...
#include "HDMMParticleEffects.h"
#include "HDMCParticleEffectManager.h"
#include "HDMCTextureManager.h"
#include <fstream>
...
namespace Harlinn::Windows::DirectX::MiniEngine
{
    ...
    namespace
    {
        std::map<std::wstring, uint32_t> s_TextureArrayLookup;
        std::vector<TextureRef> s_TextureReferences;

        uint32_t GetTextureIndex( const std::wstring& name )
        {
...
#define MAKE_XMFLOAT3(it) XMFLOAT3((float)it.value()[0], (float)it.value()[1], (float)it.value()[2])
    ...
    void ParticleEffects::InitFromJSON( const std::wstring& InitJsonFile )
    {
        using json = nlohmann::json;

        json particle_setup;
        std::ifstream( InitJsonFile ) >> particle_setup;
