...
namespace Harlinn::Windows::DirectX::MiniEngine
{
    ...
    using TEX_FR_FLAGS = ::DirectX::TEX_FR_FLAGS;
    using CNMAP_FLAGS = ::DirectX::CNMAP_FLAGS;
...
#define GetFlag(f) ((Flags & f) != 0)
    ...
    bool ConvertToDDS( const std::wstring& filePath, uint32_t Flags )
    {
        ...
        if ( ext == L"dds" )
        {
            ...
            isDDS = true;
            HRESULT hr = LoadFromDDSFile( filePath.c_str( ), DDS_FLAGS::DDS_FLAGS_NONE, &info, *image );
            if ( FAILED( hr ) )
            {
        ...
        if ( bFlipImage )
        {
            ...
            HRESULT hr = FlipRotate( image->GetImages( )[ 0 ], TEX_FR_FLAGS::TEX_FR_FLIP_VERTICAL, *timage );
        ...
        if ( bBumpMap )
        {
            ...
            HRESULT hr = ComputeNormalMap( image->GetImages( ), image->GetImageCount( ), image->GetMetadata( ),
                CNMAP_FLAGS::CNMAP_CHANNEL_LUMINANCE, 10.0f, tformat, *timage );
        ...
        else if ( info.format != tformat )
        {
            ...
            HRESULT hr = Convert( image->GetImages( ), image->GetImageCount( ), image->GetMetadata( ),
                tformat, TEX_FILTER_FLAGS::TEX_FILTER_DEFAULT, 0.5f, *timage );
        ...
        if ( info.mipLevels == 1 )
        {
            ...
            HRESULT hr = GenerateMipMaps( image->GetImages( ), image->GetImageCount( ), image->GetMetadata( ), TEX_FILTER_FLAGS::TEX_FILTER_DEFAULT, 0, *timage );
        ...
        if ( bBlockCompress )
        {
            ...
            else
            {
                ...
                HRESULT hr = Compress( image->GetImages( ), image->GetImageCount( ), image->GetMetadata( ), cformat, TEX_COMPRESS_FLAGS::TEX_COMPRESS_DEFAULT, 0.5f, *timage );
                if ( FAILED( hr ) )
                {
        ...
        // Save DDS
        HRESULT hr = SaveToDDSFile( image->GetImages( ), image->GetImageCount( ), image->GetMetadata( ), DDS_FLAGS::DDS_FLAGS_NONE, wDest.c_str( ) );
        if ( FAILED( hr ) )
        {
