...
namespace Harlinn::Windows::DirectX::MiniEngine
{
    ...
    namespace Utility
    {
        ByteArray NullFile = std::make_shared<vector<Byte> >( vector<Byte>( ) );
    }
    ...
    ByteArray ReadFileHelper( const wstring& fileName )
    {
        ...
        Utility::ByteArray byteArray = std::make_shared<vector<Byte> >( fileStat.st_size );
    ...
    ByteArray Inflate( ByteArray CompressedSource, int& err, uint32_t ChunkSize = 0x100000 )
    {
        ...
        while ( err == Z_OK || err == Z_BUF_ERROR )
        {
            ...
            strm.next_out = new Byte[ ChunkSize ];
        ...
        Utility::ByteArray byteArray = std::make_shared<vector<Byte> >( strm.total_out );
