...
namespace Harlinn::Windows::DirectX::MiniEngine
{
    ...
    namespace Math
    {
        ...
        public:
            INLINE Matrix3( ) {}
            INLINE Matrix3( const Vector3& x, const Vector3& y, const Vector3& z )
            { 
                m_mat.simd[ 0 ] = x.ToSimd( ).simd; 
                m_mat.simd[ 1 ] = y.ToSimd( ).simd;
                m_mat.simd[ 2 ] = z.ToSimd( ).simd;
            }
            INLINE Matrix3( const Matrix3& m ) 
                : m_mat(m.m_mat)
            { 
            }
            INLINE Matrix3( Quaternion q ) 
            { 
                *this = Matrix3( ::DirectX::XMMatrixRotationQuaternion( q ) ); 
            }
