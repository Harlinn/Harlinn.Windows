...
#define CONDITIONALLY_ENABLE_HDR_OUTPUT 1
    ...
    void Display::Resize( uint32_t width, uint32_t height )
    {
        ...
        s_SwapChain1.ResizeBuffers( SWAP_CHAIN_BUFFER_COUNT, width, height, SwapChainFormat, 0 );

        for ( uint32_t i = 0; i < SWAP_CHAIN_BUFFER_COUNT; ++i )
        {
            D3D12Resource displayPlane = s_SwapChain1.GetBuffer<D3D12Resource>( i );
    ...
    void Display::Initialize( void )
    {
        ...
        */
        ...
        for ( uint32_t i = 0; i < SWAP_CHAIN_BUFFER_COUNT; ++i )
        {
            auto DisplayPlane = s_SwapChain1.GetBuffer<D3D12Resource>( i );
            g_DisplayPlane[ i ].CreateFromSwapChain( L"Primary SwapChain Buffer", DisplayPlane );
        }
