...
struct TensorExtents<1> // nExtents for 1D arrays of n elements.
{
    ...
    /*implicit*/ TensorExtents(std::span<const uint32_t> extents)
    {
...
struct TensorExtents<4> // NchwExtents
{
    ...
    std::span<const uint32_t> AsSpan() const noexcept{ return std::span<const uint32_t>(data(), size()); }
    std::span<uint32_t> AsSpan() noexcept{ return std::span<uint32_t>(data(), size()); }
