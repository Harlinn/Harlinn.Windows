...
class Optimizer {
 public:
  ...
  struct PassToken {
    ...
    SPIRV_TOOLS_EXPORT PassToken(std::unique_ptr<Impl>);
    ...
    SPIRV_TOOLS_EXPORT PassToken(std::unique_ptr<opt::Pass>&& pass);

    // Tokens can only be moved. Copying is disabled.
    PassToken(const PassToken&) = delete;
    SPIRV_TOOLS_EXPORT PassToken(PassToken&&);
    PassToken& operator=(const PassToken&) = delete;
    SPIRV_TOOLS_EXPORT PassToken& operator=(PassToken&&);

    SPIRV_TOOLS_EXPORT ~PassToken();
  ...
  SPIRV_TOOLS_EXPORT explicit Optimizer(spv_target_env env);
  ...
  SPIRV_TOOLS_EXPORT const MessageConsumer& consumer() const;
  ...
  // method.
  SPIRV_TOOLS_EXPORT Optimizer& RegisterPass(PassToken&& pass);

  // Registers passes that attempt to improve performance of generated code.
  // This sequence of passes is subject to constant review and will change
  // from time to time.
  SPIRV_TOOLS_EXPORT Optimizer& RegisterPerformancePasses();

  // Registers passes that attempt to improve the size of generated code.
  // This sequence of passes is subject to constant review and will change
  // from time to time.
  SPIRV_TOOLS_EXPORT Optimizer& RegisterSizePasses();
  ...
  // This sequence of passes is subject to constant review and will change
  // from time to time.
  SPIRV_TOOLS_EXPORT Optimizer& RegisterLegalizationPasses();
  ...
  SPIRV_TOOLS_EXPORT bool RegisterPassesFromFlags(const std::vector<std::string>& flags);
  ...
  //                  HLSL front-end.
  SPIRV_TOOLS_EXPORT bool RegisterPassFromFlag(const std::string& flag);
  ...
  // returns false.
  SPIRV_TOOLS_EXPORT bool FlagHasValidForm(const std::string& flag) const;
  ...
  SPIRV_TOOLS_EXPORT void SetTargetEnv(const spv_target_env env);
  ...
  SPIRV_TOOLS_EXPORT bool Run(const uint32_t* original_binary, size_t original_binary_size,
  ...
  // module.
  SPIRV_TOOLS_EXPORT bool Run(const uint32_t* original_binary, const size_t original_binary_size,
  ...
  SPIRV_TOOLS_EXPORT std::vector<const char*> GetPassNames() const;
  ...
  SPIRV_TOOLS_EXPORT Optimizer& SetPrintAll(std::ostream* out);
  ...
  // |out| output stream.
  SPIRV_TOOLS_EXPORT Optimizer& SetTimeReport(std::ostream* out);

  // Sets the option to validate the module after each pass.
  SPIRV_TOOLS_EXPORT Optimizer& SetValidateAfterAll(bool validate);
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateNullPass();

// Creates a strip-debug-info pass.
// A strip-debug-info pass removes all debug instructions (as documented in
// Section 3.32.2 of the SPIR-V spec) of the SPIR-V module to be optimized.
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateStripDebugInfoPass();
...
// the future.
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateStripReflectInfoPass();
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateEliminateDeadFunctionsPass();
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateEliminateDeadMembersPass();
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateSetSpecConstantDefaultValuePass(
...
// it removes.
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateFlattenDecorationPass();
...
// OpSpecConstantOp.
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateFreezeSpecConstantValuePass();
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateFoldSpecConstantOpAndCompositePass();
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateUnifyConstantPass();
...
// OpSpecConstantOp.
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateEliminateDeadConstantPass();
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateStrengthReductionPass();
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateBlockMergePass();
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateInlineExhaustivePass();
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateInlineOpaquePass();
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateLocalSingleBlockLoadStoreElimPass();
...
// possible.
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateDeadBranchElimPass();
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateLocalMultiStoreElimPass();
...
// possible.
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateLocalAccessChainConvertPass();
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateLocalSingleStoreElimPass();
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateInsertExtractElimPass();
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateDeadInsertElimPass();
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateAggressiveDCEPass();
...
// stages.
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateRemoveUnusedInterfaceVariablesPass();
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreatePropagateLineInfoPass();

// Creates an empty pass.
// This is deprecated and will be removed.
// TODO(jaebaek): remove this pass after handling glslang's broken unit tests.
//                https://github.com/KhronosGroup/glslang/pull/2440
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateRedundantLineInfoElimPass();

// Creates a compact ids pass.
// The pass remaps result ids to a compact and gapless range starting from %1.
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateCompactIdsPass();
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateRemoveDuplicatesPass();
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateCFGCleanupPass();

// Create dead variable elimination pass.
// This pass will delete module scope variables, along with their decorations,
// that are not referenced.
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateDeadVariableEliminationPass();
...
// elimination.
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateMergeReturnPass();
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateLocalRedundancyEliminationPass();

// Create LICM pass.
// This pass will look for invariant instructions inside loops and hoist them to
// the loops preheader.
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateLoopInvariantCodeMotionPass();

// Creates a loop fission pass.
// This pass will split all top level loops whose register pressure exceedes the
// given |threshold|.
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateLoopFissionPass(size_t threshold);
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateLoopFusionPass(size_t max_registers_per_loop);
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateLoopPeelingPass();
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateLoopUnswitchPass();

// Create global value numbering pass.
// This pass will look for instructions where the same value is computed on all
// paths leading to the instruction.  Those instructions are deleted.
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateRedundancyEliminationPass();
...
// consider replacing.
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateScalarReplacementPass(uint32_t size_limit = 100);
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreatePrivateToLocalPass();
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateCCPPass();
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateWorkaround1209Pass();

// Creates a pass that converts if-then-else like assignments into OpSelect.
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateIfConversionPass();
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateReplaceInvalidOpcodePass();

// Creates a pass that simplifies instructions using the instruction folder.
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateSimplificationPass();
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateLoopUnrollPass(bool fully_unroll, int factor = 0);
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateSSARewritePass();
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateConvertRelaxedToHalfPass();
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateRelaxFloatOpsPass();
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateCopyPropagateArraysPass();
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateVectorDCEPass();
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateReduceLoadSizePass();
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateCombineAccessChainsPass();
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateInstBindlessCheckPass(
...
// descriptor set |desc_set|. It will write |shader_id| in each output record
// to identify the shader module which generated the record.
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateInstBuffAddrCheckPass(uint32_t desc_set,
                                                 uint32_t shader_id);
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateInstDebugPrintfPass(uint32_t desc_set,
                                               uint32_t shader_id);
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateUpgradeMemoryModelPass();
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateCodeSinkingPass();
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateFixStorageClassPass();
...
//   inclusive.
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateGraphicsRobustAccessPass();
...
// for the first index.
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateDescriptorScalarReplacementPass();
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateWrapOpKillPass();
...
// capabilities.
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateAmdExtToKhrPass();
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateInterpolateFixupPass();
...
SPIRV_TOOLS_EXPORT Optimizer::PassToken CreateConvertToSampledImagePass(
