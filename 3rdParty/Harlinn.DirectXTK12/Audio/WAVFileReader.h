...
namespace DirectX
{
    HDXTK12_EXPORT HRESULT LoadWAVAudioInMemory(
    ...
    HDXTK12_EXPORT HRESULT LoadWAVAudioFromFile(
    ...
    HDXTK12_EXPORT HRESULT LoadWAVAudioInMemoryEx(
        _In_reads_bytes_(wavDataSize) const uint8_t* wavData,
        _In_ size_t wavDataSize,
        _Out_ WAVData& result) noexcept;

    HDXTK12_EXPORT HRESULT LoadWAVAudioFromFileEx(
        _In_z_ const wchar_t* szFileName,
        _Inout_ std::unique_ptr<uint8_t[]>& wavData,
        _Out_ WAVData& result) noexcept;
}
