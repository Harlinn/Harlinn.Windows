...
namespace DirectX
{
    ...
    {
    public:
        HDXTK12_EXPORT Mouse() noexcept(false);

        HDXTK12_EXPORT Mouse(Mouse&&) noexcept;
        HDXTK12_EXPORT Mouse& operator= (Mouse&&) noexcept;

        Mouse(Mouse const&) = delete;
        Mouse& operator=(Mouse const&) = delete;

        HDXTK12_EXPORT virtual ~Mouse();
        ...
        HDXTK12_EXPORT State __cdecl GetState() const;

        // Resets the accumulated scroll wheel value
        HDXTK12_EXPORT void __cdecl ResetScrollWheelValue() noexcept;

        // Sets mouse mode (defaults to absolute)
        HDXTK12_EXPORT void __cdecl SetMode(Mode mode);

        // Signals the end of frame (recommended, but optional)
        HDXTK12_EXPORT void __cdecl EndOfInputFrame() noexcept;

        // Feature detection
        HDXTK12_EXPORT bool __cdecl IsConnected() const;

        // Cursor visibility
        HDXTK12_EXPORT bool __cdecl IsVisible() const noexcept;
        HDXTK12_EXPORT void __cdecl SetVisible(bool visible);
        ...
        HDXTK12_EXPORT void __cdecl SetWindow(HWND window);
        HDXTK12_EXPORT static void __cdecl ProcessMessage(UINT message, WPARAM wParam, LPARAM lParam);
