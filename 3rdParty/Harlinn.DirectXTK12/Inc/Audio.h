...
namespace DirectX
{
    ...
    {
    public:
        HDXTK12_EXPORT explicit AudioEngine(
        ...
        HDXTK12_EXPORT AudioEngine(AudioEngine&&) noexcept;
        HDXTK12_EXPORT AudioEngine& operator= (AudioEngine&&) noexcept;

        AudioEngine(AudioEngine const&) = delete;
        AudioEngine& operator= (AudioEngine const&) = delete;

        HDXTK12_EXPORT virtual ~AudioEngine();

        HDXTK12_EXPORT bool __cdecl Update();
            // Performs per-frame processing for the audio engine, returns false if in 'silent mode'

        HDXTK12_EXPORT bool __cdecl Reset(_In_opt_ const WAVEFORMATEX* wfx = nullptr, _In_opt_z_ const wchar_t* deviceId = nullptr);
        ...
        HDXTK12_EXPORT void __cdecl Suspend() noexcept;
        HDXTK12_EXPORT void __cdecl Resume();
            // Suspend/resumes audio processing (i.e. global pause/resume)

        HDXTK12_EXPORT float __cdecl GetMasterVolume() const noexcept;
        HDXTK12_EXPORT void __cdecl SetMasterVolume(float volume);
            // Master volume property for all sounds

        HDXTK12_EXPORT void __cdecl SetReverb(AUDIO_ENGINE_REVERB reverb);
        HDXTK12_EXPORT void __cdecl SetReverb(_In_opt_ const XAUDIO2FX_REVERB_PARAMETERS* native);
            // Sets environmental reverb for 3D positional audio (if active)

        HDXTK12_EXPORT void __cdecl SetMasteringLimit(int release, int loudness);
            // Sets the mastering volume limiter properties (if active)

        HDXTK12_EXPORT AudioStatistics __cdecl GetStatistics() const;
            // Gathers audio engine statistics

        HDXTK12_EXPORT WAVEFORMATEXTENSIBLE __cdecl GetOutputFormat() const noexcept;
            // Returns the format of the audio output device associated with the mastering voice.

        HDXTK12_EXPORT uint32_t __cdecl GetChannelMask() const noexcept;
            // Returns the output channel mask

        HDXTK12_EXPORT int __cdecl GetOutputSampleRate() const noexcept;
            // Returns the sample rate going into the mastering voice

        HDXTK12_EXPORT unsigned int __cdecl GetOutputChannels() const noexcept;
            // Returns the number of channels going into the mastering voice

        HDXTK12_EXPORT bool __cdecl IsAudioDevicePresent() const noexcept;
            // Returns true if the audio graph is operating normally, false if in 'silent mode'

        HDXTK12_EXPORT bool __cdecl IsCriticalError() const noexcept;
            // Returns true if the audio graph is halted due to a critical error (which also places the engine into 'silent mode')

        // Voice pool management.
        HDXTK12_EXPORT void __cdecl SetDefaultSampleRate(int sampleRate);
            // Sample rate for voices in the reuse pool (defaults to 44100)

        HDXTK12_EXPORT void __cdecl SetMaxVoicePool(size_t maxOneShots, size_t maxInstances);
        ...
        HDXTK12_EXPORT void __cdecl TrimVoicePool();
            // Releases any currently unused voices

        // Internal-use functions
        HDXTK12_EXPORT void __cdecl AllocateVoice(_In_ const WAVEFORMATEX* wfx,
            SOUND_EFFECT_INSTANCE_FLAGS flags, bool oneshot, _Outptr_result_maybenull_ IXAudio2SourceVoice** voice);

        HDXTK12_EXPORT void __cdecl DestroyVoice(_In_ IXAudio2SourceVoice* voice) noexcept;
            // Should only be called for instance voices, not one-shots

        HDXTK12_EXPORT void __cdecl RegisterNotify(_In_ IVoiceNotify* notify, bool usesUpdate);
        HDXTK12_EXPORT void __cdecl UnregisterNotify(_In_ IVoiceNotify* notify, bool usesOneShots, bool usesUpdate);

        // XAudio2 interface access
        HDXTK12_EXPORT IXAudio2* __cdecl GetInterface() const noexcept;
        HDXTK12_EXPORT IXAudio2MasteringVoice* __cdecl GetMasterVoice() const noexcept;
        HDXTK12_EXPORT IXAudio2SubmixVoice* __cdecl GetReverbVoice() const noexcept;
        HDXTK12_EXPORT X3DAUDIO_HANDLE& __cdecl Get3DHandle() const noexcept;
        ...
        HDXTK12_EXPORT static std::vector<RendererDetail> __cdecl GetRendererDetails();
        ...
        HDXTK12_EXPORT bool __cdecl Reset(_In_opt_ const WAVEFORMATEX* wfx = nullptr, _In_opt_z_ const __wchar_t* deviceId = nullptr);
#endif
    ...
    {
    public:
        HDXTK12_EXPORT WaveBank(_In_ AudioEngine* engine, _In_z_ const wchar_t* wbFileName);

        HDXTK12_EXPORT WaveBank(WaveBank&&) noexcept;
        HDXTK12_EXPORT WaveBank& operator= (WaveBank&&) noexcept;

        WaveBank(WaveBank const&) = delete;
        WaveBank& operator= (WaveBank const&) = delete;

        HDXTK12_EXPORT virtual ~WaveBank();

        HDXTK12_EXPORT void __cdecl Play(unsigned int index);
        HDXTK12_EXPORT void __cdecl Play(unsigned int index, float volume, float pitch, float pan);

        HDXTK12_EXPORT void __cdecl Play(_In_z_ const char* name);
        HDXTK12_EXPORT void __cdecl Play(_In_z_ const char* name, float volume, float pitch, float pan);

        HDXTK12_EXPORT std::unique_ptr<SoundEffectInstance> __cdecl CreateInstance(unsigned int index,
            SOUND_EFFECT_INSTANCE_FLAGS flags = SoundEffectInstance_Default);
        HDXTK12_EXPORT std::unique_ptr<SoundEffectInstance> __cdecl CreateInstance(_In_z_ const char* name,
            SOUND_EFFECT_INSTANCE_FLAGS flags = SoundEffectInstance_Default);

        HDXTK12_EXPORT std::unique_ptr<SoundStreamInstance> __cdecl CreateStreamInstance(unsigned int index,
            SOUND_EFFECT_INSTANCE_FLAGS flags = SoundEffectInstance_Default);
        HDXTK12_EXPORT std::unique_ptr<SoundStreamInstance> __cdecl CreateStreamInstance(_In_z_ const char* name,
            SOUND_EFFECT_INSTANCE_FLAGS flags = SoundEffectInstance_Default);

        HDXTK12_EXPORT bool __cdecl IsPrepared() const noexcept;
        HDXTK12_EXPORT bool __cdecl IsInUse() const noexcept;
        HDXTK12_EXPORT bool __cdecl IsStreamingBank() const noexcept;
        HDXTK12_EXPORT bool __cdecl IsAdvancedFormat() const noexcept;

        HDXTK12_EXPORT size_t __cdecl GetSampleSizeInBytes(unsigned int index) const noexcept;
        // Returns size of wave audio data

        HDXTK12_EXPORT size_t __cdecl GetSampleDuration(unsigned int index) const noexcept;
        // Returns the duration in samples

        HDXTK12_EXPORT size_t __cdecl GetSampleDurationMS(unsigned int index) const noexcept;
        // Returns the duration in milliseconds

        HDXTK12_EXPORT const WAVEFORMATEX* __cdecl GetFormat(unsigned int index, _Out_writes_bytes_(maxsize) WAVEFORMATEX* wfx, size_t maxsize) const noexcept;

        HDXTK12_EXPORT int __cdecl Find(_In_z_ const char* name) const;

    #ifdef USING_XAUDIO2_9
        HDXTK12_EXPORT bool __cdecl FillSubmitBuffer(unsigned int index, _Out_ XAUDIO2_BUFFER& buffer, _Out_ XAUDIO2_BUFFER_WMA& wmaBuffer) const;
    #else
        HDXTK12_EXPORT void __cdecl FillSubmitBuffer(unsigned int index, _Out_ XAUDIO2_BUFFER& buffer) const;
    #endif

        HDXTK12_EXPORT void __cdecl UnregisterInstance(_In_ IVoiceNotify* instance);

        HDXTK12_EXPORT HANDLE __cdecl GetAsyncHandle() const noexcept;

        HDXTK12_EXPORT bool __cdecl GetPrivateData(unsigned int index, _Out_writes_bytes_(datasize) void* data, size_t datasize);

#if defined(_MSC_VER) && !defined(_NATIVE_WCHAR_T_DEFINED)
        HDXTK12_EXPORT WaveBank(_In_ AudioEngine* engine, _In_z_ const __wchar_t* wbFileName);
#endif
    ...
    {
    public:
        HDXTK12_EXPORT SoundEffect(_In_ AudioEngine* engine, _In_z_ const wchar_t* waveFileName);

        HDXTK12_EXPORT SoundEffect(_In_ AudioEngine* engine, _Inout_ std::unique_ptr<uint8_t[]>& wavData,
        ...
        HDXTK12_EXPORT SoundEffect(SoundEffect&&) noexcept;
        HDXTK12_EXPORT SoundEffect& operator= (SoundEffect&&) noexcept;

        SoundEffect(SoundEffect const&) = delete;
        SoundEffect& operator= (SoundEffect const&) = delete;

        HDXTK12_EXPORT virtual ~SoundEffect();

        HDXTK12_EXPORT void __cdecl Play();
        HDXTK12_EXPORT void __cdecl Play(float volume, float pitch, float pan);

        HDXTK12_EXPORT std::unique_ptr<SoundEffectInstance> __cdecl CreateInstance(SOUND_EFFECT_INSTANCE_FLAGS flags = SoundEffectInstance_Default);

        HDXTK12_EXPORT bool __cdecl IsInUse() const noexcept;

        HDXTK12_EXPORT size_t __cdecl GetSampleSizeInBytes() const noexcept;
        // Returns size of wave audio data

        HDXTK12_EXPORT size_t __cdecl GetSampleDuration() const noexcept;
        // Returns the duration in samples

        HDXTK12_EXPORT size_t __cdecl GetSampleDurationMS() const noexcept;
        // Returns the duration in milliseconds

        HDXTK12_EXPORT const WAVEFORMATEX* __cdecl GetFormat() const noexcept;

    #ifdef USING_XAUDIO2_9
        HDXTK12_EXPORT bool __cdecl FillSubmitBuffer(_Out_ XAUDIO2_BUFFER& buffer, _Out_ XAUDIO2_BUFFER_WMA& wmaBuffer) const;
    #else
        HDXTK12_EXPORT void __cdecl FillSubmitBuffer(_Out_ XAUDIO2_BUFFER& buffer) const;
    #endif
    ...
    struct AudioListener : public X3DAUDIO_LISTENER
    {
        ...
        HDXTK12_EXPORT void __cdecl SetCone(const X3DAUDIO_CONE& listenerCone);

        HDXTK12_EXPORT bool __cdecl IsValid() const;
    };
    ...
    struct AudioEmitter : public X3DAUDIO_EMITTER
    {
        ...
        HDXTK12_EXPORT void __cdecl SetCone(const X3DAUDIO_CONE& emitterCone);

        // Set multi-channel emitter azimuths based on speaker configuration geometry.
        HDXTK12_EXPORT void __cdecl EnableDefaultMultiChannel(unsigned int channels, float radius = 1.f);

        // Set default volume, LFE, LPF, and reverb curves.
        HDXTK12_EXPORT void __cdecl EnableDefaultCurves() noexcept;
        HDXTK12_EXPORT void __cdecl EnableLinearCurves() noexcept;
    ...
    {
    public:
        HDXTK12_EXPORT SoundEffectInstance(SoundEffectInstance&&) noexcept;
        HDXTK12_EXPORT SoundEffectInstance& operator= (SoundEffectInstance&&) noexcept;

        SoundEffectInstance(SoundEffectInstance const&) = delete;
        SoundEffectInstance& operator= (SoundEffectInstance const&) = delete;

        HDXTK12_EXPORT virtual ~SoundEffectInstance();

        HDXTK12_EXPORT void __cdecl Play(bool loop = false);
        HDXTK12_EXPORT void __cdecl Stop(bool immediate = true) noexcept;
        HDXTK12_EXPORT void __cdecl Pause() noexcept;
        HDXTK12_EXPORT void __cdecl Resume();

        HDXTK12_EXPORT void __cdecl SetVolume(float volume);
        HDXTK12_EXPORT void __cdecl SetPitch(float pitch);
        HDXTK12_EXPORT void __cdecl SetPan(float pan);

        HDXTK12_EXPORT void __cdecl Apply3D(const X3DAUDIO_LISTENER& listener, const X3DAUDIO_EMITTER& emitter, bool rhcoords = true);

        HDXTK12_EXPORT bool __cdecl IsLooped() const noexcept;

        HDXTK12_EXPORT SoundState __cdecl GetState() noexcept;

        HDXTK12_EXPORT unsigned int __cdecl GetChannelCount() const noexcept;

        HDXTK12_EXPORT IVoiceNotify* __cdecl GetVoiceNotify() const noexcept;
    ...
    private:
        ...
        HDXTK12_EXPORT SoundEffectInstance(_In_ AudioEngine* engine, _In_ SoundEffect* effect, SOUND_EFFECT_INSTANCE_FLAGS flags);
        HDXTK12_EXPORT SoundEffectInstance(_In_ AudioEngine* engine, _In_ WaveBank* effect, unsigned int index, SOUND_EFFECT_INSTANCE_FLAGS flags);

        HDXTK12_EXPORT friend std::unique_ptr<SoundEffectInstance> __cdecl SoundEffect::CreateInstance(SOUND_EFFECT_INSTANCE_FLAGS);
        HDXTK12_EXPORT friend std::unique_ptr<SoundEffectInstance> __cdecl WaveBank::CreateInstance(unsigned int, SOUND_EFFECT_INSTANCE_FLAGS);
    };
    ...
    {
    public:
        HDXTK12_EXPORT SoundStreamInstance(SoundStreamInstance&&) noexcept;
        HDXTK12_EXPORT SoundStreamInstance& operator= (SoundStreamInstance&&) noexcept;

        SoundStreamInstance(SoundStreamInstance const&) = delete;
        SoundStreamInstance& operator= (SoundStreamInstance const&) = delete;

        HDXTK12_EXPORT virtual ~SoundStreamInstance();
    ...
    private:
        ...
        // Private constructors
        HDXTK12_EXPORT SoundStreamInstance(_In_ AudioEngine* engine, _In_ WaveBank* effect, unsigned int index, SOUND_EFFECT_INSTANCE_FLAGS flags);

        HDXTK12_EXPORT friend std::unique_ptr<SoundStreamInstance> __cdecl WaveBank::CreateStreamInstance(unsigned int, SOUND_EFFECT_INSTANCE_FLAGS);
    };
    ...
    {
    public:
        HDXTK12_EXPORT DynamicSoundEffectInstance(_In_ AudioEngine* engine,
            _In_ std::function<void __cdecl(DynamicSoundEffectInstance*)> bufferNeeded,
            int sampleRate, int channels, int sampleBits = 16,
            SOUND_EFFECT_INSTANCE_FLAGS flags = SoundEffectInstance_Default);

        HDXTK12_EXPORT DynamicSoundEffectInstance(DynamicSoundEffectInstance&&) noexcept;
        HDXTK12_EXPORT DynamicSoundEffectInstance& operator= (DynamicSoundEffectInstance&&) noexcept;
        ...
        HDXTK12_EXPORT virtual ~DynamicSoundEffectInstance();
        ...
        HDXTK12_EXPORT void __cdecl SubmitBuffer(_In_reads_bytes_(audioBytes) const uint8_t* pAudioData, size_t audioBytes);
        HDXTK12_EXPORT void __cdecl SubmitBuffer(_In_reads_bytes_(audioBytes) const uint8_t* pAudioData, uint32_t offset, size_t audioBytes);

        HDXTK12_EXPORT SoundState __cdecl GetState() noexcept;

        HDXTK12_EXPORT size_t __cdecl GetSampleDuration(size_t bytes) const noexcept;
        // Returns duration in samples of a buffer of a given size

        HDXTK12_EXPORT size_t __cdecl GetSampleDurationMS(size_t bytes) const noexcept;
        // Returns duration in milliseconds of a buffer of a given size

        HDXTK12_EXPORT size_t __cdecl GetSampleSizeInBytes(uint64_t duration) const noexcept;
        // Returns size of a buffer for a duration given in milliseconds

        HDXTK12_EXPORT int __cdecl GetPendingBufferCount() const noexcept;
