...
namespace DirectX
{
    ...
    {
    public:
        HDXTK12_EXPORT GamePad() noexcept(false);

        HDXTK12_EXPORT GamePad(GamePad&&) noexcept;
        HDXTK12_EXPORT GamePad& operator= (GamePad&&) noexcept;

        GamePad(GamePad const&) = delete;
        GamePad& operator=(GamePad const&) = delete;

        HDXTK12_EXPORT virtual ~GamePad();
        ...
        #pragma prefast(pop)
            ...
            HDXTK12_EXPORT void __cdecl Update(const State& state) noexcept;

            HDXTK12_EXPORT void __cdecl Reset() noexcept;

            HDXTK12_EXPORT State __cdecl GetLastState() const noexcept { return lastState; }
        ...
        HDXTK12_EXPORT State __cdecl GetState(int player, DeadZone deadZoneMode = DEAD_ZONE_INDEPENDENT_AXES);

        // Retrieve the current capabilities of the gamepad of the associated player index
        HDXTK12_EXPORT Capabilities __cdecl GetCapabilities(int player);

        // Set the vibration motor speeds of the gamepad
        HDXTK12_EXPORT bool __cdecl SetVibration(int player, float leftMotor, float rightMotor, float leftTrigger = 0.f, float rightTrigger = 0.f) noexcept;

        // Handle suspending/resuming
        HDXTK12_EXPORT void __cdecl Suspend() noexcept;
        HDXTK12_EXPORT void __cdecl Resume() noexcept;

    #ifdef USING_GAMEINPUT
        HDXTK12_EXPORT void __cdecl RegisterEvents(void* ctrlChanged) noexcept;

        // Underlying device access
        _Success_(return)
            HDXTK12_EXPORT bool __cdecl GetDevice(int player, _Outptr_ IGameInputDevice * *device) noexcept;
    #elif defined(USING_WINDOWS_GAMING_INPUT) || defined(_XBOX_ONE)
        HDXTK12_EXPORT void __cdecl RegisterEvents(void* ctrlChanged, void* userChanged) noexcept;
    #endif

        // Singleton
        HDXTK12_EXPORT static GamePad& __cdecl Get();
