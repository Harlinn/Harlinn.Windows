...
namespace DirectX
{
    inline namespace DX12
    {
        namespace Private
        {
            ...
            {
            protected:
                HDXTK12_EXPORT PrimitiveBatchBase(_In_ ID3D12Device* device, size_t maxIndices, size_t maxVertices, size_t vertexSize);

                HDXTK12_EXPORT PrimitiveBatchBase(PrimitiveBatchBase&&) noexcept;
                HDXTK12_EXPORT PrimitiveBatchBase& operator= (PrimitiveBatchBase&&) noexcept;

                PrimitiveBatchBase(PrimitiveBatchBase const&) = delete;
                PrimitiveBatchBase& operator= (PrimitiveBatchBase const&) = delete;

                HDXTK12_EXPORT virtual ~PrimitiveBatchBase();

            public:
                // Begin/End a batch of primitive drawing operations.
                HDXTK12_EXPORT void __cdecl Begin(_In_ ID3D12GraphicsCommandList* cmdList);
                HDXTK12_EXPORT void __cdecl End();

            protected:
                // Internal, untyped drawing method.
                HDXTK12_EXPORT void __cdecl Draw(D3D_PRIMITIVE_TOPOLOGY topology, bool isIndexed, _In_opt_count_(indexCount) uint16_t const* indices, size_t indexCount, size_t vertexCount, _Outptr_ void** pMappedVertices);
