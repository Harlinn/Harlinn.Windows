...
HAOM_EXPORT void od_ec_enc_init(od_ec_enc *enc, uint32_t size) {
  od_ec_enc_reset(enc);
...
HAOM_EXPORT void od_ec_enc_reset(od_ec_enc *enc) {
  enc->offs = 0;
  enc->low = 0;
  enc->rng = 0x8000;
...
HAOM_EXPORT void od_ec_enc_clear(od_ec_enc *enc) { free(enc->buf); }
...
HAOM_EXPORT void od_ec_encode_bool_q15(od_ec_enc *enc, int val, unsigned f) {
  od_ec_enc_window l;
  unsigned r;
  unsigned v;
  assert(0 < f);
...
HAOM_EXPORT void od_ec_encode_cdf_q15(od_ec_enc *enc, int s, const uint16_t *icdf,
                          int nsyms) {
  (void)nsyms;
  assert(s >= 0);
  assert(s < nsyms);
...
/*Indicates that there are no more symbols to encode.
  ...
  Return: A pointer to the start of the final buffer, or NULL if there was an
           encoding error.*/
HAOM_EXPORT unsigned char *od_ec_enc_done(od_ec_enc *enc, uint32_t *nbytes) {
  unsigned char *out;
  uint32_t storage;
  uint32_t offs;
  od_ec_enc_window m;
...
HAOM_EXPORT int od_ec_enc_tell(const od_ec_enc *enc) {
...
/*Returns the number of bits "used" by the encoded symbols so far.
  ...
  Return: The number of bits scaled by 2**OD_BITRES.
          This will always be slightly larger than the exact value (e.g., all
           rounding error is in the positive direction).*/
HAOM_EXPORT uint32_t od_ec_enc_tell_frac(const od_ec_enc *enc) {
