...
HAOM_EXPORT void die(const char *fmt, ...) {
  LOG_ERROR(NULL);
  usage_exit();
}

HAOM_EXPORT void fatal(const char *fmt, ...) {
  LOG_ERROR("Fatal");
  exit(EXIT_FAILURE);
}

HAOM_EXPORT void aom_tools_warn(const char *fmt, ...) { LOG_ERROR("Warning"); }

HAOM_EXPORT void die_codec(aom_codec_ctx_t *ctx, const char *s) {
...
HAOM_EXPORT const char *get_short_name_by_aom_encoder(aom_codec_iface_t *iface) {
  for (int i = 0; i < get_aom_encoder_count(); ++i) {
    const struct CodecInfo *info = &aom_encoders[i];
    if (info->interface() == iface) {
...
HAOM_EXPORT const char *get_short_name_by_aom_decoder(aom_codec_iface_t *iface) {
  for (int i = 0; i < get_aom_decoder_count(); ++i) {
    const struct CodecInfo *info = &aom_decoders[i];
    if (info->interface() == iface) {
      return info->short_name;
...
HAOM_EXPORT double sse_to_psnr(double samples, double peak, double sse) {
...
HAOM_EXPORT void aom_img_upshift(aom_image_t *dst, const aom_image_t *src,
                     int input_shift) {
