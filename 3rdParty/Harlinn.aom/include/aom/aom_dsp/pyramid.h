...
HAOM_EXPORT size_t aom_get_pyramid_alloc_size(int width, int height, bool image_is_16bit);

HAOM_EXPORT ImagePyramid *aom_alloc_pyramid(int width, int height, bool image_is_16bit);
...
// or -1 on error.
HAOM_EXPORT int aom_compute_pyramid(const YV12_BUFFER_CONFIG *frame, int bit_depth,
...
HAOM_EXPORT bool aom_is_pyramid_valid(ImagePyramid *pyr, int n_levels);
#endif

// Mark a pyramid as no longer containing valid data.
// This must be done whenever the corresponding frame buffer is reused
HAOM_EXPORT void aom_invalidate_pyramid(ImagePyramid *pyr);

// Release the memory associated with a pyramid
HAOM_EXPORT void aom_free_pyramid(ImagePyramid *pyr);
