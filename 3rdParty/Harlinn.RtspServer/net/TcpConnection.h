...
public:
	...
	HW_RTSPSERVER_EXPORT TcpConnection(TaskScheduler *task_scheduler, SOCKET sockfd);
	HW_RTSPSERVER_EXPORT virtual ~TcpConnection();
	...
	HW_RTSPSERVER_EXPORT void Send(std::shared_ptr<char> data, uint32_t size);
	HW_RTSPSERVER_EXPORT void Send(const char *data, uint32_t size);
    
	HW_RTSPSERVER_EXPORT void Disconnect();
...
protected:
	...
	HW_RTSPSERVER_EXPORT virtual void HandleRead();
	HW_RTSPSERVER_EXPORT virtual void HandleWrite();
	HW_RTSPSERVER_EXPORT virtual void HandleClose();
	HW_RTSPSERVER_EXPORT virtual void HandleError();
