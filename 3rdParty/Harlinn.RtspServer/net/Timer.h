...
{
public:
	HW_RTSPSERVER_EXPORT TimerId AddTimer(const TimerEvent& event, uint32_t msec);
	HW_RTSPSERVER_EXPORT void RemoveTimer(TimerId timerId);

	HW_RTSPSERVER_EXPORT int64_t GetTimeRemaining();
	HW_RTSPSERVER_EXPORT void HandleTimerEvent();

private:
	HW_RTSPSERVER_EXPORT int64_t GetTimeNow();
