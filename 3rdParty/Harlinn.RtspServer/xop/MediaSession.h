...
public:
	...
	HW_RTSPSERVER_EXPORT static MediaSession* CreateNew(std::string url_suffix="live");
	HW_RTSPSERVER_EXPORT virtual ~MediaSession();

	HW_RTSPSERVER_EXPORT bool AddSource(MediaChannelId channel_id, MediaSource* source);
	HW_RTSPSERVER_EXPORT bool RemoveSource(MediaChannelId channel_id);

	HW_RTSPSERVER_EXPORT bool StartMulticast();

	HW_RTSPSERVER_EXPORT void AddNotifyConnectedCallback(const NotifyConnectedCallback& callback);
	HW_RTSPSERVER_EXPORT void AddNotifyDisconnectedCallback(const NotifyDisconnectedCallback& callback);
	...
	HW_RTSPSERVER_EXPORT std::string GetSdpMessage(std::string ip, std::string session_name ="");

	HW_RTSPSERVER_EXPORT MediaSource* GetMediaSource(MediaChannelId channel_id);

	HW_RTSPSERVER_EXPORT bool HandleFrame(MediaChannelId channel_id, AVFrame frame);

	HW_RTSPSERVER_EXPORT bool AddClient(SOCKET rtspfd, std::shared_ptr<RtpConnection> rtp_conn);
	HW_RTSPSERVER_EXPORT void RemoveClient(SOCKET rtspfd);
