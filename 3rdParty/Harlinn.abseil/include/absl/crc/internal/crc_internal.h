...
class CRCImpl : public CRC {  // Implemention of the abstract class CRC
 public:
  ...
  ABSEIL_EXPORT static CRCImpl* NewInternal();

  ABSEIL_EXPORT void Empty(uint32_t* crc) const override;
  ...
  // is set.
  ABSEIL_EXPORT static void FillWordTable(uint32_t poly, uint32_t last, int word_size,
                            Uint32By256* t);
  ...
  ABSEIL_EXPORT static int FillZeroesTable(uint32_t poly, Uint32By256* t);
...
class CRC32 : public CRCImpl {
 public:
  ...
  ABSEIL_EXPORT void Extend(uint32_t* crc, const void* bytes, size_t length) const override;
  ABSEIL_EXPORT void ExtendByZeroes(uint32_t* crc, size_t length) const override;
  ABSEIL_EXPORT void Scramble(uint32_t* crc) const override;
  ABSEIL_EXPORT void Unscramble(uint32_t* crc) const override;
  ABSEIL_EXPORT void UnextendByZeroes(uint32_t* crc, size_t length) const override;

  ABSEIL_EXPORT void InitTables() override;
 ...
 private:
  ...
  ABSEIL_EXPORT void ExtendByZeroesImpl(uint32_t* crc, size_t length,
...
ABSEIL_EXPORT CRCImpl* TryNewCRC32AcceleratedX86ARMCombined();

// Return all possible hardware accelerated implementations. For testing only.
ABSEIL_EXPORT std::vector<std::unique_ptr<CRCImpl>> NewCRC32AcceleratedX86ARMCombinedAll();
