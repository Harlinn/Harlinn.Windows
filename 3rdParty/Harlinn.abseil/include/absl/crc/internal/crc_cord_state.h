...
class CrcCordState {
 public:
  // Constructors.
  ABSEIL_EXPORT CrcCordState();
  ABSEIL_EXPORT CrcCordState(const CrcCordState&);
  ABSEIL_EXPORT CrcCordState(CrcCordState&&);

  // Destructor. Atomically unreferences the data.
  ABSEIL_EXPORT ~CrcCordState();

  // Copy and move operators.
  ABSEIL_EXPORT CrcCordState& operator=(const CrcCordState&);
  ABSEIL_EXPORT CrcCordState& operator=(CrcCordState&&);
  ...
  ABSEIL_EXPORT absl::crc32c_t Checksum() const;
  ...
  ABSEIL_EXPORT PrefixCrc NormalizedPrefixCrcAtNthChunk(size_t n) const;

  // Poisons all chunks to so that Checksum() will likely be incorrect with high
  // probability.
  ABSEIL_EXPORT void Poison();
 ...
 private:
  ...
  ABSEIL_EXPORT static RefcountedRep* RefSharedEmptyRep();
