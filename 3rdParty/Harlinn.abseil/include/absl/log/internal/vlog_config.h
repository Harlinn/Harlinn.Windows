...
ABSEIL_EXPORT int RegisterAndInitialize(VLogSite* v);
ABSEIL_EXPORT void UpdateVLogSites();
...
class VLogSite final {
 ...
 private:
  ...
  ABSEIL_EXPORT bool SlowIsEnabled(int stale_v, int level);
  ABSL_ATTRIBUTE_NOINLINE ABSEIL_EXPORT bool SlowIsEnabled0(int stale_v);
  ABSL_ATTRIBUTE_NOINLINE ABSEIL_EXPORT bool SlowIsEnabled1(int stale_v);
  ABSL_ATTRIBUTE_NOINLINE ABSEIL_EXPORT bool SlowIsEnabled2(int stale_v);
  ABSL_ATTRIBUTE_NOINLINE ABSEIL_EXPORT bool SlowIsEnabled3(int stale_v);
  ABSL_ATTRIBUTE_NOINLINE ABSEIL_EXPORT bool SlowIsEnabled4(int stale_v);
  ABSL_ATTRIBUTE_NOINLINE ABSEIL_EXPORT bool SlowIsEnabled5(int stale_v);
...
ABSEIL_EXPORT int VLogLevel(absl::string_view file);
...
// Allocates memory.
ABSEIL_EXPORT void UpdateVModule(absl::string_view vmodule);

// Updates the global verbosity level to `v` and returns the prior value.
// Allocates memory.
ABSEIL_EXPORT int UpdateGlobalVLogLevel(int v);
...
// `global_v` otherwise.
// Allocates memory.
ABSEIL_EXPORT int PrependVModule(absl::string_view module_pattern, int log_level);

// Registers `on_update` to be called whenever `v` or `vmodule` change.
// Allocates memory.
ABSEIL_EXPORT void OnVLogVerbosityUpdate(std::function<void()> cb);

// Does not allocate memory.
ABSEIL_EXPORT VLogSite* SetVModuleListHeadForTestOnly(VLogSite* v);
