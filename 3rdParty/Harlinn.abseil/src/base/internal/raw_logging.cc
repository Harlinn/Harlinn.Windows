...
ABSEIL_EXPORT void AsyncSignalSafeWriteError(const char* s, size_t len) {
  if (!len) return;
...
ABSEIL_EXPORT void RawLog(absl::LogSeverity severity, const char* file, int line,
            const char* format, ...) {
  va_list ap;
  va_start(ap, format);
...
ABSEIL_EXPORT bool RawLoggingFullySupported() {
#ifdef ABSL_LOW_LEVEL_WRITE_SUPPORTED
  return true;
...
ABSEIL_EXPORT void RegisterLogFilterAndPrefixHook(LogFilterAndPrefixHook func) {
  log_filter_and_prefix_hook.Store(func);
}

ABSEIL_EXPORT void RegisterAbortHook(AbortHook func) { abort_hook.Store(func); }

ABSEIL_EXPORT void RegisterInternalLogFunction(InternalLogFunction func) {
