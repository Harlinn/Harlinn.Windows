...
ABSEIL_EXPORT bool ThreadIsLoggingToLogSink() { return ThreadIsLoggingStatus(); }

ABSEIL_EXPORT void LogToSinks(const absl::LogEntry& entry,
                absl::Span<absl::LogSink*> extra_sinks, bool extra_sinks_only) {
  log_internal::GlobalSinks().LogToSinks(entry, extra_sinks, extra_sinks_only);
}

ABSEIL_EXPORT void AddLogSink(absl::LogSink* sink) {
  log_internal::GlobalSinks().AddLogSink(sink);
}

ABSEIL_EXPORT void RemoveLogSink(absl::LogSink* sink) {
  log_internal::GlobalSinks().RemoveLogSink(sink);
}

ABSEIL_EXPORT void FlushLogSinks() { log_internal::GlobalSinks().FlushLogSinks(); }
