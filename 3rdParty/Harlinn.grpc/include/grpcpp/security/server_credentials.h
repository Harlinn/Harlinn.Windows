...
GRPC_EXPORT std::shared_ptr<ServerCredentials> XdsServerCredentials(
    const std::shared_ptr<ServerCredentials>& fallback_credentials);
}  // namespace experimental
...
class ServerCredentials : private grpc::GrpcLibraryCodegen {
 public:
  GRPC_EXPORT ServerCredentials();
  GRPC_EXPORT ~ServerCredentials() override;

  /// This method is not thread-safe and has to be called before the server is
  /// started. The last call to this function wins.
  GRPC_EXPORT virtual void SetAuthMetadataProcessor(
 ...
 private:
  ...
  GRPC_EXPORT virtual int AddPortToServer(const std::string& addr, grpc_server* server) = 0;
  ...
  GRPC_EXPORT virtual bool IsInsecure() const { return false; }
...
GRPC_EXPORT std::shared_ptr<ServerCredentials> SslServerCredentials(
    const grpc::SslServerCredentialsOptions& options);

GRPC_EXPORT std::shared_ptr<ServerCredentials> InsecureServerCredentials();
...
GRPC_EXPORT std::shared_ptr<ServerCredentials> AltsServerCredentials(
...
GRPC_EXPORT std::shared_ptr<ServerCredentials> LocalServerCredentials(
    grpc_local_connect_type type);

/// Builds TLS ServerCredentials given TLS options.
GRPC_EXPORT std::shared_ptr<ServerCredentials> TlsServerCredentials(
