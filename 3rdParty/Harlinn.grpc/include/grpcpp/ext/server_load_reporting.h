...
class LoadReportingServiceServerBuilderOption
 ...
 public:
  GRPC_EXPORT void UpdateArguments(::grpc::ChannelArguments* args) override;
  GRPC_EXPORT void UpdatePlugins(std::vector<std::unique_ptr<::grpc::ServerBuilderPlugin>>*
                         plugins) override;
};

// Adds the load reporting cost with \a cost_name and \a cost_value in the
// trailing metadata of the server context.
GRPC_EXPORT void AddLoadReportingCost(grpc::ServerContext* ctx,
