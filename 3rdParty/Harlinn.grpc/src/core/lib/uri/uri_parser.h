...
class URI {
 public:
  ...
  GRPC_EXPORT static absl::StatusOr<URI> Parse(absl::string_view uri_text);
  // Explicit construction by individual URI components
  GRPC_EXPORT URI(std::string scheme, std::string authority, std::string path,
      std::vector<QueryParam> query_parameter_pairs, std::string fragment_);
  URI() = default;
  // Copy construction and assignment
  GRPC_EXPORT URI(const URI& other);
  GRPC_EXPORT URI& operator=(const URI& other);
