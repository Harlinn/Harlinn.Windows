...
class BackOff {
 public:
  ...
  GRPC_EXPORT explicit BackOff(const Options& options);

  /// Returns the time at which the next attempt should start.
  GRPC_EXPORT grpc_millis NextAttemptTime();
  ...
  GRPC_EXPORT void Reset();

  GRPC_EXPORT void SetRandomSeed(unsigned int seed);
