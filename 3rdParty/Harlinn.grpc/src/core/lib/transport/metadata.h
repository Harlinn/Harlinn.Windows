...
GRPC_EXPORT extern grpc_core::DebugOnlyTraceFlag grpc_trace_metadata;
...
GRPC_EXPORT grpc_mdelem grpc_mdelem_from_slices(const grpc_slice& key,
                                    const grpc_slice& value);
...
GRPC_EXPORT grpc_mdelem grpc_mdelem_from_slices(const grpc_core::StaticMetadataSlice& key,
                                    const grpc_slice& value);

/* Like grpc_mdelem_from_slices, but key is static and val is static. */
GRPC_EXPORT grpc_mdelem grpc_mdelem_from_slices(
...
GRPC_EXPORT grpc_mdelem grpc_mdelem_from_slices(const grpc_core::ManagedMemorySlice& key,
                                    const grpc_core::ManagedMemorySlice& value);
...
GRPC_EXPORT grpc_mdelem grpc_mdelem_from_grpc_metadata(grpc_metadata* metadata);
...
GRPC_EXPORT grpc_mdelem grpc_mdelem_create(
...
GRPC_EXPORT bool grpc_mdelem_eq(grpc_mdelem a, grpc_mdelem b);
...
GRPC_EXPORT void* grpc_mdelem_get_user_data(grpc_mdelem md, void (*if_destroy_func)(void*));
GRPC_EXPORT void* grpc_mdelem_set_user_data(grpc_mdelem md, void (*destroy_func)(void*),
                                void* data);
...
#ifndef NDEBUG
GRPC_EXPORT void grpc_mdelem_trace_ref(void* md, const grpc_slice& key,
                           const grpc_slice& value, intptr_t refcnt,
                           const char* file, int line);
GRPC_EXPORT void grpc_mdelem_trace_unref(void* md, const grpc_slice& key,
                             const grpc_slice& value, intptr_t refcnt,
                             const char* file, int line);
#endif
namespace grpc_core {
...
class RefcountedMdBase {
 ...
 protected:
#ifndef NDEBUG
  GRPC_EXPORT void TraceAtStart(const char* tag);
#endif
...
class InternedMetadata : public RefcountedMdBase {
 public:
  ...
  GRPC_EXPORT InternedMetadata(const grpc_slice& key, const grpc_slice& value,
                   uint32_t hash, InternedMetadata* next);
  GRPC_EXPORT InternedMetadata(const grpc_slice& key, const grpc_slice& value,
                   uint32_t hash, InternedMetadata* next, const NoRefKey*);

  GRPC_EXPORT ~InternedMetadata();
  GRPC_EXPORT void RefWithShardLocked(mdtab_shard* shard);
  ...
  GRPC_EXPORT static size_t CleanupLinkedMetadata(BucketLink* head);
...
class AllocatedMetadata : public RefcountedMdBase {
 public:
  // TODO(arjunroy): Change to use strongly typed slices instead.
  struct NoRefKey {};
  GRPC_EXPORT AllocatedMetadata(const grpc_slice& key, const grpc_slice& value);
  GRPC_EXPORT AllocatedMetadata(const grpc_core::ManagedMemorySlice& key,
                    const grpc_core::UnmanagedMemorySlice& value);
  GRPC_EXPORT AllocatedMetadata(const grpc_core::ExternallyManagedSlice& key,
                    const grpc_core::UnmanagedMemorySlice& value);
  GRPC_EXPORT AllocatedMetadata(const grpc_slice& key, const grpc_slice& value,
                    const NoRefKey*);
  GRPC_EXPORT ~AllocatedMetadata();
...
GRPC_EXPORT void grpc_mdelem_on_final_unref(grpc_mdelem_data_storage storage, void* ptr,
...
GRPC_EXPORT void grpc_mdctx_global_init(void);
GRPC_EXPORT void grpc_mdctx_global_shutdown();
