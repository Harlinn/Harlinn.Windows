...
class BaseNode : public RefCounted<BaseNode> {
 ...
 protected:
  GRPC_EXPORT BaseNode(EntityType type, std::string name);

 public:
     GRPC_EXPORT ~BaseNode() override;
  ...
  // caller.
  GRPC_EXPORT std::string RenderJsonString();
...
class CallCountingHelper {
 public:
  GRPC_EXPORT CallCountingHelper();

  GRPC_EXPORT void RecordCallStarted();
  GRPC_EXPORT void RecordCallFailed();
  GRPC_EXPORT void RecordCallSucceeded();

  // Common rendering of the call count data and last_call_started_timestamp.
  GRPC_EXPORT void PopulateCallCounts(Json::Object* json);
 ...
 private:
  ...
  GRPC_EXPORT void CollectData(CounterData* out);
...
class ChannelNode : public BaseNode {
 public:
  GRPC_EXPORT ChannelNode(std::string target, size_t channel_tracer_max_nodes,
              bool is_internal_channel);

  // Returns the string description of the given connectivity state.
  GRPC_EXPORT static const char* GetChannelConnectivityStateChangeString(
      grpc_connectivity_state state);

  GRPC_EXPORT Json RenderJson() override;
  ...
  GRPC_EXPORT void SetConnectivityState(grpc_connectivity_state state);

  // TODO(roth): take in a RefCountedPtr to the child channel so we can retrieve
  // the human-readable name.
  GRPC_EXPORT void AddChildChannel(intptr_t child_uuid);
  GRPC_EXPORT void RemoveChildChannel(intptr_t child_uuid);

  // TODO(roth): take in a RefCountedPtr to the child subchannel so we can
  // retrieve the human-readable name.
  GRPC_EXPORT void AddChildSubchannel(intptr_t child_uuid);
  GRPC_EXPORT void RemoveChildSubchannel(intptr_t child_uuid);

 private:
  // Allows the channel trace test to access trace_.
  friend class testing::ChannelNodePeer;

  GRPC_EXPORT void PopulateChildRefs(Json::Object* json);
...
class ServerNode : public BaseNode {
 public:
  GRPC_EXPORT explicit ServerNode(size_t channel_tracer_max_nodes);

  GRPC_EXPORT ~ServerNode() override;

  GRPC_EXPORT Json RenderJson() override;

  GRPC_EXPORT std::string RenderServerSockets(intptr_t start_socket_id,
                                  intptr_t max_results);

  GRPC_EXPORT void AddChildSocket(RefCountedPtr<SocketNode> node);

  GRPC_EXPORT void RemoveChildSocket(intptr_t child_uuid);

  GRPC_EXPORT void AddChildListenSocket(RefCountedPtr<ListenSocketNode> node);

  GRPC_EXPORT void RemoveChildListenSocket(intptr_t child_uuid);
...
class SocketNode : public BaseNode {
 public:
  struct Security : public RefCounted<Security> {
    ...
    GRPC_EXPORT Json RenderJson();

    GRPC_EXPORT grpc_arg MakeChannelArg() const;

    GRPC_EXPORT static RefCountedPtr<Security> GetFromChannelArgs(
        const grpc_channel_args* args);
  };

  GRPC_EXPORT SocketNode(std::string local, std::string remote, std::string name,
             RefCountedPtr<Security> security);
  ~SocketNode() override {}

  GRPC_EXPORT Json RenderJson() override;

  GRPC_EXPORT void RecordStreamStartedFromLocal();
  GRPC_EXPORT void RecordStreamStartedFromRemote();
  ...
  GRPC_EXPORT void RecordMessagesSent(uint32_t num_sent);
  GRPC_EXPORT void RecordMessageReceived();
...
class ListenSocketNode : public BaseNode {
 public:
  GRPC_EXPORT ListenSocketNode(std::string local_addr, std::string name);
