...
class ChannelTrace {
 public:
  GRPC_EXPORT explicit ChannelTrace(size_t max_event_memory);
  GRPC_EXPORT ~ChannelTrace();
  ...
  // slice.
  GRPC_EXPORT void AddTraceEvent(Severity severity, const grpc_slice& data);
  ...
  GRPC_EXPORT void AddTraceEventWithReference(Severity severity, const grpc_slice& data,
                                  RefCountedPtr<BaseNode> referenced_entity);

  // Creates and returns the raw Json object, so a parent channelz
  // object may incorporate the json before rendering.
  GRPC_EXPORT Json RenderJson() const;

 private:
  GRPC_EXPORT friend size_t testing::GetSizeofTraceEvent(void);
  ...
  class TraceEvent {
   public:
    ...
    GRPC_EXPORT TraceEvent(Severity severity, const grpc_slice& data,
    ...
    GRPC_EXPORT Json RenderTraceEvent() const;
  ...
  GRPC_EXPORT void AddTraceEventHelper(TraceEvent* new_trace_event);
