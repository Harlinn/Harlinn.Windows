...
class ChannelzRegistry {
 public:
  ...
  GRPC_EXPORT static void Init();

  // To be called in grpc_shutdown();
  GRPC_EXPORT static void Shutdown();
 ...
 private:
  ...
  GRPC_EXPORT static ChannelzRegistry* Default();

  // globally registers an Entry. Returns its unique uuid
  GRPC_EXPORT void InternalRegister(BaseNode* node);
  ...
  GRPC_EXPORT void InternalUnregister(intptr_t uuid);

  // if object with uuid has previously been registered as the correct type,
  // returns the void* associated with that uuid. Else returns nullptr.
  GRPC_EXPORT RefCountedPtr<BaseNode> InternalGet(intptr_t uuid);

  GRPC_EXPORT std::string InternalGetTopChannels(intptr_t start_channel_id);
  GRPC_EXPORT std::string InternalGetServers(intptr_t start_server_id);

  GRPC_EXPORT void InternalLogAllEntities();
