...
GRPC_EXPORT gpr_timespec ToGprTimeSpec(absl::Duration duration);

// Converts absl::Time to gpr_timespec(GPR_CLOCK_REALTIME)
GRPC_EXPORT gpr_timespec ToGprTimeSpec(absl::Time time);

// Converts gpr_timespec(GPR_TIMESPAN) to absl::Duration
GRPC_EXPORT absl::Duration ToAbslDuration(gpr_timespec ts);

// Converts gpr_timespec(GPR_CLOCK_[MONOTONIC|REALTIME|PRECISE]) to absl::Time
GRPC_EXPORT absl::Time ToAbslTime(gpr_timespec ts);
