...
GRPC_EXPORT absl::Status StatusCreate(
...
GRPC_EXPORT void StatusSetInt(absl::Status* status, StatusIntProperty key, intptr_t value);

/// Gets the int property from the status
GRPC_EXPORT absl::optional<intptr_t> StatusGetInt(
    const absl::Status& status, StatusIntProperty key) GRPC_MUST_USE_RESULT;

/// Sets the str property to the status
GRPC_EXPORT void StatusSetStr(absl::Status* status, StatusStrProperty key,
                  absl::string_view value);

/// Gets the str property from the status
GRPC_EXPORT absl::optional<std::string> StatusGetStr(
    const absl::Status& status, StatusStrProperty key) GRPC_MUST_USE_RESULT;

/// Sets the time property to the status
GRPC_EXPORT void StatusSetTime(absl::Status* status, StatusTimeProperty key,
                   absl::Time time);

/// Gets the time property from the status
GRPC_EXPORT absl::optional<absl::Time> StatusGetTime(
    const absl::Status& status, StatusTimeProperty key) GRPC_MUST_USE_RESULT;

/// Adds a child status to status
GRPC_EXPORT void StatusAddChild(absl::Status* status, absl::Status child);

/// Returns all children status from a status
GRPC_EXPORT std::vector<absl::Status> StatusGetChildren(absl::Status status)
...
GRPC_EXPORT std::string StatusToString(const absl::Status& status) GRPC_MUST_USE_RESULT;

namespace internal {

/// Builds a upb message, google_rpc_Status from a status
/// This is for internal implementation & test only
GRPC_EXPORT google_rpc_Status* StatusToProto(absl::Status status,
                                 upb_arena* arena) GRPC_MUST_USE_RESULT;

/// Builds a status from a upb message, google_rpc_Status
/// This is for internal implementation & test only
GRPC_EXPORT absl::Status StatusFromProto(google_rpc_Status* msg) GRPC_MUST_USE_RESULT;
...
GRPC_EXPORT uintptr_t StatusAllocPtr(absl::Status s);

/// Frees the allocated status at ptr.
/// This shouldn't be used except migration purpose.
GRPC_EXPORT void StatusFreePtr(uintptr_t ptr);

/// Get the status from ptr.
/// This shouldn't be used except migration purpose.
GRPC_EXPORT absl::Status StatusGetFromPtr(uintptr_t ptr);
