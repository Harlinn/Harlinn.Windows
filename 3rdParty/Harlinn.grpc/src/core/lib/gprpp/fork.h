...
class Fork {
 public:
  ...
  GRPC_EXPORT static void GlobalInit();
  GRPC_EXPORT static void GlobalShutdown();

  // Returns true if fork suppport is enabled, false otherwise
  GRPC_EXPORT static bool Enabled();
  ...
  GRPC_EXPORT static void SetResetChildPollingEngineFunc(
      child_postfork_func reset_child_polling_engine);
  GRPC_EXPORT static child_postfork_func GetResetChildPollingEngineFunc();
  ...
  //
  GRPC_EXPORT static bool BlockExecCtx();
  GRPC_EXPORT static void AllowExecCtx();

  // Increment the count of active threads.
  GRPC_EXPORT static void IncThreadCount();

  // Decrement the count of active threads.
  GRPC_EXPORT static void DecThreadCount();

  // Await all core threads to be joined.
  GRPC_EXPORT static void AwaitThreads();

  // Test only: overrides environment variables/compile flags
  // Must be called before grpc_init()
  GRPC_EXPORT static void Enable(bool enable);

 private:
  GRPC_EXPORT static void DoIncExecCtxCount();
  GRPC_EXPORT static void DoDecExecCtxCount();

  static internal::ExecCtxState* exec_ctx_state_;
  static internal::ThreadState* thread_state_;
  GRPC_EXPORT static std::atomic<bool> support_enabled_;
