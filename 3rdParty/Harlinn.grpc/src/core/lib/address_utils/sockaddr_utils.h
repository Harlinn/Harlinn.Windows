...
/* Returns true if addr is an IPv4-mapped IPv6 address within the
   ...
   returning true. */
GRPC_EXPORT int grpc_sockaddr_is_v4mapped(const grpc_resolved_address* addr,
...
GRPC_EXPORT int grpc_sockaddr_to_v4mapped(const grpc_resolved_address* addr,
...
GRPC_EXPORT int grpc_sockaddr_is_wildcard(const grpc_resolved_address* addr, int* port_out);

/* Writes 0.0.0.0:port and [::]:port to separate sockaddrs. */
GRPC_EXPORT void grpc_sockaddr_make_wildcards(int port, grpc_resolved_address* wild4_out,
                                  grpc_resolved_address* wild6_out);

/* Writes 0.0.0.0:port. */
GRPC_EXPORT void grpc_sockaddr_make_wildcard4(int port, grpc_resolved_address* wild_out);

/* Writes [::]:port. */
GRPC_EXPORT void grpc_sockaddr_make_wildcard6(int port, grpc_resolved_address* wild_out);

/* Return the IP port number of a sockaddr */
GRPC_EXPORT int grpc_sockaddr_get_port(const grpc_resolved_address* addr);

/* Set IP port number of a sockaddr */
GRPC_EXPORT int grpc_sockaddr_set_port(grpc_resolved_address* addr, int port);
...
GRPC_EXPORT std::string grpc_sockaddr_to_string(const grpc_resolved_address* addr,
...
GRPC_EXPORT grpc_error_handle grpc_string_to_sockaddr(grpc_resolved_address* out,
                                          const char* addr, int port);

/* Returns the URI string corresponding to \a addr */
GRPC_EXPORT std::string grpc_sockaddr_to_uri(const grpc_resolved_address* addr);

/* Returns the URI scheme corresponding to \a addr */
GRPC_EXPORT const char* grpc_sockaddr_get_uri_scheme(const grpc_resolved_address* addr);

GRPC_EXPORT int grpc_sockaddr_get_family(const grpc_resolved_address* resolved_addr);

GRPC_EXPORT std::string grpc_sockaddr_get_packed_host(
    const grpc_resolved_address* resolved_addr);

// Applies a mask of \a mask_bits to IPv4/IPv6 addresses. Has no effect if the
// address type is not IPv4/IPv6.
GRPC_EXPORT void grpc_sockaddr_mask_bits(grpc_resolved_address* address,
                             uint32_t mask_bits);
...
// necessary.
GRPC_EXPORT bool grpc_sockaddr_match_subnet(const grpc_resolved_address* address,
