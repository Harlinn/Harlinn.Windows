...
GRPC_EXPORT char* gpr_dump(const char* buf, size_t len, uint32_t flags);
/* Converts array buf, of length len, into a C string according to the flags.
   The length of the returned buffer is stored in out_len.
   Result should be freed with gpr_free() */
GRPC_EXPORT char* gpr_dump_return_len(const char* buf, size_t len, uint32_t flags,
                          size_t* out_len);

/* Parses an array of bytes into an integer (base 10). Returns 1 on success,
   0 on failure. */
GRPC_EXPORT int gpr_parse_bytes_to_uint32(const char* buf, size_t len, uint32_t* result);
...
GRPC_EXPORT int gpr_ltoa(long value, char* output);
...
GRPC_EXPORT int int64_ttoa(int64_t value, char* output);

// Parses a non-negative number from a value string.  Returns -1 on error.
GRPC_EXPORT int gpr_parse_nonnegative_int(const char* value);

/* Reverse a run of bytes */
GRPC_EXPORT void gpr_reverse_bytes(char* str, int len);

/* Pad a string with flag characters. The given length specifies the minimum
   field width. The input string is never truncated. */
GRPC_EXPORT char* gpr_leftpad(const char* str, char flag, size_t length);

/* Join a set of strings, returning the resulting string.
   Total combined length (excluding null terminator) is returned in final_length
   if it is non-null. */
GRPC_EXPORT char* gpr_strjoin(const char** strs, size_t nstrs, size_t* final_length);

/* Join a set of strings using a separator, returning the resulting string.
   Total combined length (excluding null terminator) is returned in final_length
   if it is non-null. */
GRPC_EXPORT char* gpr_strjoin_sep(const char** strs, size_t nstrs, const char* sep,
                      size_t* final_length);

GRPC_EXPORT void gpr_string_split(const char* input, const char* sep, char*** strs,
                      size_t* nstrs);
...
GRPC_EXPORT std::string gpr_format_timespec(gpr_timespec);

/** Case insensitive string comparison... return <0 if lower(a)<lower(b), ==0 if
    lower(a)==lower(b), >0 if lower(a)>lower(b) */
GRPC_EXPORT int gpr_stricmp(const char* a, const char* b);
GRPC_EXPORT int gpr_strincmp(const char* a, const char* b, size_t n);

GRPC_EXPORT void* gpr_memrchr(const void* s, int c, size_t n);

/* Try to parse given string into a boolean value.
   When parsed successfully, dst will have the value and returns true.
   Otherwise, it returns false. */
GRPC_EXPORT bool gpr_parse_bool_value(const char* value, bool* dst);
