...
class Server : public InternallyRefCounted<Server> {
 public:
  ...
  GRPC_EXPORT explicit Server(const grpc_channel_args* args);
  GRPC_EXPORT ~Server() override;

  GRPC_EXPORT void Orphan() ABSL_LOCKS_EXCLUDED(mu_global_) override;
  ...
  GRPC_EXPORT bool HasOpenConnections() ABSL_LOCKS_EXCLUDED(mu_global_);
  ...
  // the listener.
  GRPC_EXPORT void AddListener(OrphanablePtr<ListenerInterface> listener);

  // Starts listening for connections.
  GRPC_EXPORT void Start() ABSL_LOCKS_EXCLUDED(mu_global_);
  ...
  GRPC_EXPORT grpc_error_handle SetupTransport(
  ...
  GRPC_EXPORT void RegisterCompletionQueue(grpc_completion_queue* cq);
  ...
  GRPC_EXPORT void SetRegisteredMethodAllocator(
      grpc_completion_queue* cq, void* method_tag,
      std::function<RegisteredCallAllocation()> allocator);
  GRPC_EXPORT void SetBatchMethodAllocator(grpc_completion_queue* cq,
                               std::function<BatchCallAllocation()> allocator);

  GRPC_EXPORT RegisteredMethod* RegisterMethod(
      const char* method, const char* host,
      grpc_server_register_method_payload_handling payload_handling,
      uint32_t flags);

  GRPC_EXPORT grpc_call_error RequestCall(grpc_call** call, grpc_call_details* details,
                              grpc_metadata_array* request_metadata,
                              grpc_completion_queue* cq_bound_to_call,
                              grpc_completion_queue* cq_for_notification,
                              void* tag);

  GRPC_EXPORT grpc_call_error RequestRegisteredCall(
  ...
  GRPC_EXPORT void ShutdownAndNotify(grpc_completion_queue* cq, void* tag)
      ABSL_LOCKS_EXCLUDED(mu_global_, mu_call_);

  GRPC_EXPORT void CancelAllCalls() ABSL_LOCKS_EXCLUDED(mu_global_);
 ...
 private:
  ...
  class ChannelData {
   public:
    ChannelData() = default;
    GRPC_EXPORT ~ChannelData();

    GRPC_EXPORT void InitTransport(RefCountedPtr<Server> server, grpc_channel* channel,
    ...
    GRPC_EXPORT ChannelRegisteredMethod* GetRegisteredMethod(const grpc_slice& host,
                                                 const grpc_slice& path,
                                                 bool is_idempotent);

    // Filter vtable functions.
    GRPC_EXPORT static grpc_error_handle InitChannelElement(
        grpc_channel_element* elem, grpc_channel_element_args* args);
    GRPC_EXPORT static void DestroyChannelElement(grpc_channel_element* elem);

   private:
    class ConnectivityWatcher;

    GRPC_EXPORT static void AcceptStream(void* arg, grpc_transport* /*transport*/,
                             const void* transport_server_data);

    GRPC_EXPORT void Destroy() ABSL_EXCLUSIVE_LOCKS_REQUIRED(server_->mu_global_);

    GRPC_EXPORT static void FinishDestroy(void* arg, grpc_error_handle error);
  ...
  class CallData {
   public:
    ...
    GRPC_EXPORT CallData(grpc_call_element* elem, const grpc_call_element_args& args,
             RefCountedPtr<Server> server);
    GRPC_EXPORT ~CallData();

    // Starts the recv_initial_metadata batch on the call.
    // Invoked from ChannelData::AcceptStream().
    GRPC_EXPORT void Start(grpc_call_element* elem);

    GRPC_EXPORT void SetState(CallState state);

    // Attempts to move from PENDING to ACTIVATED state.  Returns true
    // on success.
    GRPC_EXPORT bool MaybeActivate();

    // Publishes an incoming call to the application after it has been
    // matched.
    GRPC_EXPORT void Publish(size_t cq_idx, RequestedCall* rc);

    GRPC_EXPORT void KillZombie();

    GRPC_EXPORT void FailCallCreation();

    // Filter vtable functions.
    GRPC_EXPORT static grpc_error_handle InitCallElement(
        grpc_call_element* elem, const grpc_call_element_args* args);
    GRPC_EXPORT static void DestroyCallElement(grpc_call_element* elem,
                                   const grpc_call_final_info* /*final_info*/,
                                   grpc_closure* /*ignored*/);
    GRPC_EXPORT static void StartTransportStreamOpBatch(
        grpc_call_element* elem, grpc_transport_stream_op_batch* batch);

   private:
    // Helper functions for handling calls at the top of the call stack.
    GRPC_EXPORT static void RecvInitialMetadataBatchComplete(void* arg,
                                                 grpc_error_handle error);
    GRPC_EXPORT void StartNewRpc(grpc_call_element* elem);
    GRPC_EXPORT static void PublishNewRpc(void* arg, grpc_error_handle error);

    // Functions used inside the call stack.
    GRPC_EXPORT void StartTransportStreamOpBatchImpl(grpc_call_element* elem,
                                         grpc_transport_stream_op_batch* batch);
    GRPC_EXPORT static void RecvInitialMetadataReady(void* arg, grpc_error_handle error);
    GRPC_EXPORT static void RecvTrailingMetadataReady(void* arg, grpc_error_handle error);
  ...
  GRPC_EXPORT static void ListenerDestroyDone(void* arg, grpc_error_handle error);
  ...
  GRPC_EXPORT static void DoneRequestEvent(void* req, grpc_cq_completion* completion);

  GRPC_EXPORT void FailCall(size_t cq_idx, RequestedCall* rc, grpc_error_handle error);
  GRPC_EXPORT grpc_call_error QueueRequestedCall(size_t cq_idx, RequestedCall* rc);

  GRPC_EXPORT void MaybeFinishShutdown() ABSL_EXCLUSIVE_LOCKS_REQUIRED(mu_global_)
      ABSL_LOCKS_EXCLUDED(mu_call_);

  GRPC_EXPORT void KillPendingWorkLocked(grpc_error_handle error)
      ABSL_EXCLUSIVE_LOCKS_REQUIRED(mu_call_);

  GRPC_EXPORT static grpc_call_error ValidateServerRequest(
      grpc_completion_queue* cq_for_notification, void* tag,
      grpc_byte_buffer** optional_payload, RegisteredMethod* rm);
  GRPC_EXPORT grpc_call_error ValidateServerRequestAndCq(
      size_t* cq_idx, grpc_completion_queue* cq_for_notification, void* tag,
      grpc_byte_buffer** optional_payload, RegisteredMethod* rm);

  GRPC_EXPORT std::vector<grpc_channel*> GetChannelsLocked() const;
