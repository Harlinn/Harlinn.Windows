...
GRPC_EXPORT void grpc_auth_property_reset(grpc_auth_property* property);
...
struct grpc_auth_context
 ...
 public:
  ...
  GRPC_EXPORT void ensure_capacity();
  GRPC_EXPORT void add_property(const char* name, const char* value, size_t value_length);
  GRPC_EXPORT void add_cstring_property(const char* name, const char* value);
...
struct grpc_client_security_context {
  ...
  GRPC_EXPORT ~grpc_client_security_context();
...
GRPC_EXPORT grpc_client_security_context* grpc_client_security_context_create(
    grpc_core::Arena* arena, grpc_call_credentials* creds);
GRPC_EXPORT void grpc_client_security_context_destroy(void* ctx);
...
struct grpc_server_security_context {
  ...
  GRPC_EXPORT ~grpc_server_security_context();

  grpc_core::RefCountedPtr<grpc_auth_context> auth_context;
  grpc_security_context_extension extension;
};

GRPC_EXPORT grpc_server_security_context* grpc_server_security_context_create(
    grpc_core::Arena* arena);
GRPC_EXPORT void grpc_server_security_context_destroy(void* ctx);

/* --- Channel args for auth context --- */
#define GRPC_AUTH_CONTEXT_ARG "grpc.auth_context"

GRPC_EXPORT grpc_arg grpc_auth_context_to_arg(grpc_auth_context* c);
GRPC_EXPORT grpc_auth_context* grpc_auth_context_from_arg(const grpc_arg* arg);
GRPC_EXPORT grpc_auth_context* grpc_find_auth_context_in_args(
