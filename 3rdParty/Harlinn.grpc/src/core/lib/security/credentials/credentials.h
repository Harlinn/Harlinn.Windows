...
GRPC_EXPORT std::string grpc_get_well_known_google_credentials_file_path(void);

/* Implementation function for the different platforms. */
GRPC_EXPORT std::string grpc_get_well_known_google_credentials_file_path_impl(void);

/* Override for testing only. Not thread-safe */
typedef std::string (*grpc_well_known_credentials_path_getter)(void);
GRPC_EXPORT void grpc_override_well_known_credentials_path_getter(
...
GRPC_EXPORT grpc_arg grpc_channel_credentials_to_arg(grpc_channel_credentials* credentials);

/* Util to get the channel credentials from a channel arg. */
GRPC_EXPORT grpc_channel_credentials* grpc_channel_credentials_from_arg(
    const grpc_arg* arg);

/* Util to find the channel credentials from channel args. */
GRPC_EXPORT grpc_channel_credentials* grpc_channel_credentials_find_in_args(
...
GRPC_EXPORT void grpc_credentials_mdelem_array_add(grpc_credentials_mdelem_array* list,
                                       grpc_mdelem md);

/// Appends all elements from \a src to \a dst, taking a new ref to each one.
GRPC_EXPORT void grpc_credentials_mdelem_array_append(grpc_credentials_mdelem_array* dst,
                                          grpc_credentials_mdelem_array* src);

GRPC_EXPORT void grpc_credentials_mdelem_array_destroy(grpc_credentials_mdelem_array* list);
...
GRPC_EXPORT grpc_call_credentials* grpc_md_only_test_credentials_create(
...
GRPC_EXPORT grpc_arg grpc_server_credentials_to_arg(grpc_server_credentials* c);
GRPC_EXPORT grpc_server_credentials* grpc_server_credentials_from_arg(const grpc_arg* arg);
GRPC_EXPORT grpc_server_credentials* grpc_find_server_credentials_in_args(
    const grpc_channel_args* args);
