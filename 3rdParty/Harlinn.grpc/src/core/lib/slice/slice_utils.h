...
GRPC_EXPORT int grpc_slice_differs_refcounted(const grpc_slice& a,
...
struct ManagedMemorySlice : public grpc_slice {
  ...
  GRPC_EXPORT explicit ManagedMemorySlice(const char* string);
  GRPC_EXPORT ManagedMemorySlice(const char* buf, size_t len);
  GRPC_EXPORT explicit ManagedMemorySlice(const grpc_slice* slice);
...
struct UnmanagedMemorySlice : public grpc_slice {
  ...
  GRPC_EXPORT explicit UnmanagedMemorySlice(const char* source);
  GRPC_EXPORT UnmanagedMemorySlice(const char* source, size_t length);
  ...
  GRPC_EXPORT explicit UnmanagedMemorySlice(size_t length);
  explicit UnmanagedMemorySlice(size_t length, const ForceHeapAllocation&) {
    HeapInit(length);
  }

 private:
  GRPC_EXPORT void HeapInit(size_t length);
};
...
struct InternedSlice : public ManagedMemorySlice {
  GRPC_EXPORT explicit InternedSlice(InternedSliceRefcount* s);
};
