...
GRPC_EXPORT extern grpc_core::DebugOnlyTraceFlag grpc_trace_fd_refcount;
...
GRPC_EXPORT void grpc_set_pollset_vtable(grpc_pollset_vtable* vtable);

GRPC_EXPORT void grpc_pollset_global_init(void);
GRPC_EXPORT void grpc_pollset_global_shutdown(void);

GRPC_EXPORT size_t grpc_pollset_size(void);
/* Initialize a pollset: assumes *pollset contains all zeros */
GRPC_EXPORT void grpc_pollset_init(grpc_pollset* pollset, gpr_mu** mu);
/* Begin shutting down the pollset, and call closure when done.
 * pollset's mutex must be held */
GRPC_EXPORT void grpc_pollset_shutdown(grpc_pollset* pollset, grpc_closure* closure);
GRPC_EXPORT void grpc_pollset_destroy(grpc_pollset* pollset);
...
/* Do some work on a pollset.
   ...
   pollset
   lock */
GRPC_EXPORT grpc_error_handle grpc_pollset_work(grpc_pollset* pollset,
                                    grpc_pollset_worker** worker,
                                    grpc_millis deadline) GRPC_MUST_USE_RESULT;

/* Break one polling thread out of polling work for this pollset.
   If specific_worker is non-NULL, then kick that worker. */
GRPC_EXPORT grpc_error_handle grpc_pollset_kick(grpc_pollset* pollset,
                                    grpc_pollset_worker* specific_worker)
