...
GRPC_EXPORT gpr_timespec grpc_millis_to_timespec(grpc_millis millis, gpr_clock_type clock);
GRPC_EXPORT grpc_millis grpc_timespec_to_millis_round_down(gpr_timespec ts);
GRPC_EXPORT grpc_millis grpc_timespec_to_millis_round_up(gpr_timespec ts);
GRPC_EXPORT grpc_millis grpc_cycle_counter_to_millis_round_down(gpr_cycle_counter cycles);
GRPC_EXPORT grpc_millis grpc_cycle_counter_to_millis_round_up(gpr_cycle_counter cycles);
...
class ExecCtx {
 public:
  ...
  GRPC_EXPORT static void TestOnlyGlobalInit(gpr_timespec new_val);

  /** Global initialization for ExecCtx. Called by iomgr. */
  GRPC_EXPORT static void GlobalInit(void);

  /** Global shutdown for ExecCtx. Called by iomgr. */
  static void GlobalShutdown(void) {}
#ifdef _WIN32
  /** Gets pointer to current exec_ctx. */
  GRPC_EXPORT static ExecCtx* InternalGet( );

  GRPC_EXPORT static void InternalSet(ExecCtx* exec_ctx);
  ...
  GRPC_EXPORT static void Run(const DebugLocation& location, grpc_closure* closure,
                  grpc_error_handle error);

  GRPC_EXPORT static void RunList(const DebugLocation& location, grpc_closure_list* list);
