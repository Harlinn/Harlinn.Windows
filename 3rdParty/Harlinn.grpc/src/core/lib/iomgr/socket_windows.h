...
GRPC_EXPORT grpc_winsocket* grpc_winsocket_create(SOCKET socket, const char* name);

GRPC_EXPORT SOCKET grpc_winsocket_wrapped_socket(grpc_winsocket* socket);

/* Initiate an asynchronous shutdown of the socket. Will call off any pending
   operation to cancel them. */
GRPC_EXPORT void grpc_winsocket_shutdown(grpc_winsocket* socket);

/* Destroy a socket. Should only be called if there's no pending operation. */
GRPC_EXPORT void grpc_winsocket_destroy(grpc_winsocket* socket);

GRPC_EXPORT void grpc_socket_notify_on_write(grpc_winsocket* winsocket,
                                 grpc_closure* closure);

GRPC_EXPORT void grpc_socket_notify_on_read(grpc_winsocket* winsocket,
                                grpc_closure* closure);

GRPC_EXPORT void grpc_socket_become_ready(grpc_winsocket* winsocket,
                              grpc_winsocket_callback_info* ci);

/* Returns true if this system can create AF_INET6 sockets bound to ::1.
   The value is probed once, and cached for the life of the process. */
GRPC_EXPORT int grpc_ipv6_loopback_available(void);

GRPC_EXPORT void grpc_wsa_socket_flags_init();

GRPC_EXPORT DWORD grpc_get_default_wsa_socket_flags();
