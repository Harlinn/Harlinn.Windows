...
class Executor {
 public:
  GRPC_EXPORT explicit Executor(const char* executor_name);

  GRPC_EXPORT void Init();

  /** Is the executor multi-threaded? */
  GRPC_EXPORT bool IsThreaded() const;
  ...
  GRPC_EXPORT void SetThreading(bool threading);

  /** Shutdown the executor, running all pending work as part of the call */
  GRPC_EXPORT void Shutdown();
  ...
  GRPC_EXPORT void Enqueue(grpc_closure* closure, grpc_error_handle error, bool is_short);
  ...
  GRPC_EXPORT static void InitAll();

  GRPC_EXPORT static void Run(grpc_closure* closure, grpc_error_handle error,
                  ExecutorType executor_type = ExecutorType::DEFAULT,
                  ExecutorJobType job_type = ExecutorJobType::SHORT);

  // Shutdown ALL the executors
  GRPC_EXPORT static void ShutdownAll();

  // Set the threading mode for ALL the executors
  GRPC_EXPORT static void SetThreadingAll(bool enable);

  // Set the threading mode for ALL the executors
  GRPC_EXPORT static void SetThreadingDefault(bool enable);

  // Return if a given executor is running in threaded mode (i.e if
  // SetThreading(true) was called previously on that executor)
  GRPC_EXPORT static bool IsThreaded(ExecutorType executor_type);

  // Return if the DEFAULT executor is threaded
  GRPC_EXPORT static bool IsThreadedDefault();
...
GRPC_EXPORT void grpc_executor_global_init();
