...
class ThreadPoolWorker {
 ...
 private:
  ...
  GRPC_EXPORT void Run();  // Pulls closures from queue and executes them
...
class ThreadPool : public ThreadPoolInterface {
 public:
  ...
  GRPC_EXPORT explicit ThreadPool(int num_threads);
  ...
  GRPC_EXPORT ThreadPool(int num_threads, const char* thd_name);
  ...
  // Waits for all pending closures to complete, then shuts down thread pool.
  GRPC_EXPORT ~ThreadPool() override;

  // Adds given closure into pending queue immediately. Since closure queue has
  // infinite length, this routine will not block.
  GRPC_EXPORT void Add(grpc_completion_queue_functor* closure) override;

  GRPC_EXPORT int num_pending_closures() const override;
  GRPC_EXPORT int pool_capacity() const override;
  GRPC_EXPORT const Thread::Options& thread_options() const override;
  GRPC_EXPORT const char* thread_name() const override;
 ...
 private:
  ...
  GRPC_EXPORT void SharedThreadPoolConstructor();
  // For ThreadPool, default stack size for mobile platform is 1952K. for other
  // platforms is 64K.
  GRPC_EXPORT size_t DefaultStackSize();
  // Internal Use Only for debug checking.
  GRPC_EXPORT void AssertHasNotBeenShutDown();
};
