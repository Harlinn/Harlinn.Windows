...
GRPC_EXPORT grpc_polling_entity grpc_polling_entity_create_from_pollset_set(
    grpc_pollset_set* pollset_set);
GRPC_EXPORT grpc_polling_entity grpc_polling_entity_create_from_pollset(
    grpc_pollset* pollset);

/** If \a pollent contains a pollset, return it. Otherwise, return NULL */
GRPC_EXPORT grpc_pollset* grpc_polling_entity_pollset(grpc_polling_entity* pollent);

/** If \a pollent contains a pollset_set, return it. Otherwise, return NULL */
GRPC_EXPORT grpc_pollset_set* grpc_polling_entity_pollset_set(grpc_polling_entity* pollent);

GRPC_EXPORT bool grpc_polling_entity_is_empty(const grpc_polling_entity* pollent);

/** Add the pollset or pollset_set in \a pollent to the destination pollset_set
 * \a * pss_dst */
GRPC_EXPORT void grpc_polling_entity_add_to_pollset_set(grpc_polling_entity* pollent,
                                            grpc_pollset_set* pss_dst);

/** Delete the pollset or pollset_set in \a pollent from the destination
 * pollset_set \a * pss_dst */
GRPC_EXPORT void grpc_polling_entity_del_from_pollset_set(grpc_polling_entity* pollent,
                                              grpc_pollset_set* pss_dst);
