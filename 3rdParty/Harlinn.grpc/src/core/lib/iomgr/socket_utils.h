...
GRPC_EXPORT uint16_t grpc_htons(uint16_t hostshort);

/* A wrapper for ntohs on POSIX and WINDOWS */
GRPC_EXPORT uint16_t grpc_ntohs(uint16_t netshort);

/* A wrapper for htonl on POSIX and Windows */
GRPC_EXPORT uint32_t grpc_htonl(uint32_t hostlong);

/* A wrapper for ntohl on POSIX and WINDOWS */
GRPC_EXPORT uint32_t grpc_ntohl(uint32_t netlong);

/* A wrapper for inet_pton on POSIX and WINDOWS */
GRPC_EXPORT int grpc_inet_pton(int af, const char* src, void* dst);

/* A wrapper for inet_ntop on POSIX systems and InetNtop on Windows systems */
GRPC_EXPORT const char* grpc_inet_ntop(int af, const void* src, char* dst, size_t size);
