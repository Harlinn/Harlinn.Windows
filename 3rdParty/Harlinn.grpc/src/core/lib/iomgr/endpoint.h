...
GRPC_EXPORT void grpc_endpoint_read(grpc_endpoint* ep, grpc_slice_buffer* slices,
                        grpc_closure* cb, bool urgent);

GRPC_EXPORT absl::string_view grpc_endpoint_get_peer(grpc_endpoint* ep);

GRPC_EXPORT absl::string_view grpc_endpoint_get_local_address(grpc_endpoint* ep);

/* Get the file descriptor used by \a ep. Return -1 if \a ep is not using an fd.
 */
GRPC_EXPORT int grpc_endpoint_get_fd(grpc_endpoint* ep);
...
/* Write slices out to the socket.
   ...
   */
GRPC_EXPORT void grpc_endpoint_write(grpc_endpoint* ep, grpc_slice_buffer* slices,
                         grpc_closure* cb, void* arg);

/* Causes any pending and future read/write callbacks to run immediately with
   success==0 */
GRPC_EXPORT void grpc_endpoint_shutdown(grpc_endpoint* ep, grpc_error_handle why);
GRPC_EXPORT void grpc_endpoint_destroy(grpc_endpoint* ep);

/* Add an endpoint to a pollset or pollset_set, so that when the pollset is
   polled, events from this endpoint are considered */
GRPC_EXPORT void grpc_endpoint_add_to_pollset(grpc_endpoint* ep, grpc_pollset* pollset);
GRPC_EXPORT void grpc_endpoint_add_to_pollset_set(grpc_endpoint* ep,
                                      grpc_pollset_set* pollset_set);

/* Delete an endpoint from a pollset_set */
GRPC_EXPORT void grpc_endpoint_delete_from_pollset_set(grpc_endpoint* ep,
                                           grpc_pollset_set* pollset_set);

GRPC_EXPORT bool grpc_endpoint_can_track_err(grpc_endpoint* ep);
