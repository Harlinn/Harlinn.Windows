...
class MessageSizeParser : public ServiceConfigParser::Parser {
 public:
  GRPC_EXPORT std::unique_ptr<ServiceConfigParser::ParsedConfig> ParsePerMethodParams(
      const grpc_channel_args* /*args*/, const Json& json,
      grpc_error_handle* error) override;

  GRPC_EXPORT static void Register();

  GRPC_EXPORT static size_t ParserIndex();
};

GRPC_EXPORT int GetMaxRecvSizeFromChannelArgs(const grpc_channel_args* args);
GRPC_EXPORT int GetMaxSendSizeFromChannelArgs(const grpc_channel_args* args);
