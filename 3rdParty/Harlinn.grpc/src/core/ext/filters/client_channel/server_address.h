...
class ServerAddress {
 public:
  ...
  GRPC_EXPORT ServerAddress(const grpc_resolved_address& address, grpc_channel_args* args,
                std::map<const char*, std::unique_ptr<AttributeInterface>>
                    attributes = {});
  GRPC_EXPORT ServerAddress(const void* address, size_t address_len,
  ...
  // Copyable.
  GRPC_EXPORT ServerAddress(const ServerAddress& other);
  GRPC_EXPORT ServerAddress& operator=(const ServerAddress& other);

  // Movable.
  GRPC_EXPORT ServerAddress(ServerAddress&& other) noexcept;
  GRPC_EXPORT ServerAddress& operator=(ServerAddress&& other) noexcept;

  bool operator==(const ServerAddress& other) const { return Cmp(other) == 0; }

  GRPC_EXPORT int Cmp(const ServerAddress& other) const;

  const grpc_resolved_address& address() const { return address_; }
  const grpc_channel_args* args() const { return args_; }

  GRPC_EXPORT const AttributeInterface* GetAttribute(const char* key) const;

  // Returns a copy of the address with a modified attribute.
  // If the new value is null, the attribute is removed.
  GRPC_EXPORT ServerAddress WithAttribute(const char* key,
                              std::unique_ptr<AttributeInterface> value) const;

  GRPC_EXPORT std::string ToString() const;
