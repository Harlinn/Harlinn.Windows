...
class ServiceConfig : public RefCounted<ServiceConfig> {
 public:
  ...
  GRPC_EXPORT static RefCountedPtr<ServiceConfig> Create(const grpc_channel_args* args,
                                             absl::string_view json_string,
                                             grpc_error_handle* error);

  GRPC_EXPORT ServiceConfig(const grpc_channel_args* args, std::string json_string,
                Json json, grpc_error_handle* error);
  GRPC_EXPORT ~ServiceConfig() override;
  ...
  GRPC_EXPORT const ServiceConfigParser::ParsedConfigVector* GetMethodParsedConfigVector(
      const grpc_slice& path) const;

 private:
  // Helper functions for parsing the method configs.
  GRPC_EXPORT grpc_error_handle ParsePerMethodParams(const grpc_channel_args* args);
  GRPC_EXPORT grpc_error_handle ParseJsonMethodConfig(const grpc_channel_args* args,
                                          const Json& json);

  // Returns a path string for the JSON name object specified by json.
  // Sets *error on error.
  GRPC_EXPORT static std::string ParseJsonMethodName(const Json& json,
                                         grpc_error_handle* error);
