 ...
 public:
  ...
  class Builder {
   public:
    ...
    GRPC_EXPORT static void InitRegistry();
    GRPC_EXPORT static void ShutdownRegistry();

    /// Sets the default URI prefix to \a default_prefix.
    /// Calls InitRegistry() if it has not already been called.
    GRPC_EXPORT static void SetDefaultPrefix(const char* default_prefix);
    ...
    /// Calls InitRegistry() if it has not already been called.
    GRPC_EXPORT static void RegisterResolverFactory(
        std::unique_ptr<ResolverFactory> factory);
  };

  /// Checks whether the user input \a target is valid to create a resolver.
  GRPC_EXPORT static bool IsValidTarget(absl::string_view target);
  ...
  GRPC_EXPORT static OrphanablePtr<Resolver> CreateResolver(
  ...
  GRPC_EXPORT static std::string GetDefaultAuthority(absl::string_view target);

  /// Returns \a target with the default prefix prepended, if needed.
  GRPC_EXPORT static grpc_core::UniquePtr<char> AddDefaultPrefixIfNeeded(
      const char* target);

  /// Returns the resolver factory for \a scheme.
  /// Caller does NOT own the return value.
  GRPC_EXPORT static ResolverFactory* LookupResolverFactory(const char* scheme);
};
