...
GRPC_EXPORT extern grpc_core::TraceFlag grpc_trace_cares_address_sorting;

GRPC_EXPORT extern grpc_core::TraceFlag grpc_trace_cares_resolver;
...
GRPC_EXPORT extern void (*grpc_resolve_address_ares)(const char* name,
...
GRPC_EXPORT extern grpc_ares_request* (*grpc_dns_lookup_ares_locked)(
...
GRPC_EXPORT extern void (*grpc_cancel_ares_request_locked)(grpc_ares_request* request);

/* Initialize gRPC ares wrapper. Must be called at least once before
   grpc_resolve_address_ares(). */
GRPC_EXPORT grpc_error_handle grpc_ares_init(void);

/* Uninitialized gRPC ares wrapper. If there was more than one previous call to
   grpc_ares_init(), this function uninitializes the gRPC ares wrapper only if
   it has been called the same number of times as grpc_ares_init(). */
GRPC_EXPORT void grpc_ares_cleanup(void);

/** Schedules the desired callback for request completion
 * and destroys the grpc_ares_request */
GRPC_EXPORT void grpc_ares_complete_request_locked(grpc_ares_request* request);

/* Indicates whether or not AAAA queries should be attempted. */
/* E.g., return false if ipv6 is known to not be available. */
GRPC_EXPORT bool grpc_ares_query_ipv6();

/* Sorts destinations in lb_addrs according to RFC 6724. */
GRPC_EXPORT void grpc_cares_wrapper_address_sorting_sort(
    const grpc_ares_request* request, grpc_core::ServerAddressList* addresses);

/* Exposed in this header for C-core tests only */
GRPC_EXPORT extern void (*grpc_ares_test_only_inject_config)(ares_channel channel);
