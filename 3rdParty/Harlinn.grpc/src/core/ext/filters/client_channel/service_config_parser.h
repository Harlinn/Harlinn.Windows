...
class ServiceConfigParser {
 public:
  ...
  GRPC_EXPORT static size_t RegisterParser(std::unique_ptr<Parser> parser);

  GRPC_EXPORT static ParsedConfigVector ParseGlobalParameters(const grpc_channel_args* args,
                                                  const Json& json,
                                                  grpc_error_handle* error);

  GRPC_EXPORT static ParsedConfigVector ParsePerMethodParameters(
