...
class Resolver : public InternallyRefCounted<Resolver> {
 public:
  ...
  struct Result {
    ...
    Result() = default;
    GRPC_EXPORT ~Result();
    GRPC_EXPORT Result(const Result& other);
    GRPC_EXPORT Result(Result&& other) noexcept;
    GRPC_EXPORT Result& operator=(const Result& other);
    GRPC_EXPORT Result& operator=(Result&& other) noexcept;
  };
