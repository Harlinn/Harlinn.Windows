...
class ServerRetryThrottleData : public RefCounted<ServerRetryThrottleData> {
 public:
  GRPC_EXPORT ServerRetryThrottleData(intptr_t max_milli_tokens, intptr_t milli_token_ratio,
                          ServerRetryThrottleData* old_throttle_data);
  GRPC_EXPORT ~ServerRetryThrottleData() override;

  /// Records a failure.  Returns true if it's okay to send a retry.
  GRPC_EXPORT bool RecordFailure();

  /// Records a success.
  GRPC_EXPORT void RecordSuccess();

  intptr_t max_milli_tokens() const { return max_milli_tokens_; }
  intptr_t milli_token_ratio() const { return milli_token_ratio_; }

 private:
  GRPC_EXPORT void GetReplacementThrottleDataIfNeeded(
...
class ServerRetryThrottleMap {
 public:
  ...
  /// needed.
  GRPC_EXPORT static RefCountedPtr<ServerRetryThrottleData> GetDataForServer(
