...
class dng_hue_sat_map
	...
	public:
		...
		DNG_EXPORT dng_hue_sat_map ();

		/// Copy an existing hue sat map.

		DNG_EXPORT dng_hue_sat_map (const dng_hue_sat_map &src);

		/// Copy an existing hue sat map.

		DNG_EXPORT dng_hue_sat_map & operator= (const dng_hue_sat_map &rhs);

		/// Destructor.

		DNG_EXPORT virtual ~dng_hue_sat_map ();
		...
		DNG_EXPORT void SetDivisions (uint32 hueDivisions,
						   uint32 satDivisions,
						   uint32 valDivisions = 1);	

		/// Get a specific table entry, specified by table indices.

		DNG_EXPORT void GetDelta (uint32 hueDiv,
					   uint32 satDiv,
					   uint32 valDiv,
		...
		DNG_EXPORT void SetDeltaKnownWriteable (uint32 hueDiv,
									 uint32 satDiv,
									 uint32 valDiv,
									 const HSBModify &modify);
		...
		DNG_EXPORT void AssignNewUniqueRuntimeFingerprint ();
		...
		DNG_EXPORT bool operator== (const dng_hue_sat_map &rhs) const;
		...
		DNG_EXPORT static dng_hue_sat_map * Interpolate (const dng_hue_sat_map &map1,
