...
class dng_iptc
	...
	public:
		...
		DNG_EXPORT void Parse (const void *blockData,
					uint32 blockSize,
		...
		DNG_EXPORT dng_memory_block * Spool (dng_memory_allocator &allocator,
								  bool padForTIFF);
					
	protected:
	
		DNG_EXPORT void ParseString (dng_stream &stream,
						  dng_string &s,
						  CharSet charSet);
		
		DNG_EXPORT void SpoolString (dng_stream &stream,
						  const dng_string &s,
						  uint8 dataSet,
						  uint32 maxChars,
						  CharSet charSet);
