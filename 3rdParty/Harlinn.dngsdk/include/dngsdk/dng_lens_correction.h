...
class dng_warp_params
	...
	public:
		...
		DNG_EXPORT dng_warp_params (uint32 planes,
						 const dng_point_real64 &fCenter);

		DNG_EXPORT virtual ~dng_warp_params ();

		/// Is the entire correction a NOP for all planes?

		DNG_EXPORT virtual bool IsNOPAll () const;

		/// Is the entire correction a NOP for the specified plane?

		DNG_EXPORT virtual bool IsNOP (uint32 plane) const;

		/// Is the radial correction a NOP for all planes?

		DNG_EXPORT virtual bool IsRadNOPAll () const;

		/// Is the radial correction a NOP for the specified plane?

		DNG_EXPORT virtual bool IsRadNOP (uint32 plane) const;

		/// Is the tangential correction a NOP for all planes?

		DNG_EXPORT virtual bool IsTanNOPAll () const;

		/// Is the tangential correction a NOP for the specified plane?

		DNG_EXPORT virtual bool IsTanNOP (uint32 plane) const;

		/// Do these warp params appear valid?

		DNG_EXPORT virtual bool IsValid () const;

		/// Are these warp params valid for the specified negative?

		DNG_EXPORT virtual bool IsValidForNegative (const dng_negative &negative) const;

		/// Propagate warp parameters from first plane to all other planes.

		DNG_EXPORT virtual void PropagateToAllPlanes (uint32 totalPlanes) = 0;
		...
		DNG_EXPORT virtual real64 EvaluateInverse (uint32 plane,
										real64 r) const;
		...
		DNG_EXPORT dng_point_real64 EvaluateTangential2 (uint32 plane,
		...
		DNG_EXPORT dng_point_real64 EvaluateTangential3 (uint32 plane,
											  real64 r2,
											  const dng_point_real64 &diff) const;
		...
		DNG_EXPORT virtual void Dump () const;
...
class dng_warp_params_rectilinear: public dng_warp_params
	...
	public:
		...
		DNG_EXPORT dng_warp_params_rectilinear ();
		...
		DNG_EXPORT dng_warp_params_rectilinear (uint32 planes,
									 const dng_vector radParams [],
									 const dng_vector tanParams [],
									 const dng_point_real64 &fCenter);

		DNG_EXPORT virtual ~dng_warp_params_rectilinear ();
		...
		DNG_EXPORT virtual void PropagateToAllPlanes (uint32 totalPlanes);

		DNG_EXPORT virtual real64 Evaluate (uint32 plane,
								 real64 r) const;

		DNG_EXPORT virtual real64 EvaluateRatio (uint32 plane,
									  real64 r2) const;

		DNG_EXPORT virtual dng_point_real64 EvaluateTangential (uint32 plane,
													 real64 r2,
													 const dng_point_real64 &diff,
													 const dng_point_real64 &diff2) const;

		DNG_EXPORT virtual real64 MaxSrcRadiusGap (real64 maxDstGap) const;

		DNG_EXPORT virtual dng_point_real64 MaxSrcTanGap (dng_point_real64 minDst,
											   dng_point_real64 maxDst) const;

		DNG_EXPORT virtual real64 SafeMinRatio () const;

		DNG_EXPORT virtual real64 SafeMaxRatio () const;
...
class dng_warp_params_fisheye: public dng_warp_params
	...
	public:
		...
		DNG_EXPORT dng_warp_params_fisheye ();

		/// Create rectilinear warp parameters with the specified number of
		/// planes, radial component terms, and image center in relative
		/// coordinates.

		DNG_EXPORT dng_warp_params_fisheye (uint32 planes,
								 const dng_vector radParams [],
								 const dng_point_real64 &fCenter);

		DNG_EXPORT virtual ~dng_warp_params_fisheye ();
...
class dng_opcode_WarpRectilinear: public dng_opcode
	...
	public:
		...
		DNG_EXPORT dng_opcode_WarpRectilinear (const dng_warp_params_rectilinear &params,
									uint32 flags);
		
		DNG_EXPORT explicit dng_opcode_WarpRectilinear (dng_stream &stream);
	
		// Overridden methods.

		DNG_EXPORT virtual bool IsNOP () const;
		
		DNG_EXPORT virtual bool IsValidForNegative (const dng_negative &negative) const;
	
		DNG_EXPORT virtual void PutData (dng_stream &stream) const;

		DNG_EXPORT virtual void Apply (dng_host &host,
							dng_negative &negative,
							AutoPtr<dng_image> &image);

		// Other methods.

		DNG_EXPORT bool HasDistort () const;

		DNG_EXPORT bool HasLateralCA () const;
	...
	protected:
		...
		DNG_EXPORT static uint32 ParamBytes (uint32 planes);
...
class dng_opcode_WarpFisheye: public dng_opcode
	...
	public:
		...
		DNG_EXPORT dng_opcode_WarpFisheye (const dng_warp_params_fisheye &params,
								uint32 flags);
		
		DNG_EXPORT explicit dng_opcode_WarpFisheye (dng_stream &stream);
...
class dng_vignette_radial_params
	...
	public:
		...
		DNG_EXPORT dng_vignette_radial_params ();

		DNG_EXPORT dng_vignette_radial_params (const dng_std_vector<real64> &params,
									const dng_point_real64 &center);

		DNG_EXPORT dng_vignette_radial_params (const dng_vignette_radial_params &params);

		DNG_EXPORT bool IsNOP () const;

		DNG_EXPORT bool IsValid () const;

		// For debugging.

		DNG_EXPORT void Dump () const;
...
class dng_opcode_FixVignetteRadial: public dng_inplace_opcode
	...
	public:
		...
		DNG_EXPORT dng_opcode_FixVignetteRadial (const dng_vignette_radial_params &params,
									  uint32 flags);
		
		DNG_EXPORT explicit dng_opcode_FixVignetteRadial (dng_stream &stream);
		...
		DNG_EXPORT virtual bool IsValidForNegative (const dng_negative &) const;
		...
		DNG_EXPORT virtual void Prepare (dng_negative &negative,
							  uint32 threadCount,
	...
	protected:
		...
		DNG_EXPORT static uint32 ParamBytes ();

		DNG_EXPORT virtual dng_vignette_radial_params MakeParamsForRender (const dng_negative &negative);
