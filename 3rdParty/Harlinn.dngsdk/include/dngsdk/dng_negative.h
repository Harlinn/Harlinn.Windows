...
class dng_noise_profile
	...
	public:
		...
		DNG_EXPORT dng_noise_profile ();

		/// Create noise profile with the specified noise functions (1 per plane).

		DNG_EXPORT explicit dng_noise_profile (const dng_std_vector<dng_noise_function> &functions);
		...
		DNG_EXPORT bool IsValidForNegative (const dng_negative &negative) const;

		/// The noise function for the specified plane.

		DNG_EXPORT const dng_noise_function & NoiseFunction (uint32 plane) const;

		/// The number of noise functions in this profile.

		DNG_EXPORT uint32 NumFunctions () const;
  
        /// Equality test.
        
		DNG_EXPORT bool operator== (const dng_noise_profile &profile) const;
...
class dng_metadata
	...
	public:
		...
		DNG_EXPORT dng_metadata (dng_host &host);
		
		DNG_EXPORT dng_metadata (const dng_metadata &rhs,
					  dng_memory_allocator &allocator);
		
		DNG_EXPORT virtual ~dng_metadata ();

		/// Copy this metadata.
		
		DNG_EXPORT virtual dng_metadata * Clone (dng_memory_allocator &allocator) const;
		
		/// Setter for BaseOrientation.
			
		DNG_EXPORT void SetBaseOrientation (const dng_orientation &orientation);
		...
		DNG_EXPORT void ApplyOrientation (const dng_orientation &orientation);

		// API for IPTC metadata:
			
		DNG_EXPORT void SetIPTC (AutoPtr<dng_memory_block> &block,
					  uint64 offset);
		
		DNG_EXPORT void SetIPTC (AutoPtr<dng_memory_block> &block);
		
		DNG_EXPORT void ClearIPTC ();
		
		DNG_EXPORT const void * IPTCData () const;
		
		DNG_EXPORT uint32 IPTCLength () const;
		
		DNG_EXPORT uint64 IPTCOffset () const;
		
		DNG_EXPORT dng_fingerprint IPTCDigest (bool includePadding = true) const;
		
		DNG_EXPORT void RebuildIPTC (dng_memory_allocator &allocator,
						  bool padForTIFF);
		...
		DNG_EXPORT void ResetExif (dng_exif * newExif);
			
		DNG_EXPORT dng_memory_block * BuildExifBlock (dng_memory_allocator &allocator,
		...
		DNG_EXPORT bool SetXMP (dng_host &host,
					 const void *buffer,
					 uint32 count,
					 bool xmpInSidecar = false,
					 bool xmpIsNewer = false);
					 
		DNG_EXPORT void SetEmbeddedXMP (dng_host &host,
							 const void *buffer,
							 uint32 count);
		...
		DNG_EXPORT void ResetXMP (dng_xmp * newXMP);
	
		DNG_EXPORT void ResetXMPSidecarNewer (dng_xmp * newXMP, bool inSidecar, bool isNewer );
			
		// Synchronize metadata sources.
		
		DNG_EXPORT void SynchronizeMetadata ();
		...
		DNG_EXPORT void UpdateDateTimeToNow ();
		
		DNG_EXPORT void UpdateMetadataDateTimeToNow ();
...
class dng_negative
	...
	public:
		...
		DNG_EXPORT virtual ~dng_negative ();
		
		DNG_EXPORT static dng_negative * Make (dng_host &host);
		...
		DNG_EXPORT dng_metadata * CloneInternalMetadata () const;
	...
	public:
		...
		DNG_EXPORT virtual dng_orientation ComputeOrientation (const dng_metadata &metadata) const;
		...
		DNG_EXPORT void SetDefaultOriginalSizes ();

		/// Set all the original size fields to a specific size.
        
		DNG_EXPORT void SetOriginalSizes (const dng_point &size);

		/// The default crop area in the stage 3 image coordinates.
							
		DNG_EXPORT dng_rect DefaultCropArea () const;
		...
		DNG_EXPORT real64 TotalBaselineExposure (const dng_camera_profile_id &profileID) const;
		...
		DNG_EXPORT void SetShadowScale (const dng_urational &scale);
		...
		DNG_EXPORT void SetAnalogBalance (const dng_vector &b);
		
		/// Getter for AnalogBalance as dng_urational.

		DNG_EXPORT dng_urational AnalogBalanceR (uint32 channel) const;
		
		/// Getter for AnalogBalance as real64.

		DNG_EXPORT real64 AnalogBalance (uint32 channel) const;
		
		/// Setter for CameraNeutral.

		DNG_EXPORT void SetCameraNeutral (const dng_vector &n);
		...
		DNG_EXPORT dng_urational CameraNeutralR (uint32 channel) const;
		
		/// Setter for CameraWhiteXY.

		DNG_EXPORT void SetCameraWhiteXY (const dng_xy_coord &coord);
		...
		DNG_EXPORT const dng_xy_coord & CameraWhiteXY () const;
		
		DNG_EXPORT void GetCameraWhiteXY (dng_urational &x,
		...
		DNG_EXPORT void SetCameraCalibration1 (const dng_matrix &m);
		...
		DNG_EXPORT void SetCameraCalibration2 (const dng_matrix &m);
		...
		DNG_EXPORT void AddProfile (AutoPtr<dng_camera_profile> &profile);
		
		DNG_EXPORT void ClearProfiles ();
			
		DNG_EXPORT void ClearProfiles (bool clearBuiltinMatrixProfiles,
							bool clearReadFromDisk);
			
		DNG_EXPORT uint32 ProfileCount () const;
		
		DNG_EXPORT const dng_camera_profile & ProfileByIndex (uint32 index) const;
		
		DNG_EXPORT virtual const dng_camera_profile * ProfileByID (const dng_camera_profile_id &id,
		...
		DNG_EXPORT virtual const dng_camera_profile * ComputeCameraProfileToEmbed
													(const dng_metadata &metadata) const;
		...
		DNG_EXPORT virtual dng_color_spec * MakeColorSpec (const dng_camera_profile_id &id) const;
		
		// Compute a MD5 hash on an image, using a fixed algorithm.
		// The results must be stable across different hardware, OSes,
		// and software versions.
			
		DNG_EXPORT static dng_fingerprint FindImageDigest (dng_host &host,
		...
		DNG_EXPORT void FindRawImageDigest (dng_host &host) const;
		
		DNG_EXPORT void FindNewRawImageDigest (dng_host &host) const;
		
		DNG_EXPORT void ValidateRawImageDigest (dng_host &host);
		...
		DNG_EXPORT dng_fingerprint RawDataUniqueID () const;
		
		DNG_EXPORT void FindRawDataUniqueID (dng_host &host) const;
		
		DNG_EXPORT virtual void RecomputeRawDataUniqueID (dng_host &host);
		...
		DNG_EXPORT void FindOriginalRawFileDigest () const;
		
		DNG_EXPORT void ValidateOriginalRawFileDigest ();
		...
		DNG_EXPORT void SetBlackLevel (real64 black,
							int32 plane = -1);
							
		DNG_EXPORT void SetQuadBlacks (real64 black0,
						    real64 black1,
						    real64 black2,
						    real64 black3,
							int32 plane = -1);
	
		DNG_EXPORT void Set6x6Blacks (real64 blacks6x6 [36],
						   int32 plane = -1);
						    
		DNG_EXPORT void SetRowBlacks (const real64 *blacks,
						   uint32 count);
						   
		DNG_EXPORT void SetColumnBlacks (const real64 *blacks,
							  uint32 count);
							  
		// Sensor white level information.
		
		DNG_EXPORT uint32 WhiteLevel (uint32 plane = 0) const;
			
		DNG_EXPORT void SetWhiteLevel (uint32 white,
							int32 plane = -1);
		...
		DNG_EXPORT void SetColorKeys (ColorKeyCode color0,
						   ColorKeyCode color1,
						   ColorKeyCode color2,
		...
		DNG_EXPORT void SetBayerMosaic (uint32 phase);

		DNG_EXPORT void SetFujiMosaic (uint32 phase);

		DNG_EXPORT void SetFujiMosaic6x6 (uint32 phase);

		DNG_EXPORT void SetQuadMosaic (uint32 pattern);
			
		// BayerGreenSplit.
							
		DNG_EXPORT void SetGreenSplit (uint32 split);
		...
		DNG_EXPORT virtual void Parse (dng_host &host,
							dng_stream &stream,
							dng_info &info);
		...
		DNG_EXPORT virtual void PostParse (dng_host &host,
								dng_stream &stream,
								dng_info &info);
		...
		DNG_EXPORT virtual bool SetFourColorBayer ();
		...
		DNG_EXPORT const dng_image & RawImage () const;
  
        // Returns the raw image black level in 16-bit space.
        
		DNG_EXPORT uint16 RawImageBlackLevel () const;
		...
		DNG_EXPORT const dng_jpeg_image * RawJPEGImage () const;

		DNG_EXPORT void SetRawJPEGImage (AutoPtr<dng_jpeg_image> &jpegImage);
			
		DNG_EXPORT void ClearRawJPEGImage ();
		...
		DNG_EXPORT void FindRawJPEGImageDigest (dng_host &host) const;
		
        // Read the opcode lists.
        
		DNG_EXPORT virtual void ReadOpcodeLists (dng_host &host,
                                      dng_stream &stream,
                                      dng_info &info);
        
        // Read the stage 1 image.
        
		DNG_EXPORT virtual void ReadStage1Image (dng_host &host,
                                      dng_stream &stream,
                                      dng_info &info);
        
        // Read the enhanced image directly into the stage 3 image.
        
		DNG_EXPORT virtual void ReadEnhancedImage (dng_host &host,
                                        dng_stream &stream,
                                        dng_info &info);
        
		// Assign the stage 1 image.
		
		DNG_EXPORT void SetStage1Image (AutoPtr<dng_image> &image);
		
		// Assign the stage 2 image.
		
		DNG_EXPORT void SetStage2Image (AutoPtr<dng_image> &image);
									  
		// Assign the stage 3 image.
		
		DNG_EXPORT void SetStage3Image (AutoPtr<dng_image> &image);
		
		// Build the stage 2 (linearized and range mapped) image.
		
		DNG_EXPORT void BuildStage2Image (dng_host &host);
									   
		// Build the stage 3 (demosaiced) image.
									   
		DNG_EXPORT void BuildStage3Image (dng_host &host,
							   int32 srcPlane = -1);
		...
		DNG_EXPORT virtual bool SupportsPreservedBlackLevels (dng_host &host);
			
		// Adaptively encode a proxy image down to 8-bits/channel.

		DNG_EXPORT dng_image * EncodeRawProxy (dng_host &host,
		...
		DNG_EXPORT void ConvertToProxy (dng_host &host,
		...
		DNG_EXPORT void SetTransparencyMask (AutoPtr<dng_image> &image,
								  uint32 bitDepth = 0);
		
		DNG_EXPORT const dng_image * TransparencyMask () const;
		
		DNG_EXPORT const dng_image * RawTransparencyMask () const;
		
		DNG_EXPORT uint32 RawTransparencyMaskBitDepth () const;
		
		DNG_EXPORT void ReadTransparencyMask (dng_host &host,
								   dng_stream &stream,
								   dng_info &info);
								   
		DNG_EXPORT virtual void ResizeTransparencyToMatchStage3 (dng_host &host,
                                                      bool convertTo8Bit = false);
        
		DNG_EXPORT virtual bool NeedFlattenTransparency (dng_host &host);
		
		DNG_EXPORT virtual void FlattenTransparency (dng_host &host);
		
		DNG_EXPORT const dng_image * UnflattenedStage3Image () const;
		...
		DNG_EXPORT void SetDepthMap (AutoPtr<dng_image> &depthMap);
		...
		DNG_EXPORT void ReadDepthMap (dng_host &host,
                           dng_stream &stream,
                           dng_info &info);
        
		DNG_EXPORT virtual void ResizeDepthToMatchStage3 (dng_host &host);
	...
	protected:
		...
		DNG_EXPORT dng_negative (dng_host &host);
		
		DNG_EXPORT virtual void Initialize ();
		
		DNG_EXPORT virtual dng_linearization_info * MakeLinearizationInfo ();
		
		DNG_EXPORT void NeedLinearizationInfo ();
		
		DNG_EXPORT virtual dng_mosaic_info * MakeMosaicInfo ();
		
		DNG_EXPORT void NeedMosaicInfo ();
		
		DNG_EXPORT virtual void DoBuildStage2 (dng_host &host);
		
		DNG_EXPORT virtual void DoPostOpcodeList2 (dng_host &host);
									   
		DNG_EXPORT virtual bool NeedDefloatStage2 (dng_host &host);
		
		DNG_EXPORT virtual void DefloatStage2 (dng_host &host);
		
		DNG_EXPORT virtual void DoInterpolateStage3 (dng_host &host,
									      int32 srcPlane,
                                          dng_matrix *scaleTransforms);
									
		DNG_EXPORT virtual void DoMergeStage3 (dng_host &host,
                                    dng_matrix *scaleTransforms);
									   
		DNG_EXPORT virtual void DoBuildStage3 (dng_host &host,
									int32 srcPlane,
                                    dng_matrix *scaleTransforms);
									   
		DNG_EXPORT virtual void AdjustProfileForStage3 ();
