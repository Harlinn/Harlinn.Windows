...
class dng_color_spec
	...
	public:
		...
		DNG_EXPORT dng_color_spec (const dng_negative &negative,
		...
		DNG_EXPORT void SetWhiteXY (const dng_xy_coord &white);
		
		/// Getter for white point. Value is as XY colorspace coordinate. 
		/// \retval XY value of white point.

		DNG_EXPORT const dng_xy_coord & WhiteXY () const;
		...
		DNG_EXPORT const dng_vector & CameraWhite () const;
		...
		DNG_EXPORT const dng_matrix & CameraToPCS () const;

		/// Getter for Profile Connection Space to camera color transform.
		/// \retval A transform that takes into account all camera calibration
		/// transforms and white point. 

		DNG_EXPORT const dng_matrix & PCStoCamera () const;
		...
		DNG_EXPORT dng_xy_coord NeutralToXY (const dng_vector &neutral);

	private:
	
		DNG_EXPORT dng_matrix FindXYZtoCamera (const dng_xy_coord &white,
