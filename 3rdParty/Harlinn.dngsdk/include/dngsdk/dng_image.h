...
class dng_tile_buffer: public dng_pixel_buffer,
	...
	protected:
		...
		DNG_EXPORT dng_tile_buffer (const dng_image &image,
						 const dng_rect &tile,
						 bool dirty);
						 
		DNG_EXPORT virtual ~dng_tile_buffer ();
...
class dng_const_tile_buffer: public dng_tile_buffer
	...
	public:
		...
		DNG_EXPORT dng_const_tile_buffer (const dng_image &image,
							   const dng_rect &tile);
							   
		DNG_EXPORT virtual ~dng_const_tile_buffer ();
...
class dng_dirty_tile_buffer: public dng_tile_buffer
	...
	public:
		...
		DNG_EXPORT dng_dirty_tile_buffer (dng_image &image,
							   const dng_rect &tile);
							   
		DNG_EXPORT virtual ~dng_dirty_tile_buffer ();
...
class dng_image
	...
	protected:
		...
		DNG_EXPORT dng_image (const dng_rect &bounds,
				   uint32 planes,
				   uint32 pixelType);
		
	public:
	
		DNG_EXPORT virtual ~dng_image ();
		...
		DNG_EXPORT uint32 PixelSize () const;
		...
		DNG_EXPORT uint32 PixelRange () const;

		/// Getter for best "tile stride" for accessing image.

		DNG_EXPORT virtual dng_rect RepeatingTile () const;
		...
		DNG_EXPORT void Get (dng_pixel_buffer &buffer,
		...
		DNG_EXPORT void Put (const dng_pixel_buffer &buffer);
		...
		DNG_EXPORT virtual bool EqualArea (const dng_image &rhs,
								const dng_rect &area,
								uint32 plane,
								uint32 planes) const;
		...
		DNG_EXPORT virtual void GetRepeat (dng_pixel_buffer &buffer,
	...
	protected:
		...
		DNG_EXPORT virtual void ReleaseTileBuffer (dng_tile_buffer &buffer) const;
	
		DNG_EXPORT virtual void DoGet (dng_pixel_buffer &buffer) const;
		
		DNG_EXPORT virtual void DoPut (const dng_pixel_buffer &buffer);

		DNG_EXPORT virtual void DoCopyArea (const dng_image &src,
								 const dng_rect &area,
								 uint32 srcPlane,
								 uint32 dstPlane,
								 uint32 planes);

		DNG_EXPORT void GetEdge (dng_pixel_buffer &buffer,
					  edge_option edgeOption,
					  const dng_rect &srcArea,
					  const dng_rect &dstArea) const;
					  
		DNG_EXPORT virtual void SetConstant (uint32 value,
								  const dng_rect &area);
