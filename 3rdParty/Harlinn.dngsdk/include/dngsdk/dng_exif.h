...
class dng_exif
	...
	public:
		...
		DNG_EXPORT virtual ~dng_exif ();

		/// Make clone.
		
		DNG_EXPORT virtual dng_exif * Clone () const;

		/// Clear all EXIF fields.
		
		DNG_EXPORT void SetEmpty ();

		/// Copy all GPS-related fields.
		/// \param exif Source object from which to copy GPS fields.
		
		DNG_EXPORT void CopyGPSFrom (const dng_exif &exif);

		/// Utility to fix up common errors and rounding issues with EXIF exposure
		/// times.

		DNG_EXPORT static real64 SnapExposureTime (real64 et);
		...
		DNG_EXPORT void SetExposureTime (real64 et,
							  bool snap = true);

		/// Set shutter speed value (APEX units) and exposure time.
		/// \param ss Shutter speed in APEX units.
		
		DNG_EXPORT void SetShutterSpeedValue (real64 ss);

		/// Utility to encode f-number as a rational.
		/// \param fs The f-number to encode.
		
		DNG_EXPORT static dng_urational EncodeFNumber (real64 fs);

		/// Set the FNumber and ApertureValue fields.
		/// \param fs The f-number to set.
		
		DNG_EXPORT void SetFNumber (real64 fs);
		
		/// Set the FNumber and ApertureValue fields.
		/// \param av The aperture value (APEX units).
		
		DNG_EXPORT void SetApertureValue (real64 av);

		/// Utility to convert aperture value (APEX units) to f-number.
		/// \param av The aperture value (APEX units) to convert.
		
		DNG_EXPORT static real64 ApertureValueToFNumber (real64 av);

		/// Utility to convert aperture value (APEX units) to f-number.
		/// \param av The aperture value (APEX units) to convert.
		
		DNG_EXPORT static real64 ApertureValueToFNumber (const dng_urational &av);

		/// Utility to convert f-number to aperture value (APEX units).
		/// \param fNumber The f-number to convert.
		
		DNG_EXPORT static real64 FNumberToApertureValue (real64 fNumber);

		/// Utility to convert f-number to aperture value (APEX units).
		/// \param fNumber The f-number to convert.
		
		DNG_EXPORT static real64 FNumberToApertureValue (const dng_urational &fNumber);
		...
		DNG_EXPORT bool AtLeastVersion0230 () const;
  
        /// Returns true iff the EXIF version is at least 2.3.1.
        
		DNG_EXPORT bool AtLeastVersion0231 () const;
        
        /// Sets the EXIF version to 2.3.1.
        
		DNG_EXPORT void SetVersion0231 ();

		DNG_EXPORT bool HasLensDistortInfo () const;
		
		DNG_EXPORT void SetLensDistortInfo (const dng_vector &params);
	...
	protected:
		...
		DNG_EXPORT virtual bool Parse_ifd0 (dng_stream &stream,
							     dng_shared &shared,
							 	 uint32 parentCode,
							 	 uint32 tagCode,
							 	 uint32 tagType,
		...
		DNG_EXPORT virtual bool Parse_ifd0_main (dng_stream &stream,
							          dng_shared &shared,
						 		 	  uint32 parentCode,
						 		 	  uint32 tagCode,
						 		 	  uint32 tagType,
		...
		DNG_EXPORT virtual bool Parse_ifd0_exif (dng_stream &stream,
							          dng_shared &shared,
						 		 	  uint32 parentCode,
						 		 	  uint32 tagCode,
						 		 	  uint32 tagType,
		...
		DNG_EXPORT virtual bool Parse_gps (dng_stream &stream,
							    dng_shared &shared,
						 		uint32 parentCode,
						 		uint32 tagCode,
						 		uint32 tagType,
		...
		DNG_EXPORT virtual bool Parse_interoperability (dng_stream &stream,
							    			 dng_shared &shared,
						 					 uint32 parentCode,
											 uint32 tagCode,
											 uint32 tagType,
