...
class dng_info: private dng_uncopyable
	...
	public:
		...
		DNG_EXPORT virtual bool IsValidDNG ();
		
	protected:
		
		DNG_EXPORT virtual void ValidateMagic ();

		DNG_EXPORT virtual void ParseTag (dng_host &host,
							   dng_stream &stream,
							   dng_exif *exif,
						 	   dng_shared *shared,
						 	   dng_ifd *ifd,
		...
		DNG_EXPORT virtual bool ValidateIFD (dng_stream &stream,
						 	      uint64 ifdOffset,
						 	      int64 offsetDelta);

		DNG_EXPORT virtual void ParseIFD (dng_host &host,
							   dng_stream &stream,
							   dng_exif *exif,
						 	   dng_shared *shared,
						 	   dng_ifd *ifd,
		...
		DNG_EXPORT virtual bool ParseMakerNoteIFD (dng_host &host,
										dng_stream &stream,
										uint64 ifdSize,
								 	    uint64 ifdOffset,
								 	    int64 offsetDelta,
		...
		DNG_EXPORT virtual void ParseMakerNote (dng_host &host,
									 dng_stream &stream,
		...
		DNG_EXPORT virtual void ParseSonyPrivateData (dng_host &host,
										   dng_stream &stream,
										   uint64 count,
										   uint64 oldOffset,
										   uint64 newOffset);
							   		 
		DNG_EXPORT virtual void ParseDNGPrivateData (dng_host &host,
										  dng_stream &stream);
