...
DNG_EXPORT void OptimizeOrder (const void *&sPtr,
					void *&dPtr,
					uint32 sPixelSize,
					uint32 dPixelSize,
					uint32 &count0,
...
DNG_EXPORT void OptimizeOrder (void *&dPtr,
					uint32 dPixelSize,
					uint32 &count0,
					uint32 &count1,
					uint32 &count2,
...
class dng_pixel_buffer
	...
	private:
		...
		DNG_EXPORT void CheckPixelType (uint32 pixelType) const;
		
#endif
		
	public:
	
		DNG_EXPORT dng_pixel_buffer ();
		...
		DNG_EXPORT dng_pixel_buffer (const dng_rect &area,
						  uint32 plane, 
						  uint32 planes,
						  uint32 pixelType, 
		...
		DNG_EXPORT dng_pixel_buffer (const dng_pixel_buffer &buffer);
		
		DNG_EXPORT dng_pixel_buffer & operator= (const dng_pixel_buffer &buffer);

		DNG_EXPORT virtual ~dng_pixel_buffer ();
		...
		DNG_EXPORT void SetConstant (const dng_rect &area,
					      uint32 plane,
					      uint32 planes,
					      uint32 value);
		...
		DNG_EXPORT void SetZero (const dng_rect &area,
					  uint32 plane,
					  uint32 planes);
		...
		DNG_EXPORT void CopyArea (const dng_pixel_buffer &src,
					   const dng_rect &area,
					   uint32 srcPlane,
					   uint32 dstPlane,
					   uint32 planes);
		...
		DNG_EXPORT static dng_point RepeatPhase (const dng_rect &srcArea,
		...
		DNG_EXPORT void RepeatArea (const dng_rect &srcArea,
						 const dng_rect &dstArea);
						 
		/// Replicates a sub-area of a buffer to fill the entire buffer.
		
		DNG_EXPORT void RepeatSubArea (const dng_rect subArea,
						    uint32 repeatV = 1,
						    uint32 repeatH = 1);
		...
		DNG_EXPORT void ShiftRight (uint32 shift);
		...
		DNG_EXPORT bool EqualArea (const dng_pixel_buffer &rhs,
					    const dng_rect &area,
					    uint32 plane,
					    uint32 planes) const;
		...
		DNG_EXPORT real64 MaximumDifference (const dng_pixel_buffer &rhs,
								  const dng_rect &area,
								  uint32 plane,
								  uint32 planes) const;
