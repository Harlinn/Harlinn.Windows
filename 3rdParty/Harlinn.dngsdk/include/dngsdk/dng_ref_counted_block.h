...
class dng_ref_counted_block
	...
	public:
		...
		DNG_EXPORT dng_ref_counted_block (const dng_ref_counted_block &data);
		
		/// Assignment operatore takes a reference to right hand side and does not copy the data.
		
		DNG_EXPORT dng_ref_counted_block & operator= (const dng_ref_counted_block &data);
		...
		DNG_EXPORT void EnsureWriteable ();
