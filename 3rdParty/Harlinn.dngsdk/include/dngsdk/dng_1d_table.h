...
class dng_1d_table: private dng_uncopyable
	...
	public:
		...
		static constexpr uint32 kMinTableSize = 512;
	
	private:
	
		/// Constant denoting default size of table.

		static constexpr uint32 kDefaultTableSize = 4096;
	...
	public:
		...
		DNG_EXPORT explicit dng_1d_table (uint32 count = kDefaultTableSize);
			
		DNG_EXPORT virtual ~dng_1d_table ();
		...
		DNG_EXPORT void Initialize (dng_memory_allocator &allocator,
		...
		DNG_EXPORT void Expand16 (uint16 *table16) const;
			
	private:
	
		DNG_EXPORT void SubDivide (const dng_1d_function &function,
						uint32 lower,
						uint32 upper,
						real32 maxDelta);
