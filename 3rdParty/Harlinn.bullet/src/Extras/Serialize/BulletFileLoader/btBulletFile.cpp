...
#include "bullet/Extras/Serialize/BulletFileLoader/btBulletFile.h"
#include "bullet/Extras/Serialize/BulletFileLoader/bDefines.h"
#include "bullet/Extras/Serialize/BulletFileLoader/bDNA.h"
...
btBulletFile::btBulletFile()
	: Bullet2::bFile("", "BULLET ")
{
	mMemoryDNA = new Bullet2::bDNA();  //this memory gets released in the bFile::~bFile destructor,@todo not consistent with the rule 'who allocates it, has to deallocate it"
...
void btBulletFile::parseData()
{
	...
	const bool brokenDNA = (mFlags & Bullet2::FD_BROKEN_DNA) != 0;
	...
	//invalid/empty file?
	if (remain < sizeof( Bullet2::bChunkInd))
		return;

	char* dataPtr = mFileBuffer + mDataStart;

	Bullet2::bChunkInd dataChunk;
	dataChunk.code = 0;

	//dataPtr += ChunkUtils::getNextBlock(&dataChunk, dataPtr, mFlags);
	int seek = getNextBlock(&dataChunk, dataPtr, mFlags);

	if (mFlags & Bullet2::FD_ENDIAN_SWAP)
		swapLen(dataPtr);
	...
	while (dataChunk.code != DNA1)
	{
		if (!brokenDNA || (dataChunk.code != BT_QUANTIZED_BVH_CODE))
		{
			...
			dataPtrHead = dataPtr + Bullet2::ChunkUtils::getOffset(mFlags);
...
void btBulletFile::writeDNA(FILE* fp)
{
	...
	if ( Bullet2::VOID_IS_8)
	{
		dataChunk.len = sBulletDNAlen64;
		dataChunk.oldPtr = sBulletDNAstr64;
		fwrite(&dataChunk, sizeof( Bullet2::bChunkInd), 1, fp);
		fwrite(sBulletDNAstr64, sBulletDNAlen64, 1, fp);
	}
	else
	{
...
void btBulletFile::addStruct(const char* structType, void* data, int len, void* oldPtr, int code)
{
	...
	bParse::Bullet2::bChunkInd dataChunk;
	dataChunk.code = code;
	dataChunk.nr = 1;
	dataChunk.len = len;
