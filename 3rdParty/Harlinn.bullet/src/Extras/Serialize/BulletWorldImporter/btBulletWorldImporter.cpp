...
{
	bool ok = (bulletFile2->getFlags() & bParse::Bullet2::FD_OK) != 0;
	...
	if (m_verboseMode & bParse::Bullet2::FD_VERBOSE_DUMP_CHUNKS)
	{
...
bool btBulletWorldImporter::convertAllObjects(bParse::btBulletFile* bulletFile2)
{
	...
	for (i = 0; i < bulletFile2->m_bvhs.size(); i++)
	{
		...
		if (bulletFile2->getFlags() & bParse::Bullet2::FD_DOUBLE_PRECISION)
		{
	...
	for (i = 0; i < bulletFile2->m_constraints.size(); i++)
	{
		...
		bool isDoublePrecisionData = (bulletFile2->getFlags() & bParse::Bullet2::FD_DOUBLE_PRECISION) != 0;
