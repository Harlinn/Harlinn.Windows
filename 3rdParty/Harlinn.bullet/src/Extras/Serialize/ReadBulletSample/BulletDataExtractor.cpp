...
#include "bullet/Extras/Serialize/ReadBulletSample/BulletDataExtractor.h"
...
void* btBulletDataExtractor::convertCollisionShape( Bullet::btCollisionShapeData* shapeData)
{
	void* shape = 0;
	...
	switch (shapeData->m_shapeType)
	{
		case STATIC_PLANE_PROXYTYPE:
		{
			...
			void* shape = this->createPlaneShape(planeData->m_planeNormal, planeData->m_planeConstant, planeData->m_localScaling);
			break;
		}
		...
		case CONVEX_HULL_SHAPE_PROXYTYPE:
		{
			...
			switch (shapeData->m_shapeType)
			{
				case BOX_SHAPE_PROXYTYPE:
				{
					shape = this->createBoxShape(bsd->m_implicitShapeDimensions, bsd->m_localScaling, bsd->m_collisionMargin);
					break;
				}
				case SPHERE_SHAPE_PROXYTYPE:
				{
					shape = this->createSphereShape(bsd->m_implicitShapeDimensions.m_floats[0], bsd->m_localScaling, bsd->m_collisionMargin);
					break;
				}
#if 0
					case CAPSULE_SHAPE_PROXYTYPE:
