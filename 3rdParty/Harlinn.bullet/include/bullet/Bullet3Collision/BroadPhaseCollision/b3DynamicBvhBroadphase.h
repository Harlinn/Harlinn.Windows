...
struct b3DynamicBvhBroadphase
{
...
#endif
	/* Methods		*/
	BT_EXPORT b3DynamicBvhBroadphase(int proxyCapacity, b3OverlappingPairCache* paircache = 0);
	BT_EXPORT virtual ~b3DynamicBvhBroadphase();
	BT_EXPORT void collide(b3Dispatcher* dispatcher);
	BT_EXPORT void optimize();

	/* b3BroadphaseInterface Implementation	*/
	BT_EXPORT b3BroadphaseProxy* createProxy(const b3Vector3& aabbMin, const b3Vector3& aabbMax, int objectIndex, void* userPtr, int collisionFilterGroup, int collisionFilterMask);
	BT_EXPORT virtual void destroyProxy(b3BroadphaseProxy* proxy, b3Dispatcher* dispatcher);
	BT_EXPORT virtual void setAabb(int objectId, const b3Vector3& aabbMin, const b3Vector3& aabbMax, b3Dispatcher* dispatcher);
	BT_EXPORT virtual void rayTest(const b3Vector3& rayFrom, const b3Vector3& rayTo, b3BroadphaseRayCallback& rayCallback, const b3Vector3& aabbMin = b3MakeVector3(0, 0, 0), const b3Vector3& aabbMax = b3MakeVector3(0, 0, 0));
	BT_EXPORT virtual void aabbTest(const b3Vector3& aabbMin, const b3Vector3& aabbMax, b3BroadphaseAabbCallback& callback);

	//virtual void					getAabb(b3BroadphaseProxy* proxy,b3Vector3& aabbMin, b3Vector3& aabbMax ) const;
	BT_EXPORT virtual void getAabb(int objectId, b3Vector3& aabbMin, b3Vector3& aabbMax) const;
	BT_EXPORT virtual void calculateOverlappingPairs(b3Dispatcher* dispatcher = 0);
	BT_EXPORT virtual b3OverlappingPairCache* getOverlappingPairCache();
	BT_EXPORT virtual const b3OverlappingPairCache* getOverlappingPairCache() const;
	BT_EXPORT virtual void getBroadphaseAabb(b3Vector3& aabbMin, b3Vector3& aabbMax) const;
	BT_EXPORT virtual void printStats();

	///reset broadphase internal structures, to ensure determinism/reproducability
	BT_EXPORT virtual void resetPool(b3Dispatcher* dispatcher);

	BT_EXPORT void performDeferredRemoval(b3Dispatcher* dispatcher);
	...
	BT_EXPORT void setAabbForceUpdate(b3BroadphaseProxy* absproxy, const b3Vector3& aabbMin, const b3Vector3& aabbMax, b3Dispatcher* /*dispatcher*/);
