...
struct b3DynamicBvh
{
	...
	// Methods
	BT_EXPORT b3DynamicBvh();
	BT_EXPORT ~b3DynamicBvh();
	BT_EXPORT void clear();
	bool empty() const { return (0 == m_root); }
	BT_EXPORT void optimizeBottomUp();
	BT_EXPORT void optimizeTopDown(int bu_treshold = 128);
	BT_EXPORT void optimizeIncremental(int passes);
	BT_EXPORT b3DbvtNode* insert(const b3DbvtVolume& box, void* data);
	BT_EXPORT void update(b3DbvtNode* leaf, int lookahead = -1);
	BT_EXPORT void update(b3DbvtNode* leaf, b3DbvtVolume& volume);
	BT_EXPORT bool update(b3DbvtNode* leaf, b3DbvtVolume& volume, const b3Vector3& velocity, b3Scalar margin);
	BT_EXPORT bool update(b3DbvtNode* leaf, b3DbvtVolume& volume, const b3Vector3& velocity);
	BT_EXPORT bool update(b3DbvtNode* leaf, b3DbvtVolume& volume, b3Scalar margin);
	BT_EXPORT void remove(b3DbvtNode* leaf);
	BT_EXPORT void write(IWriter* iwriter) const;
	BT_EXPORT void clone(b3DynamicBvh& dest, IClone* iclone = 0) const;
	BT_EXPORT static int maxdepth(const b3DbvtNode* node);
	BT_EXPORT static int countLeaves(const b3DbvtNode* node);
	BT_EXPORT static void extractLeaves(const b3DbvtNode* node, b3AlignedObjectArray<const b3DbvtNode*>& leaves);
#if B3_DBVT_ENABLE_BENCHMARK
	BT_EXPORT static void benchmark();
#else
