...
protected:
	...
	BT_EXPORT int setAngularLimits(btConstraintInfo2 * info, int row_offset, const btTransform& transA, const btTransform& transB, const btVector3& linVelA, const btVector3& linVelB, const btVector3& angVelA, const btVector3& angVelB);
	BT_EXPORT int setLinearLimits(btConstraintInfo2 * info, int row, const btTransform& transA, const btTransform& transB, const btVector3& linVelA, const btVector3& linVelB, const btVector3& angVelA, const btVector3& angVelB);

	BT_EXPORT void calculateLinearInfo();
	BT_EXPORT void calculateAngleInfo();
	BT_EXPORT void testAngularLimitMotor(int axis_index);

	BT_EXPORT void calculateJacobi(btRotationalLimitMotor2 * limot, const btTransform& transA, const btTransform& transB, btConstraintInfo2* info, int srow, btVector3& ax1, int rotational, int rotAllowed);
	BT_EXPORT int get_limit_motor_info2(btRotationalLimitMotor2 * limot,
...
public:
	...
	BT_EXPORT btGeneric6DofSpring2Constraint(btRigidBody & rbA, btRigidBody & rbB, const btTransform& frameInA, const btTransform& frameInB, RotateOrder rotOrder = RO_XYZ);
	BT_EXPORT btGeneric6DofSpring2Constraint(btRigidBody & rbB, const btTransform& frameInB, RotateOrder rotOrder = RO_XYZ);

	BT_EXPORT virtual void buildJacobian() {}
	BT_EXPORT virtual void getInfo1(btConstraintInfo1 * info);
	BT_EXPORT virtual void getInfo2(btConstraintInfo2 * info);
	...
	BT_EXPORT void calculateTransforms(const btTransform& transA, const btTransform& transB);
	BT_EXPORT void calculateTransforms();
	...
	BT_EXPORT void setFrames(const btTransform& frameA, const btTransform& frameB);
	...
	BT_EXPORT void setAxis(const btVector3& axis1, const btVector3& axis2);

	BT_EXPORT void setBounce(int index, btScalar bounce);

	BT_EXPORT void enableMotor(int index, bool onOff);
	BT_EXPORT void setServo(int index, bool onOff);  // set the type of the motor (servo or not) (the motor has to be turned on for servo also)
	BT_EXPORT void setTargetVelocity(int index, btScalar velocity);
	BT_EXPORT void setServoTarget(int index, btScalar target);
	BT_EXPORT void setMaxMotorForce(int index, btScalar force);

	BT_EXPORT void enableSpring(int index, bool onOff);
	BT_EXPORT void setStiffness(int index, btScalar stiffness, bool limitIfNeeded = true);  // if limitIfNeeded is true the system will automatically limit the stiffness in necessary situations where otherwise the spring would move unrealistically too widely
	BT_EXPORT void setDamping(int index, btScalar damping, bool limitIfNeeded = true);      // if limitIfNeeded is true the system will automatically limit the damping in necessary situations where otherwise the spring would blow up
	BT_EXPORT void setEquilibriumPoint();                                                   // set the current constraint position/orientation as an equilibrium point for all DOF
	BT_EXPORT void setEquilibriumPoint(int index);                                          // set the current constraint position/orientation as an equilibrium point for given DOF
	BT_EXPORT void setEquilibriumPoint(int index, btScalar val);

	//override the default global value of a parameter (such as ERP or CFM), optionally provide the axis (0..5).
	//If no axis is provided, it uses the default axis for this constraint.
	BT_EXPORT virtual void setParam(int num, btScalar value, int axis = -1);
	BT_EXPORT virtual btScalar getParam(int num, int axis = -1) const;

	BT_EXPORT static btScalar btGetMatrixElem(const btMatrix3x3& mat, int index);
	BT_EXPORT static bool matrixToEulerXYZ(const btMatrix3x3& mat, btVector3& xyz);
	BT_EXPORT static bool matrixToEulerXZY(const btMatrix3x3& mat, btVector3& xyz);
	BT_EXPORT static bool matrixToEulerYXZ(const btMatrix3x3& mat, btVector3& xyz);
	BT_EXPORT static bool matrixToEulerYZX(const btMatrix3x3& mat, btVector3& xyz);
	BT_EXPORT static bool matrixToEulerZXY(const btMatrix3x3& mat, btVector3& xyz);
	BT_EXPORT static bool matrixToEulerZYX(const btMatrix3x3& mat, btVector3& xyz);
};
