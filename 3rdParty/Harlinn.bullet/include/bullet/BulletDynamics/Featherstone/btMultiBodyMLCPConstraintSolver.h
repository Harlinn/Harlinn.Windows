...
protected:
	...
	BT_EXPORT virtual void createMLCPFast(const btContactSolverInfo& infoGlobal);

	/// Constructs MLCP terms for constraints of two rigid bodies
	BT_EXPORT void createMLCPFastRigidBody(const btContactSolverInfo& infoGlobal);

	/// Constructs MLCP terms for constraints of two multi-bodies or one rigid body and one multibody
	BT_EXPORT void createMLCPFastMultiBody(const btContactSolverInfo& infoGlobal);

	/// Solves MLCP and returns the success
	BT_EXPORT virtual bool solveMLCP(const btContactSolverInfo& infoGlobal);

	// Documentation inherited
	BT_EXPORT btScalar solveGroupCacheFriendlySetup(
	...
	// Documentation inherited
	BT_EXPORT btScalar solveGroupCacheFriendlyIterations(
		btCollisionObject** bodies,
		int numBodies,
		btPersistentManifold** manifoldPtr,
		int numManifolds,
...
public:
	...
	BT_EXPORT explicit btMultiBodyMLCPConstraintSolver(btMLCPSolverInterface* solver);

	/// Destructor
	BT_EXPORT virtual ~btMultiBodyMLCPConstraintSolver();

	/// Sets MLCP solver. Assumed it's not null.
	BT_EXPORT void setMLCPSolver(btMLCPSolverInterface* solver);

	/// Returns the number of fallbacks of using btSequentialImpulseConstraintSolver, which happens when the MLCP
	/// solver fails.
	BT_EXPORT int getNumFallbacks() const;

	/// Sets the number of fallbacks. This function may be used to reset the number to zero.
	BT_EXPORT void setNumFallbacks(int num);

	/// Returns the constraint solver type.
	BT_EXPORT virtual btConstraintSolverType getSolverType() const;
};
