...
public:
	...
	BT_EXPORT btMultiBody(int n_links,               // NOT including the base
				btScalar mass,             // mass of base
				const btVector3 &inertia,  // inertia of base, in base frame; assumed diagonal
				bool fixedBase,            // whether the base is fixed (true) or can move (false)
				bool canSleep, bool deprecatedMultiDof = true);

	BT_EXPORT virtual ~btMultiBody();

	//note: fixed link collision with parent is always disabled
	BT_EXPORT void setupFixed(int i, //linkIndex
					btScalar mass,
	...
	BT_EXPORT void setupPrismatic(int i,
						btScalar mass,
						const btVector3 &inertia,
						int parent,
						const btQuaternion &rotParentToThis,
	...
	BT_EXPORT void setupRevolute(int i,  // 0 to num_links-1
					   btScalar mass,
					   const btVector3 &inertia,
					   int parentIndex,
	...
	BT_EXPORT void setupSpherical(int i,  // linkIndex, 0 to num_links-1
						btScalar mass,
						const btVector3 &inertia,
						int parent,
						const btQuaternion &rotParentToThis,          // rotate points in parent frame to this frame, when q = 0
	...
	BT_EXPORT void setupPlanar(int i,  // 0 to num_links-1
					 btScalar mass,
					 const btVector3 &inertia,
					 int parent,
					 const btQuaternion &rotParentToThis,  // rotate points in parent frame to this frame, when q = 0
	...
	//
	BT_EXPORT int getParent(int link_num) const;
	...
	BT_EXPORT btScalar getLinkMass(int i) const;
	BT_EXPORT const btVector3 &getLinkInertia(int i) const;
	...
	BT_EXPORT void saveKinematicState(btScalar timeStep);

	//
	// get/set pos/vel for child m_links (i = 0 to num_links-1)
	//

	BT_EXPORT btScalar getJointPos(int i) const;
	BT_EXPORT btScalar getJointVel(int i) const;

	BT_EXPORT btScalar *getJointVelMultiDof(int i);
	BT_EXPORT btScalar *getJointPosMultiDof(int i);

	BT_EXPORT const btScalar *getJointVelMultiDof(int i) const;
	BT_EXPORT const btScalar *getJointPosMultiDof(int i) const;

	BT_EXPORT void setJointPos(int i, btScalar q);
	BT_EXPORT void setJointVel(int i, btScalar qdot);
	BT_EXPORT void setJointPosMultiDof(int i, const double *q);
	BT_EXPORT void setJointVelMultiDof(int i, const double *qdot);
	BT_EXPORT void setJointPosMultiDof(int i, const float *q);
	BT_EXPORT void setJointVelMultiDof(int i, const float *qdot);
  ...
  */
	...
	BT_EXPORT const btVector3 &getRVector(int i) const;              // vector from COM(parent(i)) to COM(i), in frame i's coords
	BT_EXPORT const btQuaternion &getParentToLocalRot(int i) const;  // rotates vectors in frame parent(i) to vectors in frame i.
    BT_EXPORT const btVector3 &getInterpolateRVector(int i) const;              // vector from COM(parent(i)) to COM(i), in frame i's coords
    BT_EXPORT const btQuaternion &getInterpolateParentToLocalRot(int i) const;  // rotates vectors in frame parent(i) to vectors in frame i.

	//
	// transform vectors in local frame of link i to world frame (or vice versa)
	//
	BT_EXPORT btVector3 localPosToWorld(int i, const btVector3 &local_pos) const;
	BT_EXPORT btVector3 localDirToWorld(int i, const btVector3 &local_dir) const;
	BT_EXPORT btVector3 worldPosToLocal(int i, const btVector3 &world_pos) const;
	BT_EXPORT btVector3 worldDirToLocal(int i, const btVector3 &world_dir) const;

	//
	// transform a frame in local coordinate to a frame in world coordinate
	//
	BT_EXPORT btMatrix3x3 localFrameToWorld(int i, const btMatrix3x3 &local_frame) const;
	...
	BT_EXPORT void clearForcesAndTorques();
	BT_EXPORT void clearConstraintForces();

	BT_EXPORT void clearVelocities();
	...
	BT_EXPORT void addLinkForce(int i, const btVector3 &f);
	BT_EXPORT void addLinkTorque(int i, const btVector3 &t);
	...
	BT_EXPORT void addLinkConstraintForce(int i, const btVector3 &f);
	BT_EXPORT void addLinkConstraintTorque(int i, const btVector3 &t);

	BT_EXPORT void addJointTorque(int i, btScalar Q);
	BT_EXPORT void addJointTorqueMultiDof(int i, int dof, btScalar Q);
	BT_EXPORT void addJointTorqueMultiDof(int i, const btScalar *Q);

	const btVector3 &getBaseForce() const { return m_baseForce; }
	const btVector3 &getBaseTorque() const { return m_baseTorque; }
	BT_EXPORT const btVector3 &getLinkForce(int i) const;
	BT_EXPORT const btVector3 &getLinkTorque(int i) const;
	BT_EXPORT btScalar getJointTorque(int i) const;
	BT_EXPORT btScalar *getJointTorqueMultiDof(int i);
	...
	BT_EXPORT void computeAccelerationsArticulatedBodyAlgorithmMultiDof(btScalar dt,
	...
	BT_EXPORT void calcAccelerationDeltasMultiDof(const btScalar *force, btScalar *output,
										btAlignedObjectArray<btScalar> &scratch_r,
	...
	BT_EXPORT void stepPositionsMultiDof(btScalar dt, btScalar *pq = 0, btScalar *pqd = 0);
    
    // predict the positions
	BT_EXPORT void predictPositionsMultiDof(btScalar dt);
	...
	BT_EXPORT void fillContactJacobianMultiDof(int link,
	...
	BT_EXPORT void fillConstraintJacobianMultiDof(int link,
										const btVector3 &contact_point,
	...
	BT_EXPORT void wakeUp();
	BT_EXPORT void goToSleep();
	BT_EXPORT void checkMotionAndSleepIfRequired(btScalar timestep);

	BT_EXPORT bool hasFixedBase() const;

	BT_EXPORT bool isBaseKinematic() const;

	BT_EXPORT bool isBaseStaticOrKinematic() const;

	// set the dynamic type in the base's collision flags.
	BT_EXPORT void setBaseDynamicType(int dynamicType);
	...
	BT_EXPORT void finalizeMultiDof();
	...
	BT_EXPORT void forwardKinematics(btAlignedObjectArray<btQuaternion>& world_to_local, btAlignedObjectArray<btVector3> & local_origin);

	BT_EXPORT void compTreeLinkVelocities(btVector3 * omega, btVector3 * vel) const;

	BT_EXPORT void updateCollisionObjectWorldTransforms(btAlignedObjectArray<btQuaternion> & world_to_local, btAlignedObjectArray<btVector3> & local_origin);
	BT_EXPORT void updateCollisionObjectInterpolationWorldTransforms(btAlignedObjectArray<btQuaternion> & world_to_local, btAlignedObjectArray<btVector3> & local_origin);
	...
	BT_EXPORT static void spatialTransform(const btMatrix3x3 &rotation_matrix,  // rotates vectors in 'from' frame to vectors in 'to' frame
	...
	BT_EXPORT void setLinkDynamicType(const int i, int type);

	BT_EXPORT bool isLinkStaticOrKinematic(const int i) const;

	BT_EXPORT bool isLinkKinematic(const int i) const;

	BT_EXPORT bool isLinkAndAllAncestorsStaticOrKinematic(const int i) const;

	BT_EXPORT bool isLinkAndAllAncestorsKinematic(const int i) const;
...
private:
	BT_EXPORT btMultiBody(const btMultiBody &);     // not implemented
	BT_EXPORT void operator=(const btMultiBody &);  // not implemented

	BT_EXPORT void solveImatrix(const btVector3 &rhs_top, const btVector3 &rhs_bot, btScalar result[6]) const;
	BT_EXPORT void solveImatrix(const btSpatialForceVector &rhs, btSpatialMotionVector &result) const;
	...
	BT_EXPORT void mulMatrix(const btScalar *pA, const btScalar *pB, int rowsA, int colsA, int rowsB, int colsB, btScalar *pC) const;
