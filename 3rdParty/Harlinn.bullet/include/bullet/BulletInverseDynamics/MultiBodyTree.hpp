...
public:
	...
	BT_EXPORT int addBody(int body_index, int parent_index, JointType joint_type,
	...
	BT_EXPORT void setAcceptInvalidMassParameters(bool flag);
	/// @return the mass properties policy flag
	BT_EXPORT bool getAcceptInvalidMassProperties() const;
	/// build internal data structures
	/// call this after all bodies have been added via addBody
	/// @return 0 on success, -1 on error
	BT_EXPORT int finalize();
	/// pretty print ascii description of tree to stdout
	BT_EXPORT void printTree();
	/// print tree data to stdout
	BT_EXPORT void printTreeData();
	...
	/// @return 0 on success, -1 on error
	BT_EXPORT int calculateInverseDynamics(const vecx& q, const vecx& u, const vecx& dot_u,
								 vecx* joint_forces);
	...
	BT_EXPORT int calculateMassMatrix(const vecx& q, const bool update_kinematics,
	...
	/// @param mass_matrix matrix for storing the output (should be dim(q)xdim(q))
	/// @return -1 on error, 0 on success
	BT_EXPORT int calculateMassMatrix(const vecx& q, matxx* mass_matrix);
	...
	BT_EXPORT int calculateKinematics(const vecx& q, const vecx& u, const vecx& dot_u);
	/// Calculate position kinematics
	BT_EXPORT int calculatePositionKinematics(const vecx& q);
	/// Calculate position and velocity kinematics
	BT_EXPORT int calculatePositionAndVelocityKinematics(const vecx& q, const vecx& u);
	...
	BT_EXPORT int calculateJacobians(const vecx& q, const vecx& u);
	...
	BT_EXPORT int calculateJacobians(const vecx& q);
	...
	/// @return 0 on success, -1 on error
	BT_EXPORT int setGravityInWorldFrame(const vec3& gravity);
	/// returns number of bodies in tree
	BT_EXPORT int numBodies() const;
	/// returns number of mechanical degrees of freedom (dimension of q-vector)
	BT_EXPORT int numDoFs() const;
	...
	/// @return 0 on success, -1 on error
	BT_EXPORT int getBodyLinearVelocityCoM(const int body_index, vec3* world_velocity) const;
	/// get origin of a body-fixed frame, represented in world frame
	/// @param body_index index for frame/body
	/// @param world_origin pointer for return data
	/// @return 0 on success, -1 on error
	BT_EXPORT int getBodyAngularAcceleration(const int body_index, vec3* world_dot_omega) const;
	/// get origin of a body-fixed frame, represented in world frame
	...
	/// @param world_origin pointer for return data
	/// @return 0 on success, -1 on error
	BT_EXPORT int getBodyLinearAcceleration(const int body_index, vec3* world_acceleration) const;

#if (defined BT_ID_HAVE_MAT3X) && (defined BT_ID_WITH_JACOBIANS)
	// get translational jacobian, in world frame (dworld_velocity/du)
	BT_EXPORT int getBodyJacobianTrans(const int body_index, mat3x* world_jac_trans) const;
	// get rotational jacobian, in world frame (dworld_omega/du)
	BT_EXPORT int getBodyJacobianRot(const int body_index, mat3x* world_jac_rot) const;
	// get product of translational jacobian derivative * generatlized velocities
	BT_EXPORT int getBodyDotJacobianTransU(const int body_index, vec3* world_dot_jac_trans_u) const;
	// get product of rotational jacobian derivative * generatlized velocities
	BT_EXPORT int getBodyDotJacobianRotU(const int body_index, vec3* world_dot_jac_rot_u) const;
#endif  // BT_ID_HAVE_MAT3X
	...
	/// @return 0 on success, -1 on error
	BT_EXPORT int getParentIndex(const int body_index, int* parent_index) const;
	/// get joint type
	/// @param body_index index of the body
	/// @param joint_type the corresponding joint type
	/// @return 0 on success, -1 on failure
	BT_EXPORT int getJointType(const int body_index, JointType* joint_type) const;
	/// get joint type as string
	/// @param body_index index of the body
	/// @param joint_type string naming the corresponding joint type
	/// @return 0 on success, -1 on failure
	BT_EXPORT int getJointTypeStr(const int body_index, const char** joint_type) const;
	/// get offset translation to parent body (see addBody)
	/// @param body_index index of the body
	/// @param r the offset translation (see above)
	/// @return 0 on success, -1 on failure
	BT_EXPORT int getParentRParentBodyRef(const int body_index, vec3* r) const;
	/// get offset rotation to parent body (see addBody)
	/// @param body_index index of the body
	/// @param T the transform (see above)
	/// @return 0 on success, -1 on failure
	BT_EXPORT int getBodyTParentRef(const int body_index, mat33* T) const;
	/// get axis of motion (see addBody)
	/// @param body_index index of the body
	/// @param axis the axis (see above)
	/// @return 0 on success, -1 on failure
	BT_EXPORT int getBodyAxisOfMotion(const int body_index, vec3* axis) const;
	/// get offset for degrees of freedom of this body into the q-vector
	/// @param body_index index of the body
	/// @param q_offset offset the q vector
	/// @return -1 on error, 0 on success
	BT_EXPORT int getDoFOffset(const int body_index, int* q_offset) const;
	/// get user integer. not used by the library.
	/// @param body_index index of the body
	/// @param user_int   the user integer
	/// @return 0 on success, -1 on error
	BT_EXPORT int getUserInt(const int body_index, int* user_int) const;
	/// get user pointer. not used by the library.
	/// @param body_index index of the body
	/// @param user_ptr   the user pointer
	/// @return 0 on success, -1 on error
	BT_EXPORT int getUserPtr(const int body_index, void** user_ptr) const;
	/// set user integer. not used by the library.
	/// @param body_index index of the body
	/// @param user_int   the user integer
	/// @return 0 on success, -1 on error
	BT_EXPORT int setUserInt(const int body_index, const int user_int);
	/// set user pointer. not used by the library.
	/// @param body_index index of the body
	/// @param user_ptr   the user pointer
	/// @return 0 on success, -1 on error
	BT_EXPORT int setUserPtr(const int body_index, void* const user_ptr);
	/// set mass for a body
	/// @param body_index index of the body
	/// @param mass the mass to set
	/// @return 0 on success, -1 on failure
	BT_EXPORT int setBodyMass(const int body_index, const idScalar mass);
	...
	/// @return 0 on success, -1 on failure
	BT_EXPORT int setBodyFirstMassMoment(const int body_index, const vec3& first_mass_moment);
	...
	/// @return 0 on success, -1 on failure
	BT_EXPORT int setBodySecondMassMoment(const int body_index, const mat33& second_mass_moment);
	/// get mass for a body
	/// @param body_index index of the body
	/// @param mass the mass
	/// @return 0 on success, -1 on failure
	BT_EXPORT int getBodyMass(const int body_index, idScalar* mass) const;
	/// get first moment of mass for a body
	/// (mass * center of mass, in body fixed frame, relative to joint)
	/// @param body_index index of the body
	/// @param first_moment the vector
	/// @return 0 on success, -1 on failure
	BT_EXPORT int getBodyFirstMassMoment(const int body_index, vec3* first_mass_moment) const;
	/// get second moment of mass for a body
	/// (moment of inertia, in body fixed frame, relative to joint)
	/// @param body_index index of the body
	/// @param second_mass_moment the inertia matrix
	/// @return 0 on success, -1 on failure
	BT_EXPORT int getBodySecondMassMoment(const int body_index, mat33* second_mass_moment) const;
	/// set all user forces and moments to zero
	BT_EXPORT void clearAllUserForcesAndMoments();
	...
	/// @return 0 on success, -1 on error
	BT_EXPORT int addUserForce(const int body_index, const vec3& body_force);
	...
	/// @return 0 on success, -1 on error
	BT_EXPORT int addUserMoment(const int body_index, const vec3& body_moment);
