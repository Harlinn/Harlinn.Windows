...
struct btMiniSDF
{
	...
	BT_EXPORT bool load(const char* data, int size);
	bool isValid() const
	{
		return m_isValid;
	}
	BT_EXPORT unsigned int multiToSingleIndex(btMultiIndex const& ijk) const;

	BT_EXPORT btAlignedBox3d subdomain(btMultiIndex const& ijk) const;

	BT_EXPORT btMultiIndex singleToMultiIndex(unsigned int l) const;

	BT_EXPORT btAlignedBox3d subdomain(unsigned int l) const;

	BT_EXPORT btShapeMatrix
	shape_function_(btVector3 const& xi, btShapeGradients* gradient = 0) const;

	BT_EXPORT bool interpolate(unsigned int field_id, double& dist, btVector3 const& x, btVector3* gradient) const;
};
