...
struct btDbvt
{
	...
	BT_EXPORT btDbvtNode* insert(const btDbvtVolume& box, void* data);
	BT_EXPORT void update(btDbvtNode* leaf, int lookahead = -1);
	BT_EXPORT void update(btDbvtNode* leaf, btDbvtVolume& volume);
	BT_EXPORT bool update(btDbvtNode* leaf, btDbvtVolume& volume, const btVector3& velocity, btScalar margin);
	BT_EXPORT bool update(btDbvtNode* leaf, btDbvtVolume& volume, const btVector3& velocity);
	BT_EXPORT bool update(btDbvtNode* leaf, btDbvtVolume& volume, btScalar margin);
	BT_EXPORT void remove(btDbvtNode* leaf);
	BT_EXPORT void write(IWriter* iwriter) const;
	BT_EXPORT void clone(btDbvt& dest, IClone* iclone = 0) const;
	BT_EXPORT static int maxdepth(const btDbvtNode* node);
	BT_EXPORT static int countLeaves(const btDbvtNode* node);
	BT_EXPORT static void extractLeaves(const btDbvtNode* node, btAlignedObjectArray<const btDbvtNode*>& leaves);
