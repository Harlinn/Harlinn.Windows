...
public:
	...
	BT_EXPORT virtual void project(TVStack& x);

	// add friction force to the rhs of the linear solve
	BT_EXPORT virtual void applyDynamicFriction(TVStack& f);

	// update and solve the constraints
	BT_EXPORT virtual btScalar update(btCollisionObject** deformableBodies, int numDeformableBodies, const btContactSolverInfo& infoGlobal);

	// Add constraints to m_constraints. In addition, the constraints that each vertex own are recorded in m_constraintsDict.
	BT_EXPORT virtual void setConstraints(const btContactSolverInfo& infoGlobal);

	// Set up projections for each vertex by adding the projection direction to
	BT_EXPORT virtual void setProjection();

	BT_EXPORT virtual void reinitialize(bool nodeUpdated);

	BT_EXPORT btScalar solveSplitImpulse(btCollisionObject** deformableBodies, int numDeformableBodies, const btContactSolverInfo& infoGlobal);

	BT_EXPORT virtual void setLagrangeMultiplier();

	BT_EXPORT void checkConstraints(const TVStack& x);
};
