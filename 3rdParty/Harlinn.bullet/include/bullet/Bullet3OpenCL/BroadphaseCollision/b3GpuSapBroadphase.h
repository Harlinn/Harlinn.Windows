...
#include "bullet/Bullet3OpenCL/ParallelPrimitives/b3FillCL.h"  //b3Int2
class b3Vector3;
...
public:
	...
	BT_EXPORT b3GpuSapBroadphase(cl_context ctx, cl_device_id device, cl_command_queue q, b3GpuSapKernelType kernelType = B3_GPU_SAP_KERNEL_LOCAL_SHARED_MEMORY);
	BT_EXPORT virtual ~b3GpuSapBroadphase();
	...
	BT_EXPORT virtual void calculateOverlappingPairs(int maxPairs);
	BT_EXPORT virtual void calculateOverlappingPairsHost(int maxPairs);

	BT_EXPORT void reset();

	BT_EXPORT void init3dSap();
	BT_EXPORT virtual void calculateOverlappingPairsHostIncremental3Sap();

	BT_EXPORT virtual void createProxy(const b3Vector3& aabbMin, const b3Vector3& aabbMax, int userPtr, int collisionFilterGroup, int collisionFilterMask);
	BT_EXPORT virtual void createLargeProxy(const b3Vector3& aabbMin, const b3Vector3& aabbMax, int userPtr, int collisionFilterGroup, int collisionFilterMask);

	//call writeAabbsToGpu after done making all changes (createProxy etc)
	BT_EXPORT virtual void writeAabbsToGpu();

	BT_EXPORT virtual cl_mem getAabbBufferWS();
	BT_EXPORT virtual int getNumOverlap();
	BT_EXPORT virtual cl_mem getOverlappingPairBuffer();

	BT_EXPORT virtual b3OpenCLArray<b3Int4>& getOverlappingPairsGPU();
	BT_EXPORT virtual b3OpenCLArray<int>& getSmallAabbIndicesGPU();
	BT_EXPORT virtual b3OpenCLArray<int>& getLargeAabbIndicesGPU();
};
