...
public:
	BT_EXPORT b3GpuParallelLinearBvh(cl_context context, cl_device_id device, cl_command_queue queue);
	BT_EXPORT virtual ~b3GpuParallelLinearBvh();

	///Must be called before any other function
	BT_EXPORT void build(const b3OpenCLArray<b3SapAabb>& worldSpaceAabbs, const b3OpenCLArray<int>& smallAabbIndices,
	...
	BT_EXPORT void calculateOverlappingPairs(b3OpenCLArray<b3Int4>& out_overlappingPairs);
	...
	BT_EXPORT void testRaysAgainstBvhAabbs(const b3OpenCLArray<b3RayInfo>& rays,
								 b3OpenCLArray<int>& out_numRayRigidPairs, b3OpenCLArray<b3Int2>& out_rayRigidPairs);

private:
	BT_EXPORT void constructBinaryRadixTree();
};
