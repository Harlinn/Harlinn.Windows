...
public:
	...
	BT_EXPORT b3Solver(cl_context ctx, cl_device_id device, cl_command_queue queue, int pairCapacity);

	BT_EXPORT virtual ~b3Solver();

	BT_EXPORT void solveContactConstraint(const b3OpenCLArray<b3RigidBodyData>* bodyBuf, const b3OpenCLArray<b3InertiaData>* inertiaBuf,
								b3OpenCLArray<b3GpuConstraint4>* constraint, void* additionalData, int n, int maxNumBatches);

	BT_EXPORT void solveContactConstraintHost(b3OpenCLArray<b3RigidBodyData>* bodyBuf, b3OpenCLArray<b3InertiaData>* shapeBuf,
	...
	BT_EXPORT void convertToConstraints(const b3OpenCLArray<b3RigidBodyData>* bodyBuf,
