...
public:
	...
	BT_EXPORT b3LauncherCL(cl_command_queue queue, cl_kernel kernel, const char* name);

	BT_EXPORT virtual ~b3LauncherCL();

	BT_EXPORT void setBuffer(cl_mem clBuffer);

	BT_EXPORT void setBuffers(b3BufferInfoCL* buffInfo, int n);
	...
	BT_EXPORT int deserializeArgs(unsigned char* buf, int bufSize, cl_context ctx);

	BT_EXPORT int validateResults(unsigned char* goldBuffer, int goldBufferCapacity, cl_context ctx);

	BT_EXPORT int serializeArguments(unsigned char* destBuffer, int destBufferCapacity);
	...
	BT_EXPORT void serializeToFile(const char* fileName, int numWorkItems);
