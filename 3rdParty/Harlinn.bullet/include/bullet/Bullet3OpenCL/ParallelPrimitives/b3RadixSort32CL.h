...
private:
public:
	BT_EXPORT b3RadixSort32CL(cl_context ctx, cl_device_id device, cl_command_queue queue, int initialCapacity = 0);

	BT_EXPORT virtual ~b3RadixSort32CL();

	BT_EXPORT void execute(b3OpenCLArray<unsigned int>& keysIn, b3OpenCLArray<unsigned int>& keysOut, b3OpenCLArray<unsigned int>& valuesIn,
				 b3OpenCLArray<unsigned int>& valuesOut, int n, int sortBits = 32);

	///keys only
	BT_EXPORT void execute(b3OpenCLArray<unsigned int>& keysInOut, int sortBits = 32);

	BT_EXPORT void execute(b3OpenCLArray<b3SortData>& keyValuesInOut, int sortBits = 32);
	BT_EXPORT void executeHost(b3OpenCLArray<b3SortData>& keyValuesInOut, int sortBits = 32);
	BT_EXPORT void executeHost(b3AlignedObjectArray<b3SortData>& keyValuesInOut, int sortBits = 32);
};
