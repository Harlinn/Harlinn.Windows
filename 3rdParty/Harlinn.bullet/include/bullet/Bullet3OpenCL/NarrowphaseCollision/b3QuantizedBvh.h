...
protected:
	...
	BT_EXPORT void swapLeafNodes(int firstIndex, int secondIndex);

	BT_EXPORT void assignInternalNodeFromLeafNode(int internalNode, int leafNodeIndex);

protected:
	BT_EXPORT void buildTree(int startIndex, int endIndex);

	BT_EXPORT int calcSplittingAxis(int startIndex, int endIndex);

	BT_EXPORT int sortAndCalcSplittingIndex(int startIndex, int endIndex, int splitAxis);

	BT_EXPORT void walkStacklessTree(b3NodeOverlapCallback * nodeCallback, const b3Vector3& aabbMin, const b3Vector3& aabbMax) const;

	BT_EXPORT void walkStacklessQuantizedTreeAgainstRay(b3NodeOverlapCallback * nodeCallback, const b3Vector3& raySource, const b3Vector3& rayTarget, const b3Vector3& aabbMin, const b3Vector3& aabbMax, int startNodeIndex, int endNodeIndex) const;
	BT_EXPORT void walkStacklessQuantizedTree(b3NodeOverlapCallback * nodeCallback, unsigned short int* quantizedQueryAabbMin, unsigned short int* quantizedQueryAabbMax, int startNodeIndex, int endNodeIndex) const;
	BT_EXPORT void walkStacklessTreeAgainstRay(b3NodeOverlapCallback * nodeCallback, const b3Vector3& raySource, const b3Vector3& rayTarget, const b3Vector3& aabbMin, const b3Vector3& aabbMax, int startNodeIndex, int endNodeIndex) const;

	///tree traversal designed for small-memory processors like PS3 SPU
	BT_EXPORT void walkStacklessQuantizedTreeCacheFriendly(b3NodeOverlapCallback * nodeCallback, unsigned short int* quantizedQueryAabbMin, unsigned short int* quantizedQueryAabbMax) const;

	///use the 16-byte stackless 'skipindex' node tree to do a recursive traversal
	BT_EXPORT void walkRecursiveQuantizedTreeAgainstQueryAabb(const b3QuantizedBvhNode* currentNode, b3NodeOverlapCallback* nodeCallback, unsigned short int* quantizedQueryAabbMin, unsigned short int* quantizedQueryAabbMax) const;

	///use the 16-byte stackless 'skipindex' node tree to do a recursive traversal
	BT_EXPORT void walkRecursiveQuantizedTreeAgainstQuantizedTree(const b3QuantizedBvhNode* treeNodeA, const b3QuantizedBvhNode* treeNodeB, b3NodeOverlapCallback* nodeCallback) const;

	BT_EXPORT void updateSubtreeHeaders(int leftChildNodexIndex, int rightChildNodexIndex);

public:
	B3_DECLARE_ALIGNED_ALLOCATOR();

	BT_EXPORT b3QuantizedBvh();

	BT_EXPORT virtual ~b3QuantizedBvh();

	///***************************************** expert/internal use only *************************
	BT_EXPORT void setQuantizationValues(const b3Vector3& bvhAabbMin, const b3Vector3& bvhAabbMax, b3Scalar quantizationMargin = b3Scalar(1.0));
	QuantizedNodeArray& getLeafNodeArray() { return m_quantizedLeafNodes; }
	///buildInternal is expert use only: assumes that setQuantizationValues and LeafNodeArray are initialized
	BT_EXPORT void buildInternal();
	///***************************************** expert/internal use only *************************

	BT_EXPORT void reportAabbOverlappingNodex(b3NodeOverlapCallback * nodeCallback, const b3Vector3& aabbMin, const b3Vector3& aabbMax) const;
	BT_EXPORT void reportRayOverlappingNodex(b3NodeOverlapCallback * nodeCallback, const b3Vector3& raySource, const b3Vector3& rayTarget) const;
	BT_EXPORT void reportBoxCastOverlappingNodex(b3NodeOverlapCallback * nodeCallback, const b3Vector3& raySource, const b3Vector3& rayTarget, const b3Vector3& aabbMin, const b3Vector3& aabbMax) const;
	...
	BT_EXPORT unsigned calculateSerializeBufferSize() const;

	/// Data buffer MUST be 16 byte aligned
	BT_EXPORT virtual bool serialize(void* o_alignedDataBuffer, unsigned i_dataBufferSize, bool i_swapEndian) const;

	///deSerializeInPlace loads and initializes a BVH from a buffer in memory 'in place'
	BT_EXPORT static b3QuantizedBvh* deSerializeInPlace(void* i_alignedDataBuffer, unsigned int i_dataBufferSize, bool i_swapEndian);

	BT_EXPORT static unsigned int getAlignmentSerializationPadding();
	//////////////////////////////////////////////////////////////////////

	BT_EXPORT virtual int calculateSerializeBufferSizeNew() const;

	///fills the dataBuffer and returns the struct name (and 0 on failure)
	BT_EXPORT virtual const char* serialize(void* dataBuffer, b3Serializer* serializer) const;

	BT_EXPORT virtual void deSerializeFloat(struct b3QuantizedBvhFloatData & quantizedBvhFloatData);

	BT_EXPORT virtual void deSerializeDouble(struct b3QuantizedBvhDoubleData & quantizedBvhDoubleData);
...
private:
	...
	BT_EXPORT b3QuantizedBvh(b3QuantizedBvh & other, bool ownsMemory);
};
