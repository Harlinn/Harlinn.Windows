...
extern "C"
{
	...
	BT_EXPORT cl_context b3OpenCLUtils_createContextFromType(cl_device_type deviceType, cl_int* pErrNum, void* pGLCtx, void* pGLDC, int preferredDeviceIndex, int preferredPlatformIndex, cl_platform_id* platformId);

	BT_EXPORT int b3OpenCLUtils_getNumDevices(cl_context cxMainContext);

	BT_EXPORT cl_device_id b3OpenCLUtils_getDevice(cl_context cxMainContext, int nr);

	BT_EXPORT void b3OpenCLUtils_printDeviceInfo(cl_device_id device);

	BT_EXPORT cl_kernel b3OpenCLUtils_compileCLKernelFromString(cl_context clContext, cl_device_id device, const char* kernelSource, const char* kernelName, cl_int* pErrNum, cl_program prog, const char* additionalMacros);

	//optional
	BT_EXPORT cl_program b3OpenCLUtils_compileCLProgramFromString(cl_context clContext, cl_device_id device, const char* kernelSource, cl_int* pErrNum, const char* additionalMacros, const char* srcFileNameForCaching, bool disableBinaryCaching);

	//the following optional APIs provide access using specific platform information
	BT_EXPORT int b3OpenCLUtils_getNumPlatforms(cl_int* pErrNum);

	///get the nr'th platform, where nr is in the range [0..getNumPlatforms)
	BT_EXPORT cl_platform_id b3OpenCLUtils_getPlatform(int nr, cl_int* pErrNum);

	BT_EXPORT void b3OpenCLUtils_printPlatformInfo(cl_platform_id platform);

	BT_EXPORT const char* b3OpenCLUtils_getSdkVendorName();

	///set the path (directory/folder) where the compiled OpenCL kernel are stored
	BT_EXPORT void b3OpenCLUtils_setCachePath(const char* path);

	BT_EXPORT cl_context b3OpenCLUtils_createContextFromPlatform(cl_platform_id platform, cl_device_type deviceType, cl_int* pErrNum, void* pGLCtx, void* pGLDC, int preferredDeviceIndex, int preferredPlatformIndex);
...
struct b3OpenCLUtils
{
	...
	BT_EXPORT static void getPlatformInfo(cl_platform_id platform, b3OpenCLPlatformInfo* platformInfo);
