...
BT_EXPORT bool btIsMainThread();
BT_EXPORT bool btThreadsAreRunning();
BT_EXPORT unsigned int btGetCurrentThreadIndex();
BT_EXPORT void btResetThreadIndexCounter();  // notify that all worker threads have been destroyed
...
{
public:
	BT_EXPORT btITaskScheduler(const char* name);
	...
	// internal use only
	BT_EXPORT virtual void activate();
	BT_EXPORT virtual void deactivate();
...
BT_EXPORT void btSetTaskScheduler(btITaskScheduler* ts);

// get the current task scheduler
BT_EXPORT btITaskScheduler* btGetTaskScheduler();

// get non-threaded task scheduler (always available)
BT_EXPORT btITaskScheduler* btGetSequentialTaskScheduler();

// create a default task scheduler (Win32 or pthreads based)
BT_EXPORT btITaskScheduler* btCreateDefaultTaskScheduler();

// get OpenMP task scheduler (if available, otherwise returns null)
BT_EXPORT btITaskScheduler* btGetOpenMPTaskScheduler();

// get Intel TBB task scheduler (if available, otherwise returns null)
BT_EXPORT btITaskScheduler* btGetTBBTaskScheduler();

// get PPL task scheduler (if available, otherwise returns null)
BT_EXPORT btITaskScheduler* btGetPPLTaskScheduler();

// btParallelFor -- call this to dispatch work like a for-loop
//                 (iterations may be done out of order, so no dependencies are allowed)
BT_EXPORT void btParallelFor(int iBegin, int iEnd, int grainSize, const btIParallelForBody& body);

// btParallelSum -- call this to dispatch work like a for-loop, returns the sum of all iterations
//                 (iterations may be done out of order, so no dependencies are allowed)
BT_EXPORT btScalar btParallelSum(int iBegin, int iEnd, int grainSize, const btIParallelSumBody& body);
