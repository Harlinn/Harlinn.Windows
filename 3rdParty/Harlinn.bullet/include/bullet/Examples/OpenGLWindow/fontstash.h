...
BT_EXPORT struct sth_stash* sth_create(int cachew, int cacheh, RenderCallbacks* callbacks);

BT_EXPORT int sth_add_font(struct sth_stash* stash, const char* path);
BT_EXPORT int sth_add_font_from_memory(struct sth_stash* stash, unsigned char* buffer);

BT_EXPORT int sth_add_bitmap_font(struct sth_stash* stash, int ascent, int descent, int line_gap);
/*void sth_add_glyph(struct sth_stash* stash, int idx, unsigned int uid, const char* s,
                  short size, short base, int x, int y, int w, int h,
                  float xoffset, float yoffset, float xadvance);
				  */

BT_EXPORT void sth_begin_draw(struct sth_stash* stash);
BT_EXPORT void sth_end_draw(struct sth_stash* stash);

BT_EXPORT void sth_draw_texture(struct sth_stash* stash,
					  int idx, float size,
					  float x, float y,
					  int screenwidth, int screenheight,
					  const char* s, float* dx, float colorRGBA[4]);

BT_EXPORT void sth_flush_draw(struct sth_stash* stash);

BT_EXPORT void sth_draw_text3D(struct sth_stash* stash,
					 int idx, float fontSize,
					 float x, float y, float z,
					 const char* s, float* dx, float textScale, float colorRGBA[4], int bla);

BT_EXPORT void sth_draw_text(struct sth_stash* stash,
				   int idx, float size,
...
BT_EXPORT void sth_dim_text(struct sth_stash* stash, int idx, float size, const char* string,
				  float* minx, float* miny, float* maxx, float* maxy);

BT_EXPORT void sth_vmetrics(struct sth_stash* stash,
				  int idx, float size,
				  float* ascender, float* descender, float* lineh);

BT_EXPORT void sth_delete(struct sth_stash* stash);
