...
public:
	...
	b3Vector3( float xv, float yv, float zv, float wv = 0.f )
		: x( xv ), y( yv ), z( zv ), w( wv )
	{}
...
#if defined B3_USE_SSE
	const long scalar_cutoff = 10;
	BT_EXPORT long b3_mindot_large(const float* array, const float* vec, unsigned long array_count, float* dotOut);
#elif defined B3_USE_NEON
	const long scalar_cutoff = 4;
