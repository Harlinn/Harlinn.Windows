...
#define b3StopProfile
	...
	BT_EXPORT void b3SetCustomPrintfFunc(b3PrintfFunc* printfFunc);
	BT_EXPORT void b3SetCustomWarningMessageFunc(b3WarningMessageFunc* warningMsgFunc);
	BT_EXPORT void b3SetCustomErrorMessageFunc(b3ErrorMessageFunc* errorMsgFunc);

	///Set custom profile zone functions (zones can be nested)
	BT_EXPORT void b3SetCustomEnterProfileZoneFunc(b3EnterProfileZoneFunc* enterFunc);
	BT_EXPORT void b3SetCustomLeaveProfileZoneFunc(b3LeaveProfileZoneFunc* leaveFunc);

	///Don't use those internal functions directly, use the b3Printf or b3SetCustomPrintfFunc instead (or warning/error version)
	BT_EXPORT void b3OutputPrintfVarArgsInternal(const char* str, ...);
	BT_EXPORT void b3OutputWarningMessageVarArgsInternal(const char* str, ...);
	BT_EXPORT void b3OutputErrorMessageVarArgsInternal(const char* str, ...);
