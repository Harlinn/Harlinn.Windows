...
public:
	...
	void SetIdentity();
	BUSSIK_EXPORT void SetDiagonalEntries(double d);
	BUSSIK_EXPORT void SetDiagonalEntries(const VectorRn& d);
	BUSSIK_EXPORT void SetSuperDiagonalEntries(double d);
	BUSSIK_EXPORT void SetSuperDiagonalEntries(const VectorRn& d);
	BUSSIK_EXPORT void SetSubDiagonalEntries(double d);
	BUSSIK_EXPORT void SetSubDiagonalEntries(const VectorRn& d);
	BUSSIK_EXPORT void SetColumn(long i, const VectorRn& d);
	BUSSIK_EXPORT void SetRow(long i, const VectorRn& d);
	BUSSIK_EXPORT void SetSequence(const VectorRn& d, long startRow, long startCol, long deltaRow, long deltaCol);

	// Loads matrix in as a sub-matrix.  (i,j) is the base point. Defaults to (0,0).
	// The "Tranpose" versions load the transpose of A.
	BUSSIK_EXPORT void LoadAsSubmatrix(const MatrixRmn& A);
	void LoadAsSubmatrix(long i, long j, const MatrixRmn& A);
	BUSSIK_EXPORT void LoadAsSubmatrixTranspose(const MatrixRmn& A);
	void LoadAsSubmatrixTranspose(long i, long j, const MatrixRmn& A);

	// Norms
	double FrobeniusNormSq() const;
	BUSSIK_EXPORT double FrobeniusNorm() const;

	// Operations on VectorRn's
	BUSSIK_EXPORT void Multiply(const VectorRn& v, VectorRn& result) const;           // result = (this)*(v)
	BUSSIK_EXPORT void MultiplyTranspose(const VectorRn& v, VectorRn& result) const;  // Equivalent to mult by row vector on left
	BUSSIK_EXPORT double DotProductColumn(const VectorRn& v, long colNum) const;      // Returns dot product of v with i-th column
	...
	MatrixRmn& operator-=(const MatrixRmn& B);
	BUSSIK_EXPORT static MatrixRmn& Multiply(const MatrixRmn& A, const MatrixRmn& B, MatrixRmn& dst);           // Sets dst = A*B.
	BUSSIK_EXPORT static MatrixRmn& MultiplyTranspose(const MatrixRmn& A, const MatrixRmn& B, MatrixRmn& dst);  // Sets dst = A*(B-tranpose).
	BUSSIK_EXPORT static MatrixRmn& TransposeMultiply(const MatrixRmn& A, const MatrixRmn& B, MatrixRmn& dst);  // Sets dst = (A-transpose)*B.

	// Miscellaneous operation
	BUSSIK_EXPORT MatrixRmn& AddToDiagonal(double d);  // Adds d to each diagonal
	BUSSIK_EXPORT MatrixRmn& AddToDiagonal(const VectorRn& dVec);

	// Solving systems of linear equations
	BUSSIK_EXPORT void Solve(const VectorRn& b, VectorRn* x, MatrixRmn& AugMat) const;  // Solves the equation   (*this)*x = b;    Uses row operations.  Assumes *this is invertible.
	...
	BUSSIK_EXPORT void ConvertToRefNoFree();                   // Converts the matrix in place to row echelon form -- assumption is no free variables will be found
	...
	BUSSIK_EXPORT static void CalcGivensValues(double a, double b, double* c, double* s);
	BUSSIK_EXPORT void PostApplyGivens(double c, double s, long idx);              // Applies Givens transform to columns idx and idx+1.
	BUSSIK_EXPORT void PostApplyGivens(double c, double s, long idx1, long idx2);  // Applies Givens transform to columns idx1 and idx2.

	// Singular value decomposition
	BUSSIK_EXPORT void ComputeSVD(MatrixRmn& U, VectorRn& w, MatrixRmn& V) const;
	// Good for debugging SVD computations (I recommend this be used for any new application to check for bugs/instability).
	BUSSIK_EXPORT bool DebugCheckSVD(const MatrixRmn& U, const VectorRn& w, const MatrixRmn& V) const;
	// Compute inverse of a matrix, the result is written in R
	BUSSIK_EXPORT void ComputeInverse(MatrixRmn& R) const;
	// Debug matrix inverse computation
	BUSSIK_EXPORT bool DebugCheckInverse(const MatrixRmn& MInv) const;
...
private:
	...
	BUSSIK_EXPORT static void CalcBidiagonal(MatrixRmn& U, MatrixRmn& V, VectorRn& w, VectorRn& superDiag);
	BUSSIK_EXPORT void ConvertBidiagToDiagonal(MatrixRmn& U, MatrixRmn& V, VectorRn& w, VectorRn& superDiag) const;
	BUSSIK_EXPORT static void SvdHouseholder(double* basePt,
							   long colLength, long numCols, long colStride, long rowStride,
							   double* retFirstEntry);
	BUSSIK_EXPORT void ExpandHouseholders(long numXforms, int numZerosSkipped, const double* basePt, long colStride, long rowStride);
	BUSSIK_EXPORT static bool UpdateBidiagIndices(long* firstDiagIdx, long* lastBidiagIdx, VectorRn& w, VectorRn& superDiag, double eps);
	BUSSIK_EXPORT static void ApplyGivensCBTD(double cosine, double sine, double* a, double* b, double* c, double* d);
	BUSSIK_EXPORT static void ApplyGivensCBTD(double cosine, double sine, double* a, double* b, double* c,
								double d, double* e, double* f);
	BUSSIK_EXPORT static void ClearRowWithDiagonalZero(long firstBidiagIdx, long lastBidiagIdx,
										 MatrixRmn& U, double* wPtr, double* sdPtr, double eps);
	BUSSIK_EXPORT static void ClearColumnWithDiagonalZero(long endIdx, MatrixRmn& V, double* wPtr, double* sdPtr, double eps);
	BUSSIK_EXPORT bool DebugCalcBidiagCheck(const MatrixRmn& U, const VectorRn& w, const VectorRn& superDiag, const MatrixRmn& V) const;
};
