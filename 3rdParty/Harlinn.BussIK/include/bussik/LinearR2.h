...
public:
	constexpr VectorR2() : x(0.0), y(0.0) {}
	constexpr VectorR2(double xVal, double yVal)
		: x(xVal), y(yVal) {}
	constexpr VectorR2(const VectorHgR2& uH);

	constexpr VectorR2& SetZero()
	{
		x = 0.0;
		y = 0.0;
		return *this;
	}
	constexpr VectorR2& Set(double xx, double yy)
	{
		x = xx;
		y = yy;
		return *this;
	}
	constexpr VectorR2& Load(const double* v);
	constexpr VectorR2& Load(const float* v);
	constexpr void Dump(double* v) const;
	constexpr void Dump(float* v) const;

	BUSSIK_EXPORT static const VectorR2 Zero;
	BUSSIK_EXPORT static const VectorR2 UnitX;
	BUSSIK_EXPORT static const VectorR2 UnitY;
	BUSSIK_EXPORT static const VectorR2 NegUnitX;
	BUSSIK_EXPORT static const VectorR2 NegUnitY;

	constexpr VectorR2& operator+=(const VectorR2& v)
	{
		x += v.x;
		y += v.y;
		return (*this);
	...
	constexpr VectorR2& operator*=(double m)
	{
		x *= m;
		y *= m;
		return (*this);
	...
	constexpr VectorR2 operator-() const { return (VectorR2(-x, -y)); }
	constexpr VectorR2& ArrayProd(const VectorR2&);  // Component-wise product

	constexpr VectorR2& AddScaled(const VectorR2& u, double s);
...
inline constexpr VectorR2 operator+(const VectorR2& u, const VectorR2& v);
inline constexpr VectorR2 operator-(const VectorR2& u, const VectorR2& v);
inline constexpr VectorR2 operator*(const VectorR2& u, double m);
inline constexpr VectorR2 operator*(double m, const VectorR2& u);
inline constexpr VectorR2 operator/(const VectorR2& u, double m);
inline constexpr bool operator==(const VectorR2& u, const VectorR2& v);

inline constexpr double operator^(const VectorR2& u, const VectorR2& v);  // Dot Product
inline constexpr VectorR2 ArrayProd(const VectorR2& u, const VectorR2& v);
...
public:
	...
	BUSSIK_EXPORT LinearMapR2 Inverse() const;            // Returns inverse
	BUSSIK_EXPORT LinearMapR2& Invert();                  // Converts into inverse.
	BUSSIK_EXPORT VectorR2 Solve(const VectorR2&) const;  // Returns solution
...
BUSSIK_EXPORT ostream& operator<<(ostream& os, const VectorR2& u);

// *****************************************************
// * VectorR2 class - inlined functions				   *
// * * * * * * * * * * * * * * * * * * * * * * * * * * *

inline constexpr VectorR2& VectorR2::Load(const double* v)
{
	x = *v;
	y = *(v + 1);
	return *this;
...
inline constexpr VectorR2& VectorR2::Load(const float* v)
{
	x = *v;
	y = *(v + 1);
	return *this;
...
inline constexpr void VectorR2::Dump(double* v) const
{
	*v = x;
	*(v + 1) = y;
}

inline constexpr void VectorR2::Dump(float* v) const
{
	*v = (float)x;
	*(v + 1) = (float)y;
}

inline constexpr VectorR2& VectorR2::ArrayProd(const VectorR2& v)  // Component-wise Product
{
	x *= v.x;
	y *= v.y;
	return (*this);
...
inline constexpr VectorR2& VectorR2::AddScaled(const VectorR2& u, double s)
{
	x += s * u.x;
	y += s * u.y;
	return (*this);
...
inline constexpr VectorR2::VectorR2(const VectorHgR2& uH)
	: x(uH.x), y(uH.y)
{
	*this /= uH.w;
}
