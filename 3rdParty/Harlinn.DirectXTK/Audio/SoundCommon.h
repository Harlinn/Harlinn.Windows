...
namespace DirectX
{
    ...
    HDXTK_EXPORT bool IsValid(_In_ const WAVEFORMATEX* wfx) noexcept;


    // Helper for getting a default channel mask from channels
    HDXTK_EXPORT uint32_t GetDefaultChannelMask(int channels) noexcept;


    // Helpers for creating various wave format structures
    HDXTK_EXPORT void CreateIntegerPCM(_Out_ WAVEFORMATEX* wfx,
        int sampleRate, int channels, int sampleBits) noexcept;
    HDXTK_EXPORT void CreateFloatPCM(_Out_ WAVEFORMATEX* wfx,
        int sampleRate, int channels) noexcept;
    HDXTK_EXPORT void CreateADPCM(_Out_writes_bytes_(wfxSize) WAVEFORMATEX* wfx, size_t wfxSize,
        int sampleRate, int channels, int samplesPerBlock) noexcept(false);
#ifdef DIRECTX_ENABLE_XWMA
    HDXTK_EXPORT void CreateXWMA(_Out_ WAVEFORMATEX* wfx,
        int sampleRate, int channels, int blockAlign, int avgBytes, bool wma3) noexcept;
#endif
#ifdef DIRECTX_ENABLE_XMA2
    HDXTK_EXPORT void CreateXMA2(_Out_writes_bytes_(wfxSize) WAVEFORMATEX* wfx, size_t wfxSize,
    ...
    HDXTK_EXPORT bool ComputePan(float pan, unsigned int channels, _Out_writes_(16) float* matrix) noexcept;
    ...
    public:
        ...
        HDXTK_EXPORT SoundEffectInstanceBase(SoundEffectInstanceBase&&) = default;
        HDXTK_EXPORT SoundEffectInstanceBase& operator= (SoundEffectInstanceBase&&) = default;
        ...
        HDXTK_EXPORT void SetPan(float pan);

        HDXTK_EXPORT void Apply3D(const X3DAUDIO_LISTENER& listener, const X3DAUDIO_EMITTER& emitter, bool rhcoords);
