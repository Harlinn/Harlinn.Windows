...
namespace DirectX
{
    ...
    {
    public:
        HDXTK_EXPORT GamePad() noexcept(false);

        HDXTK_EXPORT GamePad(GamePad&&) noexcept;
        HDXTK_EXPORT GamePad& operator= (GamePad&&) noexcept;

        GamePad(GamePad const&) = delete;
        GamePad& operator=(GamePad const&) = delete;

        HDXTK_EXPORT virtual ~GamePad();
        ...
        HDXTK_EXPORT State __cdecl GetState(int player, DeadZone deadZoneMode = DEAD_ZONE_INDEPENDENT_AXES);

        // Retrieve the current capabilities of the gamepad of the associated player index
        HDXTK_EXPORT Capabilities __cdecl GetCapabilities(int player);

        // Set the vibration motor speeds of the gamepad
        HDXTK_EXPORT bool __cdecl SetVibration(int player, float leftMotor, float rightMotor, float leftTrigger = 0.f, float rightTrigger = 0.f) noexcept;

        // Handle suspending/resuming
        HDXTK_EXPORT void __cdecl Suspend() noexcept;
        HDXTK_EXPORT void __cdecl Resume() noexcept;
        ...
        HDXTK_EXPORT void __cdecl RegisterEvents(void* ctrlChanged, void* userChanged) noexcept;
    #endif

        // Singleton
        HDXTK_EXPORT static GamePad& __cdecl Get();
