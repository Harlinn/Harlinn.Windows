...
namespace DirectX
{
    ...
    {
    public:
        HDXTK_EXPORT Mouse() noexcept(false);

        HDXTK_EXPORT Mouse(Mouse&&) noexcept;
        HDXTK_EXPORT Mouse& operator= (Mouse&&) noexcept;

        Mouse(Mouse const&) = delete;
        Mouse& operator=(Mouse const&) = delete;

        HDXTK_EXPORT virtual ~Mouse();
        ...
        #pragma prefast(pop)
            ...
            HDXTK_EXPORT void __cdecl Update(const State& state) noexcept;

            HDXTK_EXPORT void __cdecl Reset() noexcept;
        ...
        HDXTK_EXPORT State __cdecl GetState() const;

        // Resets the accumulated scroll wheel value
        HDXTK_EXPORT void __cdecl ResetScrollWheelValue() noexcept;

        // Sets mouse mode (defaults to absolute)
        HDXTK_EXPORT void __cdecl SetMode(Mode mode);

        // Signals the end of frame (recommended, but optional)
        HDXTK_EXPORT void __cdecl EndOfInputFrame() noexcept;

        // Feature detection
        HDXTK_EXPORT bool __cdecl IsConnected() const;

        // Cursor visibility
        HDXTK_EXPORT bool __cdecl IsVisible() const noexcept;
        HDXTK_EXPORT void __cdecl SetVisible(bool visible);
        ...
        HDXTK_EXPORT void __cdecl SetWindow(HWND window);
        HDXTK_EXPORT static void __cdecl ProcessMessage(UINT message, WPARAM wParam, LPARAM lParam);
        ...
        // Singleton
        HDXTK_EXPORT static Mouse& __cdecl Get();
