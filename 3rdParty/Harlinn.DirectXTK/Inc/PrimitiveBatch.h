...
namespace DirectX
{
    inline namespace DX11
    {
        ...
        // Base class, not to be used directly: clients should access this via the derived PrimitiveBatch<T>.
            ...
            {
            protected:
                HDXTK_EXPORT PrimitiveBatchBase(_In_ ID3D11DeviceContext* deviceContext, size_t maxIndices, size_t maxVertices, size_t vertexSize);

                HDXTK_EXPORT PrimitiveBatchBase(PrimitiveBatchBase&&) noexcept;
                HDXTK_EXPORT PrimitiveBatchBase& operator= (PrimitiveBatchBase&&) noexcept;

                PrimitiveBatchBase(PrimitiveBatchBase const&) = delete;
                PrimitiveBatchBase& operator= (PrimitiveBatchBase const&) = delete;

                HDXTK_EXPORT virtual ~PrimitiveBatchBase();

            public:
                // Begin/End a batch of primitive drawing operations.
                HDXTK_EXPORT void __cdecl Begin();
                HDXTK_EXPORT void __cdecl End();

            protected:
                // Internal, untyped drawing method.
                HDXTK_EXPORT void __cdecl Draw(D3D11_PRIMITIVE_TOPOLOGY topology, bool isIndexed, _In_opt_count_(indexCount) uint16_t const* indices, size_t indexCount, size_t vertexCount, _Out_ void** pMappedVertices);
        ...
        public:
            ...
            HDXTK_EXPORT PrimitiveBatch(PrimitiveBatch&&) = default;
            HDXTK_EXPORT PrimitiveBatch& operator= (PrimitiveBatch&&) = default;
