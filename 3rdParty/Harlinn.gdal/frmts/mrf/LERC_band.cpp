...
#undef MASK
    ...
    LercNS::Byte* ptr = reinterpret_cast<LercNS::Byte*>(dst.buffer);
    size_t sz = 0;
    bool success = false;
...
{
    const LercNS::Byte *ptr = reinterpret_cast<LercNS::Byte *>(src.buffer);
    Lerc2::HeaderInfo hdInfo;
    Lerc2 lerc2;
    bool hasMask = false;

    // If not Lerc2 switch to Lerc
    if (!lerc2.GetHeaderInfo(ptr, src.size, hdInfo, hasMask ))
...
CPLXMLNode *LERC_Band::GetMRFConfig(GDALOpenInfo *poOpenInfo)
{
    ...
    // Try lerc2
    {
        ...
        bool hasMask = false;
        hinfo.RawInit();
        if (l2.GetHeaderInfo(reinterpret_cast<LercNS::Byte *>(psz), poOpenInfo->nHeaderBytes, hinfo, hasMask )) {
            size.x = hinfo.nCols;
            size.y = hinfo.nRows;
