...
class SliceTransform : public Customizable {
 public:
  ...
  ROCKSDB_LIBRARY_API std::string AsString() const;
...
ROCKSDB_LIBRARY_API extern const SliceTransform* NewFixedPrefixTransform(size_t prefix_len);

// The prefix is the first min(length(key),`cap_len`) bytes of the key, and
// all keys are InDomain.
ROCKSDB_LIBRARY_API extern const SliceTransform* NewCappedPrefixTransform(size_t cap_len);

// Prefix is equal to key. All keys are InDomain.
ROCKSDB_LIBRARY_API extern const SliceTransform* NewNoopTransform();
