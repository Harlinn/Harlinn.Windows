...
class WriteBufferManager {
 public:
  ...
  ROCKSDB_LIBRARY_API explicit WriteBufferManager(size_t _buffer_size,
  ...
  ROCKSDB_LIBRARY_API ~WriteBufferManager();
  ...
  ROCKSDB_LIBRARY_API size_t dummy_entries_in_cache_usage() const;
  ...
  ROCKSDB_LIBRARY_API void ReserveMem(size_t mem);

  // We are in the process of freeing `mem` bytes, so it is not considered
  // when checking the soft limit.
  ROCKSDB_LIBRARY_API void ScheduleFreeMem(size_t mem);

  ROCKSDB_LIBRARY_API void FreeMem(size_t mem);

  // Add the DB instance to the queue and block the DB.
  // Should only be called by RocksDB internally.
  ROCKSDB_LIBRARY_API void BeginWriteStall(StallInterface* wbm_stall);

  // If stall conditions have resolved, remove DB instances from queue and
  // signal them to continue.
  ROCKSDB_LIBRARY_API void MaybeEndWriteStall();

  ROCKSDB_LIBRARY_API void RemoveDBFromQueue(StallInterface* wbm_stall);
 ...
 private:
  ...
  ROCKSDB_LIBRARY_API void ReserveMemWithCache(size_t mem);
  ROCKSDB_LIBRARY_API void FreeMemWithCache(size_t mem);
};
}  // namespace ROCKSDB_NAMESPACE
