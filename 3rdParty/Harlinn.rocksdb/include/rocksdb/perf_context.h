...
struct PerfContext {
  ROCKSDB_LIBRARY_API ~PerfContext();

  PerfContext() {}

  ROCKSDB_LIBRARY_API PerfContext(const PerfContext&);
  ROCKSDB_LIBRARY_API PerfContext& operator=(const PerfContext&);
  ROCKSDB_LIBRARY_API PerfContext(PerfContext&&) noexcept;

  ROCKSDB_LIBRARY_API void Reset();  // reset all performance counters to zero

  ROCKSDB_LIBRARY_API std::string ToString(bool exclude_zero_counters = false) const;

  // enable per level perf context and allocate storage for PerfContextByLevel
  ROCKSDB_LIBRARY_API void EnablePerLevelPerfContext();

  // temporarily disable per level perf context by setting the flag to false
  ROCKSDB_LIBRARY_API void DisablePerLevelPerfContext();

  // free the space for PerfContextByLevel, also disable per level perf context
  ROCKSDB_LIBRARY_API void ClearPerLevelPerfContext();
...
ROCKSDB_LIBRARY_API PerfContext* get_perf_context();
