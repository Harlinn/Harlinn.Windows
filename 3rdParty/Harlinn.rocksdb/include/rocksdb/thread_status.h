...
struct ThreadStatus {
  ...
  ROCKSDB_LIBRARY_API static std::string GetThreadTypeName(ThreadType thread_type);

  // Obtain the name of an operation given its type.
  ROCKSDB_LIBRARY_API static const std::string& GetOperationName(OperationType op_type);

  ROCKSDB_LIBRARY_API static const std::string MicrosToString(uint64_t op_elapsed_time);

  // Obtain a human-readable string describing the specified operation stage.
  ROCKSDB_LIBRARY_API static const std::string& GetOperationStageName(OperationStage stage);

  // Obtain the name of the "i"th operation property of the
  // specified operation.
  ROCKSDB_LIBRARY_API static const std::string& GetOperationPropertyName(OperationType op_type,
                                                     int i);

  // Translate the "i"th property of the specified operation given
  // a property value.
  ROCKSDB_LIBRARY_API static std::map<std::string, uint64_t> InterpretOperationProperties(
      OperationType op_type, const uint64_t* op_properties);

  // Obtain the name of a state given its type.
  ROCKSDB_LIBRARY_API static const std::string& GetStateName(StateType state_type);
};
