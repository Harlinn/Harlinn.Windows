...
extern ROCKSDB_LIBRARY_API std::shared_ptr<Cache> NewLRUCache(
...
extern ROCKSDB_LIBRARY_API std::shared_ptr<Cache> NewLRUCache(const LRUCacheOptions& cache_opts);
...
extern ROCKSDB_LIBRARY_API std::shared_ptr<Cache> NewClockCache(
    size_t capacity, int num_shard_bits = -1,
    bool strict_capacity_limit = false,
    CacheMetadataChargePolicy metadata_charge_policy =
        kDefaultCacheMetadataChargePolicy);
