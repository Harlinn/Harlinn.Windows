...
struct TableProperties {
 public:
  ...
  ROCKSDB_LIBRARY_API std::string ToString(const std::string& prop_delim = "; ",
                       const std::string& kv_delim = "=") const;

  // Aggregate the numerical member variables of the specified
  // TableProperties.
  ROCKSDB_LIBRARY_API void Add(const TableProperties& tp);
  ...
  ROCKSDB_LIBRARY_API std::map<std::string, uint64_t> GetAggregatablePropertiesAsMap() const;
};
...
// respectively.
ROCKSDB_LIBRARY_API extern uint64_t GetDeletedKeys(const UserCollectedProperties& props);
ROCKSDB_LIBRARY_API extern uint64_t GetMergeOperands(const UserCollectedProperties& props,
