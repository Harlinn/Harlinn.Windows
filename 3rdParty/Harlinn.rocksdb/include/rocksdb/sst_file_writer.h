...
class SstFileWriter {
 public:
  ...
  // Deprecated API
  ROCKSDB_LIBRARY_API SstFileWriter(const EnvOptions& env_options, const Options& options,
  ...
  ROCKSDB_LIBRARY_API ~SstFileWriter();

  // Prepare SstFileWriter to write into file located at "file_path".
  ROCKSDB_LIBRARY_API Status Open(const std::string& file_path);

  // Add a Put key with value to currently opened file (deprecated)
  // REQUIRES: key is after any previously added key according to comparator.
  // REQUIRES: comparator is *not* timestamp-aware.
  ROCKSDB_DEPRECATED_FUNC ROCKSDB_LIBRARY_API Status Add(const Slice& user_key, const Slice& value);

  // Add a Put key with value to currently opened file
  // REQUIRES: key is after any previously added key according to comparator.
  // REQUIRES: comparator is *not* timestamp-aware.
  ROCKSDB_LIBRARY_API Status Put(const Slice& user_key, const Slice& value);
  ...
  // the comparator.
  ROCKSDB_LIBRARY_API Status Put(const Slice& user_key, const Slice& timestamp, const Slice& value);

  // Add a Merge key with value to currently opened file
  // REQUIRES: key is after any previously added key according to comparator.
  // REQUIRES: comparator is *not* timestamp-aware.
  ROCKSDB_LIBRARY_API Status Merge(const Slice& user_key, const Slice& value);

  // Add a deletion key to currently opened file
  // REQUIRES: key is after any previously added key according to comparator.
  // REQUIRES: comparator is *not* timestamp-aware.
  ROCKSDB_LIBRARY_API Status Delete(const Slice& user_key);
  ...
  // REQUIRES: the timestamp's size is equal to what is expected by
  // the comparator.
  ROCKSDB_LIBRARY_API Status Delete(const Slice& user_key, const Slice& timestamp);

  // Add a range deletion tombstone to currently opened file
  // REQUIRES: comparator is *not* timestamp-aware.
  ROCKSDB_LIBRARY_API Status DeleteRange(const Slice& begin_key, const Slice& end_key);
  ...
  ROCKSDB_LIBRARY_API Status Finish(ExternalSstFileInfo* file_info = nullptr);

  // Return the current file size.
  ROCKSDB_LIBRARY_API uint64_t FileSize();

 private:
  ROCKSDB_LIBRARY_API void InvalidatePageCache(bool closing);
  struct Rep;
