...
class SystemClock : public Customizable {
 public:
  ...
  // system.
  ROCKSDB_LIBRARY_API static const std::shared_ptr<SystemClock>& Default();
...
class SystemClockWrapper : public SystemClock {
 public:
  ROCKSDB_LIBRARY_API explicit SystemClockWrapper(const std::shared_ptr<SystemClock>& t);
  ...
  ROCKSDB_LIBRARY_API Status PrepareOptions(const ConfigOptions& options) override;
#ifndef ROCKSDB_LITE
  ROCKSDB_LIBRARY_API std::string SerializeOptions(const ConfigOptions& config_options,
