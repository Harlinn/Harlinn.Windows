...
struct ColumnFamilyOptions : public AdvancedColumnFamilyOptions {
  ...
  ROCKSDB_LIBRARY_API ColumnFamilyOptions* OldDefaults(int rocksdb_major_version = 4,
  ...
  ROCKSDB_LIBRARY_API ColumnFamilyOptions* OptimizeForSmallDb(
  ...
  ROCKSDB_LIBRARY_API ColumnFamilyOptions* OptimizeForPointLookup(uint64_t block_cache_size_mb);
  ...
  ROCKSDB_LIBRARY_API ColumnFamilyOptions* OptimizeLevelStyleCompaction(
      uint64_t memtable_memory_budget = 512 * 1024 * 1024);
  ROCKSDB_LIBRARY_API ColumnFamilyOptions* OptimizeUniversalStyleCompaction(
      uint64_t memtable_memory_budget = 512 * 1024 * 1024);
  ...
  ROCKSDB_LIBRARY_API explicit ColumnFamilyOptions(const Options& options);

  ROCKSDB_LIBRARY_API void Dump(Logger* log) const;
};
...
struct DBOptions {
  ...
  ROCKSDB_LIBRARY_API DBOptions* OldDefaults(int rocksdb_major_version = 4,
                         int rocksdb_minor_version = 6);
  ...
  // memtable to cost to
  ROCKSDB_LIBRARY_API DBOptions* OptimizeForSmallDb(std::shared_ptr<Cache>* cache = nullptr);
  ...
  ROCKSDB_LIBRARY_API DBOptions* IncreaseParallelism(int total_threads = 16);
#endif  // ROCKSDB_LITE
  ...
  ROCKSDB_LIBRARY_API explicit DBOptions(const Options& options);
...
struct Options : public DBOptions, public ColumnFamilyOptions {
  ...
  // The function recovers options to the option as in version 4.6.
  ROCKSDB_LIBRARY_API Options* OldDefaults(int rocksdb_major_version = 4,
                       int rocksdb_minor_version = 6);

  ROCKSDB_LIBRARY_API void Dump(Logger* log) const;

  ROCKSDB_LIBRARY_API void DumpCFOptions(Logger* log) const;
  ...
  ROCKSDB_LIBRARY_API Options* PrepareForBulkLoad();

  // Use this if your DB is very small (like under 1GB) and you don't want to
  // spend lots of memory for memtables.
  ROCKSDB_LIBRARY_API Options* OptimizeForSmallDb();
};
...
struct ReadOptions {
  ...
  ROCKSDB_LIBRARY_API ReadOptions();
  ROCKSDB_LIBRARY_API ReadOptions(bool cksum, bool cache);
};
...
ROCKSDB_LIBRARY_API extern Status CreateLoggerFromOptions(const std::string& dbname,
