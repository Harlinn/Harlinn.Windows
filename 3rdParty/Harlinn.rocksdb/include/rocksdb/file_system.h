...
class FileSystem {
 public:
  ROCKSDB_LIBRARY_API FileSystem();

  // No copying allowed
  FileSystem(const FileSystem&) = delete;

  ROCKSDB_LIBRARY_API virtual ~FileSystem();
  ...
  ROCKSDB_LIBRARY_API static Status Load(const std::string& value,
  ...
  ROCKSDB_LIBRARY_API static Status CreateFromString(const ConfigOptions& options,
                                 const std::string& value,
                                 std::shared_ptr<FileSystem>* result);

  // Return a default FileSystem suitable for the current operating
  // system.
  ROCKSDB_LIBRARY_API static std::shared_ptr<FileSystem> Default();
  ...
  ROCKSDB_LIBRARY_API virtual IOStatus ReuseWritableFile(const std::string& fname,
...
#undef DeleteFile
  ...
  // files.
  ROCKSDB_LIBRARY_API virtual FileOptions OptimizeForManifestRead(
  ...
  ROCKSDB_LIBRARY_API virtual FileOptions OptimizeForLogWrite(const FileOptions& file_options,
  ...
  // object.
  ROCKSDB_LIBRARY_API virtual FileOptions OptimizeForManifestWrite(
      const FileOptions& file_options) const;
  ...
  // writing table files.
  ROCKSDB_LIBRARY_API virtual FileOptions OptimizeForCompactionTableWrite(
  ...
  // is a copy of the FileOptions in the parameters, but is optimized for
  // reading table files.
  ROCKSDB_LIBRARY_API virtual FileOptions OptimizeForCompactionTableRead(
      const FileOptions& file_options,
  ...
  // is a copy of the FileOptions in the parameters, but is optimized for
  // reading blob files.
  ROCKSDB_LIBRARY_API virtual FileOptions OptimizeForBlobFileRead(
      const FileOptions& file_options,
      const ImmutableDBOptions& db_options) const;
...
ROCKSDB_LIBRARY_API extern IOStatus WriteStringToFile(FileSystem* fs, const Slice& data,
                                  const std::string& fname,
                                  bool should_sync = false);

// A utility routine: read contents of named file into *data
ROCKSDB_LIBRARY_API extern IOStatus ReadFileToString(FileSystem* fs, const std::string& fname,
                                 std::string* data);
