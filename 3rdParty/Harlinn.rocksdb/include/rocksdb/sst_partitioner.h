...
class SstPartitionerFixedPrefix : public SstPartitioner {
 public:
  ...
  ROCKSDB_LIBRARY_API PartitionerResult ShouldPartition(const PartitionerRequest& request) override;

  ROCKSDB_LIBRARY_API bool CanDoTrivialMove(const Slice& smallest_user_key,
...
class SstPartitionerFixedPrefixFactory : public SstPartitionerFactory {
 public:
  ROCKSDB_LIBRARY_API explicit SstPartitionerFixedPrefixFactory(size_t len);
  ...
  ROCKSDB_LIBRARY_API std::unique_ptr<SstPartitioner> CreatePartitioner(
...
ROCKSDB_LIBRARY_API extern std::shared_ptr<SstPartitionerFactory>
