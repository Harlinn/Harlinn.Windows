...
class ObjectLibrary {
 ...
 public:
  ...
  ROCKSDB_LIBRARY_API const Entry* FindEntry(const std::string& type,
  ...
  ROCKSDB_LIBRARY_API size_t GetFactoryCount(size_t* num_types) const;

  ROCKSDB_LIBRARY_API void Dump(Logger* logger) const;
  ...
  ROCKSDB_LIBRARY_API static std::shared_ptr<ObjectLibrary>& Default();

 private:
  // Adds the input entry to the list for the given type
  ROCKSDB_LIBRARY_API void AddEntry(const std::string& type, std::unique_ptr<Entry>& entry);
...
class ObjectRegistry {
 ...
 private:
  ...
  ROCKSDB_LIBRARY_API std::shared_ptr<Customizable> GetManagedObject(const std::string& type,
                                                 const std::string& id) const;
  ROCKSDB_LIBRARY_API Status ListManagedObjects(
  ...
  ROCKSDB_LIBRARY_API Status SetManagedObject(const std::string& type, const std::string& id,
                          const std::shared_ptr<Customizable>& c);

  ROCKSDB_LIBRARY_API const ObjectLibrary::Entry* FindEntry(const std::string& type,
                                        const std::string& name) const;
