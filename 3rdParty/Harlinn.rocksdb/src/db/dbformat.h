...
extern ROCKSDB_LIBRARY_API const ValueType kValueTypeForSeek;
extern ROCKSDB_LIBRARY_API const ValueType kValueTypeForSeekForPrev;
...
struct ParsedInternalKey {
  ...
  ROCKSDB_LIBRARY_API std::string DebugString(bool log_err_key, bool hex) const;
...
ROCKSDB_LIBRARY_API EntryType GetEntryType(ValueType value_type);

// Append the serialization of "key" to *result.
ROCKSDB_LIBRARY_API extern void AppendInternalKey(std::string* result,
                              const ParsedInternalKey& key);

// Append the serialization of "key" to *result, replacing the original
// timestamp with argument ts.
ROCKSDB_LIBRARY_API extern void AppendInternalKeyWithDifferentTimestamp(
...
ROCKSDB_LIBRARY_API extern void AppendInternalKeyFooter(std::string* result, SequenceNumber s,
                                    ValueType t);

// Append the key and a minimal timestamp to *result
ROCKSDB_LIBRARY_API extern void AppendKeyWithMinTimestamp(std::string* result, const Slice& key,
                                      size_t ts_sz);

// Append the key and a maximal timestamp to *result
ROCKSDB_LIBRARY_API extern void AppendKeyWithMaxTimestamp(std::string* result, const Slice& key,
                                      size_t ts_sz);
...
ROCKSDB_LIBRARY_API extern Status ParseInternalKey(const Slice& internal_key,
...
class InternalKeyComparator
 ...
 public:
  ...
  ROCKSDB_LIBRARY_API virtual const char* Name() const override;
  ...
  ROCKSDB_LIBRARY_API virtual void FindShortestSeparator(std::string* start,
                                     const Slice& limit) const override;
  ROCKSDB_LIBRARY_API virtual void FindShortSuccessor(std::string* key) const override;

  const Comparator* user_comparator() const {
    return user_comparator_.user_comparator();
  }

  ROCKSDB_LIBRARY_API int Compare(const InternalKey& a, const InternalKey& b) const;
  ROCKSDB_LIBRARY_API int Compare(const ParsedInternalKey& a, const ParsedInternalKey& b) const;
...
ROCKSDB_LIBRARY_API extern bool ReadKeyFromWriteBatchEntry(Slice* input, Slice* key,
                                       bool cf_record);
...
ROCKSDB_LIBRARY_API extern Status ReadRecordFromWriteBatch(Slice* input, char* tag,
