...
extern ROCKSDB_LIBRARY_API const bool kDefaultToAdaptiveMutex;
...
class CondVar {
 public:
  ...
  ROCKSDB_LIBRARY_API ~CondVar();
  ROCKSDB_LIBRARY_API void Wait();
  ROCKSDB_LIBRARY_API bool TimedWait(uint64_t expiration_time);
  ROCKSDB_LIBRARY_API void Signal();
  ROCKSDB_LIBRARY_API void SignalAll();
...
extern ROCKSDB_LIBRARY_API void InitOnce(OnceType* once, void (*initializer)());
...
extern ROCKSDB_LIBRARY_API int PhysicalCoreID();
...
ROCKSDB_LIBRARY_API int truncate(const char* path, int64_t length);
ROCKSDB_LIBRARY_API int Truncate(std::string path, int64_t length);
ROCKSDB_LIBRARY_API void Crash(const std::string& srcfile, int srcline);
ROCKSDB_LIBRARY_API extern int GetMaxOpenFiles();
ROCKSDB_LIBRARY_API std::string utf16_to_utf8(const std::wstring& utf16);
ROCKSDB_LIBRARY_API std::wstring utf8_to_utf16(const std::string& utf8);

using ThreadId = int;

extern ROCKSDB_LIBRARY_API void SetCpuPriority(ThreadId id, CpuPriority priority);

ROCKSDB_LIBRARY_API int64_t GetProcessID();

// Uses platform APIs to generate a 36-character RFC-4122 UUID. Returns
// true on success or false on failure.
ROCKSDB_LIBRARY_API bool GenerateRfcUuid(std::string* output);
