...
class WindowsThread {
  ...
  ROCKSDB_LIBRARY_API void Init(std::function<void()>&&);

 public:
  using native_handle_type = void*;

  // Construct with no thread
  ROCKSDB_LIBRARY_API WindowsThread();
  ...
  ROCKSDB_LIBRARY_API WindowsThread(WindowsThread&&) noexcept;

  ROCKSDB_LIBRARY_API WindowsThread& operator=(WindowsThread&&) noexcept;

  ROCKSDB_LIBRARY_API bool joinable() const;

  unsigned int get_id() const { return th_id_; }

  ROCKSDB_LIBRARY_API native_handle_type native_handle() const;

  ROCKSDB_LIBRARY_API static unsigned hardware_concurrency();

  ROCKSDB_LIBRARY_API void join();

  ROCKSDB_LIBRARY_API bool detach();

  ROCKSDB_LIBRARY_API void swap(WindowsThread&);
};
} // namespace port
}  // namespace ROCKSDB_NAMESPACE
