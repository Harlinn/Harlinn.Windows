...
class AnalyzerOptions {
 public:
  ...
  ROCKSDB_LIBRARY_API AnalyzerOptions();

  ROCKSDB_LIBRARY_API ~AnalyzerOptions();

  ROCKSDB_LIBRARY_API void SparseCorrelationInput(const std::string& in_str);
};
...
struct TraceStats {
  ...
  ROCKSDB_LIBRARY_API TraceStats();
  ROCKSDB_LIBRARY_API ~TraceStats();
...
class TraceAnalyzer : private TraceRecord::Handler,
 ...
 public:
  ROCKSDB_LIBRARY_API TraceAnalyzer(std::string& trace_path, std::string& output_path,
                AnalyzerOptions _analyzer_opts);
  ROCKSDB_LIBRARY_API ~TraceAnalyzer();

  ROCKSDB_LIBRARY_API Status PrepareProcessing();

  ROCKSDB_LIBRARY_API Status StartProcessing();

  ROCKSDB_LIBRARY_API Status MakeStatistics();

  ROCKSDB_LIBRARY_API Status ReProcessing();

  ROCKSDB_LIBRARY_API Status EndProcessing();

  ROCKSDB_LIBRARY_API Status WriteTraceUnit(TraceUnit& unit);

  std::vector<TypeUnit>& GetTaVector() { return ta_; }

 private:
  using TraceRecord::Handler::Handle;
  ROCKSDB_LIBRARY_API Status Handle(const WriteQueryTraceRecord& record,
                std::unique_ptr<TraceRecordResult>* result) override;
  ROCKSDB_LIBRARY_API Status Handle(const GetQueryTraceRecord& record,
                std::unique_ptr<TraceRecordResult>* result) override;
  ROCKSDB_LIBRARY_API Status Handle(const IteratorSeekQueryTraceRecord& record,
                std::unique_ptr<TraceRecordResult>* result) override;
  ROCKSDB_LIBRARY_API Status Handle(const MultiGetQueryTraceRecord& record,
                std::unique_ptr<TraceRecordResult>* result) override;

  using WriteBatch::Handler::PutCF;
  ROCKSDB_LIBRARY_API Status PutCF(uint32_t column_family_id, const Slice& key,
               const Slice& value) override;

  using WriteBatch::Handler::DeleteCF;
  ROCKSDB_LIBRARY_API Status DeleteCF(uint32_t column_family_id, const Slice& key) override;

  using WriteBatch::Handler::SingleDeleteCF;
  ROCKSDB_LIBRARY_API Status SingleDeleteCF(uint32_t column_family_id, const Slice& key) override;

  using WriteBatch::Handler::DeleteRangeCF;
  ROCKSDB_LIBRARY_API Status DeleteRangeCF(uint32_t column_family_id, const Slice& begin_key,
                       const Slice& end_key) override;

  using WriteBatch::Handler::MergeCF;
  ROCKSDB_LIBRARY_API Status MergeCF(uint32_t column_family_id, const Slice& key,
                 const Slice& value) override;
  ...
  // stdout/files.
  ROCKSDB_LIBRARY_API Status OutputAnalysisResult(TraceOperationType op_type, uint64_t timestamp,
  ...
  ROCKSDB_LIBRARY_API Status ReadTraceHeader(Trace* header);
  ROCKSDB_LIBRARY_API Status ReadTraceFooter(Trace* footer);
  ROCKSDB_LIBRARY_API Status ReadTraceRecord(Trace* trace);
  ROCKSDB_LIBRARY_API Status KeyStatsInsertion(const uint32_t& type, const uint32_t& cf_id,
                           const std::string& key, const size_t value_size,
                           const uint64_t ts);
  ROCKSDB_LIBRARY_API Status StatsUnitCorrelationUpdate(StatsUnit& unit, const uint32_t& type,
                                    const uint64_t& ts, const std::string& key);
  ROCKSDB_LIBRARY_API Status OpenStatsOutputFiles(const std::string& type, TraceStats& new_stats);
  ROCKSDB_LIBRARY_API Status CreateOutputFile(
      const std::string& type, const std::string& cf_name,
      const std::string& ending,
      std::unique_ptr<ROCKSDB_NAMESPACE::WritableFile>* f_ptr);
  ROCKSDB_LIBRARY_API Status CloseOutputFiles();

  ROCKSDB_LIBRARY_API void PrintStatistics();
  ROCKSDB_LIBRARY_API Status TraceUnitWriter(
      std::unique_ptr<ROCKSDB_NAMESPACE::WritableFile>& f_ptr, TraceUnit& unit);
  ROCKSDB_LIBRARY_API Status WriteTraceSequence(const uint32_t& type, const uint32_t& cf_id,
                            const Slice& key, const size_t value_size,
                            const uint64_t ts);
  ROCKSDB_LIBRARY_API Status MakeStatisticKeyStatsOrPrefix(TraceStats& stats);
  ROCKSDB_LIBRARY_API Status MakeStatisticCorrelation(TraceStats& stats, StatsUnit& unit);
  ROCKSDB_LIBRARY_API Status MakeStatisticQPS();
  int db_version_;
};
