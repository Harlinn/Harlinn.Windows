...
ROCKSDB_LIBRARY_API extern std::vector<std::string> StringSplit(const std::string& arg, char delim);
...
ROCKSDB_LIBRARY_API extern void AppendNumberTo(std::string* str, uint64_t num);

// Append a human-readable printout of "value" to *str.
// Escapes any non-printable characters found in "value".
ROCKSDB_LIBRARY_API extern void AppendEscapedStringTo(std::string* str, const Slice& value);
...
ROCKSDB_LIBRARY_API extern std::string NumberToHumanString(int64_t num);

// Return a human-readable version of bytes
// ex: 1048576 -> 1.00 GB
ROCKSDB_LIBRARY_API extern std::string BytesToHumanString(uint64_t bytes);

// Return a human-readable version of unix time
// ex: 1562116015 -> "Tue Jul  2 18:06:55 2019"
ROCKSDB_LIBRARY_API extern std::string TimeToHumanString(int unixtime);

// Append a human-readable time in micros.
ROCKSDB_LIBRARY_API int AppendHumanMicros(uint64_t micros, char* output, int len,
                      bool fixed_format);

// Append a human-readable size in bytes
ROCKSDB_LIBRARY_API int AppendHumanBytes(uint64_t bytes, char* output, int len);

// Return a human-readable version of "value".
// Escapes any non-printable characters found in "value".
ROCKSDB_LIBRARY_API extern std::string EscapeString(const Slice& value);

// Parse a human-readable number from "*in" into *value.  On success,
// advances "*in" past the consumed number and sets "*val" to the
// numeric value.  Otherwise, returns false and leaves *in in an
// unspecified state.
ROCKSDB_LIBRARY_API extern bool ConsumeDecimalNumber(Slice* in, uint64_t* val);
...
ROCKSDB_LIBRARY_API bool isSpecialChar(const char c);
...
ROCKSDB_LIBRARY_API char UnescapeChar(const char c);
...
// the original input char.
ROCKSDB_LIBRARY_API char EscapeChar(const char c);
...
ROCKSDB_LIBRARY_API std::string EscapeOptionString(const std::string& raw_string);
...
ROCKSDB_LIBRARY_API std::string UnescapeOptionString(const std::string& escaped_string);

ROCKSDB_LIBRARY_API std::string trim(const std::string& str);

// Returns true if "string" ends with "pattern"
ROCKSDB_LIBRARY_API bool EndsWith(const std::string& string, const std::string& pattern);

// Returns true if "string" starts with "pattern"
ROCKSDB_LIBRARY_API bool StartsWith(const std::string& string, const std::string& pattern);

#ifndef ROCKSDB_LITE
ROCKSDB_LIBRARY_API bool ParseBoolean(const std::string& type, const std::string& value);

ROCKSDB_LIBRARY_API uint8_t ParseUint8(const std::string& value);

ROCKSDB_LIBRARY_API uint32_t ParseUint32(const std::string& value);

ROCKSDB_LIBRARY_API int32_t ParseInt32(const std::string& value);
#endif

ROCKSDB_LIBRARY_API uint64_t ParseUint64(const std::string& value);

ROCKSDB_LIBRARY_API int ParseInt(const std::string& value);

ROCKSDB_LIBRARY_API int64_t ParseInt64(const std::string& value);

ROCKSDB_LIBRARY_API double ParseDouble(const std::string& value);

ROCKSDB_LIBRARY_API size_t ParseSizeT(const std::string& value);

ROCKSDB_LIBRARY_API std::vector<int> ParseVectorInt(const std::string& value);

ROCKSDB_LIBRARY_API bool SerializeIntVector(const std::vector<int>& vec, std::string* value);

extern ROCKSDB_LIBRARY_API const std::string kNullptrString;

// errnoStr() function returns a string that describes the error code passed in
// the argument err
ROCKSDB_LIBRARY_API extern std::string errnoStr(int err);
