...
namespace XMP_COMPONENT_INT_NAMESPACE {
	...
	bool ConfigurationManagerImpl::RegisterMemoryAllocator( pIMemoryAllocator memoryAllocator ) {
		IMemoryAllocator_I::SetMemoryAllocator( memoryAllocator );
		return true;
	}

	bool ConfigurationManagerImpl::RegisterErrorNotifier( pIErrorNotifier_base clientErrorNotifier ) {
	...
	bool ConfigurationManagerImpl::DisableMultiThreading() {
		mMultiThreadingEnabled = false;
		return true;
	}

	bool ConfigurationManagerImpl::IsMultiThreaded() const {
