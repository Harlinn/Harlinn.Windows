...
namespace XMP_COMPONENT_INT_NAMESPACE {
	...
	spIUTF8String UTF8StringImpl::append( const char * buf, sizet count ) {
		if ( buf && count > 0 ) {
	...
	spIUTF8String UTF8StringImpl::append( const spcIUTF8String & src, sizet srcPos, sizet count ) {
		if ( src && count > 0 ) {
	...
	spIUTF8String UTF8StringImpl::assign( const char * buf, sizet count ) {
		if ( buf && count > 0 ) {
			if ( count == npos ) // assume null terminated string
				mString.assign( buf );
			else
	...
	spIUTF8String UTF8StringImpl::assign( const spcIUTF8String & src, sizet srcPos, sizet count ) {
		if ( src && count > 0 ) {
			sizet srcSize = ValidateSrcPosParameter( src, srcPos );
	...
	spIUTF8String UTF8StringImpl::insert( sizet pos, const char * buf, sizet count ) {
		if ( buf && count > 0 ) {
	...
	spIUTF8String UTF8StringImpl::insert( sizet pos, const spcIUTF8String & src, sizet srcPos, sizet count ) {
		if ( src && count > 0 ) {
			sizet size = ValidatePosParameter( pos );
			sizet srcSize = ValidateSrcPosParameter( src, srcPos );
	...
	spIUTF8String UTF8StringImpl::erase( sizet pos, sizet count ) {
		if ( count > 0 ) {
			sizet size = ValidatePosParameter( pos );
	...
	void UTF8StringImpl::resize( sizet n ) {
	...
	spIUTF8String UTF8StringImpl::replace( sizet pos, sizet count, const char * buf, sizet srcCount ) {
	...
	spIUTF8String UTF8StringImpl::replace( sizet pos, sizet count, const spcIUTF8String & src, sizet srcPos, sizet srcCount ) {
	...
	sizet UTF8StringImpl::copy( char * buf, sizet len, sizet pos ) const {
		if ( buf && len > 0 ) {
	...
	sizet UTF8StringImpl::find( const char * buf, sizet pos, sizet count ) const {
		if ( buf && count > 0 ) {
	...
	sizet UTF8StringImpl::find( const spcIUTF8String & src, sizet pos, sizet count ) const {
	...
	sizet UTF8StringImpl::rfind( const char * buf, sizet pos, sizet count ) const {
		if ( buf && count > 0 ) {
	...
	sizet UTF8StringImpl::rfind( const spcIUTF8String & src, sizet pos, sizet count ) const {
		if ( src && src->size() > 0 && count > 0 ) {
	...
	int32 UTF8StringImpl::compare( sizet pos, sizet len, const char * buf, sizet count ) const {
	...
	int32 UTF8StringImpl::compare( sizet pos, sizet len, const spcIUTF8String & str, sizet strPos, sizet strLen ) const {
	...
	spIUTF8String UTF8StringImpl::substr( sizet pos, sizet count ) const {
		ValidatePosParameter( pos );
		try {
	...
	bool UTF8StringImpl::empty() const {
		return mString.empty();
	}

	const char * UTF8StringImpl::c_str() const __NOTHROW__ {
		return mString.c_str();
	}

	void UTF8StringImpl::clear() __NOTHROW__ {
		mString.clear();
	}

	sizet UTF8StringImpl::size() const __NOTHROW__ {
		return mString.size();
	}
