...
namespace XMP_COMPONENT_INT_NAMESPACE {
	...
	void ConfigurableImpl::TreatKeyAsCaseInsensitive( bool flag ) {
		mTreatKeyAsCaseInsensitiveCharBuffer = flag;
	}

	void ConfigurableImpl::AllowDifferentValueTypesForExistingEntries( bool flag ) {
		mAllowDifferentValueTypesForExistingEntries = flag;
	}

	void ConfigurableImpl::SetAllowedKeys( uint64 * keysTable, sizet sizeOfTable ) {
		AutoSharedLock lock ( GetMutex(), true );
	...
	void ConfigurableImpl::SetAllowedValueTypesForKeys( KeyValueTypePair * keyValueTypePairTable, sizet sizeOfTable ) {
		AutoSharedLock lock( GetMutex(), true );
