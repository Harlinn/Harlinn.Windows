...
namespace XMP_COMPONENT_INT_NAMESPACE {
	...
	uint32 IConfigurationManager_I::registerMemoryAllocator( pIMemoryAllocator_base memoryAllocator, pcIError_base & error ) __NOTHROW__ {
	...
	uint32 IConfigurationManager_I::registerErrorNotifier( pIErrorNotifier_base clientErrorNotifier, pcIError_base & error ) __NOTHROW__ {
	...
	uint32 IConfigurationManager_I::disableMultiThreading( pcIError_base & error ) __NOTHROW__ {
	...
	uint32 IConfigurationManager_I::isMultiThreaded( pcIError_base & error ) const __NOTHROW__ {
	...
	pvoid IConfigurationManager_I::GetInterfacePointer( uint64 interfaceID, uint32 interfaceVersion ) {
		return GetInterfacePointerInternal( interfaceID, interfaceVersion, true );
	}

	pvoid IConfigurationManager_I::getInterfacePointer( uint64 interfaceID, uint32 interfaceVersion, pcIError_base & error ) __NOTHROW__ {
		typedef pvoid( IConfigurationManager_I::*Func )( uint64, uint32 );
	...
	pvoid IConfigurationManager_I::GetInterfacePointerInternal( uint64 interfaceID, uint32 interfaceVersion, bool isTopLevel ) {
