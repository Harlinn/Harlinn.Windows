...
namespace XMP_COMPONENT_INT_NAMESPACE {
	...
	pcIUTF8String_base IError_I::getLocation( pcIError_base & error ) const __NOTHROW__ {
		return CallConstUnSafeFunctionReturningSharedPointer< IError_v1, pcIUTF8String_base, const IUTF8String >(
			error, this, &IError_v1::GetLocation, __FILE__, __LINE__ );

	}

	pcIUTF8String_base IError_I::getParameter( sizet index, pcIError_base & error ) const __NOTHROW__ {
	...
	pIError_base IError_I::getNextError( pcIError_base & error ) __NOTHROW__ {
	...
	pIError_base IError_I::setNextError( pIError_base nextError, pcIError_base & error ) __NOTHROW__ {
	...
	pvoid IError_I::GetInterfacePointer( uint64 interfaceID, uint32 interfaceVersion ) {
		return GetInterfacePointerInternal( interfaceID, interfaceVersion, true );
	}

	pvoid IError_I::getInterfacePointer( uint64 interfaceID, uint32 interfaceVersion, pcIError_base & error ) __NOTHROW__{
	...
	pvoid IError_I::GetInterfacePointerInternal( uint64 interfaceID, uint32 interfaceVersion, bool isTopLevel ) {
