...
namespace XMP_COMPONENT_INT_NAMESPACE {
	...
	void IConfigurable_I::SetParameter( const uint64 & key, bool value ) {
		CombinedDataValue uValue;
		uValue.boolValue = value;
		SetParameter( key, kDTBool, uValue );
	}

	void IConfigurable_I::SetParameter( const uint64 & key, uint64 value ) {
		CombinedDataValue uValue;
		uValue.uint64Value = value;
		SetParameter( key, kDTUint64, uValue );
	}

	void IConfigurable_I::SetParameter( const uint64 & key, int64 value ) {
		CombinedDataValue uValue;
		uValue.int64Value = value;
		SetParameter( key, kDTInt64, uValue );
	}

	void IConfigurable_I::SetParameter( const uint64 & key, double value ) {
		CombinedDataValue uValue;
		uValue.doubleValue = value;
		SetParameter( key, kDTDouble, uValue );
	}

	void IConfigurable_I::SetParameter( const uint64 & key, char value ) {
		CombinedDataValue uValue;
		uValue.charValue = value;
		SetParameter( key, kDTChar, uValue );
	}

	void IConfigurable_I::SetParameter( const uint64 & key, const char * value ) {
		CombinedDataValue uValue;
		uValue.constCharPtrValue = value;
		SetParameter( key, kDTConstCharBuffer, uValue );
	}

	void IConfigurable_I::SetParameter( const uint64 & key, const void * value ) {
		CombinedDataValue uValue;
		uValue.constVoidPtrValue = value;
		SetParameter( key, kDTConstVoidPtr, uValue );
	}

	void IConfigurable_I::setParameter( const uint64 & key, uint32 dataType, const CombinedDataValue & dataValue, pcIError_base & error ) __NOTHROW__ {
	...
	bool IConfigurable_I::GetParameter( const uint64 & key, bool & value ) const {
		CombinedDataValue uValue;
	...
	bool IConfigurable_I::GetParameter( const uint64 & key, uint64 & value ) const {
		CombinedDataValue uValue;
	...
	bool IConfigurable_I::GetParameter( const uint64 & key, int64 & value ) const {
		CombinedDataValue uValue;
	...
	bool IConfigurable_I::GetParameter( const uint64 & key, double & value ) const {
		CombinedDataValue uValue;
	...
	bool IConfigurable_I::GetParameter( const uint64 & key, char & value ) const {
		CombinedDataValue uValue;
	...
	bool IConfigurable_I::GetParameter( const uint64 & key, const char * & value ) const {
		CombinedDataValue uValue;
	...
	bool IConfigurable_I::GetParameter( const uint64 & key, const void * & value ) const {
		CombinedDataValue uValue;
	...
	uint32 IConfigurable_I::getParameter( const uint64 & key, uint32 dataType, CombinedDataValue & value, pcIError_base & error ) const __NOTHROW__ {
	...
	uint32 IConfigurable_I::removeParameter( const uint64 & key, pcIError_base & error ) __NOTHROW__ {
	...
	void IConfigurable_I::getAllParameters( uint64 * array, sizet count ) const __NOTHROW__ {
	...
	uint32 IConfigurable_I::getDataType( const uint64 & key, pcIError_base & error ) const __NOTHROW__ {
