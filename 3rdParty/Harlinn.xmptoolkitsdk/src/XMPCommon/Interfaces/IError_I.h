...
namespace XMP_COMPONENT_INT_NAMESPACE {
	...
	public:
		...
		//!
		void AppendParameter( const spcIUTF8String & string ) __NOTHROW__;

		virtual pIError GetActualIError() __NOTHROW__ { return this; }
	...
	protected:
		...
		//!
		virtual uint32 getCode( pcIError_base & error ) const __NOTHROW__;
		virtual uint32 getDomain( pcIError_base & error ) const __NOTHROW__;
		virtual uint32 getSeverity( pcIError_base & error ) const __NOTHROW__;
		virtual pcIUTF8String_base getMessage( pcIError_base & error ) const __NOTHROW__;
		virtual pcIUTF8String_base getLocation( pcIError_base & error ) const __NOTHROW__;
		virtual pcIUTF8String_base getParameter( sizet index, pcIError_base & error ) const __NOTHROW__;
		virtual pIError_base getNextError( pcIError_base & error ) __NOTHROW__;
		virtual pIError_base setNextError( pIError_base nextError, pcIError_base & error ) __NOTHROW__;
		virtual pvoid getInterfacePointer( uint64 interfaceID, uint32 interfaceVersion, pcIError_base & error ) __NOTHROW__;
		//! @}
		//!

		virtual pvoid GetInterfacePointerInternal( uint64 interfaceID, uint32 interfaceVersion, bool isTopLevel );
