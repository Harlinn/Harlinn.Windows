...
#include "XMPFilesPlugins\api\source\HostAPI.h"
...
void PluginManager::scanRecursive( const std::string & tempPath, std::vector<std::string>& ioFoundLibs, XMP_Int32 inLevel, XMP_Int32 inMaxNestingLevel )
{
	...
	while ( Host_IO::GetNextChild ( aFolder.folder, &childName ) ) {	
		...
		if ( okFolder ) {
			...
			StringVec::const_iterator iterFound = std::find_if( mExtensions.begin( ), mExtensions.end( ), [&fileExt]( const auto& arg ) { return arg == fileExt; } );
			...
			if ( iterFound != mExtensions.end() ) {
				...
				StringVec::const_iterator pluginNeeded = std::find_if( mPluginsNeeded.begin( ), mPluginsNeeded.end( ), [&childName]( const auto& arg ) { return arg == childName; } );
