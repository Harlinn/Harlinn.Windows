...
namespace AdobeXMPCore_Int {
	...
	public:
		...
		virtual spISharedMutex GetMutex() const;
		virtual spIDOMSerializer Clone() const;
		virtual spIUTF8String SerializeInternal(const spINode & node, XMP_OptionBits options, sizet padding, const char * newline, const char * indent, sizet baseIndent, const spcINameSpacePrefixMap & nameSpacePrefixMap) const = 0;

	protected:
		virtual ~DOMSerializerImpl() __NOTHROW__ {}
        using IDOMSerializer_I::clone;
		virtual DOMSerializerImpl * clone() const = 0;
