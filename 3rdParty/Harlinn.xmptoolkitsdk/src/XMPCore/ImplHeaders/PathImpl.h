...
namespace AdobeXMPCore_Int {
	...
	protected:
		...
		virtual spcINameSpacePrefixMap RegisterNameSpacePrefixMap( const spcINameSpacePrefixMap & map );
		virtual spIUTF8String Serialize( const spcINameSpacePrefixMap & map = spcINameSpacePrefixMap() ) const;
		virtual void AppendPathSegment( const spcIPathSegment & segment );
		virtual spcIPathSegment RemovePathSegment( sizet index );
		virtual spcIPathSegment GetPathSegment( sizet index ) const;
		virtual sizet Size() const __NOTHROW__;
		virtual void Clear() __NOTHROW__;
		virtual spIPath Clone( sizet startingIndex = 1, sizet countOfSegments = kMaxSize ) const;
