...
namespace AdobeXMPCore_Int {
	...
	public:
		...
		virtual spIUTF8String Serialize( const spINode & node, const spcINameSpacePrefixMap & nameSpacePrefixMap );
		virtual eConfigurableErrorCode ValidateValue( const uint64 & key, eDataType type, const CombinedDataValue & value ) const;
		virtual spIUTF8String SerializeInternal(const spINode & node, XMP_OptionBits options, sizet padding, const char * newline, const char * indent, sizet baseIndent, const spcINameSpacePrefixMap & nameSpacePrefixMap) const;
		void InitializeDefaultValues();

	protected:
		virtual ~RDFDOMSerializerImpl() __NOTHROW__ {}
		virtual DOMSerializerImpl * clone() const;
