...
namespace AdobeXMPCore_Int {
	...
	public:
		...
		virtual spcIUTF8String GetNameSpace( const spcIUTF8String & prefix ) const = 0;
		virtual spcIUTF8String GetPrefix( const spcIUTF8String & nameSpace ) const = 0;
		virtual void Merge( const spcINameSpacePrefixMap & otherMap ) = 0;

		virtual pINameSpacePrefixMap GetActualINameSpacePrefixMap() __NOTHROW__ { return this; }
		virtual pINameSpacePrefixMap_I GetINameSpacePrefixMap_I() __NOTHROW__ { return this; }
	...
	protected:
		...
		virtual uint32 insert( const char * prefix, sizet prefixLength, const char * nameSpace, sizet nameSpaceLength, pcIError_base & error ) __NOTHROW__;
		virtual uint32 remove( uint32 keyType, const char * key, sizet keyLength, pcIError_base & error ) __NOTHROW__;
		virtual uint32 isPresent( uint32 keyType, const char * key, sizet keyLength, pcIError_base & error ) const __NOTHROW__;
		virtual pcIUTF8String_base get( uint32 keyType, const char * key, sizet keyLength, pcIError_base & error ) const __NOTHROW__;
		virtual pINameSpacePrefixMap_base clone( pcIError_base & error ) const __NOTHROW__;
