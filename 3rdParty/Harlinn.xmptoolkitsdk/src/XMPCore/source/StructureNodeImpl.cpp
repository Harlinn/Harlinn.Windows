...
namespace AdobeXMPCore_Int {
	...
	spINode StructureNodeImpl::GetNode( const spcIUTF8String & nameSpace, const spcIUTF8String & name ) {
	...
	spINode StructureNodeImpl::GetNode( const char * nameSpace, sizet nameSpaceLength, const char * name, sizet nameLength ) {
	...
	spINode StructureNodeImpl::RemoveNode( const spcIUTF8String & nameSpace, const spcIUTF8String & name ) {
		if ( nameSpace->size() == 0 || name->size() == 0 )
			return spINode();
		QualifiedName qName( nameSpace, name );
	...
	spINode StructureNodeImpl::RemoveNode( const char * nameSpace, sizet nameSpaceLength, const char * name, sizet nameLength ) {
		spIUTF8String nameSpaceStr( IUTF8String_I::CreateUTF8String( nameSpace, nameSpaceLength ) );
		spIUTF8String nameStr( IUTF8String_I::CreateUTF8String( name, nameLength ) );
		return RemoveNode( nameSpaceStr, nameStr );
	}

	INode_v1::eNodeType StructureNodeImpl::GetChildNodeType( const char * nameSpace, sizet nameSpaceLength, const char * name, sizet nameLength ) const {
	...
	INode_v1::eNodeType StructureNodeImpl::GetNodeType() const {
		return INode_v1::kNTStructure;
	}

	void StructureNodeImpl::InsertNode( const spINode & node ) {
	...
	spINode StructureNodeImpl::ReplaceNode( const spINode & node ) {
	...
	void StructureNodeImpl::AppendNode( const spINode & node ) {
		InsertNode( node );
	}

	spINodeIterator StructureNodeImpl::Iterator() {
		AutoSharedLock lock( mSharedMutex );
	...
	sizet StructureNodeImpl::ChildCount() const __NOTHROW__ {
		AutoSharedLock lock( mSharedMutex );
		return mChildrenMap.size();
	}

	spIStructureNode StructureNodeImpl::ConvertToStructureNode() {
		return MakeUncheckedSharedPointer( this, __FILE__, __LINE__ );
	}

	bool StructureNodeImpl::HasContent() const {
		AutoSharedLock lock( mSharedMutex );
	...
	void StructureNodeImpl::ClearContents() {
		AutoSharedLock lock( mSharedMutex, true );
	...
	spINode StructureNodeImpl::CloneContents( bool ignoreEmptyNodes, bool ignoreNodesWithOnlyQualifiers, sizet qualifiersCount ) const {
		AutoSharedLock lock( mSharedMutex );
