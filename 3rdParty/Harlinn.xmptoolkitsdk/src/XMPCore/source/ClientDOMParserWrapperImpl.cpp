...
#include <xmp/XMPCore/Interfaces/IClientDOMParser.h>
...
namespace AdobeXMPCore_Int {
	...
	spINode ClientDOMParserWrapperImpl::ParseAsNode( const char * buffer, sizet bufferLength ) {
		pcIError_base pError( NULL );
		uint32 unknownExceptionCaught( 0 );
	...
	eConfigurableErrorCode ClientDOMParserWrapperImpl::ValidateValue( const uint64 & key, eDataType type, const CombinedDataValue & value ) const {
		pcIError_base pError( NULL );
		uint32 unknownExceptionCaught( 0 );
	...
	DOMParserImpl * ClientDOMParserWrapperImpl::clone() const {
