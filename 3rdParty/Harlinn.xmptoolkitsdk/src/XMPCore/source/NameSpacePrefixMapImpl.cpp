...
namespace AdobeXMPCore_Int {
	...
	bool NameSpacePrefixMapImpl::Insert( const char * prefix, sizet prefixLength, const char * nameSpace, sizet nameSpaceLength ) {
	...
	bool NameSpacePrefixMapImpl::RemovePrefix( const char * prefix, sizet prefixLength ) {
	...
	bool NameSpacePrefixMapImpl::RemoveNameSpace( const char * nameSpace, sizet nameSpaceLength ) {
	...
	bool NameSpacePrefixMapImpl::IsPrefixPresent( const char * prefix, sizet prefixLength ) const {
		if ( VerifyParameters( true, false, prefix, NULL, prefixLength, 0 ) ) {
			spcIUTF8String prefixStr = IUTF8String_I::CreateUTF8String( prefix, prefixLength );
			AutoSharedLock lock( mSharedMutex, true );
	...
	bool NameSpacePrefixMapImpl::IsNameSpacePresent( const char * nameSpace, sizet nameSpaceLength ) const {
		if ( VerifyParameters( false, true, NULL, nameSpace, 0, nameSpaceLength ) ) {
			spcIUTF8String nameSpaceStr = IUTF8String_I::CreateUTF8String( nameSpace, nameSpaceLength );
			AutoSharedLock lock( mSharedMutex, true );
	...
	spcIUTF8String NameSpacePrefixMapImpl::GetNameSpace( const char * prefix, sizet prefixLength ) const {
		if ( VerifyParameters( true, false, prefix, NULL, prefixLength, 0 ) ) {
			spcIUTF8String prefixStr = IUTF8String_I::CreateUTF8String( prefix, prefixLength );
			AutoSharedLock lock( mSharedMutex, true );
	...
	spcIUTF8String NameSpacePrefixMapImpl::GetNameSpace( const spcIUTF8String & prefix ) const {
	...
	spcIUTF8String NameSpacePrefixMapImpl::GetPrefix( const char * nameSpace, sizet nameSpaceLength ) const {
		if ( VerifyParameters( false, true, NULL, nameSpace, 0, nameSpaceLength ) ) {
			spcIUTF8String nameSpaceStr = IUTF8String_I::CreateUTF8String( nameSpace, nameSpaceLength );
			AutoSharedLock lock( mSharedMutex, true );
	...
	spcIUTF8String NameSpacePrefixMapImpl::GetPrefix( const spcIUTF8String & nameSpace ) const {
	...
	sizet NameSpacePrefixMapImpl::Size() const __NOTHROW__ {
		AutoSharedLock lock( mSharedMutex, false );
		return mNameSpaceToPrefixMap.size();
	}

	void NameSpacePrefixMapImpl::Clear() __NOTHROW__ {
		AutoSharedLock lock( mSharedMutex, true );
		mNameSpaceToPrefixMap.clear();
		mPrefixToNameSpaceMap.clear();
	}

	spINameSpacePrefixMap NameSpacePrefixMapImpl::Clone() const {
	...
	void NameSpacePrefixMapImpl::Merge( const spcINameSpacePrefixMap & otherMap ) {
		if ( otherMap ) {
