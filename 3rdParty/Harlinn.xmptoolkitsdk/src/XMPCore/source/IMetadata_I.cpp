...
namespace AdobeXMPCore_Int {
	...
	pvoid IMetadata_I::GetInterfacePointer( uint64 interfaceID, uint32 interfaceVersion ) {
		return GetInterfacePointerInternal( interfaceID, interfaceVersion, true );
	}

	pvoid IMetadata_I::getInterfacePointer( uint64 interfaceID, uint32 interfaceVersion, pcIError_base & error ) __NOTHROW__ {
	...
	pvoid IMetadata_I::GetInterfacePointerInternal( uint64 interfaceID, uint32 interfaceVersion, bool isTopLevel ) {
	...
	pcIUTF8String_base IMetadata_I::getAboutURI( pcIError_base & error ) const __NOTHROW__ {
