...
namespace AdobeXMPCore_Int {
	...
	pvoid ISimpleNode_I::GetInterfacePointer( uint64 interfaceID, uint32 interfaceVersion ) {
		return GetInterfacePointerInternal( interfaceID, interfaceVersion, true );
	}

	pvoid ISimpleNode_I::getInterfacePointer( uint64 interfaceID, uint32 interfaceVersion, pcIError_base & error ) __NOTHROW__ {
	...
	pvoid ISimpleNode_I::GetInterfacePointerInternal( uint64 interfaceID, uint32 interfaceVersion, bool isTopLevel ) {
	...
	pcIUTF8String_base ISimpleNode_I::getValue( pcIError_base & error ) const __NOTHROW__ {
	...
	void ISimpleNode_I::setValue( const char * value, sizet valueLength, pcIError_base & error ) __NOTHROW__ {
	...
	uint32 ISimpleNode_I::isURIType( pcIError_base & error ) const __NOTHROW__ {
		return CallConstUnSafeFunction< ISimpleNode_v1, uint32, bool >(
			error, this, 0, &ISimpleNode_v1::IsURIType, __FILE__, __LINE__ );
	}

	void ISimpleNode_I::setURIType( uint32 isURI, pcIError_base & error ) __NOTHROW__ {
