...
namespace AdobeXMPCore_Int {
	...
	void NodeImpl::ChangeParent( pINode parent ) {
	...
	pINode NodeImpl::GetRawParentPointer() {
		AutoSharedLock lock( mSharedMutex );
		return mpParent;
	}

	spINode NodeImpl::GetParent() {
		AutoSharedLock lock( mSharedMutex );
		if ( mpParent ) {
			if ( mIsQualifierNode )
	...
	void NodeImpl::SetName( const char * name, sizet nameLength ) {
	...
	spcIUTF8String NodeImpl::GetName() const {
		if ( this->IsArrayItem() )
	...
	void NodeImpl::SetNameSpace( const char * nameSpace, sizet nameSpaceLength ) {
	...
	spcIUTF8String NodeImpl::GetNameSpace() const {
		if ( this->IsArrayItem() )
	...
	spIPath NodeImpl::GetPath() const {
	...
	spINode NodeImpl::GetQualifier( const char * nameSpace, sizet nameSpaceLength, const char * name, sizet nameLength ) {
		{
			AutoSharedLock lock( mSharedMutex );
	...
	spINode NodeImpl::GetQualifier( const spcIUTF8String & nameSpace, const spcIUTF8String & name ) {
		{
			AutoSharedLock lock( mSharedMutex );
			if ( !mQualifiers ) return spINode();
		}
	...
	void NodeImpl::InsertQualifier( const spINode & node ) {
		CreateQualifierNode();
		mQualifiers->InsertNode( node );
		node->GetINode_I()->SetIsQualifierNode( true );
	}

	spINode NodeImpl::ReplaceQualifier( const spINode & node ) {
		CreateQualifierNode();
	...
	spINode NodeImpl::RemoveQualifier( const char * nameSpace, sizet nameSpaceLength, const char * name, sizet nameLength ) {
		CreateQualifierNode();
		return mQualifiers->RemoveNode( nameSpace, nameSpaceLength, name, nameLength );
	}

	spINode NodeImpl::RemoveQualifier( const spcIUTF8String & nameSpace, const spcIUTF8String & name ) {
		CreateQualifierNode();
		return mQualifiers->GetIStructureNode_I()->RemoveQualifier( nameSpace, name );
	}

	bool NodeImpl::IsArrayItem() const {
		AutoSharedLock lock( mSharedMutex );
		return mIndex > 0;
	}

	bool NodeImpl::HasQualifiers() const {
		{
			AutoSharedLock lock( mSharedMutex );
	...
	bool NodeImpl::IsEmpty() const {
		return !HasContent() && !HasQualifiers();
	}

	bool NodeImpl::HasChanged() const {
		return mChangeCount != 0;
	}

	void NodeImpl::AcknowledgeChanges() const __NOTHROW__ {
		{
			AutoSharedLock lock( mSharedMutex );
	...
	void NodeImpl::Clear( bool contents, bool qualifiers ) {
	...
	spINode NodeImpl::Clone( bool ignoreEmptyNodes, bool ignoreNodesWithOnlyQualifiers ) const {
		AutoSharedLock lock( mSharedMutex );
	...
	sizet NodeImpl::QualifiersCount() const __NOTHROW__ {
		{
			AutoSharedLock lock( mSharedMutex );
	...
	spINodeIterator NodeImpl::QualifiersIterator() {
		{
			AutoSharedLock lock( mSharedMutex );
	...
	INode_v1::eNodeType NodeImpl::GetParentNodeType() const {
		if ( mpParent )
			return mpParent->GetNodeType();
		return INode_v1::kNTNone;
	}

	INode_v1::eNodeType NodeImpl::GetQualifierNodeType( const char * nameSpace, sizet nameSpaceLength, const char * name, sizet nameLength ) const {
		{
			AutoSharedLock lock( mSharedMutex );
	...
	spISimpleNode NodeImpl::ConvertToSimpleNode() {
		return spISimpleNode();
	}

	spIStructureNode NodeImpl::ConvertToStructureNode() {
		return spIStructureNode();
	}

	spIArrayNode NodeImpl::ConvertToArrayNode() {
		return spIArrayNode();
	}

	spIMetadata NodeImpl::ConvertToMetadata() {
		return spIMetadata();
	}
	...
	bool NodeImpl::IsQualifierNode() const {
		return mIsQualifierNode;
	}

	sizet NodeImpl::GetIndex() const {
		return mIndex;
	}
