...
namespace AdobeXMPCore_Int {
	...
	pvoid IPath_I::GetInterfacePointer( uint64 interfaceID, uint32 interfaceVersion ) {
		return GetInterfacePointerInternal( interfaceID, interfaceVersion, true );
	}

	pvoid IPath_I::getInterfacePointer( uint64 interfaceID, uint32 interfaceVersion, pcIError_base & error ) __NOTHROW__ {
	...
	pvoid IPath_I::GetInterfacePointerInternal( uint64 interfaceID, uint32 interfaceVersion, bool isTopLevel ) {
	...
	pcINameSpacePrefixMap_base IPath_I::registerNameSpacePrefixMap( pcINameSpacePrefixMap_base map, pcIError_base & error ) __NOTHROW__ {
	...
	pIUTF8String_base IPath_I::serialize( pcINameSpacePrefixMap_base map, pcIError_base & error ) const __NOTHROW__ {
	...
	void IPath_I::appendPathSegment( pcIPathSegment_base segment, pcIError_base & error ) __NOTHROW__ {
	...
	pcIPathSegment_base IPath_I::removePathSegment( sizet index, pcIError_base & error ) __NOTHROW__ {
	...
	pcIPathSegment_base IPath_I::getPathSegment( sizet index, pcIError_base & error ) const __NOTHROW__ {
	...
	pIPath_base IPath_I::clone( sizet startingIndex, sizet countOfSegemetns, pcIError_base & error ) const __NOTHROW__ {
