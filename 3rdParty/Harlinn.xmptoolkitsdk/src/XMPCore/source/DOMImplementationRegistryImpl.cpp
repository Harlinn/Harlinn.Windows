...
namespace AdobeXMPCore_Int {
	...
	bool DOMImplementationRegistryImpl::RegisterParser( const char * key, const spcIDOMParser & parser ) {
	...
	bool DOMImplementationRegistryImpl::RegisterSerializer( const char * key, const spcIDOMSerializer & serializer ) {
		spcIUTF8String  keyStr( IUTF8String_I::CreateUTF8String( key, AdobeXMPCommon::npos ) );
		auto it = mSerializerMap.insert( std::pair< spcIUTF8String, spcIDOMSerializer >( keyStr, serializer ) );
		return it.second;
	}

	spIDOMParser DOMImplementationRegistryImpl::GetParser( const char * key ) const {
		spcIUTF8String  keyStr( IUTF8String_I::CreateUTF8String( key, AdobeXMPCommon::npos ) );
	...
	spIDOMSerializer DOMImplementationRegistryImpl::GetSerializer( const char * key ) const {
		spcIUTF8String  keyStr( IUTF8String_I::CreateUTF8String( key, AdobeXMPCommon::npos ) );
