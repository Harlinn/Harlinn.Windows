...
namespace AdobeXMPCore_Int {
	...
	pvoid IArrayNode_I::GetInterfacePointer( uint64 interfaceID, uint32 interfaceVersion ) {
		return GetInterfacePointerInternal( interfaceID, interfaceVersion, true );
	}

	pvoid IArrayNode_I::getInterfacePointer( uint64 interfaceID, uint32 interfaceVersion, pcIError_base & error ) __NOTHROW__ {
	...
	pvoid IArrayNode_I::GetInterfacePointerInternal( uint64 interfaceID, uint32 interfaceVersion, bool isTopLevel ) {
	...
	uint32 IArrayNode_I::getArrayForm( pcIError_base & error ) const __NOTHROW__ {
		return CallConstUnSafeFunction< IArrayNode_v1, uint32, eArrayForm >(
			error, this, 0, &IArrayNode_v1::GetArrayForm, __FILE__, __LINE__ );
	}

	uint32 IArrayNode_I::getChildNodeType( pcIError_base & error ) const __NOTHROW__ {
	...
	pINode_base IArrayNode_I::getNodeAtIndex( sizet index, pcIError_base & error ) __NOTHROW__ {
		typedef spINode( IArrayNode_v1::*Func )( sizet );
	...
	void IArrayNode_I::insertNodeAtIndex( pINode_base node, sizet index, pcIError_base & error ) __NOTHROW__ {
	...
	pINode_base IArrayNode_I::removeNodeAtIndex( sizet index, pcIError_base & error ) __NOTHROW__ {
		return CallUnSafeFunctionReturningSharedPointer< IArrayNode_v1, pINode_base, INode, sizet >(
			error, this, &IArrayNode_v1::RemoveNodeAtIndex, __FILE__, __LINE__, index );
	}

	pINode_base IArrayNode_I::replaceNodeAtIndex( pINode_base node, sizet index, pcIError_base & error ) __NOTHROW__ {
