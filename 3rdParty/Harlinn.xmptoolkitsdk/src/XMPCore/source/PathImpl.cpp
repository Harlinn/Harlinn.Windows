...
namespace AdobeXMPCore_Int {
	...
	spcINameSpacePrefixMap PathImpl::RegisterNameSpacePrefixMap( const spcINameSpacePrefixMap & map ) {
	...
	spIUTF8String PathImpl::Serialize( const spcINameSpacePrefixMap & map ) const {
	...
	void PathImpl::AppendPathSegment( const spcIPathSegment & segment ) {
		if ( !segment )
	...
	spcIPathSegment PathImpl::RemovePathSegment( sizet index ) {
	...
	spcIPathSegment PathImpl::GetPathSegment( sizet index ) const {
		if ( index - 1 >= mSegments.size() ) {
			NOTIFY_ERROR( IError_v1::kEDGeneral, kGECIndexOutOfBounds,
	...
	void PathImpl::Clear() __NOTHROW__ {
		mSegments.clear();
	}

	spIPath PathImpl::Clone( sizet startingIndex, sizet countOfSegments ) const {
