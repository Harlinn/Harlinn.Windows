...
namespace AdobeXMPCore_Int {
	...
	pvoid ICompositeNode_I::GetInterfacePointer( uint64 interfaceID, uint32 interfaceVersion ) {
		return GetInterfacePointerInternal( interfaceID, interfaceVersion, true );
	}

	pvoid ICompositeNode_I::getInterfacePointer( uint64 interfaceID, uint32 interfaceVersion, pcIError_base & error ) __NOTHROW__ {
	...
	pvoid ICompositeNode_I::GetInterfacePointerInternal( uint64 interfaceID, uint32 interfaceVersion, bool isTopLevel ) {
	...
	uint32 ICompositeNode_I::getNodeTypeAtPath( pcIPath_base path, pcIError_base & error ) const  __NOTHROW__ {
	...
	pINode_base ICompositeNode_I::getNodeAtPath( pcIPath_base path, pcIError_base & error )  __NOTHROW__ {
		typedef spINode( ICompositeNode_v1::*Func )( const spcIPath & );
	...
	void ICompositeNode_I::appendNode( pINode_base node, pcIError_base & error )  __NOTHROW__ {
	...
	void ICompositeNode_I::insertNodeAtPath( pINode_base node, pcIPath_base path, pcIError_base & error )  __NOTHROW__ {
	...
	pINode_base ICompositeNode_I::replaceNodeAtPath( pINode_base node, pcIPath_base path, pcIError_base & error )  __NOTHROW__ {
	...
	pINode_base ICompositeNode_I::removeNodeAtPath( pcIPath_base path, pcIError_base & error )  __NOTHROW__ {
		return CallUnSafeFunctionReturningSharedPointer< ICompositeNode_v1, pINode_base, INode, const spcIPath & >(
			error, this, &ICompositeNode_v1::RemoveNodeAtPath, __FILE__, __LINE__, IPath_v1::MakeShared( path ) );
	}

	pINodeIterator_base ICompositeNode_I::iterator( pcIError_base & error )  __NOTHROW__ {
