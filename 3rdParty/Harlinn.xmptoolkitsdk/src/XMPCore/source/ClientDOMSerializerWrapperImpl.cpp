...
#include <xmp/XMPCore/Interfaces/IClientDOMSerializer.h>
...
namespace AdobeXMPCore_Int {
	...
	DOMSerializerImpl * ClientDOMSerializerWrapperImpl::clone() const {
	...
	spIUTF8String ClientDOMSerializerWrapperImpl::Serialize( const spINode & node, const spcINameSpacePrefixMap & map ) {
	...
	eConfigurableErrorCode ClientDOMSerializerWrapperImpl::ValidateValue( const uint64 & key, eDataType type, const CombinedDataValue & value ) const {
		pcIError_base pError( NULL );
		uint32 unknownExceptionCaught( 0 );
	...
	spIUTF8String ClientDOMSerializerWrapperImpl::SerializeInternal(const spINode & node, XMP_OptionBits options, sizet padding, const char * newline, const char * indent, sizet baseIndent, const spcINameSpacePrefixMap & nameSpacePrefixMap) const {
