...
namespace AdobeXMPCore_Int {
	...
	IArrayNode_v1::eArrayForm ArrayNodeImpl::GetArrayForm() const {
		return mArrayForm;
	}

	INode_v1::eNodeType ArrayNodeImpl::GetChildNodeType() const {
	...
	spINode ArrayNodeImpl::GetNodeAtIndex( sizet index ) {
	...
	void ArrayNodeImpl::InsertNodeAtIndex( const spINode & node, sizet index ) {
		sizet actualIndex = index - 1;
		{
			AutoSharedLock lock( mSharedMutex );
			if ( actualIndex > mChildren.size() )
	...
	spINode ArrayNodeImpl::RemoveNodeAtIndex( sizet index ) {
	...
	spINode ArrayNodeImpl::ReplaceNodeAtIndex( const spINode & node, sizet index ) {
	...
	void ArrayNodeImpl::AppendNode( const spINode & node ) {
		InsertNodeAtIndex( node, ChildCount() + 1 );
	}

	spINodeIterator ArrayNodeImpl::Iterator() {
		AutoSharedLock lock( mSharedMutex );
	...
	sizet ArrayNodeImpl::ChildCount() const __NOTHROW__ {
		AutoSharedLock lock( mSharedMutex );
		return mChildren.size();
	}

	spIArrayNode ArrayNodeImpl::ConvertToArrayNode() {
		return MakeUncheckedSharedPointer( this, __FILE__, __LINE__ );
	}

	INode_v1::eNodeType ArrayNodeImpl::GetNodeType() const {
		return kNTArray;
	}

	bool ArrayNodeImpl::HasContent() const {
		AutoSharedLock lock( mSharedMutex );
		return mChildren.size() > 0;
	}

	void ArrayNodeImpl::ClearContents() {
		AutoSharedLock lock( mSharedMutex, true );
	...
	spINode ArrayNodeImpl::CloneContents( bool ignoreEmptyNodes, bool ignoreNodesWithOnlyQualifiers, sizet qualifiersCount ) const {
