...
//	}
	...
	DOMParserImpl * RDFDOMParserImpl::clone() const {
		return new RDFDOMParserImpl();
	}

	spINode RDFDOMParserImpl::ParseAsNode( const char * buffer, sizet bufferLength ) {
...
//		return metadata;
	...
	eConfigurableErrorCode RDFDOMParserImpl::ValidateValue( const uint64 & key, eDataType type, const CombinedDataValue & value ) const {
		return kCECNone;
	}
