...
namespace AdobeXMPCore_Int {
	...
	spcIUTF8String SimpleNodeImpl::GetValue() const {
		AutoSharedLock lock( mSharedMutex );
		return mValue;
	}

	void SimpleNodeImpl::SetValue( const char * value, sizet valueLength ) {
		AutoSharedLock lock( mSharedMutex, true );
		mValue->assign( value, valueLength );
		RegisterChange();
	}

	bool SimpleNodeImpl::IsURIType() const {
		AutoSharedLock lock( mSharedMutex );
		return mIsURIType;
	}

	void SimpleNodeImpl::SetURIType( bool isURI ) {
		AutoSharedLock( mSharedMutex, true );
		mIsURIType = isURI;
		RegisterChange();
	}

	INode_v1::eNodeType SimpleNodeImpl::GetNodeType() const {
		return INode_v1::kNTSimple;
	}

	bool SimpleNodeImpl::HasContent() const {
		AutoSharedLock lock( mSharedMutex );
		return !mValue->empty();
	}

	void SimpleNodeImpl::ClearContents() {
		AutoSharedLock  lock( mSharedMutex, true );
		mValue->clear();
		RegisterChange();
	}

	spINode SimpleNodeImpl::CloneContents( bool ignoreEmptyNodes, bool ignoreNodesWithOnlyQualifiers, sizet qualifiersCount ) const {
	...
	spISimpleNode SimpleNodeImpl::ConvertToSimpleNode() {
