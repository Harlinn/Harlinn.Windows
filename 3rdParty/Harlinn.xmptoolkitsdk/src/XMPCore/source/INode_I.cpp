...
namespace AdobeXMPCore_Int {
	...
	pvoid INode_I::GetInterfacePointer( uint64 interfaceID, uint32 interfaceVersion ) {
		return GetInterfacePointerInternal( interfaceID, interfaceVersion, true );
	}

	pvoid INode_I::getInterfacePointer( uint64 interfaceID, uint32 interfaceVersion, pcIError_base & error ) __NOTHROW__ {
	...
	pvoid INode_I::GetInterfacePointerInternal( uint64 interfaceID, uint32 interfaceVersion, bool isTopLevel ) {
	...
	pINode_base INode_I::getParent( pcIError_base & error ) __NOTHROW__ {
	...
	uint32 INode_I::getParentNodeType( pcIError_base & error ) const __NOTHROW__ {
		return CallConstUnSafeFunction< INode_v1, uint32, eNodeType >(
			error, this, 0, &INode_v1::GetParentNodeType, __FILE__, __LINE__ );
	}

	void INode_I::setName( const char * name, sizet nameLength, pcIError_base & error ) __NOTHROW__ {
	...
	pcIUTF8String_base INode_I::getName( pcIError_base & error ) const __NOTHROW__ {
	...
	void INode_I::setNameSpace( const char * nameSpace, sizet nameSpaceLength, pcIError_base & error ) __NOTHROW__ {
		return CallUnSafeFunctionReturningVoid< INode_v1, const char *, sizet >(
			error, this, &INode_v1::SetNameSpace, __FILE__, __LINE__, nameSpace, nameSpaceLength );

	}

	pcIUTF8String_base INode_I::getNameSpace( pcIError_base & error ) const __NOTHROW__ {
		return CallConstUnSafeFunctionReturningSharedPointer< INode_v1, pcIUTF8String_base, const IUTF8String >(
			error, this, &INode_v1::GetNameSpace, __FILE__, __LINE__ );
	}

	pIPath_base INode_I::getPath( pcIError_base & error ) const __NOTHROW__ {
	...
	pINodeIterator_base INode_I::qualifiersIterator( pcIError_base & error ) __NOTHROW__ {
	...
	uint32 INode_I::getQualifierNodeType( const char * nameSpace, sizet nameSpaceLength, const char * name, sizet nameLength, pcIError_base & error ) const __NOTHROW__ {
	...
	pINode_base INode_I::getQualifier( const char * nameSpace, sizet nameSpaceLength, const char * name, sizet nameLength, pcIError_base & error ) __NOTHROW__ {
		typedef spINode( INode_v1::*Func )( const char *, sizet, const char *, sizet );
	...
	void INode_I::insertQualifier( pINode_base base, pcIError_base & error ) __NOTHROW__ {
	...
	pINode_base INode_I::replaceQualifier( pINode_base node, pcIError_base & error ) __NOTHROW__ {
	...
	pINode_base INode_I::clone( uint32 igoreEmptyNodes, uint32 ignoreNodesWithOnlyQualifiers, pcIError_base & error ) const __NOTHROW__ {
	...
	pISimpleNode_base INode_I::convertToSimpleNode( pcIError_base & error ) __NOTHROW__ {
	...
	pIStructureNode_base INode_I::convertToStructureNode( pcIError_base & error ) __NOTHROW__ {
	...
	pIArrayNode_base INode_I::convertToArrayNode( pcIError_base & error ) __NOTHROW__ {
	...
	pIMetadata_base INode_I::convertToMetadata( pcIError_base & error ) __NOTHROW__ {
