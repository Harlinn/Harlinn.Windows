...
namespace AdobeXMPCore_Int {
	...
	pvoid IPathSegment_I::GetInterfacePointer( uint64 interfaceID, uint32 interfaceVersion ) {
		return GetInterfacePointerInternal( interfaceID, interfaceVersion, true );
	}

	pvoid IPathSegment_I::getInterfacePointer( uint64 interfaceID, uint32 interfaceVersion, pcIError_base & error ) __NOTHROW__ {
	...
	pvoid IPathSegment_I::GetInterfacePointerInternal( uint64 interfaceID, uint32 interfaceVersion, bool isTopLevel ) {
	...
	pcIUTF8String_base IPathSegment_I::getNameSpace( pcIError_base & error ) const __NOTHROW__ {
	...
	pcIUTF8String_base IPathSegment_I::getName( pcIError_base & error ) const __NOTHROW__ {
		return CallConstUnSafeFunctionReturningSharedPointer< IPathSegment_v1, pcIUTF8String_base, const IUTF8String >(
			error, this, &IPathSegment_v1::GetName, __FILE__, __LINE__ );
	}

	uint32 IPathSegment_I::getType( pcIError_base & error ) const __NOTHROW__ {
	...
	pcIUTF8String_base IPathSegment_I::getValue( pcIError_base & error ) const __NOTHROW__ {
		return CallConstUnSafeFunctionReturningSharedPointer< IPathSegment_v1, pcIUTF8String_base, const IUTF8String >(
