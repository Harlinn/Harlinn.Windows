...
namespace AdobeXMPCore_Int {
	...
	spcIUTF8String MetadataImpl::GetAboutURI() const {
		AutoSharedLock lock( mSharedMutex );
		return mAboutURI;
	}

	void MetadataImpl::SetAboutURI( const char * uri, sizet uriLength ) __NOTHROW__ {
		AutoSharedLock lock( mSharedMutex, true );
		mAboutURI->assign( uri, uriLength );
	}

	spcIUTF8String MetadataImpl::GetName() const {
		return IUTF8String_I::CreateUTF8String();
	}

	spcIUTF8String MetadataImpl::GetNameSpace() const {
		return IUTF8String_I::CreateUTF8String();
	}

	spINode MetadataImpl::GetParent() {
		return spINode();
	}

	INode::eNodeType MetadataImpl::GetParentNodeType() const {
		return INode::kNTNone;
	}

	void MetadataImpl::SetName( const char * name, sizet nameLength ) {
		return;
	}

	void MetadataImpl::SetNameSpace( const char * nameSpace, sizet nameSpaceLength ) {
		return;
	}

	void MetadataImpl::ClearContents() {
	...
	spINode MetadataImpl::CloneContents( bool ignoreEmptyNodes, bool ignoreNodesWithOnlyQualifiers, sizet qualifiersCount ) const {
	...
	spIMetadata MetadataImpl::ConvertToMetadata() {
		return MakeUncheckedSharedPointer( this, __FILE__, __LINE__ );
	}

	void MetadataImpl::EnableFeature( const char * key, sizet keyLength ) const __NOTHROW__ {
		UTF8String keyStr( key, keyLength );
		if ( keyStr.compare( "alias" ) == 0 )
			mSupportAliases = true;
	}

	void MetadataImpl::DisableFeature( const char * key, sizet keyLength ) const __NOTHROW__ {
		UTF8String keyStr( key, keyLength );
		if ( keyStr.compare( "alias" ) == 0 )
			mSupportAliases = false;
	}
	...
	spINode MetadataImpl::ReplaceNode( const spINode & node ) {
		if ( mSupportAliases  ) {
	...
	void MetadataImpl::InsertNode( const spINode & node ) {
	...
	spINode MetadataImpl::RemoveNode( const spcIUTF8String & nameSpace, const spcIUTF8String & name ) {
		if ( mSupportAliases ) {
			XMP_ExpandedXPath  exPath;
	...
	spINode MetadataImpl::GetNode( const spcIUTF8String & nameSpace, const spcIUTF8String & name ) {
		if ( mSupportAliases ) {
			XMP_ExpandedXPath  exPath;
			if (IsNodeAlias(nameSpace->c_str(), name->c_str(), exPath)) {
				spINode destNode;
