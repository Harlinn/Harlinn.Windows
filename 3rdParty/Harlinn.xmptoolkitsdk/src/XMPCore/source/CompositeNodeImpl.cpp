...
namespace AdobeXMPCore_Int {
	...
	INode_v1::eNodeType CompositeNodeImpl::GetNodeTypeAtPath( const spcIPath & path ) const {
		auto node = const_cast< CompositeNodeImpl * >( this )->GetNodeAtPath( path );
		if ( node ) return node->GetNodeType();
		return INode_v1::kNTNone;
	}

	spINode CompositeNodeImpl::GetNodeAtPath( const spcIPath & path ) {
		if ( !path )
			return spINode();
	...
	void CompositeNodeImpl::InsertNodeAtPath( const spINode & node, const spcIPath & path ) {
	...
	spINode CompositeNodeImpl::ReplaceNodeAtPath( const spINode & node, const spcIPath & path ) {
		NOTIFY_ERROR( IError_v1::kEDGeneral, kGECNotImplemented,
			"ReplaceNodeAtPath( path ) is not yet implemented", IError_v1::kESOperationFatal, false, false );
	}

	spINode CompositeNodeImpl::RemoveNodeAtPath( const spcIPath & path ) {
