...
class EBML_DLL_API EbmlMaster : public EbmlElement {
  public:
    EBML_EXPORT EbmlMaster(const EbmlSemanticContext & aContext, bool bSizeIsKnown = true);
    EBML_EXPORT EbmlMaster(const EbmlMaster & ElementToClone);
    ...
    */
    EBML_EXPORT ~EbmlMaster() override;

    EBML_EXPORT filepos_t RenderData(IOCallback & output, bool bForceRender, bool bWithDefault = false) override;
    EBML_EXPORT filepos_t ReadData(IOCallback & input, ScopeMode ReadFully) override;
    ...
    */
    ...
    EBML_EXPORT bool PushElement(EbmlElement & element);
    ...
    */
    EBML_EXPORT EbmlElement *FindElt(const EbmlCallbacks & Callbacks) const;
    /*!
      \brief find the first element corresponding to the ID of the element
    */
    EBML_EXPORT EbmlElement *FindFirstElt(const EbmlCallbacks & Callbacks, bool bCreateIfNull);
    EBML_EXPORT EbmlElement *FindFirstElt(const EbmlCallbacks & Callbacks) const;

    /*!
      \brief find the element of the same type of PasElt following in the list of elements
    */
    EBML_EXPORT EbmlElement *FindNextElt(const EbmlElement & PastElt, bool bCreateIfNull);
    EBML_EXPORT EbmlElement *FindNextElt(const EbmlElement & PastElt) const;
    EBML_EXPORT EbmlElement *AddNewElt(const EbmlCallbacks & Callbacks);

    /*!
      \brief add an element at a specified location
    */
    EBML_EXPORT bool InsertElement(EbmlElement & element, size_t position = 0);
    EBML_EXPORT bool InsertElement(EbmlElement & element, const EbmlElement & before);

    /*!
      \brief Read the data and keep the known children
    */
    EBML_EXPORT void Read(EbmlStream & inDataStream, const EbmlSemanticContext & Context, int & UpperEltFound, EbmlElement * & FoundElt, bool AllowDummyElt, ScopeMode ReadFully = SCOPE_ALL_DATA) override;

    /*!
      \brief sort Data when they can
    */
    EBML_EXPORT void Sort();
    ...
    */
    EBML_EXPORT bool CheckMandatory() const;

    /*!
      \brief Remove an element from the list of the master
    */
    EBML_EXPORT void Remove(size_t Index);
    EBML_EXPORT void Remove(EBML_MASTER_ITERATOR & Itr);
    EBML_EXPORT void Remove(EBML_MASTER_RITERATOR & Itr);
    ...
    */
    EBML_EXPORT filepos_t WriteHead(IOCallback & output, int SizeLength, bool bWithDefault = false);

    void EnableChecksum(bool bIsEnabled = true) { bChecksumUsed = bIsEnabled; }
    bool HasChecksum() const {return bChecksumUsed;}
    EBML_EXPORT bool VerifyChecksum() const;
    ...
    */
    EBML_EXPORT std::vector<std::string> FindAllMissingElements();
  ...
  private:
    ...
    */
    EBML_EXPORT bool ProcessMandatory();
};
