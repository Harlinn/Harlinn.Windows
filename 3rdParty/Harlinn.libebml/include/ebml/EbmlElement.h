...
*/
EBML_EXPORT int EBML_DLL_API CodedSizeLength(uint64 Length, unsigned int SizeLength, bool bSizeIsFinite = true);

/*!
  \brief The coded value of the EBML-coded length
  \note The size of OutBuffer must be 8 octets at least
*/
EBML_EXPORT int EBML_DLL_API CodedValueLength(uint64 Length, int CodedSize, binary * OutBuffer);

/*!
  \brief Read an EBML-coded value from a buffer
  \return the value read
*/
EBML_EXPORT uint64 EBML_DLL_API ReadCodedSizeValue(const binary * InBuffer, uint32 & BufferSize, uint64 & SizeUnknown);

/*!
  \brief The size of the EBML-coded signed length
*/
EBML_EXPORT int EBML_DLL_API CodedSizeLengthSigned(int64 Length, unsigned int SizeLength);

/*!
  \brief The coded value of the EBML-coded signed length
  \note the size of OutBuffer must be 8 octets at least
*/
EBML_EXPORT int EBML_DLL_API CodedValueLengthSigned(int64 Length, int CodedSize, binary * OutBuffer);

/*!
  \brief Read a signed EBML-coded value from a buffer
  \return the value read
*/
EBML_EXPORT int64 EBML_DLL_API ReadCodedSizeSignedValue(const binary * InBuffer, uint32 & BufferSize, uint64 & SizeUnknown);
...
class EBML_DLL_API EbmlCallbacks {
  public:
    EBML_EXPORT EbmlCallbacks(EbmlElement & (*Creator)(), const EbmlId & aGlobalId, const char * aDebugName, const EbmlSemanticContext & aContext);
  ...
  public:
    ...
    bool operator!=(const EbmlSemanticContext & aElt) const {
      return ((Size != aElt.Size) || (MyTable != aElt.MyTable) ||
        ...
        EBML_EXPORT const EbmlSemantic & GetSemantic(size_t i) const;
...
class EBML_DLL_API EbmlElement {
  public:
    EBML_EXPORT EbmlElement(uint64 aDefaultSize, bool bValueSet = false);
    EBML_EXPORT virtual ~EbmlElement();
    ...
    EBML_EXPORT static EbmlElement * FindNextElement(IOCallback & DataStream, const EbmlSemanticContext & Context, int & UpperLevel, uint64 MaxDataSize, bool AllowDummyElt, unsigned int MaxLowerLevel = 1);
    EBML_EXPORT static EbmlElement * FindNextID(IOCallback & DataStream, const EbmlCallbacks & ClassInfos, uint64 MaxDataSize);

    /*!
      \brief find the next element with the same ID
    */
    EBML_EXPORT EbmlElement * FindNext(IOCallback & DataStream, uint64 MaxDataSize);

    EBML_EXPORT EbmlElement * SkipData(EbmlStream & DataStream, const EbmlSemanticContext & Context, EbmlElement * TestReadElt = nullptr, bool AllowDummyElt = false);
    ...
    */
    ...
    EBML_EXPORT uint64 ElementSize(bool bWithDefault = false) const; /// return the size of the header+data, before writing

    EBML_EXPORT filepos_t Render(IOCallback & output, bool bWithDefault = false, bool bKeepPosition = false, bool bForceRender = false);

    EBML_EXPORT virtual filepos_t UpdateSize(bool bWithDefault = false, bool bForceRender = false) = 0; /// update the Size of the Data stored
    EBML_EXPORT virtual filepos_t GetSize() const {return Size;} /// return the size of the data stored in the element, on reading

    virtual filepos_t ReadData(IOCallback & input, ScopeMode ReadFully = SCOPE_ALL_DATA) = 0;
    EBML_EXPORT virtual void Read(EbmlStream & inDataStream, const EbmlSemanticContext & Context, int & UpperEltFound, EbmlElement * & FoundElt, bool AllowDummyElt = false, ScopeMode ReadFully = SCOPE_ALL_DATA);
    ...
    */
    EBML_EXPORT virtual bool IsSmallerThan(const EbmlElement *Cmp) const;
    EBML_EXPORT static bool CompareElements(const EbmlElement *A, const EbmlElement *B);
    ...
    */
    EBML_EXPORT bool ForceSize(uint64 NewSize);

    EBML_EXPORT filepos_t OverwriteHead(IOCallback & output, bool bKeepPosition = false);
    EBML_EXPORT filepos_t OverwriteData(IOCallback & output, bool bKeepPosition = false);

    /*!
      \brief void the content of the element (replace by EbmlVoid)
    */
    EBML_EXPORT uint64 VoidMe(IOCallback & output, bool bWithDefault = false);
  ...
  protected:
    ...
    */
    EBML_EXPORT static EbmlElement *CreateElementUsingContext(const EbmlId & aID, const EbmlSemanticContext & Context, int & LowLevel, bool IsGlobalContext, bool bAllowDummy = false, unsigned int MaxLowerLevel = 1);

    EBML_EXPORT filepos_t RenderHead(IOCallback & output, bool bForceRender, bool bWithDefault = false, bool bKeepPosition = false);
    EBML_EXPORT filepos_t MakeRenderHead(IOCallback & output, bool bKeepPosition);
