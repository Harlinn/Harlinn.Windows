...
class EBML_DLL_API EbmlFloat : public EbmlElement {
  public:
    ...
    EBML_EXPORT EbmlFloat(Precision prec = FLOAT_32);
    EBML_EXPORT EbmlFloat(double DefaultValue, Precision prec = FLOAT_32);
...
//    EbmlFloat & operator=(const float NewValue) { Value = NewValue; return *this;}
    ...
    EBML_EXPORT operator float() const;
    EBML_EXPORT operator double() const;

    EBML_EXPORT EbmlFloat &SetValue(double NewValue);
    EBML_EXPORT double GetValue() const;

    EBML_EXPORT void SetDefaultValue(double);

    EBML_EXPORT double DefaultVal() const;
