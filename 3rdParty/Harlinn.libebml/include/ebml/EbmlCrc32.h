...
DECLARE_EBML_BINARY(EbmlCrc32)
  public:
    EBML_EXPORT EbmlCrc32(const EbmlCrc32 & ElementToClone);
...
//    filepos_t UpdateSize(bool bWithDefault = false);
    ...
    EBML_EXPORT void AddElementCRC32(EbmlElement &ElementToCRC);
    EBML_EXPORT bool CheckElementCRC32(EbmlElement &ElementToCRC);

    /*!
      Use this to quickly check a CRC32 with some data
      \return True if inputCRC matches CRC32 generated from input data
    */
    EBML_EXPORT static bool CheckCRC(uint32 inputCRC, const binary *input, uint32 length);
    /*!
      Calls Update() and Finalize(), use to create a CRC32 in one go
    */
    EBML_EXPORT void FillCRC32(const binary *input, uint32 length);
    /*!
      Add data to the CRC table, in other words process some data bit by bit
    */
    EBML_EXPORT void Update(const binary *input, uint32 length);
    /*!
      Use this with Update() to Finalize() or Complete the CRC32
    */
    EBML_EXPORT void Finalize();
    /*!
    ...
    */
    ...
    protected:
#endif
    EBML_EXPORT void ResetCRC();
    EBML_EXPORT void UpdateByte(binary b);
