...
{
public:
  EBML_EXPORT ADbg(int level = 0);
  EBML_EXPORT virtual ~ADbg();

  /// \todo make an inline function to test the level first and the process
  EBML_EXPORT int OutPut(int level, const char * format,...) const;

  EBML_EXPORT int OutPut(const char * format,...) const;
...
EBML_EXPORT extern ADbg globalDebug;
