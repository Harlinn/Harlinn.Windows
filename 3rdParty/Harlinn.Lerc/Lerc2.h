...
public:
  HLERC_EXPORT Lerc2();
  HLERC_EXPORT Lerc2(int nDim, int nCols, int nRows, const Byte* pMaskBits = nullptr);    // valid / invalid bits as byte array
  virtual ~Lerc2()  {}

  static int CurrentVersion() { return 5; }

  HLERC_EXPORT bool SetEncoderToOldVersion(int version);    // call this to encode compatible to an old decoder

  HLERC_EXPORT bool Set(int nDim, int nCols, int nRows, const Byte* pMaskBits = nullptr);
  ...
  HLERC_EXPORT static bool GetHeaderInfo(const Byte* pByte, size_t nBytesRemaining, struct HeaderInfo& headerInfo, bool& bHasMask);
...
private:
  ...
  HLERC_EXPORT void Init();

  HLERC_EXPORT static unsigned int ComputeNumBytesHeaderToWrite(const struct HeaderInfo& hd);
  HLERC_EXPORT static bool WriteHeader(Byte** ppByte, const struct HeaderInfo& hd);
  HLERC_EXPORT static bool ReadHeader(const Byte** ppByte, size_t& nBytesRemaining, struct HeaderInfo& hd);

  HLERC_EXPORT bool WriteMask(Byte** ppByte) const;
  HLERC_EXPORT bool ReadMask(const Byte** ppByte, size_t& nBytesRemaining);

  HLERC_EXPORT bool DoChecksOnEncode(Byte* pBlobBegin, Byte* pBlobEnd) const;
  HLERC_EXPORT static unsigned int ComputeChecksumFletcher32(const Byte* pByte, int len);
  ...
  HLERC_EXPORT static bool PruneCandidates(std::vector<double>& roundErr, std::vector<double>& zErr,
