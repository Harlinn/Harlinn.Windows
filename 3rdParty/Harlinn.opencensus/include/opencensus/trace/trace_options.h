...
class TraceOptions final {
 public:
  ...
  OPENCENSUS_EXPORT explicit TraceOptions(const uint8_t* buf);
  ...
  OPENCENSUS_EXPORT bool operator==(const TraceOptions& that) const;

  // Returns a 2-char hex string of the TraceOptions value.
  OPENCENSUS_EXPORT std::string ToHex() const;

  // Copies the TraceOptions data to a buffer, which must hold kSize bytes.
  OPENCENSUS_EXPORT void CopyTo(uint8_t* buf) const;

  // Returns a copy of these TraceOptions with the sampled bit set to
  // is_sampled.
  OPENCENSUS_EXPORT TraceOptions WithSampling(bool is_sampled) const;
