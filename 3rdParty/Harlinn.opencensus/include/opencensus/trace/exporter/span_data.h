...
class SpanData final {
 public:
  ...
  OPENCENSUS_EXPORT SpanData(absl::string_view name, SpanContext context, SpanId parent_span_id,
  ...
  OPENCENSUS_EXPORT absl::string_view name() const;

  // The SpanContext contains the trace_id, span_id, and options (IsSampled) for
  // the current span.
  OPENCENSUS_EXPORT SpanContext context() const;

  // The parent span's, span_id. This is all zeroes if the current span is a
  // root span.
  OPENCENSUS_EXPORT SpanId parent_span_id() const;

  // The annotations attached to this span.
  OPENCENSUS_EXPORT const TimeEvents<Annotation>& annotations() const;

  // Network events (send, recv) attached to this span, e.g. when an RPC reply
  // was sent.
  OPENCENSUS_EXPORT const TimeEvents<MessageEvent>& message_events() const;

  // Links to spans in other traces.
  OPENCENSUS_EXPORT const std::vector<Link>& links() const;

  // The number of links that were dropped.
  OPENCENSUS_EXPORT int num_links_dropped() const;

  // The attributes (key -> value) attached to this span, e.g. tags attached to
  // the current span.
  OPENCENSUS_EXPORT const std::unordered_map<std::string, AttributeValue>& attributes() const;

  // The number of attributes that were dropped.
  OPENCENSUS_EXPORT int num_attributes_dropped() const;

  // True if the span ended.
  OPENCENSUS_EXPORT bool has_ended() const;

  // The start time of the span.
  OPENCENSUS_EXPORT absl::Time start_time() const;

  // The end time of the span. Set to 0 if the span hasn't ended.
  OPENCENSUS_EXPORT absl::Time end_time() const;

  // The status of the span. Unset if the span hasn't ended.
  OPENCENSUS_EXPORT Status status() const;

  // True if the parent is on a different process.
  OPENCENSUS_EXPORT bool has_remote_parent() const;

  // Returns a human-readable string for debugging. Do not rely on its format or
  // try to parse it.
  OPENCENSUS_EXPORT std::string DebugString() const;
