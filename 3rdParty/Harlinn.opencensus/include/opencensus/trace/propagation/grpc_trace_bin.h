...
OPENCENSUS_EXPORT SpanContext FromGrpcTraceBinHeader(absl::string_view header);

// Returns a value for the grpc-trace-bin header.
OPENCENSUS_EXPORT std::string ToGrpcTraceBinHeader(const SpanContext& ctx);
...
OPENCENSUS_EXPORT void ToGrpcTraceBinHeader(const SpanContext& ctx, uint8_t* out);
