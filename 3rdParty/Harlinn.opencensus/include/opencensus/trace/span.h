...
class Span final {
 public:
  ...
  OPENCENSUS_EXPORT static Span BlankSpan();
  ...
  OPENCENSUS_EXPORT static Span StartSpan(absl::string_view name, const Span* parent = nullptr,
                        const StartSpanOptions& options = StartSpanOptions());

  // Constructs a span with a remote parent.
  OPENCENSUS_EXPORT static Span StartSpanWithRemoteParent(
  ...
  OPENCENSUS_EXPORT void AddAttribute(absl::string_view key, AttributeValueRef attribute) const;
  OPENCENSUS_EXPORT void AddAttributes(AttributesRef attributes) const;
  ...
  OPENCENSUS_EXPORT void AddAnnotation(absl::string_view description,
  ...
  OPENCENSUS_EXPORT void AddSentMessageEvent(uint32_t message_id,
                           uint32_t compressed_message_size,
                           uint32_t uncompressed_message_size) const;
  OPENCENSUS_EXPORT void AddReceivedMessageEvent(uint32_t message_id,
                               uint32_t compressed_message_size,
                               uint32_t uncompressed_message_size) const;
  ...
  // attributes per link.
  OPENCENSUS_EXPORT void AddParentLink(const SpanContext& parent_ctx,
                     AttributesRef attributes = {}) const;
  OPENCENSUS_EXPORT void AddChildLink(const SpanContext& child_ctx,
                    AttributesRef attributes = {}) const;

  // Sets the status of the Span. See status_code.h for canonical codes.
  OPENCENSUS_EXPORT void SetStatus(StatusCode canonical_code,
                 absl::string_view message = "") const;

  // Set the span name.
  OPENCENSUS_EXPORT void SetName(absl::string_view name) const;

  // Marks the end of a Span. No further changes can be made to the Span after
  // End is called.
  OPENCENSUS_EXPORT void End() const;

  // Returns the SpanContext associated with this Span.
  OPENCENSUS_EXPORT const SpanContext& context() const;

  // Returns true if the Span is sampled (will be exported).
  // Sampled spans always record events.
  OPENCENSUS_EXPORT bool IsSampled() const;
  ...
  // are sampled.
  OPENCENSUS_EXPORT bool IsRecording() const;

 private:
  Span() = delete;
  OPENCENSUS_EXPORT Span(const SpanContext& context, SpanImpl* impl);
  ...
  OPENCENSUS_EXPORT friend void swap(Span& a, Span& b);
