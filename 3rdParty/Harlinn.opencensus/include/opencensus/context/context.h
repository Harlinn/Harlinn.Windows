...
class Context {
 public:
  ...
  OPENCENSUS_EXPORT static const Context& Current();
  ...
  OPENCENSUS_EXPORT std::function<void()> Wrap(std::function<void()> fn) const;
 ...
 private:
  ...
  OPENCENSUS_EXPORT Context();

  OPENCENSUS_EXPORT static Context* InternalMutableCurrent();
  OPENCENSUS_EXPORT friend void swap(Context& a, Context& b);
