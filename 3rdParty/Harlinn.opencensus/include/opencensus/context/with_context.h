...
class WithContext {
 public:
  OPENCENSUS_EXPORT explicit WithContext(const Context& ctx, bool cond = true);
  OPENCENSUS_EXPORT explicit WithContext(Context&& ctx, bool cond = true);
  OPENCENSUS_EXPORT ~WithContext();
 ...
 private:
  ...
  OPENCENSUS_EXPORT void ConditionalSwap();
