...
class TagMap final {
 public:
  ...
  OPENCENSUS_EXPORT TagMap(std::initializer_list<std::pair<TagKey, absl::string_view>> tags);
  ...
  OPENCENSUS_EXPORT TagMap(std::vector<std::pair<TagKey, std::string>> tags);
  ...
  struct Hash {
    OPENCENSUS_EXPORT std::size_t operator()(const TagMap& tags) const;
  };

  OPENCENSUS_EXPORT bool operator==(const TagMap& other) const;
 ...
 private:
  OPENCENSUS_EXPORT void Initialize();
