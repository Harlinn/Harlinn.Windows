...
class StatsExporter final {
 public:
  ...
  OPENCENSUS_EXPORT static void RemoveView(absl::string_view name);
  ...
  OPENCENSUS_EXPORT static void RegisterPushHandler(std::unique_ptr<Handler> handler);

  // Retrieves current data for all registered views, for implementing pull
  // exporters.
  OPENCENSUS_EXPORT static std::vector<std::pair<ViewDescriptor, ViewData>> GetViewData();
 ...
 private:
  ...
  OPENCENSUS_EXPORT static void ClearHandlersForTesting();
};
