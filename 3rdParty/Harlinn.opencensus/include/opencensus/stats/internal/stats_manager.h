...
class StatsManager final {
 public:
  ...
  class ViewInformation {
   public:
    OPENCENSUS_EXPORT ViewInformation(const ViewDescriptor& descriptor, absl::Mutex* mu);
    ...
    OPENCENSUS_EXPORT bool Matches(const ViewDescriptor& descriptor) const;

    OPENCENSUS_EXPORT int num_consumers() const;
    // Increments the consumer count. Requires holding *mu_.
    OPENCENSUS_EXPORT void AddConsumer();
    // Decrements the consumer count and returns the resulting count. Requires
    // holding *mu_.
    OPENCENSUS_EXPORT int RemoveConsumer();

    // Adds 'data' under 'tags' as of 'now'. Requires holding *mu_;
    OPENCENSUS_EXPORT void MergeMeasureData(const opencensus::tags::TagMap& tags,
                          const MeasureData& data, absl::Time now);

    // Retrieves a copy of the data.
    OPENCENSUS_EXPORT std::unique_ptr<ViewDataImpl> GetData() LOCKS_EXCLUDED(*mu_);

    OPENCENSUS_EXPORT const ViewDescriptor& view_descriptor() const { return descriptor_; }
   ...
   private:
    ...
    OPENCENSUS_EXPORT static DataType DataTypeForDescriptor(const ViewDescriptor& descriptor);

    ViewDataImpl data_ GUARDED_BY(*mu_);
  };

 public:
  OPENCENSUS_EXPORT static StatsManager* Get();

  // Merges all data from 'delta' at the present time.
  OPENCENSUS_EXPORT void MergeDelta(const Delta& delta) LOCKS_EXCLUDED(mu_);

  // Adds a measure--this is necessary for views to be added under that measure.
  template <typename MeasureT>
  OPENCENSUS_EXPORT void AddMeasure(Measure<MeasureT> measure) LOCKS_EXCLUDED(mu_);

  // Returns a handle that can be used to retrieve data for 'descriptor' (which
  // may point to a new or re-used ViewInformation).
  OPENCENSUS_EXPORT ViewInformation* AddConsumer(const ViewDescriptor& descriptor)
      LOCKS_EXCLUDED(mu_);

  // Removes a consumer from the ViewInformation 'handle', and deletes it if
  // that was the last consumer.
  OPENCENSUS_EXPORT void RemoveConsumer(ViewInformation* handle) LOCKS_EXCLUDED(mu_);

 private:
  // MeasureInformation stores all ViewInformation objects for a given measure.
  class MeasureInformation {
   public:
    OPENCENSUS_EXPORT explicit MeasureInformation(absl::Mutex* mu) : mu_(mu) {}
    ...
    OPENCENSUS_EXPORT ViewInformation* AddConsumer(const ViewDescriptor& descriptor);
    OPENCENSUS_EXPORT void RemoveView(const ViewInformation* handle);
