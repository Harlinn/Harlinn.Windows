...
public:
	...
	SPVC_EXPORT explicit CompilerMSL(std::vector<uint32_t> spirv);
	SPVC_EXPORT CompilerMSL(const uint32_t *ir, size_t word_count);
	SPVC_EXPORT explicit CompilerMSL(const ParsedIR &ir);
	SPVC_EXPORT explicit CompilerMSL(ParsedIR &&ir);
	...
	SPVC_EXPORT void add_msl_shader_input(const MSLShaderInput &input);
	...
	SPVC_EXPORT void add_msl_resource_binding(const MSLResourceBinding &resource);
	...
	SPVC_EXPORT void add_dynamic_buffer(uint32_t desc_set, uint32_t binding, uint32_t index);
	...
	SPVC_EXPORT void add_inline_uniform_block(uint32_t desc_set, uint32_t binding);
	...
	SPVC_EXPORT void add_discrete_descriptor_set(uint32_t desc_set);
	...
	SPVC_EXPORT void set_argument_buffer_device_address_space(uint32_t desc_set, bool device_storage);

	// Query after compilation is done. This allows you to check if an input location was used by the shader.
	SPVC_EXPORT bool is_msl_shader_input_used(uint32_t location);
	...
	SPVC_EXPORT uint32_t get_automatic_builtin_input_location(spv::BuiltIn builtin) const;
	...
	SPVC_EXPORT bool is_msl_resource_binding_used(spv::ExecutionModel model, uint32_t set, uint32_t binding) const;
	...
	SPVC_EXPORT uint32_t get_automatic_msl_resource_binding(uint32_t id) const;
	...
	SPVC_EXPORT uint32_t get_automatic_msl_resource_binding_secondary(uint32_t id) const;
	...
	SPVC_EXPORT uint32_t get_automatic_msl_resource_binding_tertiary(uint32_t id) const;
	...
	SPVC_EXPORT uint32_t get_automatic_msl_resource_binding_quaternary(uint32_t id) const;
	...
	SPVC_EXPORT void remap_constexpr_sampler(VariableID id, const MSLConstexprSampler &sampler);
	...
	SPVC_EXPORT void remap_constexpr_sampler_by_binding(uint32_t desc_set, uint32_t binding, const MSLConstexprSampler &sampler);
	...
	SPVC_EXPORT void set_fragment_output_components(uint32_t location, uint32_t components);

	SPVC_EXPORT void set_combined_sampler_suffix(const char *suffix);
	SPVC_EXPORT const char *get_combined_sampler_suffix() const;
...
protected:
	...
	// Use Metal's native frame-buffer fetch API for subpass inputs.
	SPVC_EXPORT void emit_texture_op(const Instruction &i, bool sparse) override;
	SPVC_EXPORT void emit_binary_unord_op(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1, const char *op);
	SPVC_EXPORT void emit_instruction(const Instruction &instr) override;
	SPVC_EXPORT void emit_glsl_op(uint32_t result_type, uint32_t result_id, uint32_t op, const uint32_t *args,
	                  uint32_t count) override;
	SPVC_EXPORT void emit_spv_amd_shader_trinary_minmax_op(uint32_t result_type, uint32_t result_id, uint32_t op,
	                                           const uint32_t *args, uint32_t count) override;
	SPVC_EXPORT void emit_header() override;
	SPVC_EXPORT void emit_function_prototype(SPIRFunction &func, const Bitset &return_flags) override;
	SPVC_EXPORT void emit_sampled_image_op(uint32_t result_type, uint32_t result_id, uint32_t image_id, uint32_t samp_id) override;
	SPVC_EXPORT void emit_subgroup_op(const Instruction &i) override;
	SPVC_EXPORT std::string to_texture_op(const Instruction &i, bool sparse, bool *forward,
	                          SmallVector<uint32_t> &inherited_expressions) override;
	SPVC_EXPORT void emit_fixup() override;
	SPVC_EXPORT std::string to_struct_member(const SPIRType &type, uint32_t member_type_id, uint32_t index,
	                             const std::string &qualifier = "");
	SPVC_EXPORT void emit_struct_member(const SPIRType &type, uint32_t member_type_id, uint32_t index,
	                        const std::string &qualifier = "", uint32_t base_offset = 0) override;
	SPVC_EXPORT void emit_struct_padding_target(const SPIRType &type) override;
	SPVC_EXPORT std::string type_to_glsl(const SPIRType &type, uint32_t id = 0) override;
	SPVC_EXPORT void emit_block_hints(const SPIRBlock &block) override;

	// Allow Metal to use the array<T> template to make arrays a value type
	SPVC_EXPORT std::string type_to_array_glsl(const SPIRType &type) override;

	// Threadgroup arrays can't have a wrapper type
	SPVC_EXPORT std::string variable_decl(const SPIRVariable &variable) override;

	SPVC_EXPORT bool variable_decl_is_remapped_storage(const SPIRVariable &variable, spv::StorageClass storage) const override;

	// GCC workaround of lambdas calling protected functions (for older GCC versions)
	SPVC_EXPORT std::string variable_decl(const SPIRType &type, const std::string &name, uint32_t id = 0) override;

	SPVC_EXPORT std::string image_type_glsl(const SPIRType &type, uint32_t id = 0) override;
	SPVC_EXPORT std::string sampler_type(const SPIRType &type, uint32_t id);
	SPVC_EXPORT std::string builtin_to_glsl(spv::BuiltIn builtin, spv::StorageClass storage) override;
	SPVC_EXPORT std::string to_func_call_arg(const SPIRFunction::Parameter &arg, uint32_t id) override;
	SPVC_EXPORT std::string to_name(uint32_t id, bool allow_alias = true) const override;
	SPVC_EXPORT std::string to_function_name(const TextureFunctionNameArguments &args) override;
	SPVC_EXPORT std::string to_function_args(const TextureFunctionArguments &args, bool *p_forward) override;
	SPVC_EXPORT std::string to_initializer_expression(const SPIRVariable &var) override;
	SPVC_EXPORT std::string to_zero_initialized_expression(uint32_t type_id) override;

	SPVC_EXPORT std::string unpack_expression_type(std::string expr_str, const SPIRType &type, uint32_t physical_type_id,
	...
	SPVC_EXPORT bool builtin_translates_to_nonarray(spv::BuiltIn builtin) const override;

	SPVC_EXPORT std::string bitcast_glsl_op(const SPIRType &result_type, const SPIRType &argument_type) override;
	SPVC_EXPORT bool emit_complex_bitcast(uint32_t result_id, uint32_t id, uint32_t op0) override;
	SPVC_EXPORT bool skip_argument(uint32_t id) const override;
	SPVC_EXPORT std::string to_member_reference(uint32_t base, const SPIRType &type, uint32_t index, bool ptr_chain) override;
	SPVC_EXPORT std::string to_qualifiers_glsl(uint32_t id) override;
	SPVC_EXPORT void replace_illegal_names() override;
	SPVC_EXPORT void declare_undefined_values() override;
	SPVC_EXPORT void declare_constant_arrays();

	SPVC_EXPORT void replace_illegal_entry_point_names();
	SPVC_EXPORT void sync_entry_point_aliases_and_names();

	SPVC_EXPORT static const std::unordered_set<std::string> &get_reserved_keyword_set();
	SPVC_EXPORT static const std::unordered_set<std::string> &get_illegal_func_names();

	// Constant arrays of non-primitive types (i.e. matrices) won't link properly into Metal libraries
	SPVC_EXPORT void declare_complex_constant_arrays();

	SPVC_EXPORT bool is_patch_block(const SPIRType &type);
	SPVC_EXPORT bool is_non_native_row_major_matrix(uint32_t id) override;
	SPVC_EXPORT bool member_is_non_native_row_major_matrix(const SPIRType &type, uint32_t index) override;
	SPVC_EXPORT std::string convert_row_major_matrix(std::string exp_str, const SPIRType &exp_type, uint32_t physical_type_id,
	                                     bool is_packed) override;

	SPVC_EXPORT void preprocess_op_codes();
	SPVC_EXPORT void localize_global_variables();
	SPVC_EXPORT void extract_global_variables_from_functions();
	SPVC_EXPORT void mark_packable_structs();
	SPVC_EXPORT void mark_as_packable(SPIRType &type);

	std::unordered_map<uint32_t, std::set<uint32_t>> function_global_vars;
	SPVC_EXPORT void extract_global_variables_from_function(uint32_t func_id, std::set<uint32_t> &added_arg_ids,
	                                            std::unordered_set<uint32_t> &global_var_ids,
	                                            std::unordered_set<uint32_t> &processed_func_ids);
	SPVC_EXPORT uint32_t add_interface_block(spv::StorageClass storage, bool patch = false);
	SPVC_EXPORT uint32_t add_interface_block_pointer(uint32_t ib_var_id, spv::StorageClass storage);
	...
	SPVC_EXPORT std::string to_tesc_invocation_id();
	SPVC_EXPORT void emit_local_masked_variable(const SPIRVariable &masked_var, bool strip_array);
	SPVC_EXPORT void add_variable_to_interface_block(spv::StorageClass storage, const std::string &ib_var_ref, SPIRType &ib_type,
	                                     SPIRVariable &var, InterfaceBlockMeta &meta);
	SPVC_EXPORT void add_composite_variable_to_interface_block(spv::StorageClass storage, const std::string &ib_var_ref,
	                                               SPIRType &ib_type, SPIRVariable &var, InterfaceBlockMeta &meta);
	SPVC_EXPORT void add_plain_variable_to_interface_block(spv::StorageClass storage, const std::string &ib_var_ref,
	                                           SPIRType &ib_type, SPIRVariable &var, InterfaceBlockMeta &meta);
	SPVC_EXPORT bool add_component_variable_to_interface_block(spv::StorageClass storage, const std::string &ib_var_ref,
	                                               SPIRVariable &var, const SPIRType &type,
	                                               InterfaceBlockMeta &meta);
	SPVC_EXPORT void add_plain_member_variable_to_interface_block(spv::StorageClass storage, const std::string &ib_var_ref,
	                                                  SPIRType &ib_type, SPIRVariable &var, uint32_t index,
	                                                  InterfaceBlockMeta &meta);
	SPVC_EXPORT void add_composite_member_variable_to_interface_block(spv::StorageClass storage, const std::string &ib_var_ref,
	                                                      SPIRType &ib_type, SPIRVariable &var, uint32_t index,
	                                                      InterfaceBlockMeta &meta);
	SPVC_EXPORT void add_tess_level_input_to_interface_block(const std::string &ib_var_ref, SPIRType &ib_type, SPIRVariable &var);

	SPVC_EXPORT void fix_up_interface_member_indices(spv::StorageClass storage, uint32_t ib_type_id);

	SPVC_EXPORT void mark_location_as_used_by_shader(uint32_t location, const SPIRType &type,
	                                     spv::StorageClass storage, bool fallback = false);
	SPVC_EXPORT uint32_t ensure_correct_builtin_type(uint32_t type_id, spv::BuiltIn builtin);
	SPVC_EXPORT uint32_t ensure_correct_input_type(uint32_t type_id, uint32_t location,
	                                   uint32_t num_components, bool strip_array);

	SPVC_EXPORT void emit_custom_templates();
	SPVC_EXPORT void emit_custom_functions();
	SPVC_EXPORT void emit_resources();
	SPVC_EXPORT void emit_specialization_constants_and_structs();
	SPVC_EXPORT void emit_interface_block(uint32_t ib_var_id);
	SPVC_EXPORT bool maybe_emit_array_assignment(uint32_t id_lhs, uint32_t id_rhs);
	SPVC_EXPORT uint32_t get_resource_array_size(uint32_t id) const;

	SPVC_EXPORT void fix_up_shader_inputs_outputs();

	SPVC_EXPORT std::string func_type_decl(SPIRType &type);
	SPVC_EXPORT std::string entry_point_args_classic(bool append_comma);
	SPVC_EXPORT std::string entry_point_args_argument_buffer(bool append_comma);
	SPVC_EXPORT std::string entry_point_arg_stage_in();
	SPVC_EXPORT void entry_point_args_builtin(std::string &args);
	SPVC_EXPORT void entry_point_args_discrete_descriptors(std::string &args);
	SPVC_EXPORT std::string to_qualified_member_name(const SPIRType &type, uint32_t index);
	SPVC_EXPORT std::string ensure_valid_name(std::string name, std::string pfx);
	SPVC_EXPORT std::string to_sampler_expression(uint32_t id);
	SPVC_EXPORT std::string to_swizzle_expression(uint32_t id);
	SPVC_EXPORT std::string to_buffer_size_expression(uint32_t id);
	SPVC_EXPORT bool is_sample_rate() const;
	SPVC_EXPORT bool is_direct_input_builtin(spv::BuiltIn builtin);
	SPVC_EXPORT std::string builtin_qualifier(spv::BuiltIn builtin);
	SPVC_EXPORT std::string builtin_type_decl(spv::BuiltIn builtin, uint32_t id = 0);
	SPVC_EXPORT std::string built_in_func_arg(spv::BuiltIn builtin, bool prefix_comma);
	SPVC_EXPORT std::string member_attribute_qualifier(const SPIRType &type, uint32_t index);
	SPVC_EXPORT std::string argument_decl(const SPIRFunction::Parameter &arg);
	SPVC_EXPORT std::string round_fp_tex_coords(std::string tex_coords, bool coord_is_fp);
	SPVC_EXPORT uint32_t get_metal_resource_index(SPIRVariable &var, SPIRType::BaseType basetype, uint32_t plane = 0);
	SPVC_EXPORT uint32_t get_member_location(uint32_t type_id, uint32_t index, uint32_t *comp = nullptr) const;
	SPVC_EXPORT uint32_t get_or_allocate_builtin_input_member_location(spv::BuiltIn builtin,
	                                                       uint32_t type_id, uint32_t index, uint32_t *comp = nullptr);

	SPVC_EXPORT uint32_t get_physical_tess_level_array_size(spv::BuiltIn builtin) const;
	...
	SPVC_EXPORT uint32_t get_declared_type_size_msl(const SPIRType &type, bool packed, bool row_major) const;
	SPVC_EXPORT uint32_t get_declared_type_array_stride_msl(const SPIRType &type, bool packed, bool row_major) const;
	SPVC_EXPORT uint32_t get_declared_type_matrix_stride_msl(const SPIRType &type, bool packed, bool row_major) const;
	SPVC_EXPORT uint32_t get_declared_type_alignment_msl(const SPIRType &type, bool packed, bool row_major) const;

	SPVC_EXPORT uint32_t get_declared_struct_member_size_msl(const SPIRType &struct_type, uint32_t index) const;
	SPVC_EXPORT uint32_t get_declared_struct_member_array_stride_msl(const SPIRType &struct_type, uint32_t index) const;
	SPVC_EXPORT uint32_t get_declared_struct_member_matrix_stride_msl(const SPIRType &struct_type, uint32_t index) const;
	SPVC_EXPORT uint32_t get_declared_struct_member_alignment_msl(const SPIRType &struct_type, uint32_t index) const;

	SPVC_EXPORT uint32_t get_declared_input_size_msl(const SPIRType &struct_type, uint32_t index) const;
	SPVC_EXPORT uint32_t get_declared_input_array_stride_msl(const SPIRType &struct_type, uint32_t index) const;
	SPVC_EXPORT uint32_t get_declared_input_matrix_stride_msl(const SPIRType &struct_type, uint32_t index) const;
	SPVC_EXPORT uint32_t get_declared_input_alignment_msl(const SPIRType &struct_type, uint32_t index) const;

	SPVC_EXPORT const SPIRType &get_physical_member_type(const SPIRType &struct_type, uint32_t index) const;
	SPVC_EXPORT SPIRType get_presumed_input_type(const SPIRType &struct_type, uint32_t index) const;

	SPVC_EXPORT uint32_t get_declared_struct_size_msl(const SPIRType &struct_type, bool ignore_alignment = false,
	                                      bool ignore_padding = false) const;

	SPVC_EXPORT std::string to_component_argument(uint32_t id);
	SPVC_EXPORT void align_struct(SPIRType &ib_type, std::unordered_set<uint32_t> &aligned_structs);
	SPVC_EXPORT void mark_scalar_layout_structs(const SPIRType &ib_type);
	SPVC_EXPORT void mark_struct_members_packed(const SPIRType &type);
	SPVC_EXPORT void ensure_member_packing_rules_msl(SPIRType &ib_type, uint32_t index);
	SPVC_EXPORT bool validate_member_packing_rules_msl(const SPIRType &type, uint32_t index) const;
	SPVC_EXPORT std::string get_argument_address_space(const SPIRVariable &argument);
	SPVC_EXPORT std::string get_type_address_space(const SPIRType &type, uint32_t id, bool argument = false);
	SPVC_EXPORT const char *to_restrict(uint32_t id, bool space = true);
	SPVC_EXPORT SPIRType &get_stage_in_struct_type();
	SPVC_EXPORT SPIRType &get_stage_out_struct_type();
	SPVC_EXPORT SPIRType &get_patch_stage_in_struct_type();
	SPVC_EXPORT SPIRType &get_patch_stage_out_struct_type();
	SPVC_EXPORT std::string get_tess_factor_struct_name();
	SPVC_EXPORT SPIRType &get_uint_type();
	SPVC_EXPORT uint32_t get_uint_type_id();
	SPVC_EXPORT void emit_atomic_func_op(uint32_t result_type, uint32_t result_id, const char *op, uint32_t mem_order_1,
	                         uint32_t mem_order_2, bool has_mem_order_2, uint32_t op0, uint32_t op1 = 0,
	                         bool op1_is_pointer = false, bool op1_is_literal = false, uint32_t op2 = 0);
	SPVC_EXPORT const char *get_memory_order(uint32_t spv_mem_sem);
	SPVC_EXPORT void add_pragma_line(const std::string &line);
	SPVC_EXPORT void add_typedef_line(const std::string &line);
	SPVC_EXPORT void emit_barrier(uint32_t id_exe_scope, uint32_t id_mem_scope, uint32_t id_mem_sem);
	SPVC_EXPORT void emit_array_copy(const std::string &lhs, uint32_t lhs_id, uint32_t rhs_id,
	                     spv::StorageClass lhs_storage, spv::StorageClass rhs_storage) override;
	SPVC_EXPORT void build_implicit_builtins();
	SPVC_EXPORT uint32_t build_constant_uint_array_pointer();
	SPVC_EXPORT void emit_entry_point_declarations() override;
	...
	SPVC_EXPORT void cast_to_builtin_store(uint32_t target_id, std::string &expr, const SPIRType &expr_type) override;
	SPVC_EXPORT void cast_from_builtin_load(uint32_t source_id, std::string &expr, const SPIRType &expr_type) override;
	SPVC_EXPORT void emit_store_statement(uint32_t lhs_expression, uint32_t rhs_expression) override;

	SPVC_EXPORT void analyze_sampled_image_usage();

	SPVC_EXPORT bool access_chain_needs_stage_io_builtin_translation(uint32_t base) override;
	SPVC_EXPORT void prepare_access_chain_for_scalar_access(std::string &expr, const SPIRType &type, spv::StorageClass storage,
	                                            bool &is_packed) override;
	SPVC_EXPORT void fix_up_interpolant_access_chain(const uint32_t *ops, uint32_t length);
	SPVC_EXPORT bool emit_tessellation_access_chain(const uint32_t *ops, uint32_t length);
	SPVC_EXPORT bool emit_tessellation_io_load(uint32_t result_type, uint32_t id, uint32_t ptr);
	SPVC_EXPORT bool is_out_of_bounds_tessellation_level(uint32_t id_lhs);

	SPVC_EXPORT void ensure_builtin(spv::StorageClass storage, spv::BuiltIn builtin);

	SPVC_EXPORT void mark_implicit_builtin(spv::StorageClass storage, spv::BuiltIn builtin, uint32_t id);

	SPVC_EXPORT std::string convert_to_f32(const std::string &expr, uint32_t components);
	...
	SPVC_EXPORT const MSLConstexprSampler *find_constexpr_sampler(uint32_t id) const;
	...
	SPVC_EXPORT void analyze_argument_buffers();
	SPVC_EXPORT bool descriptor_set_is_argument_buffer(uint32_t desc_set) const;
	SPVC_EXPORT MSLResourceBinding &get_argument_buffer_resource(uint32_t desc_set, uint32_t arg_idx);
	SPVC_EXPORT void add_argument_buffer_padding_buffer_type(SPIRType &struct_type, uint32_t &mbr_idx, uint32_t &arg_buff_index, MSLResourceBinding &rez_bind);
	SPVC_EXPORT void add_argument_buffer_padding_image_type(SPIRType &struct_type, uint32_t &mbr_idx, uint32_t &arg_buff_index, MSLResourceBinding &rez_bind);
	SPVC_EXPORT void add_argument_buffer_padding_sampler_type(SPIRType &struct_type, uint32_t &mbr_idx, uint32_t &arg_buff_index, MSLResourceBinding &rez_bind);
	SPVC_EXPORT void add_argument_buffer_padding_type(uint32_t mbr_type_id, SPIRType &struct_type, uint32_t &mbr_idx, uint32_t &arg_buff_index, uint32_t count);

	SPVC_EXPORT uint32_t get_target_components_for_fragment_location(uint32_t location) const;
	SPVC_EXPORT uint32_t build_extended_vector_type(uint32_t type_id, uint32_t components,
	                                    SPIRType::BaseType basetype = SPIRType::Unknown);
	SPVC_EXPORT uint32_t build_msl_interpolant_type(uint32_t type_id, bool is_noperspective);

	bool suppress_missing_prototypes = false;

	SPVC_EXPORT void add_spv_func_and_recompile(SPVFuncImpl spv_func);

	SPVC_EXPORT void activate_argument_buffer_resources();

	SPVC_EXPORT bool type_is_msl_framebuffer_fetch(const SPIRType &type) const;
	SPVC_EXPORT bool type_is_pointer(const SPIRType &type) const;
	SPVC_EXPORT bool type_is_pointer_to_pointer(const SPIRType &type) const;
	SPVC_EXPORT bool is_supported_argument_buffer_type(const SPIRType &type) const;

	SPVC_EXPORT bool variable_storage_requires_stage_io(spv::StorageClass storage) const;

	SPVC_EXPORT bool has_additional_fixed_sample_mask() const { return msl_options.additional_fixed_sample_mask != 0xffffffff; }
	SPVC_EXPORT std::string additional_fixed_sample_mask_str() const;
	...
	struct OpCodePreprocessor : OpcodeHandler
	{
		...
		SPVC_EXPORT bool handle(spv::Op opcode, const uint32_t *args, uint32_t length) override;
		SPVC_EXPORT CompilerMSL::SPVFuncImpl get_spv_func_impl(spv::Op opcode, const uint32_t *args);
		SPVC_EXPORT void check_resource_write(uint32_t var_id);
	...
	struct SampledImageScanner : OpcodeHandler
	{
		...
		SPVC_EXPORT bool handle(spv::Op opcode, const uint32_t *args, uint32_t) override;
	...
	struct MemberSorter
	{
		...
		SPVC_EXPORT void sort();
		SPVC_EXPORT bool operator()(uint32_t mbr_idx1, uint32_t mbr_idx2);
		SPVC_EXPORT MemberSorter(SPIRType &t, Meta &m, SortAspect sa);
