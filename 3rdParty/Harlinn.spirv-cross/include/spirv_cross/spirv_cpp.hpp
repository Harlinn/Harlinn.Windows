...
private:
	...
	SPVC_EXPORT void emit_c_linkage();
	...
	SPVC_EXPORT void emit_shared(const SPIRVariable &var);
	SPVC_EXPORT void emit_block_struct(SPIRType &type);
	SPVC_EXPORT std::string variable_decl(const SPIRType &type, const std::string &name, uint32_t id) override;
