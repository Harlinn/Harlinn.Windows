...
public:
	SPVC_EXPORT ParsedIR();

	// Due to custom allocations from object pools, we cannot use a default copy constructor.
	SPVC_EXPORT ParsedIR(const ParsedIR &other);
	SPVC_EXPORT ParsedIR &operator=(const ParsedIR &other);
	...
	SPVC_EXPORT ParsedIR(ParsedIR &&other) SPIRV_CROSS_NOEXCEPT;
	SPVC_EXPORT ParsedIR &operator=(ParsedIR &&other) SPIRV_CROSS_NOEXCEPT;

	// Resizes ids, meta and block_meta.
	SPVC_EXPORT void set_id_bounds(uint32_t bounds);
	...
	SPVC_EXPORT void mark_used_as_array_length(ID id);
	SPVC_EXPORT uint32_t increase_bound_by(uint32_t count);
	SPVC_EXPORT Bitset get_buffer_block_flags(const SPIRVariable &var) const;
	SPVC_EXPORT Bitset get_buffer_block_type_flags(const SPIRType &type) const;
	...
	{
	public:
		SPVC_EXPORT explicit LoopLock(uint32_t *counter);
		LoopLock(const LoopLock &) = delete;
		void operator=(const LoopLock &) = delete;
		SPVC_EXPORT LoopLock(LoopLock &&other) SPIRV_CROSS_NOEXCEPT;
		SPVC_EXPORT LoopLock &operator=(LoopLock &&other) SPIRV_CROSS_NOEXCEPT;
		SPVC_EXPORT ~LoopLock();
	...
	SPVC_EXPORT LoopLock create_loop_hard_lock() const;
	SPVC_EXPORT LoopLock create_loop_soft_lock() const;
	...
	SPVC_EXPORT void reset_all_of_type(Types type);

	SPVC_EXPORT Meta *find_meta(ID id);
	SPVC_EXPORT const Meta *find_meta(ID id) const;
	...
	SPVC_EXPORT void make_constant_null(uint32_t id, uint32_t type, bool add_to_typed_id_set);

	SPVC_EXPORT void fixup_reserved_names();

	SPVC_EXPORT static void sanitize_underscores(std::string &str);
	SPVC_EXPORT static void sanitize_identifier(std::string &str, bool member, bool allow_reserved_prefixes);
	SPVC_EXPORT static bool is_globally_reserved_identifier(std::string &str, bool allow_reserved_prefixes);

	SPVC_EXPORT uint32_t get_spirv_version() const;
