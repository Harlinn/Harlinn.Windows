...
public:
	...
	// layout specified.
	SPVC_EXPORT void set_root_constant_layouts(std::vector<RootConstants> layout);
	...
	SPVC_EXPORT void add_vertex_attribute_remap(const HLSLVertexAttributeRemap &vertex_attributes);
	...
	SPVC_EXPORT VariableID remap_num_workgroups_builtin();

	// Controls how resource bindings are declared in the output HLSL.
	SPVC_EXPORT void set_resource_binding_flags(HLSLBindingFlags flags);
	...
	SPVC_EXPORT void add_hlsl_resource_binding(const HLSLResourceBinding &resource);
	SPVC_EXPORT bool is_hlsl_resource_binding_used(spv::ExecutionModel model, uint32_t set, uint32_t binding) const;

	// Controls which storage buffer bindings will be forced to be declared as UAVs.
	SPVC_EXPORT void set_hlsl_force_storage_buffer_as_uav(uint32_t desc_set, uint32_t binding);

private:
	SPVC_EXPORT std::string type_to_glsl(const SPIRType &type, uint32_t id = 0) override;
	SPVC_EXPORT std::string image_type_hlsl(const SPIRType &type, uint32_t id);
	SPVC_EXPORT std::string image_type_hlsl_modern(const SPIRType &type, uint32_t id);
	SPVC_EXPORT std::string image_type_hlsl_legacy(const SPIRType &type, uint32_t id);
	SPVC_EXPORT void emit_function_prototype(SPIRFunction &func, const Bitset &return_flags) override;
	SPVC_EXPORT void emit_hlsl_entry_point();
	SPVC_EXPORT void emit_header() override;
	SPVC_EXPORT void emit_resources();
	SPVC_EXPORT void declare_undefined_values() override;
	SPVC_EXPORT void emit_interface_block_globally(const SPIRVariable &type);
	SPVC_EXPORT void emit_interface_block_in_struct(const SPIRVariable &var, std::unordered_set<uint32_t> &active_locations);
	SPVC_EXPORT void emit_interface_block_member_in_struct(const SPIRVariable &var, uint32_t member_index,
	                                           uint32_t location,
	                                           std::unordered_set<uint32_t> &active_locations);
	SPVC_EXPORT void emit_builtin_inputs_in_struct();
	SPVC_EXPORT void emit_builtin_outputs_in_struct();
	SPVC_EXPORT void emit_texture_op(const Instruction &i, bool sparse) override;
	SPVC_EXPORT void emit_instruction(const Instruction &instruction) override;
	SPVC_EXPORT void emit_glsl_op(uint32_t result_type, uint32_t result_id, uint32_t op, const uint32_t *args,
	                  uint32_t count) override;
	SPVC_EXPORT void emit_buffer_block(const SPIRVariable &type) override;
	SPVC_EXPORT void emit_push_constant_block(const SPIRVariable &var) override;
	SPVC_EXPORT void emit_uniform(const SPIRVariable &var) override;
	SPVC_EXPORT void emit_modern_uniform(const SPIRVariable &var);
	SPVC_EXPORT void emit_legacy_uniform(const SPIRVariable &var);
	SPVC_EXPORT void emit_specialization_constants_and_structs();
	SPVC_EXPORT void emit_composite_constants();
	...
	SPVC_EXPORT std::string layout_for_member(const SPIRType &type, uint32_t index) override;
	SPVC_EXPORT std::string to_interpolation_qualifiers(const Bitset &flags) override;
	SPVC_EXPORT std::string bitcast_glsl_op(const SPIRType &result_type, const SPIRType &argument_type) override;
	SPVC_EXPORT bool emit_complex_bitcast(uint32_t result_type, uint32_t id, uint32_t op0) override;
	...
	SPVC_EXPORT std::string to_resource_binding(const SPIRVariable &var);
	SPVC_EXPORT std::string to_resource_binding_sampler(const SPIRVariable &var);
	SPVC_EXPORT std::string to_resource_register(HLSLBindingFlagBits flag, char space, uint32_t binding, uint32_t set);
	...
	SPVC_EXPORT void emit_access_chain(const Instruction &instruction);
	SPVC_EXPORT void emit_load(const Instruction &instruction);
	SPVC_EXPORT void read_access_chain(std::string *expr, const std::string &lhs, const SPIRAccessChain &chain);
	SPVC_EXPORT void read_access_chain_struct(const std::string &lhs, const SPIRAccessChain &chain);
	SPVC_EXPORT void read_access_chain_array(const std::string &lhs, const SPIRAccessChain &chain);
	SPVC_EXPORT void write_access_chain(const SPIRAccessChain &chain, uint32_t value, const SmallVector<uint32_t> &composite_chain);
	SPVC_EXPORT void write_access_chain_struct(const SPIRAccessChain &chain, uint32_t value,
	                               const SmallVector<uint32_t> &composite_chain);
	SPVC_EXPORT void write_access_chain_array(const SPIRAccessChain &chain, uint32_t value,
	                              const SmallVector<uint32_t> &composite_chain);
	SPVC_EXPORT std::string write_access_chain_value(uint32_t value, const SmallVector<uint32_t> &composite_chain, bool enclose);
	SPVC_EXPORT void emit_store(const Instruction &instruction);
	SPVC_EXPORT void emit_atomic(const uint32_t *ops, uint32_t length, spv::Op op);
	...
	SPVC_EXPORT void emit_struct_member(const SPIRType &type, uint32_t member_type_id, uint32_t index, const std::string &qualifier,
	                        uint32_t base_offset = 0) override;

	SPVC_EXPORT const char *to_storage_qualifiers_glsl(const SPIRVariable &var) override;
	SPVC_EXPORT void replace_illegal_names() override;

	SPVC_EXPORT bool is_hlsl_force_storage_buffer_as_uav(ID id) const;
	...
	SPVC_EXPORT void require_texture_query_variant(uint32_t var_id);
	SPVC_EXPORT void emit_texture_size_variants(uint64_t variant_mask, const char *vecsize_qualifier, bool uav,
	...
	SPVC_EXPORT BitcastType get_bitcast_type(uint32_t result_type, uint32_t op0);

	SPVC_EXPORT void emit_builtin_variables();
	bool require_output = false;
	bool require_input = false;
	SmallVector<HLSLVertexAttributeRemap> remap_vertex_attributes;

	SPVC_EXPORT uint32_t type_to_consumed_locations(const SPIRType &type) const;

	SPVC_EXPORT std::string to_semantic(uint32_t location, spv::ExecutionModel em, spv::StorageClass sc);
	...
	SPVC_EXPORT void validate_shader_model();

	SPVC_EXPORT std::string get_unique_identifier();
	...
	SPVC_EXPORT void remap_hlsl_resource_binding(HLSLBindingFlagBits type, uint32_t &desc_set, uint32_t &binding);
