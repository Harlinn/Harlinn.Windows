...
public:
	...
	SPVC_EXPORT explicit Compiler(std::vector<uint32_t> ir);
	SPVC_EXPORT Compiler(const uint32_t *ir, size_t word_count);
	...
	SPVC_EXPORT explicit Compiler(const ParsedIR &ir);
	SPVC_EXPORT explicit Compiler(ParsedIR &&ir);
	...
	SPVC_EXPORT virtual std::string compile();

	// Gets the identifier (OpName) of an ID. If not defined, an empty string will be returned.
	SPVC_EXPORT const std::string &get_name(ID id) const;

	// Applies a decoration to an ID. Effectively injects OpDecorate.
	SPVC_EXPORT void set_decoration(ID id, spv::Decoration decoration, uint32_t argument = 0);
	SPVC_EXPORT void set_decoration_string(ID id, spv::Decoration decoration, const std::string &argument);
	...
	SPVC_EXPORT void set_name(ID id, const std::string &name);

	// Gets a bitmask for the decorations which are applied to ID.
	// I.e. (1ull << spv::DecorationFoo) | (1ull << spv::DecorationBar)
	SPVC_EXPORT const Bitset &get_decoration_bitset(ID id) const;

	// Returns whether the decoration has been applied to the ID.
	SPVC_EXPORT bool has_decoration(ID id, spv::Decoration decoration) const;
	...
	// 0 will be returned.
	SPVC_EXPORT uint32_t get_decoration(ID id, spv::Decoration decoration) const;
	SPVC_EXPORT const std::string &get_decoration_string(ID id, spv::Decoration decoration) const;

	// Removes the decoration for an ID.
	SPVC_EXPORT void unset_decoration(ID id, spv::Decoration decoration);
	...
	SPVC_EXPORT const SPIRType &get_type(TypeID id) const;

	// Gets the SPIR-V type of a variable.
	SPVC_EXPORT const SPIRType &get_type_from_variable(VariableID id) const;

	// Gets the underlying storage class for an OpVariable.
	SPVC_EXPORT spv::StorageClass get_storage_class(VariableID id) const;

	// If get_name() is an empty string, get the fallback name which will be used
	// instead in the disassembled source.
	SPVC_EXPORT virtual const std::string get_fallback_name(ID id) const;
	...
	SPVC_EXPORT virtual const std::string get_block_fallback_name(VariableID id) const;

	// Given an OpTypeStruct in ID, obtain the identifier for member number "index".
	// This may be an empty string.
	SPVC_EXPORT const std::string &get_member_name(TypeID id, uint32_t index) const;

	// Given an OpTypeStruct in ID, obtain the OpMemberDecoration for member number "index".
	SPVC_EXPORT uint32_t get_member_decoration(TypeID id, uint32_t index, spv::Decoration decoration) const;
	SPVC_EXPORT const std::string &get_member_decoration_string(TypeID id, uint32_t index, spv::Decoration decoration) const;

	// Sets the member identifier for OpTypeStruct ID, member number "index".
	SPVC_EXPORT void set_member_name(TypeID id, uint32_t index, const std::string &name);
	...
	SPVC_EXPORT const std::string &get_member_qualified_name(TypeID type_id, uint32_t index) const;

	// Gets the decoration mask for a member of a struct, similar to get_decoration_mask.
	SPVC_EXPORT const Bitset &get_member_decoration_bitset(TypeID id, uint32_t index) const;

	// Returns whether the decoration has been applied to a member of a struct.
	SPVC_EXPORT bool has_member_decoration(TypeID id, uint32_t index, spv::Decoration decoration) const;

	// Similar to set_decoration, but for struct members.
	SPVC_EXPORT void set_member_decoration(TypeID id, uint32_t index, spv::Decoration decoration, uint32_t argument = 0);
	SPVC_EXPORT void set_member_decoration_string(TypeID id, uint32_t index, spv::Decoration decoration,
	                                  const std::string &argument);

	// Unsets a member decoration, similar to unset_decoration.
	SPVC_EXPORT void unset_member_decoration(TypeID id, uint32_t index, spv::Decoration decoration);
	...
	SPVC_EXPORT SmallVector<BufferRange> get_active_buffer_ranges(VariableID id) const;

	// Returns the effective size of a buffer block.
	SPVC_EXPORT size_t get_declared_struct_size(const SPIRType &struct_type) const;
	...
	SPVC_EXPORT size_t get_declared_struct_size_runtime_array(const SPIRType &struct_type, size_t array_size) const;

	// Returns the effective size of a buffer block struct member.
	SPVC_EXPORT size_t get_declared_struct_member_size(const SPIRType &struct_type, uint32_t index) const;
	...
	SPVC_EXPORT std::unordered_set<VariableID> get_active_interface_variables() const;
	...
	SPVC_EXPORT void set_enabled_interface_variables(std::unordered_set<VariableID> active_variables);

	// Query shader resources, use ids with reflection interface to modify or query binding points, etc.
	SPVC_EXPORT ShaderResources get_shader_resources() const;
	...
	// accessed.
	SPVC_EXPORT ShaderResources get_shader_resources(const std::unordered_set<VariableID> &active_variables) const;
	...
	SPVC_EXPORT void set_remapped_variable_state(VariableID id, bool remap_enable);
	SPVC_EXPORT bool get_remapped_variable_state(VariableID id) const;
	...
	SPVC_EXPORT void set_subpass_input_remapped_components(VariableID id, uint32_t components);
	SPVC_EXPORT uint32_t get_subpass_input_remapped_components(VariableID id) const;
	...
	SPVC_EXPORT SmallVector<EntryPoint> get_entry_points_and_stages() const;
	SPVC_EXPORT void set_entry_point(const std::string &entry, spv::ExecutionModel execution_model);
	...
	SPVC_EXPORT void rename_entry_point(const std::string &old_name, const std::string &new_name,
	                        spv::ExecutionModel execution_model);
	SPVC_EXPORT const SPIREntryPoint &get_entry_point(const std::string &name, spv::ExecutionModel execution_model) const;
	SPVC_EXPORT SPIREntryPoint &get_entry_point(const std::string &name, spv::ExecutionModel execution_model);
	SPVC_EXPORT const std::string &get_cleansed_entry_point_name(const std::string &name,
	                                                 spv::ExecutionModel execution_model) const;

	// Traverses all reachable opcodes and sets active_builtins to a bitmask of all builtin variables which are accessed in the shader.
	SPVC_EXPORT void update_active_builtins();
	SPVC_EXPORT bool has_active_builtin(spv::BuiltIn builtin, spv::StorageClass storage) const;

	// Query and modify OpExecutionMode.
	SPVC_EXPORT const Bitset &get_execution_mode_bitset() const;

	SPVC_EXPORT void unset_execution_mode(spv::ExecutionMode mode);
	SPVC_EXPORT void set_execution_mode(spv::ExecutionMode mode, uint32_t arg0 = 0, uint32_t arg1 = 0, uint32_t arg2 = 0);
	...
	SPVC_EXPORT uint32_t get_execution_mode_argument(spv::ExecutionMode mode, uint32_t index = 0) const;
	SPVC_EXPORT spv::ExecutionModel get_execution_model() const;

	SPVC_EXPORT bool is_tessellation_shader() const;
	...
	// for most use cases.
	SPVC_EXPORT uint32_t get_work_group_size_specialization_constants(SpecializationConstant &x, SpecializationConstant &y,
	...
	SPVC_EXPORT VariableID build_dummy_sampler_for_combined_images();
	...
	SPVC_EXPORT void build_combined_image_samplers();
	...
	SPVC_EXPORT SmallVector<SpecializationConstant> get_specialization_constants() const;
	SPVC_EXPORT SPIRConstant &get_constant(ConstantID id);
	SPVC_EXPORT const SPIRConstant &get_constant(ConstantID id) const;
	...
	SPVC_EXPORT uint32_t type_struct_member_offset(const SPIRType &type, uint32_t index) const;
	SPVC_EXPORT uint32_t type_struct_member_array_stride(const SPIRType &type, uint32_t index) const;
	SPVC_EXPORT uint32_t type_struct_member_matrix_stride(const SPIRType &type, uint32_t index) const;
	...
	SPVC_EXPORT bool get_binary_offset_for_decoration(VariableID id, spv::Decoration decoration, uint32_t &word_offset) const;
	...
	SPVC_EXPORT bool buffer_is_hlsl_counter_buffer(VariableID id) const;
	...
	// only return true if OpSource was reported HLSL.
	// To rely on this functionality, ensure that the SPIR-V module is not stripped.
	SPVC_EXPORT bool buffer_get_hlsl_counter_buffer(VariableID id, uint32_t &counter_id) const;

	// Gets the list of all SPIR-V Capabilities which were declared in the SPIR-V module.
	SPVC_EXPORT const SmallVector<spv::Capability> &get_declared_capabilities() const;

	// Gets the list of all SPIR-V extensions which were declared in the SPIR-V module.
	SPVC_EXPORT const SmallVector<std::string> &get_declared_extensions() const;
	...
	SPVC_EXPORT std::string get_remapped_declared_block_name(VariableID id) const;
	...
	SPVC_EXPORT Bitset get_buffer_block_flags(VariableID id) const;
...
protected:
	...
	SPVC_EXPORT void add_loop_level();
	...
	SPVC_EXPORT uint32_t get_pointee_type_id(uint32_t type_id) const;

	// Gets the SPIR-V type underlying the given type, which might be a pointer.
	SPVC_EXPORT const SPIRType &get_pointee_type(const SPIRType &type) const;

	// Gets the SPIR-V type underlying the given type_id, which might be a pointer.
	SPVC_EXPORT const SPIRType &get_pointee_type(uint32_t type_id) const;

	// Gets the ID of the SPIR-V type underlying a variable.
	SPVC_EXPORT uint32_t get_variable_data_type_id(const SPIRVariable &var) const;

	// Gets the SPIR-V type underlying a variable.
	SPVC_EXPORT SPIRType &get_variable_data_type(const SPIRVariable &var);

	// Gets the SPIR-V type underlying a variable.
	SPVC_EXPORT const SPIRType &get_variable_data_type(const SPIRVariable &var) const;

	// Gets the SPIR-V element type underlying an array variable.
	SPVC_EXPORT SPIRType &get_variable_element_type(const SPIRVariable &var);

	// Gets the SPIR-V element type underlying an array variable.
	SPVC_EXPORT const SPIRType &get_variable_element_type(const SPIRVariable &var) const;

	// Sets the qualified member identifier for OpTypeStruct ID, member number "index".
	SPVC_EXPORT void set_member_qualified_name(uint32_t type_id, uint32_t index, const std::string &name);
	SPVC_EXPORT void set_qualified_name(uint32_t id, const std::string &name);

	// Returns if the given type refers to a sampled image.
	SPVC_EXPORT bool is_sampled_image_type(const SPIRType &type);

	SPVC_EXPORT const SPIREntryPoint &get_entry_point() const;
	SPVC_EXPORT SPIREntryPoint &get_entry_point();
	SPVC_EXPORT static bool is_tessellation_shader(spv::ExecutionModel model);

	SPVC_EXPORT virtual std::string to_name(uint32_t id, bool allow_alias = true) const;
	SPVC_EXPORT bool is_builtin_variable(const SPIRVariable &var) const;
	SPVC_EXPORT bool is_builtin_type(const SPIRType &type) const;
	SPVC_EXPORT bool is_hidden_variable(const SPIRVariable &var, bool include_builtins = false) const;
	SPVC_EXPORT bool is_immutable(uint32_t id) const;
	SPVC_EXPORT bool is_member_builtin(const SPIRType &type, uint32_t index, spv::BuiltIn *builtin) const;
	SPVC_EXPORT bool is_scalar(const SPIRType &type) const;
	SPVC_EXPORT bool is_vector(const SPIRType &type) const;
	SPVC_EXPORT bool is_matrix(const SPIRType &type) const;
	SPVC_EXPORT bool is_array(const SPIRType &type) const;
	SPVC_EXPORT uint32_t expression_type_id(uint32_t id) const;
	SPVC_EXPORT const SPIRType &expression_type(uint32_t id) const;
	SPVC_EXPORT bool expression_is_lvalue(uint32_t id) const;
	SPVC_EXPORT bool variable_storage_is_aliased(const SPIRVariable &var);
	SPVC_EXPORT SPIRVariable *maybe_get_backing_variable(uint32_t chain);

	SPVC_EXPORT void register_read(uint32_t expr, uint32_t chain, bool forwarded);
	SPVC_EXPORT void register_write(uint32_t chain);
	...
	SPVC_EXPORT void flush_dependees(SPIRVariable &var);
	SPVC_EXPORT void flush_all_active_variables();
	SPVC_EXPORT void flush_control_dependent_expressions(uint32_t block);
	SPVC_EXPORT void flush_all_atomic_capable_variables();
	SPVC_EXPORT void flush_all_aliased_variables();
	SPVC_EXPORT void register_global_read_dependencies(const SPIRBlock &func, uint32_t id);
	SPVC_EXPORT void register_global_read_dependencies(const SPIRFunction &func, uint32_t id);
	std::unordered_set<uint32_t> invalid_expressions;

	SPVC_EXPORT void update_name_cache(std::unordered_set<std::string> &cache, std::string &name);
	...
	SPVC_EXPORT void update_name_cache(std::unordered_set<std::string> &cache_primary,
	                       const std::unordered_set<std::string> &cache_secondary, std::string &name);

	SPVC_EXPORT bool function_is_pure(const SPIRFunction &func);
	SPVC_EXPORT bool block_is_pure(const SPIRBlock &block);

	SPVC_EXPORT bool execution_is_branchless(const SPIRBlock &from, const SPIRBlock &to) const;
	SPVC_EXPORT bool execution_is_direct_branch(const SPIRBlock &from, const SPIRBlock &to) const;
	SPVC_EXPORT bool execution_is_noop(const SPIRBlock &from, const SPIRBlock &to) const;
	SPVC_EXPORT SPIRBlock::ContinueBlockType continue_block_type(const SPIRBlock &continue_block) const;

	SPVC_EXPORT void force_recompile();
	SPVC_EXPORT void clear_force_recompile();
	SPVC_EXPORT bool is_forcing_recompilation() const;
	bool is_force_recompile = false;

	SPVC_EXPORT bool block_is_loop_candidate(const SPIRBlock &block, SPIRBlock::Method method) const;

	SPVC_EXPORT bool types_are_logically_equivalent(const SPIRType &a, const SPIRType &b) const;
	SPVC_EXPORT void inherit_expression_dependencies(uint32_t dst, uint32_t source);
	SPVC_EXPORT void add_implied_read_expression(SPIRExpression &e, uint32_t source);
	SPVC_EXPORT void add_implied_read_expression(SPIRAccessChain &e, uint32_t source);

	// For proper multiple entry point support, allow querying if an Input or Output
	// variable is part of that entry points interface.
	SPVC_EXPORT bool interface_variable_exists_in_entry_point(uint32_t id) const;
	...
	SPVC_EXPORT void set_ir(const ParsedIR &parsed);
	SPVC_EXPORT void set_ir(ParsedIR &&parsed);
	SPVC_EXPORT void parse_fixup();
	...
	struct CombinedImageSamplerHandler : OpcodeHandler
	{
		...
		SPVC_EXPORT bool handle(spv::Op opcode, const uint32_t *args, uint32_t length) override;
		SPVC_EXPORT bool begin_function_scope(const uint32_t *args, uint32_t length) override;
		SPVC_EXPORT bool end_function_scope(const uint32_t *args, uint32_t length) override;
		...
		SPVC_EXPORT uint32_t remap_parameter(uint32_t id);
		SPVC_EXPORT void push_remap_parameters(const SPIRFunction &func, const uint32_t *args, uint32_t length);
		SPVC_EXPORT void pop_remap_parameters();
		SPVC_EXPORT void register_combined_image_sampler(SPIRFunction &caller, VariableID combined_id, VariableID texture_id,
	...
	struct ActiveBuiltinHandler : OpcodeHandler
	{
		...
		SPVC_EXPORT void handle_builtin(const SPIRType &type, spv::BuiltIn builtin, const Bitset &decoration_flags);
		SPVC_EXPORT void add_if_builtin(uint32_t id);
		SPVC_EXPORT void add_if_builtin_or_block(uint32_t id);
		SPVC_EXPORT void add_if_builtin(uint32_t id, bool allow_blocks);
	};

	SPVC_EXPORT bool traverse_all_reachable_opcodes(const SPIRBlock &block, OpcodeHandler &handler) const;
	SPVC_EXPORT bool traverse_all_reachable_opcodes(const SPIRFunction &block, OpcodeHandler &handler) const;
	...
	SPVC_EXPORT bool get_common_basic_type(const SPIRType &type, SPIRType::BaseType &base_type);
	...
	SPVC_EXPORT void analyze_parameter_preservation(
	...
	SPVC_EXPORT void analyze_image_and_sampler_usage();
	...
	struct CombinedImageSamplerUsageHandler : OpcodeHandler
	{
		...
		SPVC_EXPORT void add_hierarchy_to_comparison_ids(uint32_t ids);
		bool need_subpass_input = false;
		SPVC_EXPORT void add_dependency(uint32_t dst, uint32_t src);
	};

	SPVC_EXPORT void build_function_control_flow_graphs_and_analyze();
	std::unordered_map<uint32_t, std::unique_ptr<CFG>> function_cfgs;
	SPVC_EXPORT const CFG &get_cfg_for_current_function() const;
	SPVC_EXPORT const CFG &get_cfg_for_function(uint32_t id) const;

	struct CFGBuilder : OpcodeHandler
	{
		SPVC_EXPORT explicit CFGBuilder(Compiler &compiler_);

		SPVC_EXPORT bool follow_function_call(const SPIRFunction &func) override;
		SPVC_EXPORT bool handle(spv::Op op, const uint32_t *args, uint32_t length) override;
		Compiler &compiler;
		std::unordered_map<uint32_t, std::unique_ptr<CFG>> function_cfgs;
	};
	...
	{
		SPVC_EXPORT AnalyzeVariableScopeAccessHandler(Compiler &compiler_, SPIRFunction &entry_);

		SPVC_EXPORT bool follow_function_call(const SPIRFunction &) override;
		SPVC_EXPORT void set_current_block(const SPIRBlock &block) override;

		SPVC_EXPORT void notify_variable_access(uint32_t id, uint32_t block);
		SPVC_EXPORT bool id_is_phi_variable(uint32_t id) const;
		SPVC_EXPORT bool id_is_potential_temporary(uint32_t id) const;
		SPVC_EXPORT bool handle(spv::Op op, const uint32_t *args, uint32_t length) override;
		SPVC_EXPORT bool handle_terminator(const SPIRBlock &block) override;
	...
	{
		SPVC_EXPORT StaticExpressionAccessHandler(Compiler &compiler_, uint32_t variable_id_);
		SPVC_EXPORT bool follow_function_call(const SPIRFunction &) override;
		SPVC_EXPORT bool handle(spv::Op op, const uint32_t *args, uint32_t length) override;
	...
	{
		SPVC_EXPORT explicit PhysicalStorageBufferPointerHandler(Compiler &compiler_);
		SPVC_EXPORT bool handle(spv::Op op, const uint32_t *args, uint32_t length) override;
		Compiler &compiler;
		std::unordered_set<uint32_t> types;
	};
	SPVC_EXPORT void analyze_non_block_pointer_types();
	SmallVector<uint32_t> physical_storage_non_block_pointer_types;

	SPVC_EXPORT void analyze_variable_scope(SPIRFunction &function, AnalyzeVariableScopeAccessHandler &handler);
	SPVC_EXPORT void find_function_local_luts(SPIRFunction &function, const AnalyzeVariableScopeAccessHandler &handler,
	                              bool single_function);
	SPVC_EXPORT bool may_read_undefined_variable_in_block(const SPIRBlock &block, uint32_t var);
	...
	struct InterlockedResourceAccessHandler : OpcodeHandler
	{
		...
		SPVC_EXPORT void access_potential_resource(uint32_t id);
	};
	...
	struct InterlockedResourceAccessPrepassHandler : OpcodeHandler
	{
		...
		SPVC_EXPORT void rearm_current_block(const SPIRBlock &block) override;
		SPVC_EXPORT bool handle(spv::Op op, const uint32_t *args, uint32_t length) override;
		SPVC_EXPORT bool begin_function_scope(const uint32_t *args, uint32_t length) override;
		SPVC_EXPORT bool end_function_scope(const uint32_t *args, uint32_t length) override;
	...
	SPVC_EXPORT void analyze_interlocked_resource_usage();
	...
	SPVC_EXPORT void make_constant_null(uint32_t id, uint32_t type);

	std::unordered_map<uint32_t, std::string> declared_block_names;

	SPVC_EXPORT bool instruction_to_result_type(uint32_t &result_type, uint32_t &result_id, spv::Op op, const uint32_t *args,
	                                uint32_t length);

	SPVC_EXPORT Bitset combined_decoration_for_member(const SPIRType &type, uint32_t index) const;
	SPVC_EXPORT static bool is_desktop_only_format(spv::ImageFormat format);

	SPVC_EXPORT bool image_is_comparison(const SPIRType &type, uint32_t id) const;

	SPVC_EXPORT void set_extended_decoration(uint32_t id, ExtendedDecorations decoration, uint32_t value = 0);
	SPVC_EXPORT uint32_t get_extended_decoration(uint32_t id, ExtendedDecorations decoration) const;
	SPVC_EXPORT bool has_extended_decoration(uint32_t id, ExtendedDecorations decoration) const;
	SPVC_EXPORT void unset_extended_decoration(uint32_t id, ExtendedDecorations decoration);

	SPVC_EXPORT void set_extended_member_decoration(uint32_t type, uint32_t index, ExtendedDecorations decoration,
	                                    uint32_t value = 0);
	SPVC_EXPORT uint32_t get_extended_member_decoration(uint32_t type, uint32_t index, ExtendedDecorations decoration) const;
	SPVC_EXPORT bool has_extended_member_decoration(uint32_t type, uint32_t index, ExtendedDecorations decoration) const;
	SPVC_EXPORT void unset_extended_member_decoration(uint32_t type, uint32_t index, ExtendedDecorations decoration);

	SPVC_EXPORT bool type_is_array_of_pointers(const SPIRType &type) const;
	SPVC_EXPORT bool type_is_top_level_physical_pointer(const SPIRType &type) const;
	SPVC_EXPORT bool type_is_block_like(const SPIRType &type) const;
	SPVC_EXPORT bool type_is_opaque_value(const SPIRType &type) const;

	SPVC_EXPORT bool reflection_ssbo_instance_name_is_significant() const;
	SPVC_EXPORT std::string get_remapped_declared_block_name(uint32_t id, bool fallback_prefer_instance_name) const;

	SPVC_EXPORT bool flush_phi_required(BlockID from, BlockID to) const;

	SPVC_EXPORT uint32_t evaluate_spec_constant_u32(const SPIRConstantOp &spec) const;
	SPVC_EXPORT uint32_t evaluate_constant_u32(uint32_t id) const;

	SPVC_EXPORT bool is_vertex_like_shader() const;

private:
	// Used only to implement the old deprecated get_entry_point() interface.
	SPVC_EXPORT const SPIREntryPoint &get_first_entry_point(const std::string &name) const;
	SPVC_EXPORT SPIREntryPoint &get_first_entry_point(const std::string &name);
};
} // namespace SPIRV_CROSS_NAMESPACE
