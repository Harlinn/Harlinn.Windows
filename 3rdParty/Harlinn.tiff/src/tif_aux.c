...
HTIFF_EXPORT uint32_t _TIFFMultiply32(TIFF *tif, uint32_t first, uint32_t second,
                         const char *where)
{
...
HTIFF_EXPORT uint64_t _TIFFMultiply64(TIFF *tif, uint64_t first, uint64_t second,
                         const char *where)
{
...
HTIFF_EXPORT tmsize_t _TIFFMultiplySSize(TIFF *tif, tmsize_t first, tmsize_t second,
                            const char *where)
{
    if (first <= 0 || second <= 0)
    {
...
HTIFF_EXPORT tmsize_t _TIFFCastUInt64ToSSize(TIFF *tif, uint64_t val, const char *module)
{
...
HTIFF_EXPORT void *_TIFFCheckRealloc(TIFF *tif, void *buffer, tmsize_t nmemb,
...
HTIFF_EXPORT void *_TIFFCheckMalloc(TIFF *tif, tmsize_t nmemb, tmsize_t elem_size,
                       const char *what)
{
 ...
 */
HTIFF_EXPORT int TIFFVGetFieldDefaulted(TIFF *tif, uint32_t tag, va_list ap)
{
 ...
 * value if the tag is not present in the directory.
 */
HTIFF_EXPORT int TIFFGetFieldDefaulted(TIFF *tif, uint32_t tag, ...)
{
    int ok;
    va_list ap;
...
HTIFF_EXPORT float _TIFFClampDoubleToFloat(double val)
{
    if (val > FLT_MAX)
        return FLT_MAX;
    if (val < -FLT_MAX)
...
HTIFF_EXPORT uint32_t _TIFFClampDoubleToUInt32(double val)
{
    if (val < 0)
        return 0;
...
HTIFF_EXPORT int _TIFFSeekOK(TIFF *tif, toff_t off)
{
