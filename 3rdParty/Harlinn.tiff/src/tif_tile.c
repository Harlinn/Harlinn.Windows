 ...
 */
HTIFF_EXPORT uint32_t TIFFComputeTile(TIFF *tif, uint32_t x, uint32_t y, uint32_t z,
                         uint16_t s)
{
 ...
 */
HTIFF_EXPORT int TIFFCheckTile(TIFF *tif, uint32_t x, uint32_t y, uint32_t z, uint16_t s)
{
    TIFFDirectory *td = &tif->tif_dir;
 ...
 */
HTIFF_EXPORT uint32_t TIFFNumberOfTiles(TIFF *tif)
{
    TIFFDirectory *td = &tif->tif_dir;
    uint32_t dx = td->td_tilewidth;
    uint32_t dy = td->td_tilelength;
 ...
 */
HTIFF_EXPORT uint64_t TIFFTileRowSize64(TIFF *tif)
{
...
HTIFF_EXPORT tmsize_t TIFFTileRowSize(TIFF *tif)
{
 ...
 */
HTIFF_EXPORT uint64_t TIFFVTileSize64(TIFF *tif, uint32_t nrows)
{
...
HTIFF_EXPORT tmsize_t TIFFVTileSize(TIFF *tif, uint32_t nrows)
{
 ...
 */
HTIFF_EXPORT uint64_t TIFFTileSize64(TIFF *tif)
{
    return (TIFFVTileSize64(tif, tif->tif_dir.td_tilelength));
}
HTIFF_EXPORT tmsize_t TIFFTileSize(TIFF *tif)
{
 ...
 */
HTIFF_EXPORT void TIFFDefaultTileSize(TIFF *tif, uint32_t *tw, uint32_t *th)
{
    (*tif->tif_deftilesize)(tif, tw, th);
}

HTIFF_EXPORT void _TIFFDefaultTileSize(TIFF *tif, uint32_t *tw, uint32_t *th)
{
    (void)tif;
    if (*(int32_t *)tw < 1)
        *tw = 256;
