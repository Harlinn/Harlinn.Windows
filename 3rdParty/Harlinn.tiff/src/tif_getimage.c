 ...
 */
HTIFF_EXPORT int TIFFRGBAImageOK(TIFF *tif, char emsg[EMSG_BUF_SIZE])
{
...
HTIFF_EXPORT void TIFFRGBAImageEnd(TIFFRGBAImage *img)
{
    if (img->Map)
    {
...
HTIFF_EXPORT int TIFFRGBAImageBegin(TIFFRGBAImage *img, TIFF *tif, int stop,
...
HTIFF_EXPORT int TIFFRGBAImageGet(TIFFRGBAImage *img, uint32_t *raster, uint32_t w,
                     uint32_t h)
{
    if (img->get == NULL)
    {
 ...
 */
HTIFF_EXPORT int TIFFReadRGBAImageOriented(TIFF *tif, uint32_t rwidth, uint32_t rheight,
 ...
 */
HTIFF_EXPORT int TIFFReadRGBAImage(TIFF *tif, uint32_t rwidth, uint32_t rheight,
...
HTIFF_EXPORT int TIFFReadRGBAStrip(TIFF *tif, uint32_t row, uint32_t *raster)

{
    return TIFFReadRGBAStripExt(tif, row, raster, 0);
}

HTIFF_EXPORT int TIFFReadRGBAStripExt(TIFF *tif, uint32_t row, uint32_t *raster,
                         int stop_on_error)
...
HTIFF_EXPORT int TIFFReadRGBATile(TIFF *tif, uint32_t col, uint32_t row, uint32_t *raster)

{
    return TIFFReadRGBATileExt(tif, col, row, raster, 0);
}

HTIFF_EXPORT int TIFFReadRGBATileExt(TIFF *tif, uint32_t col, uint32_t row, uint32_t *raster,
                        int stop_on_error)
{
    char emsg[EMSG_BUF_SIZE] = "";
    TIFFRGBAImage img;
