...
HTIFF_EXPORT int TIFFReadScanline(TIFF *tif, void *buf, uint32_t row, uint16_t sample)
{
    int e;
 ...
 */
HTIFF_EXPORT tmsize_t TIFFReadEncodedStrip(TIFF *tif, uint32_t strip, void *buf,
                              tmsize_t size)
{
    static const char module[] = "TIFFReadEncodedStrip";
    TIFFDirectory *td = &tif->tif_dir;
 ...
 */
HTIFF_EXPORT tmsize_t _TIFFReadEncodedStripAndAllocBuffer(TIFF *tif, uint32_t strip,
                                             void **buf,
 ...
 */
HTIFF_EXPORT tmsize_t TIFFReadRawStrip(TIFF *tif, uint32_t strip, void *buf, tmsize_t size)
{
 ...
 */
HTIFF_EXPORT tmsize_t TIFFReadTile(TIFF *tif, void *buf, uint32_t x, uint32_t y, uint32_t z,
                      uint16_t s)
{
 ...
 * amount into the user-supplied buffer.
 */
HTIFF_EXPORT tmsize_t TIFFReadEncodedTile(TIFF *tif, uint32_t tile, void *buf, tmsize_t size)
{
 ...
 */
HTIFF_EXPORT tmsize_t _TIFFReadTileAndAllocBuffer(TIFF *tif, void **buf,
 ...
 * * calls regular TIFFReadEncodedTile() if *buf != NULL
 */
HTIFF_EXPORT tmsize_t _TIFFReadEncodedTileAndAllocBuffer(TIFF *tif, uint32_t tile,
 ...
 */
HTIFF_EXPORT tmsize_t TIFFReadRawTile(TIFF *tif, uint32_t tile, void *buf, tmsize_t size)
{
 ...
 * raw data.
 */
HTIFF_EXPORT int TIFFReadBufferSetup(TIFF *tif, void *bp, tmsize_t size)
{
 ...
 */
HTIFF_EXPORT int TIFFReadFromUserBuffer(TIFF *tif, uint32_t strile, void *inbuf,
...
HTIFF_EXPORT void _TIFFNoPostDecode(TIFF *tif, uint8_t *buf, tmsize_t cc)
{
    (void)tif;
    (void)buf;
    (void)cc;
...
HTIFF_EXPORT void _TIFFSwab16BitData(TIFF *tif, uint8_t *buf, tmsize_t cc)
{
    (void)tif;
    assert((cc & 1) == 0);
...
HTIFF_EXPORT void _TIFFSwab24BitData(TIFF *tif, uint8_t *buf, tmsize_t cc)
{
    (void)tif;
    assert((cc % 3) == 0);
...
HTIFF_EXPORT void _TIFFSwab32BitData(TIFF *tif, uint8_t *buf, tmsize_t cc)
{
    (void)tif;
    assert((cc & 3) == 0);
...
HTIFF_EXPORT void _TIFFSwab64BitData(TIFF *tif, uint8_t *buf, tmsize_t cc)
{
    (void)tif;
    assert((cc & 7) == 0);
