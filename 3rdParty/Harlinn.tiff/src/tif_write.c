...
HTIFF_EXPORT int TIFFWriteScanline(TIFF *tif, void *buf, uint32_t row, uint16_t sample)
{
 ...
 */
HTIFF_EXPORT tmsize_t TIFFWriteEncodedStrip(TIFF *tif, uint32_t strip, void *data,
                               tmsize_t cc)
{
 ...
 * NB: Image length must be setup before writing.
 */
HTIFF_EXPORT tmsize_t TIFFWriteRawStrip(TIFF *tif, uint32_t strip, void *data, tmsize_t cc)
{
 ...
 */
HTIFF_EXPORT tmsize_t TIFFWriteTile(TIFF *tif, void *buf, uint32_t x, uint32_t y, uint32_t z,
                       uint16_t s)
{
 ...
 */
HTIFF_EXPORT tmsize_t TIFFWriteEncodedTile(TIFF *tif, uint32_t tile, void *data, tmsize_t cc)
{
 ...
 *     the image on each write (as TIFFWriteScanline does).
 */
HTIFF_EXPORT tmsize_t TIFFWriteRawTile(TIFF *tif, uint32_t tile, void *data, tmsize_t cc)
{
...
HTIFF_EXPORT int TIFFSetupStrips(TIFF *tif)
{
    TIFFDirectory *td = &tif->tif_dir;
 ...
 */
HTIFF_EXPORT int TIFFWriteCheck(TIFF *tif, int tiles, const char *module)
{
 ...
 */
HTIFF_EXPORT int TIFFWriteBufferSetup(TIFF *tif, void *bp, tmsize_t size)
{
 ...
 */
HTIFF_EXPORT int TIFFFlushData1(TIFF *tif)
{
 ...
 */
HTIFF_EXPORT void TIFFSetWriteOffset(TIFF *tif, toff_t off)
{
    tif->tif_curoff = off;
    tif->tif_lastvalidoff = 0;
}
