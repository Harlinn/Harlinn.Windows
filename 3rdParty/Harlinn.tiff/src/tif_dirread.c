 ...
 */
HTIFF_EXPORT int TIFFReadDirectory(TIFF *tif)
{
 ...
 */
HTIFF_EXPORT int TIFFReadCustomDirectory(TIFF *tif, toff_t diroff,
 ...
 * function to read it.
 */
HTIFF_EXPORT int TIFFReadEXIFDirectory(TIFF *tif, toff_t diroff)
{
 ...
 */
HTIFF_EXPORT int TIFFReadGPSDirectory(TIFF *tif, toff_t diroff)
{
 ...
 */
HTIFF_EXPORT int _TIFFCheckDirNumberAndOffset(TIFF *tif, tdir_t dirn, uint64_t diroff)
{
 ...
 */
HTIFF_EXPORT int _TIFFGetDirNumberFromOffset(TIFF *tif, uint64_t diroff, tdir_t *dirn)
{
    if (diroff == 0) /* no more directories */
        return 0;
 ...
 * Otherwise returns 0 or if an error occurred.
 */
HTIFF_EXPORT int _TIFFGetOffsetFromDirNumber(TIFF *tif, tdir_t dirn, uint64_t *diroff)
{
 ...
 */
HTIFF_EXPORT int _TIFFRemoveEntryFromDirectoryListByOffset(TIFF *tif, uint64_t diroff)
{
    if (tif->tif_map_dir_offset_to_number == NULL)
        return 1;
 ...
 * tile/strile */
HTIFF_EXPORT uint64_t TIFFGetStrileOffset(TIFF *tif, uint32_t strile)
{
...
/* Return the value of the TileOffsets/StripOffsets array for the specified
 * tile/strile */
HTIFF_EXPORT uint64_t TIFFGetStrileOffsetWithErr(TIFF *tif, uint32_t strile, int *pbErr)
{
    TIFFDirectory *td = &tif->tif_dir;
...
HTIFF_EXPORT uint64_t TIFFGetStrileByteCount(TIFF *tif, uint32_t strile)
{
...
/* Return the value of the TileByteCounts/StripByteCounts array for the
 * specified tile/strile */
HTIFF_EXPORT uint64_t TIFFGetStrileByteCountWithErr(TIFF *tif, uint32_t strile, int *pbErr)
{
    TIFFDirectory *td = &tif->tif_dir;
...
HTIFF_EXPORT int _TIFFFillStriles(TIFF *tif) { return _TIFFFillStrilesInternal(tif, 1); }
