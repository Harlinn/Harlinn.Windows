...
#include <piex/tiff_directory/tiff_directory.h>
...
HPIEX_EXPORT bool Get16u(StreamInterface* stream, const std::uint32_t offset,
...
// success. Returns false when something is wrong.
HPIEX_EXPORT bool Get32u(StreamInterface* stream, const std::uint32_t offset,
...
HPIEX_EXPORT std::vector<std::uint8_t> GetData(const size_t offset, const size_t length,
...
HPIEX_EXPORT bool GetEndianness(const std::uint32_t tiff_offset, StreamInterface* stream,
                   tiff_directory::Endian* endian);

// Retrieves an image from tiff_directory. Return false when something is wrong.
HPIEX_EXPORT bool GetImageData(const tiff_directory::TiffDirectory& tiff_directory,
                  StreamInterface* stream, Image* image);

// Retrieves the width and height from the jpeg image returning true on
// success. Returns false when something is wrong.
HPIEX_EXPORT bool GetJpegDimensions(const std::uint32_t jpeg_offset, StreamInterface* stream,
...
HPIEX_EXPORT bool IsThumbnail(const Image& image,
...
HPIEX_EXPORT bool ParseDirectory(const std::uint32_t tiff_offset,
...
// otherwise.
HPIEX_EXPORT bool GetExifOrientation(StreamInterface* stream, const std::uint32_t offset,
                        std::uint32_t* orientation);

// Reads the width and height of the full resolution image. The tag groups are
// exclusive.
HPIEX_EXPORT bool GetFullDimension32(const tiff_directory::TiffDirectory& tiff_directory,
                        std::uint32_t* width, std::uint32_t* height);

// Reads the width and height of the crop information if available.
// Returns false if an error occurred.
HPIEX_EXPORT bool GetFullCropDimension(const tiff_directory::TiffDirectory& tiff_directory,
                          std::uint32_t* width, std::uint32_t* height);

// Reads 1 or more rational values for a tag and stores results into data.
// Returns false if an error occurred.
HPIEX_EXPORT bool GetRational(const tiff_directory::TiffDirectory::Tag& tag,
...
class TiffParser {
 public:
  ...
  HPIEX_EXPORT explicit TiffParser(StreamInterface* stream);
  HPIEX_EXPORT TiffParser(StreamInterface* stream, const std::uint32_t offset);
  ...
  HPIEX_EXPORT bool GetPreviewImageData(const TiffContent& tiff_content,
                           PreviewImageData* image_metadata);

  // Returns false if called more that once or something with the Tiff data is
  // wrong.
  HPIEX_EXPORT bool Parse(const TagSet& desired_tags, const std::uint16_t max_number_ifds,
 ...
 private:
  ...
  HPIEX_EXPORT bool ParseIfd(const std::uint32_t ifd_offset, const TagSet& desired_tags,
                const std::uint16_t max_number_ifds, IfdVector* tiff_directory);
  HPIEX_EXPORT bool ParseGpsData(const tiff_directory::TiffDirectory* tiff_ifd,
                    TiffContent* tiff_content);
