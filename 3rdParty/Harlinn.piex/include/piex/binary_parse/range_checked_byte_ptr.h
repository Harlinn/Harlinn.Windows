...
class PagedByteArray {
 public:
  ...
  HPIEX_EXPORT virtual ~PagedByteArray();
...
class RangeCheckedBytePtr {
 private:
  ...
  HPIEX_EXPORT RangeCheckedBytePtr();
 ...
 public:
  ...
  // non-null pointer.
  HPIEX_EXPORT explicit RangeCheckedBytePtr(const unsigned char *array, const size_t len);
  ...
  // destroyed.
  HPIEX_EXPORT explicit RangeCheckedBytePtr(PagedByteArray *array);
  ...
  HPIEX_EXPORT static RangeCheckedBytePtr invalidPointer();
  ...
  HPIEX_EXPORT RangeCheckedBytePtr pointerToSubArray(size_t pos, size_t length) const;
  ...
  HPIEX_EXPORT size_t offsetInArray() const;
  ...
  // to this pointer.
  HPIEX_EXPORT std::string substr(size_t pos, size_t length) const;

  // Returns 'length' number of bytes from the array starting at position 'pos'
  // relative to this pointer.
  HPIEX_EXPORT std::vector<unsigned char> extractBytes(size_t pos, size_t length) const;
 ...
 private:
  HPIEX_EXPORT void loadPageForOffset(size_t offset) const;
  HPIEX_EXPORT void restrictPageToSubArray() const;
};
...
HPIEX_EXPORT int memcmp(const RangeCheckedBytePtr &x, const RangeCheckedBytePtr &y,
           size_t num);
...
HPIEX_EXPORT int strcmp(const RangeCheckedBytePtr &x, const std::string &y);
...
HPIEX_EXPORT size_t strlen(const RangeCheckedBytePtr &src);
...
//     // error handling
//   }
HPIEX_EXPORT int16 Get16s(const RangeCheckedBytePtr &input, const bool big_endian,
             MemoryStatus *status);
HPIEX_EXPORT uint16 Get16u(const RangeCheckedBytePtr &input, const bool big_endian,
              MemoryStatus *status);
HPIEX_EXPORT int32 Get32s(const RangeCheckedBytePtr &input, const bool big_endian,
             MemoryStatus *status);
HPIEX_EXPORT uint32 Get32u(const RangeCheckedBytePtr &input, const bool big_endian,
              MemoryStatus *status);

inline size_t RangeCheckedBytePtr::remainingLength() const {
  if (!errorOccurred()) {
...
inline bool RangeCheckedBytePtr::errorOccurred() const {
  return error_flag_ != RANGE_CHECKED_BYTE_SUCCESS;
}

inline unsigned char RangeCheckedBytePtr::operator[](size_t i) const {
...
inline unsigned char RangeCheckedBytePtr::operator*() const { return (*this)[0]; }

inline RangeCheckedBytePtr &RangeCheckedBytePtr::operator++() {
  if (current_pos_ < sub_array_end_) {
    current_pos_++;
  } else {
#ifdef BREAK_IF_DEBUGGING_AND_OUT_OF_RANGE
...
inline RangeCheckedBytePtr RangeCheckedBytePtr::operator++(int) {
...
inline RangeCheckedBytePtr &RangeCheckedBytePtr::operator+=(size_t x) {
...
inline RangeCheckedBytePtr operator+(const RangeCheckedBytePtr &p, size_t x) {
