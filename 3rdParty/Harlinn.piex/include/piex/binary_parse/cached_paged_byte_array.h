...
#endif
#include <piex/binary_parse/range_checked_byte_ptr.h>
...
class CachedPagedByteArray : public PagedByteArray {
 public:
  ...
  // 'cache_size'.
  HPIEX_EXPORT explicit CachedPagedByteArray(const PagedByteArray* paged_byte_array,
                                size_t cache_size);

  HPIEX_EXPORT virtual size_t length() const { return paged_byte_array_->length(); }

  HPIEX_EXPORT virtual size_t pageSize() const { return paged_byte_array_->pageSize(); }

  HPIEX_EXPORT virtual void getPage(size_t page_index, const unsigned char** begin,
 ...
 private:
  ...
  // returns false.
  HPIEX_EXPORT bool getFromCache(size_t page_index, size_t* cache_index) const;
