    ...
    while (json_peek(s) != JSON_ARRAY_END && !json_get_error(s)) {
        ...
        while (json_peek(s) != JSON_OBJECT_END && !json_get_error(s)) {
            if (json_next(s) != JSON_STRING)
                return -EINVAL;
            char *key = _strdup(json_get_string(s, NULL));
                               ...
                               unsigned index)
{
    if (index >= MAX_FEATURE_COUNT)
        return -EINVAL;
    model->feature[index].name = _strdup(name);
...
#ifdef _MSC_VER
    char* cfg_name = (char*)_alloca(cfg_name_sz);
#else
    char cfg_name[cfg_name_sz];
#endif
    const size_t generated_key_sz = 4 + 1;
#ifdef _MSC_VER
    char* generated_key = (char*)_alloca( generated_key_sz);
#else
