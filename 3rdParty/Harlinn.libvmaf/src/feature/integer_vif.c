...
pad_top_and_bottom(VifBuffer buf, unsigned h, int fwidth)
{
    ...
    for (unsigned i = 1; i <= fwidth_half; ++i) {
        ...
        memcpy((BYTE*)ref - offset, (BYTE*)ref + offset, buf.stride);
        memcpy( (BYTE*)dis - offset, (BYTE*)dis + offset, buf.stride);
        memcpy( (BYTE*)ref + buf.stride * (h - 1) + buf.stride * i,
                (BYTE*)ref + buf.stride * (h - 1) - buf.stride * i,
                 buf.stride);
        memcpy( (BYTE*)dis + buf.stride * (h - 1) + buf.stride * i,
                (BYTE*)dis + buf.stride * (h - 1) - buf.stride * i,
                 buf.stride);
    }
}
                ...
                unsigned bpc, unsigned w, unsigned h)
{
    ...
    s->buf.data = data; (BYTE*)data += pad_size;
    s->buf.ref = data; (BYTE*)data += frame_size + pad_size + pad_size;
    s->buf.dis = data; (BYTE*)data += frame_size + pad_size;
    s->buf.mu1 = data; (BYTE*)data += h * s->buf.stride_16;
    s->buf.mu2 = data; (BYTE*)data += h * s->buf.stride_16;
    s->buf.mu1_32 = data; (BYTE*)data += h * s->buf.stride_32;
    s->buf.mu2_32 = data; (BYTE*)data += h * s->buf.stride_32;
    s->buf.ref_sq = data; (BYTE*)data += h * s->buf.stride_32;
    s->buf.dis_sq = data; (BYTE*)data += h * s->buf.stride_32;
    s->buf.ref_dis = data; (BYTE*)data += h * s->buf.stride_32;
    s->buf.tmp.mu1 = data; (BYTE*)data += s->buf.stride_tmp;
    s->buf.tmp.mu2 = data; (BYTE*)data += s->buf.stride_tmp;
    s->buf.tmp.ref = data; (BYTE*)data += s->buf.stride_tmp;
    s->buf.tmp.dis = data; (BYTE*)data += s->buf.stride_tmp;
    s->buf.tmp.ref_dis = data; (BYTE*)data += s->buf.stride_tmp;
    s->buf.tmp.ref_convol = data; (BYTE*)data += s->buf.stride_tmp;
                   ...
                   unsigned index, VmafFeatureCollector *feature_collector)
{
    ...
    for (unsigned i = 0; i < h; i++) {
        ...
        (BYTE*)ref_in += ref_pic->stride[0];
        (BYTE*)dis_in += dist_pic->stride[0];
        (BYTE*)ref_out += s->buf.stride;
        (BYTE*)dis_out += s->buf.stride;
    }
