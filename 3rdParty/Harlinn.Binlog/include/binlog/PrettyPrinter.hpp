...
{
public:
  HBINLOG_EXPORT PrettyPrinter(std::string eventFormat, std::string timeFormat);
   ...
   */
  HBINLOG_EXPORT void printEvent(
    std::ostream& ostr,
    const Event& event,
   ...
   */
  HBINLOG_EXPORT bool printStruct(detail::OstreamBuffer& out, mserialize::Visitor::StructBegin sb, Range& input) const;

private:
  HBINLOG_EXPORT void printEventField(
  ...
  HBINLOG_EXPORT void printEventMessage(detail::OstreamBuffer& out, const Event& event) const;

  HBINLOG_EXPORT void printProducerLocalTime(detail::OstreamBuffer& out, std::uint64_t clockValue) const;
  HBINLOG_EXPORT void printUTCTime(detail::OstreamBuffer& out, std::uint64_t clockValue) const;

  HBINLOG_EXPORT void printTime(detail::OstreamBuffer& out, BrokenDownTime& bdt, int tzoffset, const char* tzname) const;
  HBINLOG_EXPORT void printTimeField(detail::OstreamBuffer& out, char spec, BrokenDownTime& bdt, int tzoffset, const char* tzname) const;
