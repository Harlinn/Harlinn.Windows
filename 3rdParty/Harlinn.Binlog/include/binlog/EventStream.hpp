...
public:
   ...
   */
  HBINLOG_EXPORT const Event* nextEvent(EntryStream& input);
...
private:
  HBINLOG_EXPORT void readEventSource(Range range);

  HBINLOG_EXPORT void readWriterProp(Range range);

  HBINLOG_EXPORT void readClockSync(Range range);

  HBINLOG_EXPORT void readEvent(std::uint64_t eventSourceId, Range range);
