...
void* ZEXPORT mem_simple_create_file( zlib_filefunc64_32_def* api, void* buffer, size_t buf_len)
{
    MEMFILE* handle = malloc(sizeof(*handle));
    if ( handle )
    {
        api->zfile_func64.zopen64_file = NULL;
        api->zfile_func64.zread_file = mem_read;
        api->zfile_func64.zwrite_file = mem_write;
        api->zfile_func64.ztell64_file = mem_tell;
        api->zfile_func64.zseek64_file = mem_seek;
        api->zfile_func64.zclose_file = mem_close;
        api->zfile_func64.zerror_file = mem_error;
        api->zfile_func64.opaque = handle;
        handle->position = 0;
