...
xmlGzfileOpen_real (const char *filename) {
    ...
    if (!strcmp(filename, "-")) {
        int duped_fd = _dup(_fileno(stdin));
        fd = gzdopen(duped_fd, "rb");
        if (fd == Z_NULL && duped_fd >= 0) {
            _close(duped_fd);  /* gzdOpen() does not close on failure */
        }
...
xmlGzfileOpenW (const char *filename, int compression) {
    ...
    if (!strcmp(filename, "-")) {
        int duped_fd = _dup(_fileno(stdout));
        fd = gzdopen(duped_fd, "rb");
        if (fd == Z_NULL && duped_fd >= 0) {
            _close(duped_fd);  /* gzdOpen() does not close on failure */
        }
...
xmlXzfileOpen_real (const char *filename) {
    ...
    if (!strcmp(filename, "-")) {
        fd = __libxml2_xzdopen(_dup(_fileno(stdin)), "rb");
	return((void *) fd);
    }
