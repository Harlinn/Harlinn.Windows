...
#ifdef BUILDING_HARLINN_SJPEG
#define HSJPEG_EXPORT __declspec(dllexport)
#else
#define HSJPEG_EXPORT __declspec(dllimport)
#pragma comment(lib,"Harlinn.sjpeg.lib")
#endif
...
HSJPEG_EXPORT uint32_t SjpegVersion();
...
HSJPEG_EXPORT size_t SjpegCompress(const uint8_t* rgb, int width, int height, float quality,
                     uint8_t** out_data);
...
//
HSJPEG_EXPORT size_t SjpegEncode(const uint8_t* rgb,
...
HSJPEG_EXPORT void SjpegFreeBuffer(const uint8_t* buffer);
...
HSJPEG_EXPORT bool SjpegDimensions(const uint8_t* data, size_t size,
...
HSJPEG_EXPORT int SjpegFindQuantizer(const uint8_t* data, size_t size,
                       uint8_t quant[2][64]);
...
HSJPEG_EXPORT float SjpegEstimateQuality(const uint8_t matrix[64], bool for_chroma);

// Generate a default quantization matrix for the given quality factor,
// in a libjpeg-6b fashion.
HSJPEG_EXPORT void SjpegQuantMatrix(float quality, bool for_chroma, uint8_t matrix[64]);
...
HSJPEG_EXPORT SjpegYUVMode SjpegRiskiness(const uint8_t* rgb, int width, int height,
...
HSJPEG_EXPORT bool SjpegCompress(const uint8_t* rgb,
                   int width, int height, float quality, std::string* output);

HSJPEG_EXPORT bool SjpegDimensions(const std::string& jpeg_data,
                     int* width, int* height, int* is_yuv420);

HSJPEG_EXPORT int SjpegFindQuantizer(const std::string& jpeg_data, uint8_t quant[2][64]);
...
struct EncoderParam {
  HSJPEG_EXPORT EncoderParam();
  HSJPEG_EXPORT explicit EncoderParam(float quality_factor);

  // Sets the compression factor. 0 = lowest quality, 100 = best quality.
  // The call will actually initialize quant[][].
  HSJPEG_EXPORT void SetQuality(float quality_factor);
  ...
  HSJPEG_EXPORT void SetQuantization(const uint8_t m[2][64], float reduction = 100.f);
  HSJPEG_EXPORT const uint8_t* GetQuantMatrix(int idx) const { return quant_[idx]; }
  ...
  HSJPEG_EXPORT void SetLimitQuantization(bool limit_quantization = true, int tolerance = 0);

  // Set the minimal quantization matrices directly, irrespective of the value
  // of quant_[][].
  HSJPEG_EXPORT void SetMinQuantization(const uint8_t m[2][64], int min_quant_tolerance = 0);
  ...
  HSJPEG_EXPORT void ResetMetadata();      // clears the above
 ...
 protected:
  HSJPEG_EXPORT void Init(float quality_factor);
...
HSJPEG_EXPORT bool Encode(const uint8_t* rgb, int width, int height, int stride,
            const EncoderParam& param, std::string* output);

// This version returns data in *out_data. Returns 0 in case of error.
HSJPEG_EXPORT size_t Encode(const uint8_t* rgb, int width, int height, int stride,
...
struct SearchHook {
  ...
  HSJPEG_EXPORT virtual bool Setup(const EncoderParam& param);
  // Set up the next matrices to try, corresponding to the current q value.
  // 'idx' is 0 for luma, 1 for chroma
  HSJPEG_EXPORT virtual void NextMatrix(int idx, uint8_t dst[64]);
  // return true if the search is finished
  HSJPEG_EXPORT virtual bool Update(float result);
  virtual ~SearchHook() {}
};
...
HSJPEG_EXPORT std::shared_ptr<ByteSink> MakeByteSink(std::string* output);
// Vector-based template, specialized for uint8_t
template<typename T>
HSJPEG_EXPORT std::shared_ptr<ByteSink> MakeByteSink(std::vector<T>* output);
template<> HSJPEG_EXPORT std::shared_ptr<ByteSink> MakeByteSink(std::vector<uint8_t>* output);
