...
extern HSJPEG_EXPORT bool SupportsSSE2();
extern HSJPEG_EXPORT bool SupportsNEON();
...
HSJPEG_EXPORT FdctFunc GetFdct();

// these are the default luma/chroma matrices (JPEG spec section K.1)
extern HSJPEG_EXPORT const uint8_t kDefaultMatrices[2][64];
extern HSJPEG_EXPORT const uint8_t kZigzag[64];

// scoring tables in score_7.cc
extern HSJPEG_EXPORT const int kRGBSize;
extern HSJPEG_EXPORT const uint8_t kSharpnessScore[];

// internal riskiness scoring functions:
extern HSJPEG_EXPORT double DCTRiskinessScore(const int16_t yuv[3 * 8],
                                int16_t scores[8 * 8]);
extern HSJPEG_EXPORT double BlockRiskinessScore(const uint8_t* rgb, int stride,
                                  int16_t scores[8 * 8]);
extern HSJPEG_EXPORT int YUVToRiskIdx(int16_t y, int16_t u, int16_t v);
...
extern HSJPEG_EXPORT float GetQFactor(float q);   // convert quality factor -> scale factor
extern HSJPEG_EXPORT void CopyQuantMatrix(const uint8_t in[64], uint8_t out[64]);
extern HSJPEG_EXPORT void SetQuantMatrix(const uint8_t in[64], float q_factor,
                           uint8_t out[64]);
extern HSJPEG_EXPORT void SetMinQuantMatrix(const uint8_t* const m, uint8_t out[64],
                              int tolerance);
...
struct Encoder {
 public:
  HSJPEG_EXPORT Encoder(int W, int H, int step, const uint8_t* rgb, ByteSink* sink);
  HSJPEG_EXPORT virtual ~Encoder();
  bool Ok() const { return ok_; }

  // setters
  HSJPEG_EXPORT void SetQuality(float q);
  HSJPEG_EXPORT void SetCompressionMethod(int method);

  // all-in-one init from EncoderParam.
  HSJPEG_EXPORT bool InitFromParam(const EncoderParam& param);

  // Main call. Return false in case of parameter error (setting empty output).
  HSJPEG_EXPORT bool Encode();
 ...
 private:
  // setters
  HSJPEG_EXPORT void SetQuantMatrices(const uint8_t m[2][64]);
  HSJPEG_EXPORT void SetMinQuantMatrices(const uint8_t m[2][64], int tolerance);
  HSJPEG_EXPORT void SetDefaultMinQuantMatrices();

  HSJPEG_EXPORT void SetQuantizationBias(int bias, bool use_adaptive);
  HSJPEG_EXPORT void SetQuantizationDeltas(int qdelta_luma, int qdelta_chroma);

  typedef enum { ICC, EXIF, XMP, MARKERS } MetadataType;
  HSJPEG_EXPORT void SetMetadata(const std::string& data, MetadataType type);

 private:
  HSJPEG_EXPORT bool CheckBuffers();  // returns false in case of memory alloc error

  HSJPEG_EXPORT void WriteAPP0();
  HSJPEG_EXPORT bool WriteAPPMarkers(const std::string& data);
  HSJPEG_EXPORT bool WriteEXIF(const std::string& data);
  HSJPEG_EXPORT bool WriteICCP(const std::string& data);
  HSJPEG_EXPORT bool WriteXMP(const std::string& data);
  HSJPEG_EXPORT void WriteDQT();
  HSJPEG_EXPORT void WriteSOF();
  HSJPEG_EXPORT void WriteDHT();
  HSJPEG_EXPORT void WriteSOS();
  HSJPEG_EXPORT void WriteEOI();

  HSJPEG_EXPORT void ResetDCs();

  // collect transformed coeffs (unquantized) only
  HSJPEG_EXPORT void CollectCoeffs();

  // 2-pass Huffman optimizing scan
  HSJPEG_EXPORT void ResetEntropyStats();
  HSJPEG_EXPORT void AddEntropyStats(const DCTCoeffs* const coeffs,
                       const RunLevel* const run_levels);
  HSJPEG_EXPORT void CompileEntropyStats();
  HSJPEG_EXPORT void StoreOptimalHuffmanTables(size_t nb_mbs, const DCTCoeffs* coeffs);

  HSJPEG_EXPORT void SinglePassScan();           // finalizing scan
  HSJPEG_EXPORT void SinglePassScanOptimized();  // optimize the Huffman table + finalize scan

  // quantize and compute run/levels from already stored coeffs
  HSJPEG_EXPORT void StoreRunLevels(DCTCoeffs* coeffs);
  // just write already stored run_levels & coeffs:
  HSJPEG_EXPORT void FinalPassScan(size_t nb_mbs, const DCTCoeffs* coeffs);

  // dichotomy loop
  HSJPEG_EXPORT void LoopScan();

  // Histogram pass
  HSJPEG_EXPORT void CollectHistograms();

  HSJPEG_EXPORT void BuildHuffmanCodes(const HuffmanTable* const tab,
  ...
  HSJPEG_EXPORT static QuantizeBlockFunc quantize_block_;
  HSJPEG_EXPORT static QuantizeBlockFunc GetQuantizeBlockFunc();

  HSJPEG_EXPORT static int TrellisQuantizeBlock(const int16_t in[64], int idx,
                                  const Quantizer* const Q,
                                  DCTCoeffs* const out,
                                  RunLevel* const rl);
  ...
  typedef uint32_t (*QuantizeErrorFunc)(const int16_t in[64],
                                        const Quantizer* const Q);
  HSJPEG_EXPORT static QuantizeErrorFunc quantize_error_;
  HSJPEG_EXPORT static QuantizeErrorFunc GetQuantizeErrorFunc();

  HSJPEG_EXPORT void CodeBlock(const DCTCoeffs* const coeffs, const RunLevel* const rl);
  // returns DC code (4bits for length, 12bits for suffix), updates DC_predictor
  HSJPEG_EXPORT static uint16_t GenerateDCDiffCode(int DC, int* const DC_predictor);

  HSJPEG_EXPORT static void FinalizeQuantMatrix(Quantizer* const q, int bias);
  HSJPEG_EXPORT void SetCostCodes(int idx);
  HSJPEG_EXPORT void InitCodes(bool only_ac);

  HSJPEG_EXPORT size_t HeaderSize() const;
  HSJPEG_EXPORT void BlocksSize(int nb_mbs, const DCTCoeffs* coeffs,
                  const RunLevel* rl, sjpeg::BitCounter* const bc) const;
  HSJPEG_EXPORT float ComputeSize(const DCTCoeffs* coeffs);
  HSJPEG_EXPORT float ComputePSNR() const;

 protected:
  HSJPEG_EXPORT bool SetError();   // sets ok_ to true
  ...
  HSJPEG_EXPORT const uint8_t* GetReplicatedSamples(const uint8_t* rgb,    // block source
  ...
  HSJPEG_EXPORT const uint8_t* GetReplicatedYUVSamples(const uint8_t* in, int step,
                                         int sub_w, int sub_h, int w, int h);
  // set blocks that are totally outside of the picture to an average value
  HSJPEG_EXPORT void AverageExtraLuma(int sub_w, int sub_h, int16_t* out);
  uint8_t replicated_buffer_[3 * 16 * 16];   // tmp buffer for replication

  sjpeg::RGBToYUVBlockFunc get_yuv_block_;
  HSJPEG_EXPORT static sjpeg::RGBToYUVBlockFunc get_yuv444_block_;
 ...
 private:
  ...
  HSJPEG_EXPORT bool AllocateBlocks(size_t num_blocks);  // returns false in case of error
  HSJPEG_EXPORT void DesallocateBlocks();
  ...
  typedef void (*StoreHistoFunc)(const int16_t in[64], Histo* const histos,
                                 int nb_blocks);
  HSJPEG_EXPORT static StoreHistoFunc store_histo_;
  HSJPEG_EXPORT static StoreHistoFunc GetStoreHistoFunc();  // select between the above.
  ...
  HSJPEG_EXPORT void AnalyseHisto();
  HSJPEG_EXPORT void ResetHisto();  // initialize histos_[]
  Histo histos_[2];
  ...
  HSJPEG_EXPORT static const float kHistoWeight[QSIZE];

  HSJPEG_EXPORT static void (*fDCT_)(int16_t* in, int num_blocks);
  HSJPEG_EXPORT static void InitializeStaticPointers();
};
