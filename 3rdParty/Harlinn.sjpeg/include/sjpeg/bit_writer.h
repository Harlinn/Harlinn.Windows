...
class MemorySink : public ByteSink {
 public:
  HSJPEG_EXPORT explicit MemorySink(size_t expected_size);
  HSJPEG_EXPORT virtual ~MemorySink();
  HSJPEG_EXPORT virtual bool Commit(size_t used_size, size_t extra_size, uint8_t** data);
  virtual bool Finalize() { /* nothing to do */ return true; }
  HSJPEG_EXPORT virtual void Reset();
  HSJPEG_EXPORT void Release(uint8_t** buf_ptr, size_t* size_ptr);
...
class BitWriter {
 public:
  HSJPEG_EXPORT explicit BitWriter(ByteSink* const sink);
  ...
  HSJPEG_EXPORT void PutPackedCode(uint32_t code) { PutBits(code >> 16, code & 0xff); }

  // Write pending bits, and align bitstream with extra '1' bits.
  HSJPEG_EXPORT void Flush();
...
struct BitCounter {
  ...
  HSJPEG_EXPORT void AddBits(const uint32_t bits, size_t nbits);
