...
// reads a disk file
HSJPEG_EXPORT extern std::string ReadFile(const char filename[]);
// save data to disk file
HSJPEG_EXPORT extern int SaveFile(const char* name, const std::string& out, bool quiet);

// returns the current time in seconds.
HSJPEG_EXPORT extern double GetStopwatchTime();

// try to systematically decode the input as a JPEG, PNG, PPM (in this order)
extern HSJPEG_EXPORT std::vector<uint8_t> ReadImage(const std::string& in,
                                      int* const width, int* const height,
                                      sjpeg::EncoderParam* const param);

// Return CRC32 signature for data block. 'crc' is the current checksum value.
extern HSJPEG_EXPORT uint32_t GetCRC32(const std::string& data, uint32_t crc = 0);
...
extern HSJPEG_EXPORT const char* ImageTypeName(ImageType type);
...
extern HSJPEG_EXPORT ImageType GuessImageType(const std::string& input);
...
extern HSJPEG_EXPORT ImageReader GuessImageReader(const std::string& input);

// quickly try to guess the image format and read it.
extern HSJPEG_EXPORT std::vector<uint8_t> ReadImageQuick(const std::string& in,
                                           int* const width, int* const height,
                                           sjpeg::EncoderParam* const param);

// Directly callable functions, in case auto-detection didn't work.
extern HSJPEG_EXPORT std::vector<uint8_t> ReadJPEG(const std::string& in,
                                     int* const width, int* const height,
                                     sjpeg::EncoderParam* const param);
extern HSJPEG_EXPORT std::vector<uint8_t> ReadPNG(const std::string& input,
                                    int* const width_ptr, int* const height_ptr,
                                    sjpeg::EncoderParam* const param);
extern HSJPEG_EXPORT std::vector<uint8_t> ReadPPM(const std::string& input,
                                    int* const width, int* const height,
                                    sjpeg::EncoderParam* const param);
