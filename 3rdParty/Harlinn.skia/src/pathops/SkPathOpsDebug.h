...
#define DEBUG_WINDING_AT_T 1
    ...
    extern SK_API int gDumpTSectNum;
#endif
...
public:
    ...
    SK_API static int gContourID;
    SK_API static int gSegmentID;
#endif

#if DEBUG_SORT
    SK_API static int gSortCountDefault;
    SK_API static int gSortCount;
#endif

#if DEBUG_ACTIVE_OP
    SK_API static const char* kPathOpStr[];
#endif
    SK_API static bool gRunFail;
    SK_API static bool gVeryVerbose;

#if DEBUG_ACTIVE_SPANS
    SK_API static SkString gActiveSpans;
#endif
#if DEBUG_DUMP_VERIFY
    SK_API static bool gDumpOp;
    SK_API static bool gVerifyOp;
#endif

    SK_API static const char* OpStr(SkPathOp );
    SK_API static void MathematicaIze(char* str, size_t bufferSize);
    SK_API static bool ValidWind(int winding);
    SK_API static void WindingPrintf(int winding);

    SK_API static void ShowActiveSpans(SkOpContourHead* contourList);
    SK_API static void ShowOnePath(const SkPath& path, const char* name, bool includeDeclaration);
    SK_API static void ShowPath(const SkPath& one, const SkPath& two, SkPathOp op, const char* name);

    SK_API static bool ChaseContains(const SkTDArray<SkOpSpanBase*>& , const SkOpSpanBase* );

    SK_API static void CheckHealth(class SkOpContourHead* contourList);

#if DEBUG_COIN
    SK_API static void DumpCoinDict();
    SK_API static void DumpGlitchType(GlitchType );
#endif
...
#if DEBUG_DUMP_VERIFY
SK_API void DumpOp(const SkPath& one, const SkPath& two, SkPathOp op,
        const char* testName);
SK_API void DumpOp(FILE* file, const SkPath& one, const SkPath& two, SkPathOp op,
        const char* testName);
SK_API void DumpSimplify(const SkPath& path, const char* testName);
SK_API void DumpSimplify(FILE* file, const SkPath& path, const char* testName);
SK_API void ReportOpFail(const SkPath& one, const SkPath& two, SkPathOp op);
SK_API void ReportSimplifyFail(const SkPath& path);
SK_API void VerifyOp(const SkPath& one, const SkPath& two, SkPathOp op,
    const SkPath& result);
SK_API void VerifySimplify(const SkPath& path, const SkPath& result);
#endif

// global path dumps for msvs Visual Studio 17 to use from Immediate Window
SK_API void Dump(const SkOpContour& );
SK_API void DumpAll(const SkOpContour& );
SK_API void DumpAngles(const SkOpContour& );
SK_API void DumpContours(const SkOpContour& );
SK_API void DumpContoursAll(const SkOpContour& );
SK_API void DumpContoursAngles(const SkOpContour& );
SK_API void DumpContoursPts(const SkOpContour& );
SK_API void DumpContoursPt(const SkOpContour& , int segmentID);
SK_API void DumpContoursSegment(const SkOpContour& , int segmentID);
SK_API void DumpContoursSpan(const SkOpContour& , int segmentID);
SK_API void DumpContoursSpans(const SkOpContour& );
SK_API void DumpPt(const SkOpContour& , int );
SK_API void DumpPts(const SkOpContour& , const char* prefix = "seg");
SK_API void DumpSegment(const SkOpContour& , int );
SK_API void DumpSegments(const SkOpContour& , const char* prefix = "seg", SkPathOp op = (SkPathOp) -1);
SK_API void DumpSpan(const SkOpContour& , int );
SK_API void DumpSpans(const SkOpContour& );

SK_API void Dump(const SkOpSegment& );
SK_API void DumpAll(const SkOpSegment& );
SK_API void DumpAngles(const SkOpSegment& );
SK_API void DumpCoin(const SkOpSegment& );
SK_API void DumpPts(const SkOpSegment& , const char* prefix = "seg");

SK_API void Dump(const SkOpPtT& );
SK_API void DumpAll(const SkOpPtT& );

SK_API void Dump(const SkOpSpanBase& );
SK_API void DumpCoin(const SkOpSpanBase& );
SK_API void DumpAll(const SkOpSpanBase& );

SK_API void DumpCoin(const SkOpSpan& );
SK_API bool DumpSpan(const SkOpSpan& );

SK_API void Dump(const SkDConic& );
SK_API void DumpID(const SkDConic& , int id);

SK_API void Dump(const SkDCubic& );
SK_API void DumpID(const SkDCubic& , int id);

SK_API void Dump(const SkDLine& );
SK_API void DumpID(const SkDLine& , int id);

SK_API void Dump( const SkDQuad& );
SK_API void DumpID(const SkDQuad& , int id);

SK_API void Dump(const SkDPoint& );

SK_API void Dump(const SkOpAngle& );

// generates tools/path_sorter.htm and path_visualizer.htm compatible data
SK_API void DumpQ(const SkDQuad& quad1, const SkDQuad& quad2, int testNo);
SK_API void DumpT(const SkDQuad& quad, double t);

// global path dumps for msvs Visual Studio 17 to use from Immediate Window
SK_API void Dump( const SkPath& path );
SK_API void DumpHex(const SkPath& path);
