...
struct SkDQuad {
    inline static constexpr int kPointCount = 3;
    inline static constexpr int kPointLast = kPointCount - 1;
    inline static constexpr int kMaxIntersections = 4;
    ...
    SK_API static int AddValidTs(double s[], int realRoots, double* t);
    SK_API void align(int endIndex, SkDPoint* dstPt) const;
    SK_API SkDQuadPair chopAt(double t) const;
    SK_API SkDVector dxdyAtT(double t) const;
    SK_API static int FindExtrema(const double src[], double tValue[1]);
     ...
     */
    SK_API int horizontalIntersect(double yIntercept, double roots[2]) const;

    SK_API bool hullIntersects(const SkDQuad& , bool* isLinear) const;
    SK_API bool hullIntersects(const SkDConic& , bool* isLinear) const;
    SK_API bool hullIntersects(const SkDCubic& , bool* isLinear) const;
    SK_API bool isLinear(int startIndex, int endIndex) const;
    static constexpr int maxIntersections() { return kMaxIntersections; }
    SK_API bool monotonicInX() const;
    SK_API bool monotonicInY() const;
    SK_API void otherPts(int oddMan, const SkDPoint* endPt[2]) const;
    static constexpr int pointCount() { return kPointCount; }
    static constexpr int pointLast() { return kPointLast; }
    SK_API SkDPoint ptAtT(double t) const;
    SK_API static int RootsReal(double A, double B, double C, double t[2]);
    SK_API static int RootsValidT(const double A, const double B, const double C, double s[2]);
    SK_API static void SetABC(const double* quad, double* a, double* b, double* c);
    SK_API SkDQuad subDivide(double t1, double t2) const;
    ...
    SK_API SkDPoint subDivide(const SkDPoint& a, const SkDPoint& c, double t1, double t2) const;
     ...
     */
    SK_API int verticalIntersect(double xIntercept, double roots[2]) const;

    SK_API SkDCubic debugToCubic() const;
...
public:
    ...
    SK_API bool hullIntersects(const SkDCubic& cubic, bool* isLinear) const override;
