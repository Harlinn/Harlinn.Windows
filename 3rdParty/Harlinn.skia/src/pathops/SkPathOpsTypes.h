...
#include <skia/private/SkFloatingPoint.h>
#include <skia/private/SkSafe_math.h>
...
public:
    SK_API SkOpGlobalState(SkOpContourHead* head,
...
#ifdef SK_DEBUG
    SK_API const class SkOpAngle* debugAngle(int id) const;
    SK_API const SkOpCoincidence* debugCoincidence() const;
    SK_API SkOpContour* debugContour(int id) const;
    SK_API const class SkOpPtT* debugPtT(int id) const;
#endif

    SK_API static bool DebugRunFail();

#ifdef SK_DEBUG
    SK_API const class SkOpSegment* debugSegment(int id) const;
    bool debugSkipAssert() const { return fDebugSkipAssert; }
    SK_API const class SkOpSpanBase* debugSpan(int id) const;
    const char* debugTestName() const { return fDebugTestName; }
#endif

#if DEBUG_T_SECT_LOOP_COUNT
    SK_API void debugAddLoopCount(SkIntersections* , const SkIntersectionHelper& ,
        const SkIntersectionHelper& );
    SK_API void debugDoYourWorst(SkOpGlobalState* );
    SK_API void debugLoopReport();
    SK_API void debugResetLoopCounts();
#endif
    ...
    SK_API void debugSetPhase(const char* funcName  DEBUG_COIN_DECLARE_PARAMS()) const;
#endif

#if DEBUG_COIN
    SK_API void debugAddToCoinChangedDict();
    SK_API void debugAddToGlobalCoinDicts();
    ...
    SK_API static void DumpCoinDict();
#endif
...
SK_API bool AlmostEqualUlps(float a, float b);
inline bool AlmostEqualUlps(double a, double b) {
    return AlmostEqualUlps(SkDoubleToScalar(a), SkDoubleToScalar(b));
}

SK_API bool AlmostEqualUlpsNoNormalCheck(float a, float b);
...
SK_API bool AlmostEqualUlps_Pin(float a, float b);
...
SK_API bool AlmostDequalUlps(float a, float b);
SK_API bool AlmostDequalUlps(double a, double b);

SK_API bool NotAlmostEqualUlps(float a, float b);
inline bool NotAlmostEqualUlps(double a, double b) {
    return NotAlmostEqualUlps(SkDoubleToScalar(a), SkDoubleToScalar(b));
}

SK_API bool NotAlmostEqualUlps_Pin(float a, float b);
inline bool NotAlmostEqualUlps_Pin(double a, double b) {
    return NotAlmostEqualUlps_Pin(SkDoubleToScalar(a), SkDoubleToScalar(b));
}

SK_API bool NotAlmostDequalUlps(float a, float b);
...
SK_API int UlpsDistance(float a, float b);
...
inline constexpr double FLT_EPSILON_CUBED = FLT_EPSILON * FLT_EPSILON * FLT_EPSILON;
inline constexpr double FLT_EPSILON_HALF = FLT_EPSILON / 2;
inline constexpr double FLT_EPSILON_DOUBLE = FLT_EPSILON * 2;
inline constexpr double FLT_EPSILON_ORDERABLE_ERR = FLT_EPSILON * 16;
inline constexpr double FLT_EPSILON_SQUARED = FLT_EPSILON * FLT_EPSILON;
// Use a compile-time constant for FLT_EPSILON_SQRT to avoid initializers.
// A 17 digit constant guarantees exact results.
inline constexpr double FLT_EPSILON_SQRT = 0.00034526697709225118; // sqrt(FLT_EPSILON);
inline constexpr double FLT_EPSILON_INVERSE = 1 / FLT_EPSILON;
inline constexpr double DBL_EPSILON_ERR = DBL_EPSILON * 4;  // FIXME: tune -- allow a few bits of error
inline constexpr double DBL_EPSILON_SUBDIVIDE_ERR = DBL_EPSILON * 16;
inline constexpr double ROUGH_EPSILON = FLT_EPSILON * 64;
inline constexpr double MORE_ROUGH_EPSILON = FLT_EPSILON * 256;
inline constexpr double WAY_ROUGH_EPSILON = FLT_EPSILON * 2048;
inline constexpr double BUMP_EPSILON = FLT_EPSILON * 4096;

inline constexpr SkScalar INVERSE_NUMBER_RANGE = FLT_EPSILON_ORDERABLE_ERR;

inline constexpr bool zero_or_one(double x) {
    return x == 0 || x == 1;
}
...
inline constexpr bool approximately_greater(double x, double y) {
    return x - FLT_EPSILON >= y;
}

inline constexpr bool approximately_greater_double(double x, double y) {
    return x - FLT_EPSILON_DOUBLE >= y;
}

inline constexpr bool approximately_greater_orderable(double x, double y) {
    return x - FLT_EPSILON_ORDERABLE_ERR >= y;
}

inline constexpr bool approximately_greater_or_equal(double x, double y) {
    return x + FLT_EPSILON > y;
}

inline constexpr bool approximately_greater_or_equal_double(double x, double y) {
    return x + FLT_EPSILON_DOUBLE > y;
}

inline constexpr bool approximately_greater_or_equal_orderable(double x, double y) {
    return x + FLT_EPSILON_ORDERABLE_ERR > y;
}

inline constexpr bool approximately_lesser(double x, double y) {
    return x + FLT_EPSILON <= y;
}

inline constexpr bool approximately_lesser_double(double x, double y) {
    return x + FLT_EPSILON_DOUBLE <= y;
}

inline constexpr bool approximately_lesser_orderable(double x, double y) {
    return x + FLT_EPSILON_ORDERABLE_ERR <= y;
}

inline constexpr bool approximately_lesser_or_equal(double x, double y) {
    return x - FLT_EPSILON < y;
}

inline constexpr bool approximately_lesser_or_equal_double(double x, double y) {
    return x - FLT_EPSILON_DOUBLE < y;
}

inline constexpr bool approximately_lesser_or_equal_orderable(double x, double y) {
    return x - FLT_EPSILON_ORDERABLE_ERR < y;
}

inline constexpr bool approximately_greater_than_one(double x) {
    return x > 1 - FLT_EPSILON;
}

inline constexpr bool precisely_greater_than_one(double x) {
    return x > 1 - DBL_EPSILON_ERR;
}

inline constexpr bool approximately_less_than_zero(double x) {
    return x < FLT_EPSILON;
}

inline constexpr bool precisely_less_than_zero(double x) {
    return x < DBL_EPSILON_ERR;
}

inline constexpr bool approximately_negative(double x) {
    return x < FLT_EPSILON;
}

inline constexpr bool approximately_negative_orderable(double x) {
    return x < FLT_EPSILON_ORDERABLE_ERR;
}

inline constexpr bool precisely_negative(double x) {
    return x < DBL_EPSILON_ERR;
}

inline constexpr bool approximately_one_or_less(double x) {
    return x < 1 + FLT_EPSILON;
}

inline constexpr bool approximately_one_or_less_double(double x) {
    return x < 1 + FLT_EPSILON_DOUBLE;
}

inline constexpr bool approximately_positive(double x) {
    return x > -FLT_EPSILON;
}

inline constexpr bool approximately_positive_squared(double x) {
    return x > -(FLT_EPSILON_SQUARED);
}

inline constexpr bool approximately_zero_or_more(double x) {
    return x > -FLT_EPSILON;
}

inline constexpr bool approximately_zero_or_more_double(double x) {
    return x > -FLT_EPSILON_DOUBLE;
}

inline constexpr bool approximately_between_orderable(double a, double b, double c) {
    return a <= c
...
inline constexpr bool approximately_between(double a, double b, double c) {
    return a <= c ? approximately_negative(a - b) && approximately_negative(b - c)
            : approximately_negative(b - a) && approximately_negative(c - b);
}

inline constexpr bool precisely_between(double a, double b, double c) {
...
inline constexpr bool roughly_negative(double x) {
    return x < ROUGH_EPSILON;
}

inline constexpr bool roughly_between(double a, double b, double c) {
...
inline constexpr double SkDInterp(double A, double B, double t) {
    return A + (B - A) * t;
}

SK_API double SkDCubeRoot(double x);

/* Returns -1 if negative, 0 if zero, 1 if positive
*/
inline constexpr int SkDSign(double x) {
    return (x > 0) - (x < 0);
}

/* Returns 0 if negative, 1 if zero, 2 if positive
*/
inline constexpr int SKDSide(double x) {
    return (x > 0) + (x >= 0);
}

/* Returns 1 if negative, 2 if zero, 4 if positive
*/
inline constexpr int SkDSideBit(double x) {
    return 1 << SKDSide(x);
}

inline constexpr double SkPinT(double t) {
