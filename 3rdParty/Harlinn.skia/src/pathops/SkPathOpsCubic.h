...
struct SkDCubic {
    ...
    SK_API void align(int endIndex, int ctrlIndex, SkDPoint* dstPt) const;
    SK_API double binarySearch(double min, double max, double axisIntercept, SearchAxis xAxis) const;
    SK_API double calcPrecision() const;
    SK_API SkDCubicPair chopAt(double t) const;
    SK_API static void Coefficients(const double* cubic, double* A, double* B, double* C, double* D);
    SK_API static int ComplexBreak(const SkPoint pts[4], SkScalar* t);
    SK_API int convexHull(char order[kPointCount]) const;

    void debugInit() {
        sk_bzero(fPts, sizeof(fPts));
    }

    SK_API void debugSet(const SkDPoint* pts);

    void dump() const;  // callable from the debugger when the implementation code is linked in
    void dumpID(int id) const;
    void dumpInner() const;
    SK_API SkDVector dxdyAtT(double t) const;
    SK_API bool endsAreExtremaInXOrY() const;
    SK_API static int FindExtrema(const double src[], double tValue[2]);
    SK_API int findInflections(double tValues[2]) const;
    ...
    SK_API int findMaxCurvature(double tValues[]) const;

#ifdef SK_DEBUG
    SkOpGlobalState* globalState() const { return fDebugGlobalState; }
#endif

    SK_API bool hullIntersects(const SkDCubic& c2, bool* isLinear) const;
    SK_API bool hullIntersects(const SkDConic& c, bool* isLinear) const;
    SK_API bool hullIntersects(const SkDQuad& c2, bool* isLinear) const;
    SK_API bool hullIntersects(const SkDPoint* pts, int ptCount, bool* isLinear) const;
    SK_API bool isLinear(int startIndex, int endIndex) const;
    SK_API static int maxIntersections() { return kMaxIntersections; }
    SK_API bool monotonicInX() const;
    SK_API bool monotonicInY() const;
    SK_API void otherPts(int index, const SkDPoint* o1Pts[kPointCount - 1]) const;
    SK_API static int pointCount() { return kPointCount; }
    SK_API static int pointLast() { return kPointLast; }
    SK_API SkDPoint ptAtT(double t) const;
    SK_API static int RootsReal(double A, double B, double C, double D, double t[3]);
    SK_API static int RootsValidT(const double A, const double B, const double C, double D, double s[3]);

    SK_API int searchRoots(double extremes[6], int extrema, double axisIntercept,
                    SearchAxis xAxis, double* validRoots) const;

    SK_API bool toFloatPoints(SkPoint* ) const;
    /**
     *  Return the number of valid roots (0 < root < 1) for this cubic intersecting the
     *  specified horizontal line.
     */
    SK_API int horizontalIntersect(double yIntercept, double roots[3]) const;
    /**
     *  Return the number of valid roots (0 < root < 1) for this cubic intersecting the
     *  specified vertical line.
     */
    SK_API int verticalIntersect(double xIntercept, double roots[3]) const;
...
// add debug only global pointer so asserts can be skipped by fuzzers
    ...
    SK_API SkDCubic subDivide(double t1, double t2) const;
    ...
    SK_API void subDivide(const SkDPoint& a, const SkDPoint& d, double t1, double t2, SkDPoint p[2]) const;
    ...
    SK_API double top(const SkDCubic& dCurve, double startT, double endT, SkDPoint*topPt) const;
    SK_API SkDQuad toQuad() const;
...
public:
...
#endif
    SK_API bool hullIntersects(const SkDQuad& quad, bool* isLinear) const override;
    SK_API bool hullIntersects(const SkDConic& conic, bool* isLinear) const override;
    ...
    SK_API int intersectRay(SkIntersections* i, const SkDLine& line) const override;
    ...
    SK_API void setBounds(SkDRect* ) const override;
