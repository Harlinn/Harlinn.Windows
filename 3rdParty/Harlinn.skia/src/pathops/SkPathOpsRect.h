...
struct SkDRect {
    ...
    SK_API void setBounds(const SkDConic& curve, const SkDConic& sub, double tStart, double tEnd);

    void setBounds(const SkDCubic& curve) {
        setBounds(curve, curve, 0, 1);
    }

    SK_API void setBounds(const SkDCubic& curve, const SkDCubic& sub, double tStart, double tEnd);

    void setBounds(const SkDQuad& curve) {
        setBounds(curve, curve, 0, 1);
    }

    SK_API void setBounds(const SkDQuad& curve, const SkDQuad& sub, double tStart, double tEnd);

    SK_API void setBounds(const SkTCurve& curve);
