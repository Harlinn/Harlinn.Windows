...
public:
    ...
    SK_API static std::unique_ptr<GrFragmentProcessor> MakeColor(SkPMColor4f color);
     ...
     */
    SK_API static std::unique_ptr<GrFragmentProcessor> MulInputByChildAlpha(
     ...
     */
    SK_API static std::unique_ptr<GrFragmentProcessor> ApplyPaintAlpha(
            std::unique_ptr<GrFragmentProcessor> child);
     ...
     */
    SK_API static std::unique_ptr<GrFragmentProcessor> ModulateRGBA(
     ...
     *  child.
     */
    SK_API static std::unique_ptr<GrFragmentProcessor> OverrideInput(std::unique_ptr<GrFragmentProcessor>,
                                                              const SkPMColor4f&);
     ...
     *  optimization.
     */
    SK_API static std::unique_ptr<GrFragmentProcessor> DisableCoverageAsAlpha(
     ...
     */
    SK_API static std::unique_ptr<GrFragmentProcessor> UseDestColorAsInput(
            std::unique_ptr<GrFragmentProcessor>);
     ...
     *  the output.
     */
    SK_API static std::unique_ptr<GrFragmentProcessor> SwizzleOutput(std::unique_ptr<GrFragmentProcessor>,
                                                              const GrSwizzle&);
     ...
     *  the output to [0, 1].
     */
    SK_API static std::unique_ptr<GrFragmentProcessor> ClampOutput(std::unique_ptr<GrFragmentProcessor>);
     ...
     */
    SK_API static std::unique_ptr<GrFragmentProcessor> Compose(std::unique_ptr<GrFragmentProcessor> f,
     ...
     */
    SK_API static std::unique_ptr<GrFragmentProcessor> ColorMatrix(
            std::unique_ptr<GrFragmentProcessor> child,
     ...
     */
    SK_API static std::unique_ptr<GrFragmentProcessor> SurfaceColor();
     ...
     */
    SK_API static std::unique_ptr<GrFragmentProcessor> DeviceSpace(std::unique_ptr<GrFragmentProcessor>);
     ...
     */
    SK_API static std::unique_ptr<GrFragmentProcessor> Rect(std::unique_ptr<GrFragmentProcessor>,
                                                     GrClipEdgeType,
                                                     SkRect);

    SK_API static GrFPResult Circle(std::unique_ptr<GrFragmentProcessor>,
                             GrClipEdgeType,
                             SkPoint center,
                             float radius);

    SK_API static GrFPResult Ellipse(std::unique_ptr<GrFragmentProcessor>,
                              GrClipEdgeType,
                              SkPoint center,
                              SkPoint radii,
                              const GrShaderCaps&);
     ...
     */
    SK_API static std::unique_ptr<GrFragmentProcessor> HighPrecision(std::unique_ptr<GrFragmentProcessor>);
    ...
    SK_API std::unique_ptr<ProgramImpl> makeProgramImpl() const;
    ...
    SK_API int numNonNullChildProcessors() const;
    ...
    SkDEBUGCODE( SK_API bool isInstantiated() const;)
     ...
     */
    SK_API bool isEqual(const GrFragmentProcessor& that) const;

    SK_API void visitProxies(const GrVisitProxyFunc&) const;

    SK_API void visitTextureEffects(const std::function<void(const GrTextureEffect&)>&) const;

    SK_API void visitWithImpls(const std::function<void(const GrFragmentProcessor&, ProgramImpl&)>&,
                        ProgramImpl&) const;

    SK_API GrTextureEffect* asTextureEffect();
    SK_API const GrTextureEffect* asTextureEffect() const;

#if GR_TEST_UTILS
    // Generates debug info for this processor tree by recursively calling dumpInfo() on this
    // processor and its children.
    SK_API SkString dumpTreeInfo() const;
#endif
...
protected:
     ...
     */
    SK_API void registerChild(std::unique_ptr<GrFragmentProcessor> child,
     ...
     */
    SK_API void cloneAndRegisterAllChildProcessors(const GrFragmentProcessor& src);
...
public:
    ...
    SK_API void setData(const GrGLSLProgramDataManager& pdman, const GrFragmentProcessor& processor);
     ...
     */
    SK_API SkString invokeChild(int childIndex,
                         const char* inputColor,
                         const char* destColor,
                         EmitArgs& parentArgs,
                         SkSL::String skslCoords = "");
     ...
     */
    SK_API SkString invokeChildWithMatrix(int childIndex,
                                   const char* inputColor,
                                   const char* destColor,
                                   EmitArgs& parentArgs);
