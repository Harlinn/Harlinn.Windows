...
public:
    SK_API GrAtlasManager(GrProxyProvider*, size_t maxTextureBytes, GrDrawOpAtlas::AllowMultitexturing);
    SK_API ~GrAtlasManager() override;
    ...
    SK_API void addGlyphToBulkAndSetUseToken(GrDrawOpAtlas::BulkUseTokenUpdater*, GrMaskFormat, GrGlyph*,
    ...
    SK_API GrDrawOpAtlas::ErrorCode addToAtlas(GrResourceProvider*, GrDeferredUploadTarget*, GrMaskFormat,
...
#ifdef SK_DEBUG
    SK_API void dump(GrDirectContext*) const;
#endif

    SK_API void setAtlasDimensionsToMinimum_ForTesting();
    SK_API void setMaxPages_TestingOnly(uint32_t maxPages);

private:
    SK_API bool initAtlas(GrMaskFormat);
