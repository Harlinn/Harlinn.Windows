...
public:
     ...
     */
    SK_API GrSemaphoresSubmitted flushSurfaces(
     ...
     */
    SK_API bool validPMUPMConversionExists();

    /**
     * These functions create premul <-> unpremul effects, using specialized round-trip effects.
     */
    SK_API std::unique_ptr<GrFragmentProcessor> createPMToUPMEffect(std::unique_ptr<GrFragmentProcessor>);
    SK_API std::unique_ptr<GrFragmentProcessor> createUPMToPMEffect(std::unique_ptr<GrFragmentProcessor>);
    ...
    SK_API bool compile(const GrProgramDesc&, const GrProgramInfo&);
...
#if GR_TEST_UTILS
    /** Reset GPU stats */
    SK_API void resetGpuStats() const;

    /** Prints cache stats to the string if GR_CACHE_STATS == 1. */
    SK_API void dumpCacheStats(SkString*) const;
    SK_API void dumpCacheStatsKeyValuePairs(SkTArray<SkString>* keys, SkTArray<double>* values) const;
    SK_API void printCacheStats() const;

    /** Prints GPU stats to the string if GR_GPU_STATS == 1. */
    SK_API void dumpGpuStats( SkString* ) const;
    SK_API void dumpGpuStatsKeyValuePairs( SkTArray<SkString>* keys, SkTArray<double>* values ) const;
    SK_API void printGpuStats() const;

    /** These are only active if GR_GPU_STATS == 1. */
    SK_API void resetContextStats();
    SK_API void dumpContextStats(SkString*) const;
    SK_API void dumpContextStatsKeyValuePairs(SkTArray<SkString>* keys, SkTArray<double>* values) const;
    SK_API void printContextStats() const;
    ...
    SK_API sk_sp<SkImage> testingOnly_getFontAtlasImage(GrMaskFormat format, unsigned int index = 0);

    SK_API void testingOnly_flushAndRemoveOnFlushCallbackObject(GrOnFlushCallbackObject*);
#endif
