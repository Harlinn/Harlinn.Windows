...
public:
    ...
    SK_API GrPipeline(GrScissorTest,
               sk_sp<const GrXferProcessor>,
               const GrSwizzle& writeSwizzle,
               InputFlags = InputFlags::kNone);

    SK_API GrPipeline(const InitArgs& args, sk_sp<const GrXferProcessor>, const GrAppliedHardClip&);
    SK_API GrPipeline(const InitArgs&, GrProcessorSet&&, GrAppliedClip&&);
    ...
    SK_API GrXferBarrierType xferBarrierType(const GrCaps&) const;

    // Used by Vulkan and Metal to cache their respective pipeline objects
    SK_API void genKey(GrProcessorKeyBuilder*, const GrCaps&) const;

    const GrSwizzle& writeSwizzle() const { return fWriteSwizzle; }

    SK_API void visitProxies(const GrVisitProxyFunc&) const;

    SK_API void setDstTextureUniforms(const GrGLSLProgramDataManager& pdm,
