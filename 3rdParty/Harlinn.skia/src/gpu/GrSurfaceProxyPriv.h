...
public:
    ...
    // Don't. Just don't.
    SK_API void exactify(bool allocatedCaseOnly);

    void setLazyDimensions(SkISize dimensions) { fProxy->setLazyDimensions(dimensions); }

    SK_API bool doLazyInstantiation(GrResourceProvider*);
...
private:
    ...
    SK_API const GrSurfaceProxyPriv* operator&() const;
    SK_API GrSurfaceProxyPriv* operator&();
