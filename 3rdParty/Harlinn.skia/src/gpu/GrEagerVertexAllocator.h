...
public:
    ...
    SK_API void* lock(size_t stride, int eagerCount) final;

    // Mark "final" as a hint for the compiler to not use the vtable.
    SK_API void unlock(int actualCount) final;
...
public:
    ...
    SK_API void* lock(size_t stride, int eagerCount) override;
    SK_API void unlock(int actualCount) override;

    SK_API sk_sp<GrThreadSafeCache::VertexData> detachVertexData();
