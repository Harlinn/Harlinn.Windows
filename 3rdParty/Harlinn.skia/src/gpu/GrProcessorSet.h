...
public:
    SK_API GrProcessorSet(GrPaint&&);
    SK_API GrProcessorSet(SkBlendMode);
    SK_API GrProcessorSet(std::unique_ptr<GrFragmentProcessor> colorFP);
    SK_API GrProcessorSet(GrProcessorSet&&);
    GrProcessorSet(const GrProcessorSet&) = delete;
    GrProcessorSet& operator=(const GrProcessorSet&) = delete;

    SK_API ~GrProcessorSet();
    ...
    SK_API bool operator==(const GrProcessorSet& that) const;
     ...
     */
    SK_API Analysis finalize(const GrProcessorAnalysisColor&, const GrProcessorAnalysisCoverage,
    ...
    SK_API static const GrProcessorSet& EmptySet();
    SK_API static GrProcessorSet MakeEmptySet();
    static constexpr Analysis EmptySetAnalysis() { return Analysis(Empty::kEmpty); }

#if GR_TEST_UTILS
    SK_API SkString dumpProcessors() const;
#endif
