...
public:
     ...
     */
    SK_API static std::unique_ptr<GrSkSLFP> MakeWithData(
...
private:
    ...
    SK_API GrSkSLFP(sk_sp<SkRuntimeEffect> effect, const char* name, OptFlags optFlags);
    SK_API GrSkSLFP(const GrSkSLFP& other);

    SK_API void addChild(std::unique_ptr<GrFragmentProcessor> child, bool mergeOptFlags);
    SK_API void setInput(std::unique_ptr<GrFragmentProcessor> input);
    SK_API void setDestColorFP(std::unique_ptr<GrFragmentProcessor> destColorFP);
    ...
    SK_API SkPMColor4f constantOutputForConstantInput(const SkPMColor4f&) const override;
