...
public:
    ...
    public:
        SK_API static sk_sp<CpuBufferCache> Make(int maxBuffersToCache);

        SK_API sk_sp<GrCpuBuffer> makeBuffer(size_t size, bool mustBeInitialized);
        SK_API void releaseAll();

    private:
        SK_API CpuBufferCache(int maxBuffersToCache);
     ...
     */
    SK_API void putBack(size_t bytes);
...
protected:
     ...
     *                              GPU buffer objects.
     */
    SK_API GrBufferAllocPool(GrGpu* gpu, GrGpuBufferType bufferType, sk_sp<CpuBufferCache> cpuBufferCache);

    SK_API virtual ~GrBufferAllocPool();
     ...
     */
    SK_API void* makeSpace(size_t size, size_t alignment, sk_sp<const GrBuffer>* buffer, size_t* offset);
     ...
     * @return pointer to where the client should write the data.
     */
    SK_API void* makeSpaceAtLeast(size_t minSize,
                           size_t fallbackSize,
                           size_t alignment,
    ...
    SK_API sk_sp<GrBuffer> getBuffer(size_t size);
...
private:
    ...
    SK_API bool createBlock(size_t requestSize);
    SK_API void destroyBlock();
    SK_API void deleteBlocks();
    SK_API void flushCpuData(const BufferBlock& block, size_t flushSize);
    SK_API void resetCpuData(size_t newSize);
#ifdef SK_DEBUG
    SK_API void validate(bool unusedBlockAllowed = false) const;
#endif
...
public:
     ...
     *                              GPU buffer objects.
     */
    SK_API GrVertexBufferAllocPool(GrGpu* gpu, sk_sp<CpuBufferCache> cpuBufferCache);
     ...
     */
    SK_API void* makeSpace(size_t vertexSize,
                    int vertexCount,
                    sk_sp<const GrBuffer>* buffer,
                    int* startVertex);
     ...
     * @return pointer to first vertex.
     */
    SK_API void* makeSpaceAtLeast(size_t vertexSize,
                           int minVertexCount,
...
public:
     ...
     *                              GPU buffer objects.
     */
    SK_API GrIndexBufferAllocPool(GrGpu* gpu, sk_sp<CpuBufferCache> cpuBufferCache);
     ...
     */
    SK_API void* makeSpace(int indexCount, sk_sp<const GrBuffer>* buffer, int* startIndex);
     ...
     * @return pointer to first index.
     */
    SK_API void* makeSpaceAtLeast(int minIndexCount,
