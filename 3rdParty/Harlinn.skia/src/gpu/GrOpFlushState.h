...
public:
    ...
    SK_API void doUpload(GrDeferredTextureUploadFn&, bool shouldPrepareSurfaceForSampling = false);

    /** Called as ops are executed. Must be called in the same order as the ops were prepared. */
    SK_API void executeDrawsAndUploadsForMeshDrawOp(const GrOp* op, const SkRect& chainBounds,
    ...
    SK_API GrDeferredUploadToken addInlineUpload(GrDeferredTextureUploadFn&&) final;
    SK_API GrDeferredUploadToken addASAPUpload(GrDeferredTextureUploadFn&&) final;

    /** Overrides of GrMeshDrawTarget. */
    SK_API void recordDraw(const GrGeometryProcessor*,
                    const GrSimpleMesh[],
                    int meshCnt,
                    const GrSurfaceProxy* const primProcProxies[],
                    GrPrimitiveType) final;
    SK_API void* makeVertexSpace(size_t vertexSize, int vertexCount, sk_sp<const GrBuffer>*,
                          int* startVertex) final;
    SK_API uint16_t* makeIndexSpace(int indexCount, sk_sp<const GrBuffer>*, int* startIndex) final;
    SK_API void* makeVertexSpaceAtLeast(size_t vertexSize, int minVertexCount, int fallbackVertexCount,
                                 sk_sp<const GrBuffer>*, int* startVertex,
                                 int* actualVertexCount) final;
    SK_API uint16_t* makeIndexSpaceAtLeast(int minIndexCount, int fallbackIndexCount,
    ...
    SK_API void putBackIndices(int indexCount) final;
    SK_API void putBackVertices(int vertices, size_t vertexStride) final;
    ...
    SK_API GrAppliedClip detachAppliedClip() final;
    ...
    SK_API const GrCaps& caps() const final;
    GrThreadSafeCache* threadSafeCache() const final;
    GrResourceProvider* resourceProvider() const final { return fResourceProvider; }

    SK_API GrStrikeCache* strikeCache() const final;
    ...
    SK_API GrAtlasManager* atlasManager() const final;
    SK_API skgpu::v1::SmallPathAtlasMgr* smallPathAtlasManager() const final;
    ...
    SK_API void drawMesh(const GrSimpleMesh& mesh);
