...
public:
    ...
    SK_API void addOp(const GrOp*, GrRenderTargetProxy::UniqueID proxyID);

    SK_API void opsCombined(const GrOp* consumer, const GrOp* consumed);
    ...
    SK_API void toJson(SkJSONWriter& writer) const;

    // returns a json string of all of the ops associated with a given client id
    SK_API void toJson(SkJSONWriter& writer, int clientID) const;
    ...
    SK_API void getBoundsByClientID(SkTArray<OpInfo>* outInfo, int clientID);
    SK_API void getBoundsByOpsTaskID(OpInfo* outInfo, int opsTaskID);

    SK_API void fullReset();
