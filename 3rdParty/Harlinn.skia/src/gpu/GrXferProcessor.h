...
public:
     ...
     * key.
     */
    SK_API void addToKey(const GrShaderCaps&,
     ...
     * source blending.
     */
    SK_API bool hasSecondaryOutput() const;
...
protected:
    SK_API GrXferProcessor(ClassID classID);
    SK_API GrXferProcessor(ClassID classID, bool willReadDstColor, GrProcessorAnalysisCoverage);
...
public:
    ...
    SK_API static sk_sp<const GrXferProcessor> MakeXferProcessor(const GrXPFactory*,
                                                          const GrProcessorAnalysisColor&,
                                                          GrProcessorAnalysisCoverage,
                                                          const GrCaps& caps,
                                                          GrClampType);

    SK_API static AnalysisProperties GetAnalysisProperties(const GrXPFactory*,
                                                    const GrProcessorAnalysisColor&,
...
public:
     ...
     */
    SK_API void emitCode(const EmitArgs&);
    ...
    /** A ProgramImpl instance can be reused with any GrXferProcessor that produces the same key.
        ...
        ProgramImpl.
     */
    SK_API void setData(const GrGLSLProgramDataManager& pdm, const GrXferProcessor& xp);

protected:
    ProgramImpl() = default;

    SK_API static void DefaultCoverageModulation(GrGLSLXPFragmentBuilder* fragBuilder,
...
private:
    ...
    SK_API virtual void emitWriteSwizzle(GrGLSLXPFragmentBuilder*,
                                  const GrSwizzle&,
                                  const char* outColor,
