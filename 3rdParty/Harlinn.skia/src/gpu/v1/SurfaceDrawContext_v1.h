...
public:
    SK_API static std::unique_ptr<SurfaceDrawContext> Make(GrRecordingContext*,
                                                    GrColorType,
    ...
    SK_API static std::unique_ptr<SurfaceDrawContext> MakeWithFallback(
            GrRecordingContext*,
            GrColorType,
            sk_sp<SkColorSpace>,
            SkBackingFit,
    ...
    SK_API static std::unique_ptr<SurfaceDrawContext> MakeFromBackendTexture(
            GrRecordingContext*,
            GrColorType,
            sk_sp<SkColorSpace>,
    ...
    SK_API SurfaceDrawContext(GrRecordingContext*,
    ...
    SK_API ~SurfaceDrawContext() override;

    /**
     *  Draw everywhere (respecting the clip) with the paint.
     */
    SK_API void drawPaint(const GrClip*, GrPaint&&, const SkMatrix& viewMatrix);
     ...
     */
    SK_API void drawRect(const GrClip*,
                  GrPaint&& paint,
                  GrAA,
     ...
     */
    SK_API void fillRectToRect(const GrClip*,
                        GrPaint&&,
                        GrAA,
                        const SkMatrix&,
    ...
    SK_API void drawQuadSet(const GrClip* clip, GrPaint&& paint, GrAA aa, const SkMatrix& viewMatrix,
     ...
     * device space.
     */
    SK_API void drawTexture(const GrClip*,
                     GrSurfaceProxyView,
                     SkAlphaType,
     ...
     */
    SK_API void drawTextureSet(const GrClip*,
                        GrTextureSetEntry[],
                        int cnt,
                        int proxyRunCnt,
                        GrSamplerState::Filter,
     ...
     */
    SK_API void drawRRect(const GrClip*,
                   GrPaint&&,
                   GrAA,
                   const SkMatrix& viewMatrix,
                   const SkRRect& rrect,
     ...
     */
    SK_API bool drawFastShadow(const GrClip*,
                        const SkMatrix& viewMatrix,
                        const SkPath& path,
     ...
     */
    SK_API void drawPath(const GrClip*,
                  GrPaint&&,
                  GrAA,
                  const SkMatrix& viewMatrix,
                  const SkPath&,
     ...
     */
    SK_API void drawShape(const GrClip*,
                   GrPaint&&,
                   GrAA,
                   const SkMatrix& viewMatrix,
                   GrStyledShape&&);
     ...
     */
    SK_API void drawVertices(const GrClip*,
                      GrPaint&& paint,
     ...
     */
    SK_API void drawAtlas(const GrClip*,
                   GrPaint&& paint,
                   const SkMatrix& viewMatrix,
                   int spriteCount,
     ...
     */
    SK_API void drawRegion(const GrClip*,
                    GrPaint&& paint,
                    GrAA aa,
                    const SkMatrix& viewMatrix,
     ...
     */
    SK_API void drawOval(const GrClip*,
                  GrPaint&& paint,
                  GrAA,
                  const SkMatrix& viewMatrix,
                  const SkRect& oval,
     ...
     */
    SK_API void drawArc(const GrClip*,
                 GrPaint&& paint,
                 GrAA,
                 const SkMatrix& viewMatrix,
                 const SkRect& oval,
     ...
     */
    SK_API void drawImageLattice(const GrClip*,
                          GrPaint&&,
                          const SkMatrix& viewMatrix,
                          GrSurfaceProxyView,
     ...
     */
    SK_API void drawGlyphRunList(const GrClip*,
     ...
     * @param glyphRunList    text, text positions, and paint.
     */
    SK_API void drawGlyphRunListWithCache(const GrClip*,
                                   const SkMatrixProvider& viewMatrix,
                                   const SkGlyphRunList& glyphRunList,
                                   const SkPaint& paint);
     ...
     * @param glyphRunList    text, text positions, and paint.
     */
    SK_API void drawGlyphRunListNoCache(const GrClip*,
                                 const SkMatrixProvider& viewMatrix,
                                 const SkGlyphRunList& glyphRunList,
                                 const SkPaint& paint);
     ...
     * command stream.
     */
    SK_API void drawDrawable(std::unique_ptr<SkDrawable::GpuDrawHandler>, const SkRect& bounds);

    // called to note the last clip drawn to the stencil buffer.
    // TODO: remove after clipping overhaul.
    SK_API void setLastClip(uint32_t clipStackGenID,
                     const SkIRect& devClipBounds,
                     int numClipAnalyticElements);

    // called to determine if we have to render the clip into SB.
    // TODO: remove after clipping overhaul.
    SK_API bool mustRenderClip(uint32_t clipStackGenID,
                        const SkIRect& devClipBounds,
                        int numClipAnalyticElements);
    ...
    SK_API bool stencilPath(const GrHardClip*,
                     GrAA doStencilMSAA,
                     const SkMatrix& viewMatrix,
                     const SkPath&);
     ...
     */
    SK_API bool drawAndStencilPath(const GrHardClip*,
    ...
    SK_API SkBudgeted isBudgeted() const;

    SK_API int maxWindowRectangles() const;
    ...
    SK_API void addDrawOp(const GrClip*,
                   GrOp::Owner,
     ...
     */
    SK_API bool waitOnSemaphores(int numSemaphores, const GrBackendSemaphore waitSemaphores[],
...
private:
    ...
    SK_API void willReplaceOpsTask(OpsTask* prevTask, OpsTask* nextTask) override;

    SK_API OpsTask::CanDiscardPreviousOps canDiscardPreviousOpsOnFullClear() const override;
    SK_API void setNeedsStencil();

    SK_API void internalStencilClear(const SkIRect* scissor, bool insideStencilMask);
    ...
    SK_API QuadOptimization attemptQuadOptimization(const GrClip* clip,
    ...
    SK_API void drawFilledQuad(const GrClip* clip,
                        GrPaint&& paint,
                        GrAA aa,
                        DrawQuad* quad,
                        const GrUserStencilSettings* ss = nullptr);
    ...
    SK_API void drawTexturedQuad(const GrClip* clip,
    ...
    SK_API void drawStrokedLine(const GrClip*, GrPaint&&, GrAA, const SkMatrix&, const SkPoint[2],
                         const SkStrokeRec&);

    // Tries to detect if the given shape is a simple, and draws it without path rendering if
    // we know how.
    SK_API bool drawSimpleShape(const GrClip*, GrPaint*, GrAA, const SkMatrix&, const GrStyledShape&);
    ...
    // one more time.
    SK_API void drawShapeUsingPathRenderer(const GrClip*, GrPaint&&, GrAA, const SkMatrix&,
    ...
    SK_API bool SK_WARN_UNUSED_RESULT setupDstProxyView(const SkRect& opBounds,
                                                 bool opRequiresMSAA,
                                                 GrDstProxyView* result);

    SK_API OpsTask* replaceOpsTaskIfModifiesColor();
