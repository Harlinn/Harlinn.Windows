...
public:
    ...
    SK_API ClipStack(const SkIRect& deviceBounds, const SkMatrixProvider* matrixProvider, bool forceAA);

    SK_API ~ClipStack() override;
    ...
    SK_API void clipShader(sk_sp<SkShader> shader);

    SK_API void replaceClip(const SkIRect& rect);

    // GrClip implementation
    SK_API GrClip::Effect apply(GrRecordingContext*, skgpu::v1::SurfaceDrawContext*, GrDrawOp*, GrAAType,
                         GrAppliedClip*, SkRect* bounds) const override;
    SK_API GrClip::PreClipResult preApply(const SkRect& drawBounds, GrAA aa) const override;
    SK_API SkIRect getConservativeBounds() const override;
...
private:
    ...
    public:
        ...
        SK_API RawElement(const SkMatrix& localToDevice, const GrShape& shape, GrAA aa, SkClipOp op);

        // Common clip type interface
        SkClipOp        op() const { return fOp; }
        const SkIRect&  outerBounds() const { return fOuterBounds; }
        SK_API bool            contains(const SaveRecord& s) const;
        SK_API bool            contains(const Draw& d) const;
        SK_API bool            contains(const RawElement& e) const;
        ...
        SK_API ClipState       clipType() const;
        ...
        SK_API void markInvalid(const SaveRecord& current);
        SK_API void restoreValid(const SaveRecord& current);
        ...
        SK_API void updateForElement(RawElement* added, const SaveRecord& current);

        SK_API void simplify(const SkIRect& deviceBounds, bool forceAA);

    private:
        SK_API bool combine(const RawElement& other, const SaveRecord& current);
    ...
    public:
        ...
        SK_API Mask(const SaveRecord& current, const SkIRect& bounds);
        ...
        SK_API bool appliesToDraw(const SaveRecord& current, const SkIRect& drawBounds) const;
        SK_API void invalidate(GrProxyProvider* proxyProvider);
    ...
    public:
        ...
        SK_API explicit SaveRecord(const SkIRect& deviceBounds);

        SK_API SaveRecord(const SaveRecord& prior, int startingMaskIndex, int startingElementIndex);
        ...
        SK_API ClipState       state() const;
        SK_API uint32_t        genID() const;
        ...
        // (e.g. turned it empty).
        SK_API bool addElement(RawElement&& toAdd, RawElement::Stack* elements);

        SK_API void addShader(sk_sp<SkShader> shader);
        SK_API void reset(const SkIRect& bounds);
        ...
        SK_API void removeElements(RawElement::Stack* elements);

        // Restore element validity now that this record is the new top of the stack.
        SK_API void restoreElements(RawElement::Stack* elements);

        SK_API void invalidateMasks(GrProxyProvider* proxyProvider, Mask::Stack* masks);

    private:
        // These functions modify 'elements' and element-dependent state of the record
        // (such as valid index and fState).
        SK_API bool appendElement(RawElement&& toAdd, RawElement::Stack* elements);
        SK_API void replaceWithElement(RawElement&& toAdd, RawElement::Stack* elements);
    ...
    SK_API void clip(RawElement&& element);
    ...
    SK_API SaveRecord& writableSaveRecord(bool* wasDeferred);
    ...
    SK_API static GrFPResult GetSWMaskFP(GrRecordingContext* context, Mask::Stack* masks,
