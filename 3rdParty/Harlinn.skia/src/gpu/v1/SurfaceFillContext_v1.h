...
public:
    SK_API SurfaceFillContext(GrRecordingContext*,
                       GrSurfaceProxyView readView,
                       GrSurfaceProxyView writeView,
                       const GrColorInfo&,
                       bool flushTimeOpsTask = false);

    SK_API void discard() override;

    SK_API void fillRectWithFP(const SkIRect& dstRect, std::unique_ptr<GrFragmentProcessor> fp) override;

    SK_API bool blitTexture(GrSurfaceProxyView view,
                     const SkIRect& srcRect,
                     const SkIPoint& dstPoint) override;

    SK_API OpsTask* getOpsTask();
    SK_API sk_sp<GrRenderTask> refRenderTask() override;
...
protected:
     ...
     */
    SK_API static void ClearToGrPaint(std::array<float, 4> color, GrPaint* paint);

    SK_API void addOp(GrOp::Owner);

    SK_API OpsTask* replaceOpsTask();
...
private:
    ...
    SK_API void internalClear(const SkIRect* scissor,
                       std::array<float, 4> color,
                       bool upgradePartialToFull = false) override;

    SK_API void addDrawOp(GrOp::Owner);
