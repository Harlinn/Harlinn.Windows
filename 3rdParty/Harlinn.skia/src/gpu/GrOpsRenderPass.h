...
public:
    ...
    SK_API void bindPipeline(const GrProgramInfo&, const SkRect& drawBounds);
    ...
    // pipeline.
    SK_API void setScissorRect(const SkIRect&);
    ...
    SK_API void bindTextures(const GrGeometryProcessor&,
                      const GrSurfaceProxy* const geomProcTextures[],
                      const GrPipeline&);

    SK_API void bindBuffers(sk_sp<const GrBuffer> indexBuffer, sk_sp<const GrBuffer> instanceBuffer,
    ...
    SK_API void draw(int vertexCount, int baseVertex);
    SK_API void drawIndexed(int indexCount, int baseIndex, uint16_t minIndexValue, uint16_t maxIndexValue,
                     int baseVertex);

    // Requires caps.drawInstancedSupport().
    SK_API void drawInstanced(int instanceCount, int baseInstance, int vertexCount, int baseVertex);

    // Requires caps.drawInstancedSupport().
    SK_API void drawIndexedInstanced(int indexCount, int baseIndex, int instanceCount, int baseInstance,
                              int baseVertex);
    ...
    SK_API void drawIndirect(const GrBuffer* drawIndirectBuffer, size_t bufferOffset, int drawCount);
    ...
    // If caps.nativeDrawIndirectSupport() is unavailable, then 'drawIndirectBuffer' must be a
    // GrCpuBuffer in order to polyfill. Performance may suffer in this scenario.
    SK_API void drawIndexedIndirect(const GrBuffer* drawIndirectBuffer, size_t bufferOffset,
                             int drawCount);
    ...
    SK_API void drawIndexPattern(int patternIndexCount, int patternRepeatCount,
     ...
     */
    SK_API void clear(const GrScissorState& scissor, std::array<float, 4> color);

    /**
     * Same as clear() but modifies the stencil; check caps.performStencilClearsAsDraws() and
     * caps.performPartialClearsAsDraws().
     */
    SK_API void clearStencilClip(const GrScissorState& scissor, bool insideStencilMask);

    /**
     * Executes the SkDrawable object for the underlying backend.
     */
    SK_API void executeDrawable(std::unique_ptr<SkDrawable::GpuDrawHandler>);
