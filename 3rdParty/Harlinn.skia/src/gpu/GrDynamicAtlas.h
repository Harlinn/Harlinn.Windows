...
public:
    ...
    SK_API static sk_sp<GrTextureProxy> MakeLazyAtlasProxy(LazyInstantiateAtlasCallback&&,
    ...
    SK_API GrDynamicAtlas(GrColorType colorType, InternalMultisample, SkISize initialSize,
                   int maxAtlasSize, const GrCaps&,
                   RectanizerAlgorithm = RectanizerAlgorithm::kSkyline);
    SK_API virtual ~GrDynamicAtlas();

    SK_API void reset(SkISize initialSize, const GrCaps& desc);

    GrColorType colorType() const { return fColorType; }
    int maxAtlasSize() const { return fMaxAtlasSize; }
    GrTextureProxy* textureProxy() const { return fTextureProxy.get(); }
    SK_API GrSurfaceProxyView readView(const GrCaps&) const;
    SK_API GrSurfaceProxyView writeView(const GrCaps&) const;
    ...
    SK_API void instantiate(GrOnFlushResourceProvider*, sk_sp<GrTexture> backingTexture = nullptr);

private:
    class Node;

    SK_API Node* makeNode(Node* previous, int l, int t, int r, int b);
    SK_API bool internalPlaceRect(int w, int h, SkIPoint16* loc);
