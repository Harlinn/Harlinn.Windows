...
public:
     ...
     */
    SK_API static std::unique_ptr<GrDrawOpAtlas> Make(GrProxyProvider*,
    ...
    SK_API ErrorCode addToAtlas(GrResourceProvider*, GrDeferredUploadTarget*,
    ...
    SK_API void compact(GrDeferredUploadToken startTokenForNextFlush);

    SK_API void instantiate(GrOnFlushResourceProvider*);

    uint32_t maxPages() const {
        return fMaxPages;
    }

    SK_API int numAllocated_TestingOnly() const;
    SK_API void setMaxPages_TestingOnly(uint32_t maxPages);

private:
    SK_API GrDrawOpAtlas(GrProxyProvider*, const GrBackendFormat& format, GrColorType, int width,
    ...
    public:
        ...
        SK_API void uploadToTexture(GrDeferredTextureUploadWritePixelsFn&, GrTextureProxy*);
        SK_API void resetRects();
    ...
    private:
        SK_API Plot(int pageIndex, int plotIndex, GenerationCounter* generationCounter,
             int offX, int offY, int width, int height, GrColorType colorType);

        SK_API ~Plot() override;
    ...
    SK_API bool uploadToPage(unsigned int pageIdx, GrDeferredUploadTarget*, int width, int height,
                      const void* image, AtlasLocator*);

    SK_API bool createPages(GrProxyProvider*, GenerationCounter*);
    SK_API bool activateNewPage(GrResourceProvider*);
    SK_API void deactivateLastPage();

    SK_API void processEviction(PlotLocator);
...
public:
    ...
    SK_API GrDrawOpAtlasConfig(int maxTextureSize, size_t maxBytes);
    ...
    SK_API SkISize atlasDimensions(GrMaskFormat type) const;
    SK_API SkISize plotDimensions(GrMaskFormat type) const;
