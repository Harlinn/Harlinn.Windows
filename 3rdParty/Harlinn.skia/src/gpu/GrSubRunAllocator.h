...
class GrBagOfBytes {
public:
    SK_API GrBagOfBytes(char* block, size_t blockSize, size_t firstHeapAllocation);
    SK_API explicit GrBagOfBytes(size_t firstHeapAllocation = 0);
    SK_API ~GrBagOfBytes();
    ...
    SK_API void* alignedBytes(int unsafeSize, int unsafeAlignment);
...
private:
    ...
    struct Block {
        SK_API Block(char* previous, char* startOfBlock);
    ...
    SK_API void setupBytesAndCapacity(char* bytes, int size);

    // Adjust fEndByte and fCapacity to satisfy the size and alignment request.
    SK_API void needMoreBytes(int size, int alignment);
...
public:
    ...
    SK_API GrSubRunAllocator(char* block, int blockSize, int firstHeapAllocation);
    SK_API explicit GrSubRunAllocator(int firstHeapAllocation = 0);
    ...
    SK_API void* alignedBytes(int size, int alignment);
