...
public:
     ...
     */
    SK_API static std::unique_ptr<GrMemoryPool> Make(size_t preallocSize, size_t minAllocSize);
     ...
     */
    SK_API void* allocate(size_t size);
    /**
     * p must have been returned by allocate().
     */
    SK_API void release(void* p);
...
private:
    ...
    SK_API GrMemoryPool(size_t preallocSize, size_t minAllocSize);
