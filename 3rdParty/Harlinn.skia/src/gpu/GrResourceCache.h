...
public:
    SK_API GrResourceCache(GrSingleOwner* owner,
                    GrDirectContext::DirectContextID owningContextID,
                    uint32_t familyID);
    SK_API ~GrResourceCache();
    ...
    SK_API ResourceAccess resourceAccess();
    ...
    SK_API void setLimit(size_t bytes);
     ...
     * the cache.
     */
    SK_API void abandonAll();

    /**
     * Releases the backend API resources owned by all GrGpuResource objects and removes them from
     * the cache.
     */
    SK_API void releaseAll();

    /**
     * Find a resource that matches a scratch key.
     */
    SK_API GrGpuResource* findAndRefScratchResource(const GrScratchKey& scratchKey);
     ...
     */
    SK_API bool purgeToMakeHeadroom(size_t desiredHeadroomBytes);
    ...
    /** Returns true if the cache would like a flush to occur in order to make more resources
        purgeable. */
    SK_API bool requestsFlush() const;

    /** Maintain a ref to this texture until we receive a GrTextureFreedMessage. */
    SK_API void insertDelayedTextureUnref(GrTexture*);
    ...
    SK_API void getStats(Stats*) const;

#if GR_TEST_UTILS
    SK_API void dumpStats(SkString*) const;

    SK_API void dumpStatsKeyValuePairs(SkTArray<SkString>* keys, SkTArray<double>* value) const;
#endif

#endif // GR_CACHE_STATS

#if GR_TEST_UTILS
    SK_API int countUniqueKeysWithTag(const char* tag) const;

    SK_API void changeTimestamp(uint32_t newTimestamp);
#endif
...
private:
    ...
    ////
    SK_API void insertResource(GrGpuResource*);
    SK_API void removeResource(GrGpuResource*);
    SK_API void notifyARefCntReachedZero(GrGpuResource*, GrGpuResource::LastRemovedRef);
    SK_API void changeUniqueKey(GrGpuResource*, const GrUniqueKey&);
    SK_API void removeUniqueKey(GrGpuResource*);
    SK_API void willRemoveScratchKey(const GrGpuResource*);
    SK_API void didChangeBudgetStatus(GrGpuResource*);
    SK_API void refResource(GrGpuResource* resource);
    /// @}

    SK_API void refAndMakeResourceMRU(GrGpuResource*);
    SK_API void processFreedGpuResources();
    SK_API void addToNonpurgeableArray(GrGpuResource*);
    SK_API void removeFromNonpurgeableArray(GrGpuResource*);

    bool wouldFit(size_t bytes) const { return fBudgetedBytes+bytes <= fMaxBytes; }

    SK_API uint32_t getNextTimestamp();

    SK_API void purgeUnlockedResources(const GrStdSteadyClock::time_point* purgeTime,
                                bool scratchResourcesOnly);

#ifdef SK_DEBUG
    SK_API bool isInCache(const GrGpuResource* r) const;
    ...
    public:
        SK_API TextureAwaitingUnref();
        SK_API TextureAwaitingUnref(GrTexture* texture);
        TextureAwaitingUnref(const TextureAwaitingUnref&) = delete;
        TextureAwaitingUnref& operator=(const TextureAwaitingUnref&) = delete;
        SK_API TextureAwaitingUnref(TextureAwaitingUnref&&);
        SK_API TextureAwaitingUnref& operator=(TextureAwaitingUnref&&);
        SK_API ~TextureAwaitingUnref();
        SK_API void addRef();
        SK_API void unref();
        SK_API bool finished();
