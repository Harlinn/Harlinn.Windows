...
public:
    ...
    SK_API GrShape& operator=(const GrShape& shape);
    ...
    SK_API uint32_t stateKey() const;
    ...
    SK_API bool simplify(unsigned flags = kAll_Flags);

    // True if the given bounding box is completely inside the shape, if it's conservatively treated
    // as a filled, closed shape.
    SK_API bool conservativeContains(const SkRect& rect) const;
    SK_API bool conservativeContains(const SkPoint& point) const;
    ...
    SK_API bool convex(bool simpleFill = true) const;

    // The bounding box of the shape.
    SK_API SkRect bounds() const;

    // The segment masks that describe the shape, were it to be converted to an SkPath
    SK_API uint32_t segmentMask() const;

    // Convert the shape into a path that describes the same geometry.
    SK_API void asPath(SkPath* out, bool simpleFill = true) const;
...
private:
    ...
    SK_API bool simplifyPath(unsigned flags);
    SK_API bool simplifyArc(unsigned flags);
    ...
    SK_API void simplifyLine(const SkPoint& p1, const SkPoint& p2, unsigned flags);
    SK_API void simplifyPoint(const SkPoint& point, unsigned flags);

    // RRects and rects care about winding for path effects and will set the path winding state
    // of the shape as well.
    SK_API void simplifyRRect(const SkRRect& rrect, SkPathDirection dir, unsigned start, unsigned flags);
    SK_API void simplifyRect(const SkRect& rect, SkPathDirection dir, unsigned start, unsigned flags);
