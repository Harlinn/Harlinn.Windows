...
protected:
    ...
    SK_API void pathToContours(float tolerance, const SkRect& clipBounds, VertexList* contours,
                        bool* isLinear) const;

    // 2) Build a mesh of edges connecting the vertices:
    SK_API void contoursToMesh(VertexList* contours, int contourCnt, VertexList* mesh,
                        const Comparator&) const;

    // 3) Sort the vertices in Y (and secondarily in X):
    SK_API static void SortedMerge(VertexList* front, VertexList* back, VertexList* result,
                            const Comparator&);
    SK_API static void SortMesh(VertexList* vertices, const Comparator&);
    ...
    SK_API SimplifyResult simplify(VertexList* mesh, const Comparator&) const;

    // 5) Tessellate the simplified mesh into monotone polygons:
    SK_API virtual Poly* tessellate(const VertexList& vertices, const Comparator&) const;

    // 6) Triangulate the monotone polygons directly into a vertex buffer:
    SK_API void* polysToTriangles(Poly* polys, void* data, SkPathFillType overrideFillType) const;
    ...
    SK_API void* emitMonotonePoly(const MonotonePoly*, void* data) const;
    SK_API void* emitTriangle(Vertex* prev, Vertex* curr, Vertex* next, int winding, void* data) const;
    SK_API void* emitPoly(const Poly*, void *data) const;
    SK_API Poly* makePoly(Poly** head, Vertex* v, int winding) const;
    SK_API void appendPointToContour(const SkPoint& p, VertexList* contour) const;
    SK_API void appendQuadraticToContour(const SkPoint[3], SkScalar toleranceSqd,
                                  VertexList* contour) const;
    SK_API void generateCubicPoints(const SkPoint&, const SkPoint&, const SkPoint&, const SkPoint&,
                             SkScalar tolSqd, VertexList* contour, int pointsLeft) const;
    SK_API bool applyFillType(int winding) const;
    SK_API Edge* makeEdge(Vertex* prev, Vertex* next, EdgeType type, const Comparator&) const;
    SK_API void setTop(Edge* edge, Vertex* v, EdgeList* activeEdges, Vertex** current,
                const Comparator&) const;
    SK_API void setBottom(Edge* edge, Vertex* v, EdgeList* activeEdges, Vertex** current,
                   const Comparator&) const;
    SK_API void mergeEdgesAbove(Edge* edge, Edge* other, EdgeList* activeEdges, Vertex** current,
                         const Comparator&) const;
    SK_API void mergeEdgesBelow(Edge* edge, Edge* other, EdgeList* activeEdges, Vertex** current,
                         const Comparator&) const;
    SK_API Edge* makeConnectingEdge(Vertex* prev, Vertex* next, EdgeType, const Comparator&,
                             int windingScale = 1) const;
    SK_API void mergeVertices(Vertex* src, Vertex* dst, VertexList* mesh, const Comparator&) const;
    SK_API static void FindEnclosingEdges(Vertex* v, EdgeList* edges, Edge** left, Edge** right);
    SK_API void mergeCollinearEdges(Edge* edge, EdgeList* activeEdges, Vertex** current,
                             const Comparator&) const;
    SK_API bool splitEdge(Edge* edge, Vertex* v, EdgeList* activeEdges, Vertex** current,
                   const Comparator&) const;
    SK_API bool intersectEdgePair(Edge* left, Edge* right, EdgeList* activeEdges, Vertex** current,
                           const Comparator&) const;
    SK_API Vertex* makeSortedVertex(const SkPoint&, uint8_t alpha, VertexList* mesh, Vertex* reference,
                             const Comparator&) const;
    SK_API void computeBisector(Edge* edge1, Edge* edge2, Vertex*) const;
    SK_API bool checkForIntersection(Edge* left, Edge* right, EdgeList* activeEdges, Vertex** current,
                              VertexList* mesh, const Comparator&) const;
    SK_API void sanitizeContours(VertexList* contours, int contourCnt) const;
    SK_API bool mergeCoincidentVertices(VertexList* mesh, const Comparator&) const;
    SK_API void buildEdges(VertexList* contours, int contourCnt, VertexList* mesh,
                    const Comparator&) const;
    SK_API Poly* contoursToPolys(VertexList* contours, int contourCnt) const;
    SK_API Poly* pathToPolys(float tolerance, const SkRect& clipBounds,
                      bool* isLinear) const;
    SK_API static int64_t CountPoints(Poly* polys, SkPathFillType overrideFillType);
    SK_API int polysToTriangles(Poly*, GrEagerVertexAllocator*) const;
...
struct GrTriangulator::VertexList {
    ...
    Vertex* fHead;
    Vertex* fTail;
    SK_API void insert(Vertex* v, Vertex* prev, Vertex* next);
...
struct GrTriangulator::Edge {
    ...
    SK_API void insertAbove(Vertex*, const Comparator&);
    SK_API void insertBelow(Vertex*, const Comparator&);
    SK_API void disconnect();
    SK_API bool intersect(const Edge& other, SkPoint* p, uint8_t* alpha = nullptr) const;
};
...
struct GrTriangulator::EdgeList {
    ...
    Edge* fHead;
    Edge* fTail;
    SK_API void insert(Edge* edge, Edge* prev, Edge* next);
    SK_API void insert(Edge* edge, Edge* prev);
    SK_API void append(Edge* e) { insert(e, fTail, nullptr); }
    SK_API void remove(Edge* edge);
    void removeAll() {
        while (fHead) {
            this->remove(fHead);
        }
...
struct GrTriangulator::Poly {
    SK_API Poly(Vertex* v, int winding);

    SK_API Poly* addEdge(Edge* e, Side side, SkArenaAlloc* alloc);
...
struct GrTriangulator::Comparator {
    ...
    SK_API bool sweep_lt(const SkPoint& a, const SkPoint& b) const;
