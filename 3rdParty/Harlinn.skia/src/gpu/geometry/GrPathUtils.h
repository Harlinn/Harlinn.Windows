...
// in subsequent math.
SK_API SkScalar scaleToleranceToSrc(SkScalar devTol,
...
SK_API uint32_t quadraticPointCount(const SkPoint points[], SkScalar tol);

// Returns the number of points actually written to 'points', will be <= to 'pointsLeft'
SK_API uint32_t generateQuadraticPoints(const SkPoint& p0,
                                 const SkPoint& p1,
                                 const SkPoint& p2,
                                 SkScalar tolSqd,
                                 SkPoint** points,
...
// This is a power of two and will not exceed kMaxPointsPerCurve.
SK_API uint32_t cubicPointCount(const SkPoint points[], SkScalar tol);

// Returns the number of points actually written to 'points', will be <= to 'pointsLeft'
SK_API uint32_t generateCubicPoints(const SkPoint& p0,
                             const SkPoint& p1,
                             const SkPoint& p2,
                             const SkPoint& p3,
                             SkScalar tolSqd,
...
public:
    ...
    SK_API void set(const SkPoint controlPts[3]);
...
//     | ..M.. |   | 1 |      | m |
//
SK_API void getConicKLM(const SkPoint p[3], const SkScalar weight, SkMatrix* klm);
...
SK_API void convertCubicToQuads(const SkPoint p[4],
                         SkScalar tolScale,
...
SK_API void convertCubicToQuadsConstrainToTangents(const SkPoint p[4],
                                            SkScalar tolScale,
...
SK_API int findCubicConvex180Chops(const SkPoint[], float T[2], bool* areCusps);
