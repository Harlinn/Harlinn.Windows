...
public:
    ...
    SK_API GrStyledShape(const GrStyledShape&);

    SK_API static GrStyledShape MakeArc(const SkRect& oval, SkScalar startAngleDegrees,
                                 SkScalar sweepAngleDegrees, bool useCenter, const GrStyle& style,
                                 DoSimplify = DoSimplify::kYes);

    SK_API GrStyledShape& operator=(const GrStyledShape& that);
     ...
     */
    SK_API static GrStyledShape MakeFilled(const GrStyledShape& original,
    ...
    SK_API bool asRRect(SkRRect* rrect, SkPathDirection* dir, unsigned* start, bool* inverted) const;
     ...
     */
    SK_API bool asLine(SkPoint pts[2], bool* inverted) const;

    // Can this shape be drawn as a pair of filled nested rectangles?
    SK_API bool asNestedRects(SkRect rects[2]) const;
     ...
     * status).
     */
    SK_API SkRect styledBounds() const;
     ...
     */
    SK_API int unstyledKeySize() const;
     ...
     * for this shape.
     */
    SK_API void writeUnstyledKey(uint32_t* key) const;
     ...
     * does nothing.
     */
    SK_API void addGenIDChangeListener(sk_sp<SkIDChangeListener>) const;
     ...
     */
    SK_API void simplify();

private:
    /** Constructor used by the applyStyle() function */
    SK_API GrStyledShape(const GrStyledShape& parentShape, GrStyle::Apply, SkScalar scale);
     ...
     */
    SK_API void setInheritedKey(const GrStyledShape& parentShape, GrStyle::Apply, SkScalar scale);
     ...
     */
    SK_API void simplifyStroke();

    /** Gets the path that gen id listeners should be added to. */
    SK_API const SkPath* originalPathForListeners() const;
