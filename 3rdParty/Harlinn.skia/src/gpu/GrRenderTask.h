...
public:
    SK_API GrRenderTask();
    SkDEBUGCODE( SK_API ~GrRenderTask() override);

    SK_API void makeClosed(GrRecordingContext*);

    void prePrepare(GrRecordingContext* context) { this->onPrePrepare(context); }

    // These two methods are only invoked at flush time
    SK_API void prepare(GrOpFlushState* flushState);
    ...
    SK_API virtual void disown(GrDrawingManager*);
     ...
     */
    SK_API void addDependency(GrDrawingManager*, GrSurfaceProxy* dependedOn, GrMipmapped,
     ...
     * depends on.
     */
    SK_API void addDependenciesFromOtherTask(GrRenderTask* otherTask);
    ...
    SK_API void replaceDependency(const GrRenderTask* toReplace, GrRenderTask* replaceWith);
    SK_API void replaceDependent(const GrRenderTask* toReplace, GrRenderTask* replaceWith);


    /*
     * Does this renderTask depend on 'dependedOn'?
     */
    SK_API bool dependsOn(const GrRenderTask* dependedOn) const;
     ...
     */
    SK_API virtual void dump(const SkString& label,
                      SkString indent,
    ...
    // it is required)?
    SK_API bool isInstantiated() const;
...
protected:
    ...
    SK_API void addTarget(GrDrawingManager*, sk_sp<GrSurfaceProxy>);
...
private:
    ...
    SK_API void addDependency(GrRenderTask* dependedOn);
    SK_API void addDependent(GrRenderTask* dependent);
    SkDEBUGCODE(bool isDependent(const GrRenderTask* dependent) const;)
    SkDEBUGCODE(void validate() const;)

    SK_API static uint32_t CreateUniqueID();
