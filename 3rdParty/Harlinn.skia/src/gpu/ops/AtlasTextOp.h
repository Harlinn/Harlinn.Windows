...
public:
    ...
    SK_API void* operator new(size_t s);
    SK_API void operator delete(void* b) noexcept;
    SK_API static void ClearCache();
    ...
    struct Geometry {
        ...
        SK_API static Geometry* MakeForBlob(const GrAtlasSubRun& subRun,
                                     const SkMatrix& drawMatrix,
                                     SkPoint drawOrigin,
                                     SkIRect clipRect,
                                     sk_sp<GrTextBlob> blob,
        ...
        SK_API void fillVertexData(void* dst, int offset, int count) const;
    ...
    SK_API void visitProxies(const GrVisitProxyFunc&) const override;

    SK_API FixedFunctionFlags fixedFunctionFlags() const override;

    SK_API GrProcessorSet::Analysis finalize(const GrCaps&, const GrAppliedClip*, GrClampType) override;
    ...
    SK_API static GrOp::Owner CreateOpTestingOnly(skgpu::v1::SurfaceDrawContext*,
                                           const SkPaint&,
                                           const SkFont&,
...
private:
    ...
    SK_API AtlasTextOp(MaskType maskType,
                bool needsTransform,
                int glyphCount,
                SkRect deviceRect,
                Geometry* geo,
    ...
    SK_API void onPrepareDraws(GrMeshDrawTarget*) override;
    SK_API void onExecute(GrOpFlushState*, const SkRect& chainBounds) override;

#if GR_TEST_UTILS
    SK_API SkString onDumpInfo() const override;
#endif
    ...
    SK_API inline void createDrawForGeneratedGlyphs(
            GrMeshDrawTarget* target, FlushInfo* flushInfo) const;

    MaskType maskType() const { return static_cast<MaskType>(fMaskType); }

    SK_API CombineResult onCombineIfPossible(GrOp* t, SkArenaAlloc*, const GrCaps& caps) override;

    SK_API GrGeometryProcessor* setupDfProcessor(SkArenaAlloc*,
                                          const GrShaderCaps&,
