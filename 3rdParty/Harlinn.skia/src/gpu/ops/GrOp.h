...
public:
    ...
    SK_API CombineResult combineIfPossible(GrOp* that, SkArenaAlloc* alloc, const GrCaps& caps);
     ...
     */
    SK_API void chainConcat(GrOp::Owner);
     ...
     */
    SK_API GrOp::Owner cutChain();
    SkDEBUGCODE( SK_API void validateChain(GrOp* expectedTail = nullptr) const);

#ifdef SK_DEBUG
    SK_API virtual void validate() const {}
#endif

protected:
    SK_API GrOp(uint32_t classID);
...
private:
    ...
    SK_API static std::atomic<uint32_t> gCurrOpUniqueID;
    SK_API static std::atomic<uint32_t> gCurrOpClassID;
};
