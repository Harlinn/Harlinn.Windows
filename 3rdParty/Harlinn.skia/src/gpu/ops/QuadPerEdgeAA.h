...
namespace skgpu::v1::QuadPerEdgeAA {
    ...
    SK_API IndexBufferOption CalcIndexBufferOption(GrAAType aa, int numQuads);

    // Gets the minimum ColorType that can represent a color.
    SK_API ColorType MinColorType(SkPMColor4f);
    ...
    public:
        ...
        SK_API int deviceDimensionality() const;
        // Will always be 0 if hasLocalCoords is false, otherwise will be 2 or 3
        SK_API int localDimensionality() const;

        int verticesPerQuad() const { return fUsesCoverageAA ? 8 : 4; }

        SK_API CoverageMode coverageMode() const;
        SK_API size_t vertexSize() const;
    ...
    class Tessellator {
    public:
        SK_API explicit Tessellator(const VertexSpec& spec, char* vertices);
        ...
        SK_API void append(GrQuad* deviceQuad, GrQuad* localQuad,
    ...
    SK_API GrGeometryProcessor* MakeProcessor(SkArenaAlloc*, const VertexSpec&);

    SK_API GrGeometryProcessor* MakeTexturedProcessor(SkArenaAlloc*,
                                               const VertexSpec&,
                                               const GrShaderCaps&,
    ...
    SK_API sk_sp<const GrBuffer> GetIndexBuffer(GrMeshDrawTarget*, IndexBufferOption);

    // What is the maximum number of quads allowed for the specified indexBuffer option?
    SK_API int QuadLimit(IndexBufferOption);
    ...
    SK_API void IssueDraw(const GrCaps&, GrOpsRenderPass*, const VertexSpec&, int runningQuadCount,
