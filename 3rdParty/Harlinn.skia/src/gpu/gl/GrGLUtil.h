...
SK_API GrGLStandard GrGLGetStandardInUseFromString(const char* versionString);
SK_API GrGLSLVersion GrGLGetVersion(const GrGLInterface*);
SK_API GrGLSLVersion GrGLGetVersionFromString(const char*);
...
SK_API GrGLDriverInfo GrGLGetDriverInfo(const GrGLInterface*);

/**
 * Helpers for glGetError()
 */

SK_API void GrGLCheckErr(const GrGLInterface* gl,
                  const char* location,
                  const char* call);
...
// Conditionally checks glGetError based on compile-time and run-time flags.
    ...
    extern SK_API bool gCheckErrorGL;
...
#if GR_GL_LOG_CALLS
    extern SK_API bool gLogCallsGL;
...
SK_API GrGLenum GrToGLStencilFunc(GrStencilTest test);

/**
 * Returns true if the format is compressed.
 */
SK_API bool GrGLFormatIsCompressed(GrGLFormat);
