...
public:
    ...
    SK_API static sk_sp<GrGLRenderTarget> MakeWrapped(GrGLGpu*,
                                               const SkISize&,
                                               GrGLFormat,
                                               int sampleCount,
                                               const IDs&,
    ...
    SK_API GrBackendRenderTarget getBackendRenderTarget() const override;

    SK_API GrBackendFormat backendFormat() const override;

    SK_API bool canAttemptStencilAttachment(bool useMultisampleFBO) const override;
    ...
    SK_API bool ensureDynamicMSAAAttachment();
    ...
    SK_API void bindForResolve(ResolveDirection);

protected:
    // Constructor for subclasses.
    SK_API GrGLRenderTarget(GrGLGpu*,
                     const SkISize&,
                     GrGLFormat,
                     int sampleCount,
                     const IDs&);

    SK_API void init(GrGLFormat, const IDs&);

    // Binds the render target to the given target and ensures its stencil attachment is valid.
    SK_API void bindInternal(GrGLenum fboTarget, bool useMultisampleFBO);
...
private:
    ...
    SK_API GrGLRenderTarget(
            GrGLGpu*, const SkISize&, GrGLFormat, int sampleCount, const IDs&,
            sk_sp<GrGLAttachment> stencil);

    SK_API void setFlags(const GrGLCaps&, const IDs&);

    SK_API GrGLGpu* getGLGpu() const;
    SK_API bool completeStencilAttachment(GrAttachment* stencil, bool useMultisampleFBO) override;
