...
public:
     ...
     */
    SK_API void enableVertexArrays(const GrGLGpu*, int enabledCount,
...
public:
    SK_API GrGLVertexArray(GrGLint id, int attribCount);
     ...
     * returned.
     */
    SK_API GrGLAttribArrayState* bind(GrGLGpu*);

    /**
     * This is a version of the above function that also binds an index buffer to the vertex
     * array object.
     */
    SK_API GrGLAttribArrayState* bindWithIndexBuffer(GrGLGpu* gpu, const GrBuffer* indexBuffer);

    GrGLuint arrayID() const { return fID; }

    SK_API void invalidateCachedState();
