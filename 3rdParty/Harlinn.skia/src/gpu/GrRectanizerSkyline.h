...
public:
    ...
    SK_API bool addRect(int w, int h, SkIPoint16* loc) final;
...
private:
    ...
    SK_API bool rectangleFits(int skylineIndex, int width, int height, int* y) const;
    // Update the skyline structure to include a width x height rect located
    // at x,y.
    SK_API void addSkylineLevel(int skylineIndex, int x, int y, int width, int height);
