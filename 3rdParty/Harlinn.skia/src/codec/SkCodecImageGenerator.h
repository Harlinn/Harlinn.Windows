...
public:
     ...
     */
    SK_API static std::unique_ptr<SkImageGenerator> MakeFromEncodedCodec(sk_sp<SkData>);

    SK_API static std::unique_ptr<SkImageGenerator> MakeFromCodec(std::unique_ptr<SkCodec>);
     ...
     */
    SK_API SkISize getScaledDimensions(float desiredScale) const;
     ...
     */
    SK_API bool getPixels(const SkImageInfo& info, void* pixels, size_t rowBytes, const SkCodec::Options* options = nullptr);
...
protected:
    SK_API sk_sp<SkData> onRefEncodedData() override;

    SK_API bool onGetPixels(const SkImageInfo& info,
                     void* pixels,
                     size_t rowBytes,
                     const Options& opts) override;

    SK_API bool onQueryYUVAInfo(const SkYUVAPixmapInfo::SupportedDataTypes&,
                         SkYUVAPixmapInfo*) const override;

    SK_API bool onGetYUVAPlanes(const SkYUVAPixmaps& yuvaPixmaps) override;
