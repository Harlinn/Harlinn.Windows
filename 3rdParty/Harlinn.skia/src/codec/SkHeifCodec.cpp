...
bool SkHeifCodec::IsSupported(const void* buffer, size_t bytesRead,
    ...
    uint32_t numCompatibleBrands = static_cast<uint32_t>( (chunkDataSize - 8) / 4 );
    bool isHeif = false;
...
int SkHeifCodec::onGetFrameCount() {
    ...
    if (fFrameHolder.size() == 0) {
        ...
        fFrameHolder.reserve(static_cast<int>(frameCount));
        for (size_t i = 0; i < frameCount; i++) {
            ...
            frame->setDuration( static_cast<int>( frameInfo.mDurationUs / 1000 ) );
...
SkCodec::Result SkHeifCodec::onGetPixels(const SkImageInfo& dstInfo,
    ...
    if (fUseAnimation) {
        ...
        fFrameHolder.editFrameAt(options.fFrameIndex)->setDuration(
                static_cast<int>(fFrameInfo.mDurationUs / 1000));
    } else {
