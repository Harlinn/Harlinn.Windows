...
public:
     ...
     */
    SK_API SkString toString() const;
...
protected:
    ...
    SK_API void init_tagged(Tag);
    SK_API void init_tagged_pointer(Tag, void*);
...
public:
    ...
    SK_API explicit BoolValue(bool);
...
public:
    ...
    SK_API explicit NumberValue(int32_t);
    SK_API explicit NumberValue(float);
...
public:
    SK_API ArrayValue(const Value* src, size_t size, SkArenaAlloc& alloc);
};
...
public:
    ...
    SK_API StringValue();
    SK_API StringValue(const char* src, size_t size, SkArenaAlloc& alloc);
...
public:
    SK_API ObjectValue(const Member* src, size_t size, SkArenaAlloc& alloc);

    SK_API const  Value& operator[](const char*) const;
...
public:
    SK_API DOM(const char*, size_t);

    const Value& root() const { return fRoot; }

    SK_API void write(SkWStream*) const;
