...
public:
    ...
    SK_API static std::unique_ptr<Type> MakeArrayType(skstd::string_view name, const Type& componentType,
                                               int columns);

    /** Converts a component type and a size (float, 10) into an array name ("float[10]"). */
    SK_API String getArrayName(int arraySize) const;
     ...
     */
    SK_API static std::unique_ptr<Type> MakeGenericType(const char* name, std::vector<const Type*> types);

    /** Create a type for literal scalars. */
    SK_API static std::unique_ptr<Type> MakeLiteralType(const char* name, const Type& scalarType,
                                                 int8_t priority);

    /** Create a matrix type. */
    SK_API static std::unique_ptr<Type> MakeMatrixType(skstd::string_view name, const char* abbrev,
                                                const Type& componentType, int columns,
                                                int8_t rows);

    /** Create a sampler type. */
    SK_API static std::unique_ptr<Type> MakeSamplerType(const char* name, const Type& textureType);

    /** Create a scalar type. */
    SK_API static std::unique_ptr<Type> MakeScalarType(skstd::string_view name, const char* abbrev,
     ...
     */
    SK_API static std::unique_ptr<Type> MakeSpecialType(const char* name, const char* abbrev,
                                                 Type::TypeKind typeKind);

    /** Creates a struct type with the given fields. */
    SK_API static std::unique_ptr<Type> MakeStructType(int line, skstd::string_view name,
                                                std::vector<Field> fields);

    /** Create a texture type. */
    SK_API static std::unique_ptr<Type> MakeTextureType(const char* name, SpvDim_ dimensions,
                                                 bool isDepth, bool isArrayedTexture,
                                                 bool isMultisampled, bool isSampled);

    /** Create a vector type. */
    SK_API static std::unique_ptr<Type> MakeVectorType(skstd::string_view name, const char* abbrev,
                                                const Type& componentType, int columns);
    ...
    SK_API const Type* clone(SymbolTable* symbolTable) const;
    ...
    SK_API bool isAllowedInES2(const Context& context) const;
     ...
     */
    SK_API CoercionCost coercionCost(const Type& other) const;
    ...
    SK_API bool isOrContainsArray() const;

    /**
     * Returns true if this type is a struct that is too deeply nested.
     */
    SK_API bool isTooDeeplyNested() const;

    /**
     * Returns the corresponding vector or matrix type with the specified number of columns and
     * rows.
     */
    SK_API const Type& toCompound(const Context& context, int columns, int rows) const;
     ...
     */
    SK_API const Type* applyPrecisionQualifiers(const Context& context,
                                         Modifiers* modifiers,
                                         SymbolTable* symbols,
                                         int line) const;
     ...
     */
    SK_API std::unique_ptr<Expression> coerceExpression(std::unique_ptr<Expression> expr,
                                                 const Context& context) const;

    /** Detects any IntLiterals in the expression which can't fit in this type. */
    SK_API bool checkForOutOfRangeLiteral(const Context& context, const Expression& expr) const;

    /** Checks if `value` can fit in this type. The type must be scalar. */
    SK_API bool checkForOutOfRangeLiteral(const Context& context, double value, int line) const;
     ...
     */
    SK_API SKSL_INT convertArraySize(const Context& context, std::unique_ptr<Expression> size) const;
...
private:
    SK_API bool isTooDeeplyNested(int limit) const;
