...
public:
     ...
     */
    SK_API static SkSL::Context& Context();

    /**
     * Returns the Settings used by DSL operations in the current thread.
     */
    SK_API static SkSL::ProgramSettings& Settings();
     ...
     */
    SK_API static std::shared_ptr<SkSL::SymbolTable>& SymbolTable();
     ...
     * given modifiers.
     */
    SK_API static const SkSL::Modifiers* Modifiers(const SkSL::Modifiers& modifiers);
     ...
     */
    SK_API static RTAdjustData& RTAdjustState();
     ...
     */
    SK_API static void StartFragmentProcessor(GrFragmentProcessor::ProgramImpl* processor,
                                       GrFragmentProcessor::ProgramImpl::EmitArgs* emitArgs);

    /**
     * Pops the processor / emitArgs pair associated with the current thread.
     */
    SK_API static void EndFragmentProcessor();
#else
    static bool InFragmentProcessor() {
        return false;
    }
    ...
    SK_API static void SetErrorReporter(ErrorReporter* errorReporter);
     ...
     */
    SK_API static void ReportError(skstd::string_view msg, PositionInfo info = PositionInfo::Capture());

    /**
     * Forwards any pending errors to the DSL ErrorReporter.
     */
    SK_API static void ReportErrors(PositionInfo pos);

    SK_API static ThreadContext& Instance();

    SK_API static void SetInstance(std::unique_ptr<ThreadContext> instance);
