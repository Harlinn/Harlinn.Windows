...
public:
     ...
     */
    SK_API static bool ManglingEnabled();

    SK_API static skstd::string_view Name(skstd::string_view name);

    /**
     * Returns the SkSL variable corresponding to a DSL var.
     */
    SK_API static const SkSL::Variable* Var(DSLVarBase& var);

    /**
     * Creates an SkSL variable corresponding to a DSLParameter.
     */
    SK_API static std::unique_ptr<SkSL::Variable> CreateParameterVar(DSLParameter& var);

    /**
     * Returns the SkSL declaration corresponding to a DSLVar.
     */
    SK_API static std::unique_ptr<SkSL::Statement> Declaration(DSLVarBase& var);

    /**
     * For use in testing only: marks the variable as having been declared, so that it can be
     * destroyed without generating errors.
     */
    SK_API static void MarkDeclared(DSLVarBase& var);

    /**
     * Returns whether DSLVars should automatically be marked declared upon creation. This is used
     * to simplify testing.
     */
    SK_API static bool MarkVarsDeclared();
     ...
     * of it.
     */
    SK_API static void AddVarDeclaration(DSLStatement& existing, DSLVar& additional);

    /**
     * Clears any elements or symbols which have been output.
     */
    SK_API static void Reset();

#if !defined(SKSL_STANDALONE) && SK_SUPPORT_GPU
    SK_API static GrGLSLUniformHandler::UniformHandle VarUniformHandle(const DSLGlobalVar& var);
#endif
