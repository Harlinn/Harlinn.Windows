...
public:
     ...
     */
    SK_API static bool GetConstantInt(const Expression& value, SKSL_INT* out);

    /**
     * If value is a literal or const scalar variable with a known value, returns true and stores
     * the value in out. Otherwise returns false.
     */
    SK_API static bool GetConstantValue(const Expression& value, double* out);
     ...
     */
    SK_API static const Expression* GetConstantValueForVariable(const Expression& value);
     ...
     */
    SK_API static std::unique_ptr<Expression> MakeConstantValueForVariable(
            std::unique_ptr<Expression> expr);

    /** Simplifies the binary expression `left OP right`. Returns null if it can't be simplified. */
    SK_API static std::unique_ptr<Expression> Simplify(const Context& context,
                                                int line,
