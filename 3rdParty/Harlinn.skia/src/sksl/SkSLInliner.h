...
public:
    ...
    SK_API bool analyze(const std::vector<std::unique_ptr<ProgramElement>>& elements,
...
private:
    ...
    SK_API void buildCandidateList(const std::vector<std::unique_ptr<ProgramElement>>& elements,
                            std::shared_ptr<SymbolTable> symbols, ProgramUsage* usage,
                            InlineCandidateList* candidateList);

    SK_API std::unique_ptr<Expression> inlineExpression(int line,
                                                 VariableRewriteMap* varMap,
                                                 SymbolTable* symbolTableForExpression,
                                                 const Expression& expression);
    SK_API std::unique_ptr<Statement> inlineStatement(int line,
                                               VariableRewriteMap* varMap,
     ...
     */
    SK_API static const Variable* RemapVariable(const Variable* variable,
                                         const VariableRewriteMap* varMap);

    /** Determines if a given function has multiple and/or early returns. */
    SK_API static ReturnComplexity GetReturnComplexity(const FunctionDefinition& funcDef);

    using InlinabilityCache = std::unordered_map<const FunctionDeclaration*, bool>;
    SK_API bool candidateCanBeInlined(const InlineCandidate& candidate,
                               const ProgramUsage& usage,
                               InlinabilityCache* cache);

    using FunctionSizeCache = std::unordered_map<const FunctionDeclaration*, int>;
    SK_API int getFunctionSize(const FunctionDeclaration& fnDecl, FunctionSizeCache* cache);
    ...
    SK_API InlinedCall inlineCall(FunctionCall*,
                           std::shared_ptr<SymbolTable>,
                           const ProgramUsage&,
                           const FunctionDeclaration* caller);

    /** Adds a scope to inlined bodies returned by `inlineCall`, if one is required. */
    SK_API void ensureScopedBlocks(Statement* inlinedBody, Statement* parentStmt);

    /** Checks whether inlining is viable for a FunctionCall, modulo recursion and function size. */
    SK_API bool isSafeToInline(const FunctionDefinition* functionDef, const ProgramUsage& usage);
