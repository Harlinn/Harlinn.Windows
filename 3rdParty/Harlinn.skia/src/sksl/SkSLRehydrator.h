...
public:
    ...
    SK_API Rehydrator(const Context* context, std::shared_ptr<SymbolTable> symbolTable,
               const uint8_t* src, size_t length);

    SK_API std::vector<std::unique_ptr<ProgramElement>> elements();

    SK_API std::shared_ptr<SymbolTable> symbolTable(bool inherit = true);
...
private:
    ...
    SK_API Layout layout();

    SK_API Modifiers modifiers();

    SK_API const Symbol* symbol();

    SK_API std::unique_ptr<ProgramElement> element();

    SK_API std::unique_ptr<Statement> statement();

    SK_API std::unique_ptr<Expression> expression();

    SK_API ExpressionArray expressionArray();

    SK_API const Type* type();
