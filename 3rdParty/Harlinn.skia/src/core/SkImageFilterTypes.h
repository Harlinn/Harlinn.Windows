...
public:
    ...
    SK_API bool SK_WARN_UNUSED_RESULT decomposeCTM(const SkMatrix& ctm,
    ...
    // unchanged.
    SK_API void concatLocal(const SkMatrix& local) { fParamToLayerMatrix.preConcat(local); }
    ...
    SK_API bool adjustLayerSpace(const SkMatrix& layer);
    ...
    void applyOrigin(const LayerSpace<SkIPoint>& origin) {
        SkAssertResult(this->adjustLayerSpace(SkMatrix::Translate(static_cast<SkScalar>(-origin.x()), static_cast<SkScalar>( -origin.y() ) )));
    }
...
private:
    ...
    template<typename T>
    SK_API static T map(const T& geom, const SkMatrix& matrix);
};
