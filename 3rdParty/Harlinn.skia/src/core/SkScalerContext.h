...
public:
    ...
    SK_API void    getMatrixFrom2x2(SkMatrix*) const;
    SK_API void    getLocalMatrix(SkMatrix*) const;
    SK_API void    getSingleMatrix(SkMatrix*) const;
     ...
     */
    SK_API bool computeMatrices(PreMatrixScale preMatrixScale,
    ...
    SK_API SkAxisAlignment computeAxisAlignmentForHText() const;
    ...
    SK_API void setLuminanceColor(SkColor c);
...
public:
    ...
    SK_API SkScalerContext(sk_sp<SkTypeface>, const SkScalerContextEffects&, const SkDescriptor*);
    SK_API virtual ~SkScalerContext();
    ...
    SK_API SkGlyph     makeGlyph(SkPackedGlyphID);
    SK_API void        getImage(const SkGlyph&);
    SK_API bool SK_WARN_UNUSED_RESULT getPath(SkPackedGlyphID, SkPath*);
    SK_API void        getFontMetrics(SkFontMetrics*);

    /** Return the size in bytes of the associated gamma lookup table
     */
    SK_API static size_t GetGammaLUTSize(SkScalar contrast, SkScalar paintGamma, SkScalar deviceGamma,
     ...
     */
    SK_API static bool   GetGammaLUTData(SkScalar contrast, SkScalar paintGamma, SkScalar deviceGamma,
                                  uint8_t* data);

    SK_API static void MakeRecAndEffects(const SkFont& font, const SkPaint& paint,
    ...
    SK_API static void MakeRecAndEffectsFromFont(const SkFont& font,
                                          SkScalerContextRec* rec,
                                          SkScalerContextEffects* effects) {
        SkPaint paint;
    ...
    SK_API static std::unique_ptr<SkScalerContext> MakeEmpty(
            sk_sp<SkTypeface> typeface, const SkScalerContextEffects& effects,
            const SkDescriptor* desc);

    SK_API static SkDescriptor* AutoDescriptorGivenRecAndEffects(
        const SkScalerContextRec& rec,
        const SkScalerContextEffects& effects,
        SkAutoDescriptor* ad);

    SK_API static std::unique_ptr<SkDescriptor> DescriptorGivenRecAndEffects(
        const SkScalerContextRec& rec,
        const SkScalerContextEffects& effects);

    SK_API static void DescriptorBufferGiveRec(const SkScalerContextRec& rec, void* buffer);
    SK_API static bool CheckBufferSizeForRec(const SkScalerContextRec& rec,
                                      const SkScalerContextEffects& effects,
                                      size_t size);

    SK_API static SkMaskGamma::PreBlend GetMaskPreBlend(const SkScalerContextRec& rec);
    ...
    */
    ...
    SK_API static SkDescriptor* CreateDescriptorAndEffectsUsingPaint(
...
private:
    ...
    SK_API static SkScalerContextRec PreprocessRec(const SkTypeface& typeface,
                                            const SkScalerContextEffects& effects,
                                            const SkDescriptor& desc);
    ...
    SK_API bool internalGetPath(SkPackedGlyphID id, SkPath* devPath, bool* hairline);
    SK_API SkGlyph internalMakeGlyph(SkPackedGlyphID packedID, SkMask::Format format);
