...
public:
    SK_API explicit SkRasterPipeline(SkArenaAlloc*);
    ...
    SK_API void append(StockStage, void* = nullptr);
    void append(StockStage stage, const void* ctx) { this->append(stage, const_cast<void*>(ctx)); }
    SK_API void append(StockStage, uintptr_t ctx);

    // Append all stages to this pipeline.
    SK_API void extend(const SkRasterPipeline&);

    // Runs the pipeline in 2d from (x,y) inclusive to (x+w,y+h) exclusive.
    SK_API void run(size_t x, size_t y, size_t w, size_t h) const;

    // Allocates a thunk which amortizes run() setup cost in alloc.
    SK_API std::function<void(size_t, size_t, size_t, size_t)> compile() const;
    ...
    SK_API void append_matrix(SkArenaAlloc*, const SkMatrix&);

    // Appends a stage for a constant uniform color.
    // Tries to optimize the stage based on the color.
    SK_API void append_constant_color(SkArenaAlloc*, const float rgba[4]);
    ...
    SK_API void append_set_rgb(SkArenaAlloc*, const float rgb[3]);

    void append_set_rgb(SkArenaAlloc* alloc, const SkColor4f& color) {
        this->append_set_rgb(alloc, color.vec());
    }

    SK_API void append_load    (SkColorType, const SkRasterPipeline_MemoryCtx*);
    SK_API void append_load_dst(SkColorType, const SkRasterPipeline_MemoryCtx*);
    SK_API void append_store   (SkColorType, const SkRasterPipeline_MemoryCtx*);

    SK_API void append_gamut_clamp_if_normalized(const SkImageInfo&);

    SK_API void append_transfer_function(const skcms_TransferFunction&);
...
private:
    ...
    SK_API StartPipelineFn build_pipeline(void**) const;

    SK_API void unchecked_append(StockStage, void*);
