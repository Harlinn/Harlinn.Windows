 ...
 */
SK_API std::unique_ptr<skgpu::v1::SurfaceDrawContext> GaussianBlur(
        GrRecordingContext*,
...
SK_API bool ComputeBlurredRRectParams(const SkRRect& srcRRect,
...
SK_API int CreateIntegralTable(float sixSigma, SkBitmap* table);

SK_API void Compute1DGaussianKernel(float* kernel, float sigma, int radius);

SK_API void Compute1DLinearGaussianKernel(float* kernel, float* offset, float sigma, int radius);
