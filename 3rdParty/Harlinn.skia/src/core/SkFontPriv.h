...
public:
     ...
     */
    SK_API static SkRect GetFontBounds(const SkFont&);
    ...
    SK_API static int CountTextElements(const void* text, size_t byteLength, SkTextEncoding);

    SK_API static void GlyphsToUnichars(const SkFont&, const uint16_t glyphs[], int count, SkUnichar[]);

    SK_API static void Flatten(const SkFont&, SkWriteBuffer& buffer);
    SK_API static bool Unflatten(SkFont*, SkReadBuffer& buffer);
