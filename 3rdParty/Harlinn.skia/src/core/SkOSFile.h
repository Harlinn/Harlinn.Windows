...
SK_API FILE* sk_fopen(const char path[], SkFILE_Flags);
SK_API void    sk_fclose(FILE*);

SK_API size_t  sk_fgetsize(FILE*);

SK_API size_t  sk_fwrite(const void* buffer, size_t byteCount, FILE*);

SK_API void    sk_fflush(FILE*);
SK_API void    sk_fsync(FILE*);

SK_API size_t  sk_ftell(FILE*);
...
SK_API bool    sk_mkdir(const char* path);
