...
private:
    ...
    SK_API static const RunRecord* NextUnchecked(const RunRecord* run);

    SK_API static size_t PosCount(uint32_t glyphCount,
                           SkTextBlob::GlyphPositioning positioning,
                           SkSafeMath* safe);

    SK_API uint32_t* textSizePtr() const;

    SK_API void grow(uint32_t count);
...
public:
    SK_API SkTextBlobRunIterator(const SkTextBlob* blob);
    ...
    SK_API GlyphPositioning positioning() const;
    SK_API unsigned scalarsPerGlyph() const;
