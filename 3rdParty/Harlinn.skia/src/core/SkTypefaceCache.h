...
public:
     ...
     */
    SK_API void add(sk_sp<SkTypeface>);
     ...
     */
    SK_API sk_sp<SkTypeface> findByProcAndRef(FindProc proc, void* ctx) const;
     ...
     */
    SK_API void purgeAll();

    /**
     *  Helper: returns a unique fontID to pass to the constructor of
     *  your subclass of SkTypeface
     */
    SK_API static SkFontID NewFontID();

    // These are static wrappers around a global instance of a cache.

    SK_API static void Add(sk_sp<SkTypeface>);
    SK_API static sk_sp<SkTypeface> FindByProcAndRef(FindProc proc, void* ctx);
...
private:
    SK_API static SkTypefaceCache& Get();

    SK_API void purge(int count);
