...
/** Given a quadratic equation Ax^2 + Bx + C = 0, return 0, 1, 2 roots for the
    equation.
*/
SK_API int SkFindUnitQuadRoots(SkScalar A, SkScalar B, SkScalar C, SkScalar roots[2]);

/** Measures the angle between two vectors, in the range [0, pi].
*/
SK_API float SkMeasureAngleBetweenVectors(SkVector, SkVector);
...
*/
SK_API SkVector SkFindBisector(SkVector, SkVector);

///////////////////////////////////////////////////////////////////////////////

SK_API SkPoint SkEvalQuadAt(const SkPoint src[3], SkScalar t);
SK_API SkPoint SkEvalQuadTangentAt(const SkPoint src[3], SkScalar t);

/** Set pt to the point on the src quadratic specified by t. t must be
    0 <= t <= 1.0
*/
SK_API void SkEvalQuadAt(const SkPoint src[3], SkScalar t, SkPoint* pt, SkVector* tangent = nullptr);

/** Given a src quadratic bezier, chop it at the specified t value,
    where 0 < t < 1, and return the two new quadratics in dst:
    dst[0..2] and dst[2..4]
*/
SK_API void SkChopQuadAt(const SkPoint src[3], SkPoint dst[5], SkScalar t);

/** Given a src quadratic bezier, chop it at the specified t == 1/2,
    The new quads are returned in dst[0..2] and dst[2..4]
*/
SK_API void SkChopQuadAtHalf(const SkPoint src[3], SkPoint dst[5]);
...
*/
SK_API float SkFindQuadMidTangent(const SkPoint src[4]);
...
/** Given the 3 coefficients for a quadratic bezier (either X or Y values), look
    ...
    1                   0 < tValues[0] < 1
*/
SK_API int SkFindQuadExtrema(SkScalar a, SkScalar b, SkScalar c, SkScalar tValues[1]);
...
*/
SK_API int SkChopQuadAtYExtrema(const SkPoint src[3], SkPoint dst[5]);
SK_API int SkChopQuadAtXExtrema(const SkPoint src[3], SkPoint dst[5]);
...
*/
SK_API SkScalar SkFindQuadMaxCurvature(const SkPoint src[3]);
...
*/
SK_API int SkChopQuadAtMaxCurvature(const SkPoint src[3], SkPoint dst[5]);
...
/** Set pt to the point on the src cubic specified by t. t must be
    0 <= t <= 1.0
*/
SK_API void SkEvalCubicAt(const SkPoint src[4], SkScalar t, SkPoint* locOrNull,
...
/** Given a src cubic bezier, chop it at the specified t value,
    ...
    dst[0..3] and dst[3..6]
*/
SK_API void SkChopCubicAt(const SkPoint src[4], SkPoint dst[7], SkScalar t);
...
*/
SK_API void SkChopCubicAt(const SkPoint src[4], SkPoint dst[10], float t0, float t1);
...
*/
SK_API void SkChopCubicAt(const SkPoint src[4], SkPoint dst[], const SkScalar t[],
                   int t_count);

/** Given a src cubic bezier, chop it at the specified t == 1/2,
    The new cubics are returned in dst[0..3] and dst[3..6]
*/
SK_API void SkChopCubicAtHalf(const SkPoint src[4], SkPoint dst[7]);
...
*/
SK_API float SkMeasureNonInflectCubicRotation(const SkPoint[4]);

/** Given a src cubic bezier, returns the T value whose tangent angle is halfway between the
    tangents at p0 and p3.
*/
SK_API float SkFindCubicMidTangent(const SkPoint src[4]);
...
*/
SK_API int SkFindCubicExtrema(SkScalar a, SkScalar b, SkScalar c, SkScalar d,
                       SkScalar tValues[2]);
...
/** Given 4 points on a cubic bezier, chop it into 1, 2, 3 beziers such that
    ...
    If dst == null, it is ignored and only the count is returned.
*/
SK_API int SkChopCubicAtYExtrema(const SkPoint src[4], SkPoint dst[10]);
SK_API int SkChopCubicAtXExtrema(const SkPoint src[4], SkPoint dst[10]);

/** Given a cubic bezier, return 0, 1, or 2 t-values that represent the
    inflection points.
*/
SK_API int SkFindCubicInflections(const SkPoint src[4], SkScalar tValues[2]);
...
*/
SK_API int SkChopCubicAtInflections(const SkPoint src[4], SkPoint dst[10]);

SK_API int SkFindCubicMaxCurvature(const SkPoint src[4], SkScalar tValues[3]);
SK_API int SkChopCubicAtMaxCurvature(const SkPoint src[4], SkPoint dst[13],
                              SkScalar tValues[3] = nullptr);
/** Returns t value of cusp if cubic has one; returns -1 otherwise.
 */
SK_API SkScalar SkFindCubicCusp(const SkPoint src[4]);

SK_API bool SkChopMonoCubicAtX(SkPoint src[4], SkScalar y, SkPoint dst[7]);
SK_API bool SkChopMonoCubicAtY(SkPoint src[4], SkScalar x, SkPoint dst[7]);
...
*/
SK_API SkCubicType SkClassifyCubic(const SkPoint p[4], double t[2] = nullptr, double s[2] = nullptr,
                            double d[4] = nullptr);
...
struct SkConic {
     ...
     *  be used.
     */
    SK_API void evalAt(SkScalar t, SkPoint* pos, SkVector* tangent = nullptr) const;
    SK_API bool SK_WARN_UNUSED_RESULT chopAt(SkScalar t, SkConic dst[2]) const;
    SK_API void chopAt(SkScalar t1, SkScalar t2, SkConic* dst) const;
    SK_API void chop(SkConic dst[2]) const;

    SK_API SkPoint evalAt(SkScalar t) const;
    SK_API SkVector evalTangentAt(SkScalar t) const;

    SK_API void computeAsQuadError(SkVector* err) const;
    SK_API bool asQuadTol(SkScalar tol) const;
    ...
    SK_API float findMidTangent() const;
    SK_API bool findXExtrema(SkScalar* t) const;
    SK_API bool findYExtrema(SkScalar* t) const;
    SK_API bool chopAtXExtrema(SkConic dst[2]) const;
    SK_API bool chopAtYExtrema(SkConic dst[2]) const;

    SK_API void computeTightBounds(SkRect* bounds) const;
    SK_API void computeFastBounds(SkRect* bounds) const;
...
//    bool findMaxCurvature(SkScalar* t) const;  // unimplemented
    ...
    SK_API static SkScalar TransformW(const SkPoint[3], SkScalar w, const SkMatrix&);

    enum {
        kMaxConicsForArc = 5
    };
    SK_API static int BuildUnitArc(const SkVector& start, const SkVector& stop, SkRotationDirection,
