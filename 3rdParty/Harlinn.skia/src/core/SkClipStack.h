...
#include <skia/private/SkDeque.h>
...
#include <skia/private/GrResourceKey.h>
#endif
...
public:
    ...
    public:
        ...
        SK_API bool operator== (const Element& element) const;
        ...
        SK_API void asDeviceSpacePath(SkPath* path) const;
        ...
        SK_API void invertShapeFillType();
         ...
         */
        SK_API const SkRect& getBounds() const;

        /**
         * Conservatively checks whether the clip shape contains the rect/rrect. (Whether the shape
         * is inverse filled is not considered.)
         */
        SK_API bool contains(const SkRect& rect) const;
        SK_API bool contains(const SkRRect& rrect) const;
    ...
    private:
        ...
        SK_API void initCommon(int saveCount, SkClipOp op, bool doAA);
        SK_API void initRect(int saveCount, const SkRect&, const SkMatrix&, SkClipOp, bool doAA);
        SK_API void initRRect(int saveCount, const SkRRect&, const SkMatrix&, SkClipOp, bool doAA);
        SK_API void initPath(int saveCount, const SkPath&, const SkMatrix&, SkClipOp, bool doAA);
        SK_API void initAsPath(int saveCount, const SkPath&, const SkMatrix&, SkClipOp, bool doAA);
        SK_API void initShader(int saveCount, sk_sp<SkShader>);
        SK_API void initReplaceRect(int saveCount, const SkRect&, bool doAA);

        SK_API void setEmpty();
        ...
        /* This method checks to see if two rect clips can be safely merged into one. The issue here
          ...
          anti-aliasing. */
        SK_API bool rectRectIntersectAllowed(const SkRect& newR, bool newAA) const;
        /** Determines possible finite bounds for the Element given the previous element of the
            stack */
        SK_API void updateBoundAndGenID(const Element* prior);
    ...
    SK_API SkClipStack();
    SK_API SkClipStack(void* storage, size_t size);
    SK_API SkClipStack(const SkClipStack& b);
    SK_API ~SkClipStack();

    SK_API SkClipStack& operator=(const SkClipStack& b);
    SK_API bool operator==(const SkClipStack& b) const;
    SK_API bool operator!=(const SkClipStack& b) const { return !(*this == b); }
     ...
     */
    SK_API void getBounds(SkRect* canvFiniteBound,
                   BoundsType* boundType,
                   bool* isIntersectionOfRects = nullptr) const;

    SK_API SkRect bounds(const SkIRect& deviceBounds) const;
    SK_API bool isEmpty(const SkIRect& deviceBounds) const;
    ...
    SK_API void clipRect(const SkRect&, const SkMatrix& matrix, SkClipOp, bool doAA);
    SK_API void clipRRect(const SkRRect&, const SkMatrix& matrix, SkClipOp, bool doAA);
    SK_API void clipPath(const SkPath&, const SkMatrix& matrix, SkClipOp, bool doAA);
    SK_API void clipShader(sk_sp<SkShader>);
    // An optimized version of clipDevRect(emptyRect, kIntersect, ...)
    SK_API void clipEmpty();

    SK_API void replaceClip(const SkRect& devRect, bool doAA);
     ...
     */
    SK_API bool isRRect(const SkRect& bounds, SkRRect* rrect, bool* aa) const;
    ...
    SK_API uint32_t getTopmostGenID() const;
...
public:
    ...
    public:
        ...
        SK_API Iter(const SkClipStack& stack, IterStart startLoc);

        /**
         *  Return the clip element for this iterator. If next()/prev() returns NULL, then the
         *  iterator is done.
         */
        SK_API const Element* next();
        SK_API const Element* prev();
         ...
         */
        SK_API const Element* skipToTopmost(SkClipOp op);

        /**
         * Restarts the iterator on a clip stack.
         */
        SK_API void reset(const SkClipStack& stack, IterStart startLoc);
     ...
     */
    SK_API void getConservativeBounds(int offsetX,
                               int offsetY,
                               int maxWidth,
                               int maxHeight,
                               SkRect* devBounds,
...
private:
    ...
    SK_API bool internalQuickContains(const SkRect& devRect) const;
    SK_API bool internalQuickContains(const SkRRect& devRRect) const;

    /**
     * Helper for clipDevPath, etc.
     */
    SK_API void pushElement(const Element& element);

    /**
     * Restore the stack back to the specified save count.
     */
    SK_API void restoreTo(int saveCount);

    /**
     * Return the next unique generation ID.
     */
    SK_API static uint32_t GetNextGenID();
};
