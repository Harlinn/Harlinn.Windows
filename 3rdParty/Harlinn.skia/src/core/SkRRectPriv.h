...
public:
    ...
    SK_API static bool IsNearlySimpleCircular(const SkRRect& rr, SkScalar tolerance = SK_ScalarNearlyZero);
    ...
    SK_API static bool AllCornersCircular(const SkRRect& rr, SkScalar tolerance = SK_ScalarNearlyZero);

    SK_API static bool ReadFromBuffer(SkRBuffer* buffer, SkRRect* rr);

    SK_API static void WriteToBuffer(const SkRRect& rr, SkWBuffer* buffer);
    ...
    // in the round rect.
    SK_API static SkRect InnerBounds(const SkRRect& rr);
    ...
    SK_API static SkRRect ConservativeIntersect(const SkRRect& a, const SkRRect& b);
};
