...
public:
    SK_API SkCachedData(void* mallocData, size_t size);
    SK_API SkCachedData(size_t size, SkDiscardableMemory*);
    SK_API virtual ~SkCachedData();
...
private:
    ...
    SK_API void internalRef(bool fromCache) const;
    SK_API void internalUnref(bool fromCache) const;

    SK_API void inMutexRef(bool fromCache);
    SK_API bool inMutexUnref(bool fromCache);  // returns true if we should delete "this"
    SK_API void inMutexLock();
    SK_API void inMutexUnlock();
