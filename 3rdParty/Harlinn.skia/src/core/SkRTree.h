...
public:
    ...
    SK_API void insert(const SkRect[], int N) override;
    SK_API void search(const SkRect& query, std::vector<int>* results) const override;
    SK_API size_t bytesUsed() const override;
...
private:
    ...
    SK_API void search(Node* root, const SkRect& query, std::vector<int>* results) const;

    // Consumes the input array.
    SK_API Branch bulkLoad(std::vector<Branch>* branches, int level = 0);

    // How many times will bulkLoad() call allocateNodeAtLevel()?
    SK_API static int CountNodes(int branches);

    SK_API Node* allocateNodeAtLevel(uint16_t level);
