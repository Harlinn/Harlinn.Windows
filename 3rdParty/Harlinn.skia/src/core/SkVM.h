...
namespace skvm {
    ...
    class Assembler {
    public:
        SK_API explicit Assembler(void* buf);

        SK_API size_t size() const;
        ...
        SK_API void bytes(const void*, int);
        SK_API void byte(uint8_t);
        SK_API void word(uint32_t);
        ...
        SK_API void align(int mod);

        SK_API void int3();
        SK_API void vzeroupper();
        SK_API void ret();
        ...
        SK_API void vpand (Ymm dst, Ymm x, Operand y);
        SK_API void vpandn(Ymm dst, Ymm x, Operand y);
        SK_API void vpor  (Ymm dst, Ymm x, Operand y);
        SK_API void vpxor (Ymm dst, Ymm x, Operand y);

        SK_API void vpaddd (Ymm dst, Ymm x, Operand y);
        SK_API void vpsubd (Ymm dst, Ymm x, Operand y);
        SK_API void vpmulld(Ymm dst, Ymm x, Operand y);

        SK_API void vpaddw   (Ymm dst, Ymm x, Operand y);
        SK_API void vpsubw   (Ymm dst, Ymm x, Operand y);
        SK_API void vpmullw  (Ymm dst, Ymm x, Operand y);

        SK_API void vpabsw   (Ymm dst, Operand x);
        SK_API void vpavgw   (Ymm dst, Ymm x, Operand y);  // dst = (x+y+1)>>1, unsigned.
        SK_API void vpmulhrsw(Ymm dst, Ymm x, Operand y);  // dst = (x*y + (1<<14)) >> 15, signed.
        SK_API void vpminsw  (Ymm dst, Ymm x, Operand y);
        SK_API void vpminuw  (Ymm dst, Ymm x, Operand y);
        SK_API void vpmaxsw  (Ymm dst, Ymm x, Operand y);
        SK_API void vpmaxuw  (Ymm dst, Ymm x, Operand y);

        SK_API void vaddps(Ymm dst, Ymm x, Operand y);
        SK_API void vsubps(Ymm dst, Ymm x, Operand y);
        SK_API void vmulps(Ymm dst, Ymm x, Operand y);
        SK_API void vdivps(Ymm dst, Ymm x, Operand y);
        SK_API void vminps(Ymm dst, Ymm x, Operand y);
        SK_API void vmaxps(Ymm dst, Ymm x, Operand y);

        SK_API void vsqrtps(Ymm dst, Operand x);

        SK_API void vfmadd132ps(Ymm dst, Ymm x, Operand y);
        SK_API void vfmadd213ps(Ymm dst, Ymm x, Operand y);
        SK_API void vfmadd231ps(Ymm dst, Ymm x, Operand y);

        SK_API void vfmsub132ps(Ymm dst, Ymm x, Operand y);
        SK_API void vfmsub213ps(Ymm dst, Ymm x, Operand y);
        SK_API void vfmsub231ps(Ymm dst, Ymm x, Operand y);

        SK_API void vfnmadd132ps(Ymm dst, Ymm x, Operand y);
        SK_API void vfnmadd213ps(Ymm dst, Ymm x, Operand y);
        SK_API void vfnmadd231ps(Ymm dst, Ymm x, Operand y);

        SK_API void vpackusdw(Ymm dst, Ymm x, Operand y);
        SK_API void vpackuswb(Ymm dst, Ymm x, Operand y);

        SK_API void vpunpckldq(Ymm dst, Ymm x, Operand y);
        SK_API void vpunpckhdq(Ymm dst, Ymm x, Operand y);

        SK_API void vpcmpeqd(Ymm dst, Ymm x, Operand y);
        SK_API void vpcmpgtd(Ymm dst, Ymm x, Operand y);
        SK_API void vpcmpeqw(Ymm dst, Ymm x, Operand y);
        SK_API void vpcmpgtw(Ymm dst, Ymm x, Operand y);

        SK_API void vcmpps   (Ymm dst, Ymm x, Operand y, int imm);
        SK_API void vcmpeqps (Ymm dst, Ymm x, Operand y) { this->vcmpps(dst,x,y,0); }
        SK_API void vcmpltps (Ymm dst, Ymm x, Operand y) { this->vcmpps(dst,x,y,1); }
        SK_API void vcmpleps (Ymm dst, Ymm x, Operand y) { this->vcmpps(dst,x,y,2); }
        SK_API void vcmpneqps(Ymm dst, Ymm x, Operand y) { this->vcmpps(dst,x,y,4); }

        // Sadly, the x parameter cannot be a general Operand for these shifts.
        SK_API void vpslld(Ymm dst, Ymm x, int imm);
        SK_API void vpsrld(Ymm dst, Ymm x, int imm);
        SK_API void vpsrad(Ymm dst, Ymm x, int imm);

        SK_API void vpsllw( Ymm dst, Ymm x, int imm );
        SK_API void vpsrlw( Ymm dst, Ymm x, int imm );
        SK_API void vpsraw(Ymm dst, Ymm x, int imm);

        SK_API void vpermq( Ymm dst, Operand x, int imm );
        SK_API void vperm2f128( Ymm dst, Ymm x, Operand y, int imm );
        SK_API void vpermps   (Ymm dst, Ymm ix, Operand src);        // dst[i] = src[ix[i]]

        enum Rounding { NEAREST, FLOOR, CEIL, TRUNC, CURRENT };
        SK_API void vroundps(Ymm dst, Operand x, Rounding);

        SK_API void vmovdqa(Ymm dst, Operand x);
        SK_API void vmovups(Ymm dst, Operand x);
        SK_API void vmovups(Xmm dst, Operand x);
        SK_API void vmovups(Operand dst, Ymm x);
        SK_API void vmovups(Operand dst, Xmm x);

        SK_API void vcvtdq2ps (Ymm dst, Operand x);
        SK_API void vcvttps2dq(Ymm dst, Operand x);
        SK_API void vcvtps2dq (Ymm dst, Operand x);

        SK_API void vcvtps2ph(Operand dst, Ymm x, Rounding);
        SK_API void vcvtph2ps(Ymm dst, Operand x);

        SK_API void vpblendvb(Ymm dst, Ymm x, Operand y, Ymm z);

        SK_API void vpshufb(Ymm dst, Ymm x, Operand y);

        SK_API void vptest(Ymm x, Operand y);

        SK_API void vbroadcastss(Ymm dst, Operand y);

        SK_API void vpmovzxwd(Ymm dst, Operand src);   // dst = src, 128-bit, uint16_t -> int
        SK_API void vpmovzxbd(Ymm dst, Operand src);   // dst = src,  64-bit, uint8_t  -> int

        SK_API void vmovq(Operand dst, Xmm src);  // dst = src,  64-bit
        SK_API void vmovd(Operand dst, Xmm src);  // dst = src,  32-bit
        SK_API void vmovd(Xmm dst, Operand src);  // dst = src,  32-bit

        SK_API void vpinsrd(Xmm dst, Xmm src, Operand y, int imm);  // dst = src; dst[imm] = y, 32-bit
        SK_API void vpinsrw(Xmm dst, Xmm src, Operand y, int imm);  // dst = src; dst[imm] = y, 16-bit
        SK_API void vpinsrb(Xmm dst, Xmm src, Operand y, int imm);  // dst = src; dst[imm] = y,  8-bit

        SK_API void vextracti128(Operand dst, Ymm src, int imm);    // dst = src[imm], 128-bit
        SK_API void vpextrd     (Operand dst, Xmm src, int imm);    // dst = src[imm],  32-bit
        SK_API void vpextrw     (Operand dst, Xmm src, int imm);    // dst = src[imm],  16-bit
        SK_API void vpextrb     (Operand dst, Xmm src, int imm);    // dst = src[imm],   8-bit

        // if (mask & 0x8000'0000) {
        //     dst = base[scale*ix];
        // }
        // mask = 0;
        SK_API void vgatherdps(Ymm dst, Scale scale, Ymm ix, GP64 base, Ymm mask);


        SK_API void label(Label*);

        SK_API void jmp(Label*);
        SK_API void je (Label*);
        SK_API void jne(Label*);
        SK_API void jl (Label*);
        SK_API void jc (Label*);

        SK_API void add( Operand dst, int imm );
        SK_API void sub( Operand dst, int imm );
        SK_API void cmp( Operand dst, int imm );
        SK_API void mov( Operand dst, int imm );
        SK_API void movb(Operand dst, int imm);

        SK_API void add( Operand dst, GP64 x );
        SK_API void sub( Operand dst, GP64 x );
        SK_API void cmp( Operand dst, GP64 x );
        SK_API void mov( Operand dst, GP64 x );
        SK_API void movb(Operand dst, GP64 x);

        SK_API void add( GP64 dst, Operand x );
        SK_API void sub( GP64 dst, Operand x );
        SK_API void cmp( GP64 dst, Operand x );
        SK_API void mov( GP64 dst, Operand x );
        SK_API void movb(GP64 dst, Operand x);
        ...
        SK_API void movzbq(GP64 dst, Operand x);  // dst = x, uint8_t  -> int
        SK_API void movzwq(GP64 dst, Operand x);  // dst = x, uint16_t -> int
        ...
        SK_API DOpNM fmin4s; 
        SK_API DOpNM fmax4s;
        SK_API DOpNM fcmeq4s; 
        SK_API DOpNM fcmgt4s; 
        SK_API DOpNM fcmge4s;
        SK_API DOpNM tbl;
        SK_API DOpNM uzp14s; 
        SK_API DOpNM uzp24s;
        SK_API DOpNM zip14s; 
        ...
        // d += n*m
        SK_API void fmla4s(V d, V n, V m);

        // d -= n*m
        SK_API void fmls4s(V d, V n, V m);

        // d = op(n,imm)
        using DOpNImm = void(V d, V n, int imm);
        SK_API DOpNImm sli4s;
        SK_API DOpNImm shl4s;
        SK_API DOpNImm sshr4s;
        SK_API DOpNImm ushr4s;
        SK_API DOpNImm ushr8h;

        // d = op(n)
        using DOpN = void(V d, V n);
        SK_API DOpN not16b;    // d = ~n
        SK_API DOpN fneg4s;    // d = -n
        SK_API DOpN fsqrt4s;   // d = sqrtf(n)
        SK_API DOpN scvtf4s;   // int -> float
        SK_API DOpN fcvtzs4s;  // truncate float -> int
        SK_API DOpN fcvtns4s;  // round float -> int  (nearest even)
        SK_API DOpN frintp4s;  // round float -> int as float, toward plus infinity  (ceil)
        SK_API DOpN frintm4s;  // round float -> int as float, toward minus infinity (floor)
        SK_API DOpN fcvtn;     // f32 -> f16 in low half
        SK_API DOpN fcvtl;     // f16 in low half -> f32
        SK_API DOpN xtns2h;    // u32 -> u16
        SK_API DOpN xtnh2b;    // u16 -> u8
        SK_API DOpN uxtlb2h;   // u8 -> u16    (TODO: this is a special case of ushll.8h)
        SK_API DOpN uxtlh2s;   // u16 -> u32   (TODO: this is a special case of ushll.4s)
        SK_API DOpN uminv4s;   // dst[0] = min(n[0],n[1],n[2],n[3]), n as unsigned

        SK_API void brk (int imm16);
        SK_API void ret (X);
        SK_API void add (X d, X n, int imm12);
        SK_API void sub (X d, X n, int imm12);
        SK_API void subs(X d, X n, int imm12);  // subtract setting condition flags

        enum Shift { LSL,LSR,ASR,ROR };
        SK_API void add (X d, X n, X m, Shift=LSL, int imm6=0);  // d=n+Shift(m,imm6), for Shift != ROR.
        ...
        SK_API void cbz (X t, Label* l);
        SK_API void cbnz(X t, Label* l);

        // TODO: there are ldur variants with unscaled imm, useful?
        SK_API void ldrd(X dst, X src, int imm12=0);  // 64-bit dst = *(src+imm12*8)
        SK_API void ldrs(X dst, X src, int imm12=0);  // 32-bit dst = *(src+imm12*4)
        SK_API void ldrh(X dst, X src, int imm12=0);  // 16-bit dst = *(src+imm12*2)
        SK_API void ldrb(X dst, X src, int imm12=0);  //  8-bit dst = *(src+imm12)

        SK_API void ldrq(V dst, Label*);  // 128-bit PC-relative load

        SK_API void ldrq(V dst, X src, int imm12=0);  // 128-bit dst = *(src+imm12*16)
        SK_API void ldrd(V dst, X src, int imm12=0);  //  64-bit dst = *(src+imm12*8)
        SK_API void ldrs(V dst, X src, int imm12=0);  //  32-bit dst = *(src+imm12*4)
        SK_API void ldrh(V dst, X src, int imm12=0);  //  16-bit dst = *(src+imm12*2)
        SK_API void ldrb(V dst, X src, int imm12=0);  //   8-bit dst = *(src+imm12)

        SK_API void strs(X src, X dst, int imm12=0);  // 32-bit *(dst+imm12*4) = src

        SK_API void strq(V src, X dst, int imm12=0);  // 128-bit *(dst+imm12*16) = src
        SK_API void strd(V src, X dst, int imm12=0);  //  64-bit *(dst+imm12*8)  = src
        SK_API void strs(V src, X dst, int imm12=0);  //  32-bit *(dst+imm12*4)  = src
        SK_API void strh(V src, X dst, int imm12=0);  //  16-bit *(dst+imm12*2)  = src
        SK_API void strb(V src, X dst, int imm12=0);  //   8-bit *(dst+imm12)    = src

        SK_API void movs(X dst, V src, int lane);  // dst = 32-bit src[lane]
        SK_API void inss(V dst, X src, int lane);  // dst[lane] = 32-bit src

        SK_API void dup4s  (V dst, X src);  // Each 32-bit lane = src

        SK_API void ld1r4s (V dst, X src);  // Each 32-bit lane = *src
        SK_API void ld1r8h (V dst, X src);  // Each 16-bit lane = *src
        SK_API void ld1r16b(V dst, X src);  // Each  8-bit lane = *src

        SK_API void ld24s( V dst, X src );  // deinterleave(dst,dst+1)             = 256-bit *src
        SK_API void ld44s( V dst, X src );  // deinterleave(dst,dst+1,dst+2,dst+3) = 512-bit *src
        SK_API void st24s(V src, X dst);  // 256-bit *dst = interleave_32bit_lanes(src,src+1)
        SK_API void st44s(V src, X dst);  // 512-bit *dst = interleave_32bit_lanes(src,src+1,src+2,src+3)

        SK_API void ld24s(V dst, X src, int lane);  // Load 2 32-bit values into given lane of dst..dst+1
        SK_API void ld44s(V dst, X src, int lane);  // Load 4 32-bit values into given lane of dst..dst+3
    ...
    private:
        ...
        SK_API void op(int prefix, int map, int opcode, int dst, int x, Operand y, W,L);
        ...
        SK_API void op(int opcode, Operand dst, GP64 x);
        SK_API void op(int opcode, int opcode_ext, Operand dst, int imm);

        SK_API void jump(uint8_t condition, Label*);
        SK_API int disp32(Label*);
        SK_API void imm_byte_after_operand(const Operand&, int byte);

        // aarch64

        // Opcode for 3-arguments ops is split between hi and lo:
        //    [11 bits hi] [5 bits m] [6 bits lo] [5 bits n] [5 bits d]
        SK_API void op(uint32_t hi, V m, uint32_t lo, V n, V d);
        ...
        SK_API void op(uint32_t op22, V n, V d, int imm=0);
        SK_API void op(uint32_t op22, X n, V d, int imm=0) { this->op(op22,(V)n,   d,imm); }
        SK_API void op(uint32_t op22, V n, X d, int imm=0) { this->op(op22,   n,(V)d,imm); }
        SK_API void op(uint32_t op22, X n, X d, int imm=0) { this->op(op22,(V)n,(V)d,imm); }
        SK_API void op(uint32_t op22,           int imm=0) { this->op(op22,(V)0,(V)0,imm); }
        ...
        SK_API void b(Condition, Label*);
        SK_API int disp19(Label*);
    };
    ...
    SK_API PixelFormat SkColorType_to_PixelFormat(SkColorType);
    ...
    public:
        ...
        SK_API Builder();
        SK_API explicit Builder(Features);

        SK_API Program done(const char* debug_name = nullptr, bool allow_jit=true) const;

        // Mostly for debugging, tests, etc.
        std::vector<Instruction> program() const { return fProgram; }
        SK_API std::vector<OptimizedInstruction> optimize() const;
        ...
        SK_API void assert_true(I32 cond, I32 debug);
        ...
        SK_API void trace_line( I32 mask, int line );
        SK_API void trace_var( I32 mask, int slot, I32 val );
        SK_API void trace_var( I32 mask, int slot, F32 val );
        SK_API void trace_var( I32 mask, int slot, bool b );
        SK_API void trace_call_enter( I32 mask, int line );
        SK_API void trace_call_exit( I32 mask, int line );

        // Store {8,16,32,64,128}-bit varying.
        SK_API void store8  (Ptr ptr, I32 val);
        SK_API void store16 (Ptr ptr, I32 val);
        SK_API void store32 (Ptr ptr, I32 val);
        void storeF  (Ptr ptr, F32 val) { store32(ptr, pun_to_I32(val)); }
        SK_API void store64 (Ptr ptr, I32 lo, I32 hi);              // *ptr = lo|(hi<<32)
        SK_API void store128(Ptr ptr, I32 x, I32 y, I32 z, I32 w);  // *ptr = x|(y<<32)|(z<<64)|(w<<96)

        // Returns varying {n, n-1, n-2, ..., 1}, where n is the argument to Program::eval().
        SK_API I32 index();

        // Load {8,16,32,64,128}-bit varying.
        SK_API I32 load8  (Ptr ptr);
        SK_API I32 load16 (Ptr ptr);
        SK_API I32 load32 (Ptr ptr);
        F32 loadF  (Ptr ptr) { return pun_to_F32(load32(ptr)); }
        SK_API I32 load64 (Ptr ptr, int lane);  // Load 32-bit lane 0-1 of  64-bit value.
        SK_API I32 load128(Ptr ptr, int lane);  // Load 32-bit lane 0-3 of 128-bit value.

        // Load i32/f32 uniform with byte-count offset.
        SK_API I32 uniform32(UPtr ptr, int offset);
        ...
        SK_API I32 array32  (UPtr ptr, int offset, int index);
        F32 arrayF   (UPtr ptr, int offset, int index) {
            return pun_to_F32(array32(ptr, offset, index));
        }

        // Push and load this color as a uniform.
        SK_API Color uniformColor(SkColor4f, Uniforms*);

        // Gather u8,u16,i32 with varying element-count index from *(ptr + byte-count offset).
        SK_API I32 gather8 (UPtr ptr, int offset, I32 index);
        SK_API I32 gather16(UPtr ptr, int offset, I32 index);
        SK_API I32 gather32(UPtr ptr, int offset, I32 index);
        ...
        SK_API F32        sqrt(F32);
        SK_API F32 approx_log2(F32);
        SK_API F32 approx_pow2(F32);
        ...
        SK_API F32 approx_powf(F32 base, F32 exp);
        ...
        SK_API F32 approx_sin(F32 radians);
        F32 approx_cos(F32 radians) { return approx_sin(add(radians, SK_ScalarPI/2)); }
        SK_API F32 approx_tan(F32 radians);

        SK_API F32 approx_asin(F32 x);
        F32 approx_acos(F32 x) { return sub(SK_ScalarPI/2, approx_asin(x)); }
        SK_API F32 approx_atan(F32 x);
        SK_API F32 approx_atan2(F32 y, F32 x);

        SK_API F32 lerp(F32   lo, F32   hi, F32   t);
        ...
        SK_API I32 select(I32 cond, I32 t, I32 f);  // cond ? t : f
        ...
        SK_API I32 extract(I32 x, int bits, I32 z);   // (x>>bits) & z
        ...
        SK_API I32 pack(I32 x, I32 y, int bits);   // x | (y<<bits)
        ...
        SK_API F32 from_unorm(int bits, I32);   // E.g. from_unorm(8, x) -> x * (1/255.0f)
        SK_API I32   to_unorm(int bits, F32);   // E.g.   to_unorm(8, x) -> round(x * 255)

        SK_API Color   load(PixelFormat, Ptr ptr);
        SK_API void   store(PixelFormat, Ptr ptr, Color);
        SK_API Color gather(PixelFormat, UPtr ptr, int offset, I32 index);
        Color gather(PixelFormat f, Uniform u, I32 index) {
            return gather(f, u.ptr, u.offset, index);
        }

        SK_API void   premul(F32* r, F32* g, F32* b, F32 a);
        SK_API void unpremul(F32* r, F32* g, F32* b, F32 a);

        Color   premul(Color c) {   this->premul(&c.r, &c.g, &c.b, c.a); return c; }
        Color unpremul(Color c) { this->unpremul(&c.r, &c.g, &c.b, c.a); return c; }

        SK_API Color lerp(Color lo, Color hi, F32 t);
        SK_API Color blend(SkBlendMode, Color src, Color dst);

        Color clamp01(Color c) {
            return { clamp01(c.r), clamp01(c.g), clamp01(c.b), clamp01(c.a) };
        }

        SK_API HSLA  to_hsla(Color);
        SK_API Color to_rgba(HSLA);

        SK_API void dump(SkWStream* = nullptr) const;

        SK_API uint64_t hash() const;

        SK_API Val push(Instruction);
    ...
    SK_API std::vector<Instruction>          eliminate_dead_code(std::vector<Instruction>);
    SK_API std::vector<OptimizedInstruction> finalize           (std::vector<Instruction>);
    ...
    class Program {
    public:
        SK_API Program(const std::vector<OptimizedInstruction>& instructions,
        ...
        SK_API Program(Program&&);
        SK_API Program& operator=(Program&&);

        Program(const Program&) = delete;
        Program& operator=(const Program&) = delete;

        SK_API void eval(int n, void* args[]) const;
        ...
        SK_API std::vector<InterpreterInstruction> instructions() const;
        SK_API int  nargs() const;
        SK_API int  nregs() const;
        SK_API int  loop () const;
        SK_API bool empty() const;

        SK_API bool hasJIT() const;  // Has this Program been JITted?

        SK_API void dump(SkWStream* = nullptr) const;
        SK_API void disassemble( SkWStream* = nullptr ) const;

    private:
        SK_API void setupInterpreter(const std::vector<OptimizedInstruction>&);
        SK_API void setupJIT        (const std::vector<OptimizedInstruction>&, const char* debug_name);
        SK_API void setupLLVM       (const std::vector<OptimizedInstruction>&, const char* debug_name);

        SK_API bool jit(const std::vector<OptimizedInstruction>&,
                 int* stack_hint, uint32_t* registers_used,
                 Assembler*) const;

        SK_API void waitForLLVM() const;
        SK_API void dropJIT();
