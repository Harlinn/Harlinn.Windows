...
struct SkGlyphPositionRoundingSpec {
    SK_API SkGlyphPositionRoundingSpec(bool isSubpixel, SkAxisAlignment axisAlignment);
    const SkVector halfAxisSampleFreq;
    const SkIPoint ignorePositionMask;
    const SkIPoint ignorePositionFieldMask;

private:
    SK_API static SkVector HalfAxisSampleFreq(bool isSubpixel, SkAxisAlignment axisAlignment);
    SK_API static SkIPoint IgnorePositionMask(bool isSubpixel, SkAxisAlignment axisAlignment);
    SK_API static SkIPoint IgnorePositionFieldMask(bool isSubpixel, SkAxisAlignment axisAlignment);
};
...
public:
    ...
    SK_API SkGlyphRunListPainter(const SkSurfaceProps& props,
    ...
    // strike cache.
    SK_API SkGlyphRunListPainter(const SkSurfaceProps&, const GrColorInfo&);
    SK_API explicit SkGlyphRunListPainter(const skgpu::v1::SurfaceDrawContext&);
    ...
    SK_API void drawForBitmapDevice(
    ...
    SK_API void processGlyphRun(const SkGlyphRun& glyphRun,
...
private:
    SK_API SkGlyphRunListPainter(const SkSurfaceProps& props, SkColorType colorType,
                          SkScalerContextFlags flags, SkStrikeForGPUCacheInterface* strikeCache);

    struct ScopedBuffers {
        SK_API ScopedBuffers(SkGlyphRunListPainter* painter, size_t size);
        // Required until C++17 copy elision
        SK_API ScopedBuffers( const ScopedBuffers& ) = default;
        SK_API ~ScopedBuffers();
        SkGlyphRunListPainter* fPainter;
    };

    SK_API ScopedBuffers SK_WARN_UNUSED_RESULT ensureBuffers(const SkGlyphRunList& glyphRunList);
    SK_API ScopedBuffers SK_WARN_UNUSED_RESULT ensureBuffers(const SkGlyphRun& glyphRun);
