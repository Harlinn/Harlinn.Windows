...
public:
...
#if SK_SUPPORT_GPU
    SK_API sk_sp<GrTextStrike> findOrCreateGrStrike(GrStrikeCache* grStrikeCache) const;
#endif
    ...
    SK_API void updateDelta(size_t increase);
...
public:
    ...
    SK_API static SkStrikeCache* GlobalStrikeCache();

    SK_API sk_sp<SkStrike> findStrike(const SkDescriptor& desc) SK_EXCLUDES(fLock);

    SK_API sk_sp<SkStrike> createStrike(
            const SkStrikeSpec& strikeSpec,
            SkFontMetrics* maybeMetrics = nullptr,
            std::unique_ptr<SkStrikePinner> = nullptr) SK_EXCLUDES(fLock);

    SK_API sk_sp<SkStrike> findOrCreateStrike(const SkStrikeSpec& strikeSpec) SK_EXCLUDES(fLock);

    SK_API SkScopedStrikeForGPU findOrCreateScopedStrike(
    ...
    SK_API void purgeAll() SK_EXCLUDES(fLock); // does not change budget

    SK_API int getCacheCountLimit() const SK_EXCLUDES(fLock);
    SK_API int setCacheCountLimit(int limit) SK_EXCLUDES(fLock);
    SK_API int getCacheCountUsed() const SK_EXCLUDES(fLock);

    SK_API size_t getCacheSizeLimit() const SK_EXCLUDES(fLock);
    SK_API size_t setCacheSizeLimit(size_t limit) SK_EXCLUDES(fLock);
    SK_API size_t getTotalMemoryUsed() const SK_EXCLUDES(fLock);

private:
    friend class SkStrike;  // for SkStrike::updateDelta
    SK_API sk_sp<SkStrike> internalFindStrikeOrNull(const SkDescriptor& desc) SK_REQUIRES(fLock);
    SK_API sk_sp<SkStrike> internalCreateStrike(
            const SkStrikeSpec& strikeSpec,
            SkFontMetrics* maybeMetrics = nullptr,
            std::unique_ptr<SkStrikePinner> = nullptr) SK_REQUIRES(fLock);

    // The following methods can only be called when mutex is already held.
    SK_API void internalRemoveStrike(SkStrike* strike) SK_REQUIRES(fLock);
    SK_API void internalAttachToHead(sk_sp<SkStrike> strike) SK_REQUIRES(fLock);

    // Checkout budgets, modulated by the specified min-bytes-needed-to-purge,
    // and attempt to purge caches to match.
    // Returns number of bytes freed.
    SK_API size_t internalPurge(size_t minBytesNeeded = 0) SK_REQUIRES(fLock);

    // A simple accounting of what each glyph cache reports and the strike cache total.
    SK_API void validate() const SK_REQUIRES(fLock);

    SK_API void forEachStrike(std::function<void(const SkStrike&)> visitor) const SK_EXCLUDES(fLock);
