...
public:
    SK_API SkSharedMutex();
    SK_API ~SkSharedMutex();
    // Acquire lock for exclusive use.
    SK_API void acquire() SK_ACQUIRE();

    // Release lock for exclusive use.
    SK_API void release() SK_RELEASE_CAPABILITY();

    // Fail if exclusive is not held.
    SK_API void assertHeld() const SK_ASSERT_CAPABILITY(this);

    // Acquire lock for shared use.
    SK_API void acquireShared() SK_ACQUIRE_SHARED();

    // Release lock for shared use.
    SK_API void releaseShared() SK_RELEASE_SHARED_CAPABILITY();

    // Fail if shared lock not held.
    SK_API void assertHeldShared() const SK_ASSERT_SHARED_CAPABILITY(this);
