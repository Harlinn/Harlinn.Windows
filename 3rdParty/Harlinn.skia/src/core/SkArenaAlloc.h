...
#include <skia/private/SkTFitsIn.h>
...
extern SK_API std::array<const uint32_t, 47> SkFibonacci47;
...
class SkArenaAlloc {
public:
    SK_API SkArenaAlloc(char* block, size_t blockSize, size_t firstHeapAllocation);
...
private:
    ...
    SK_API static char* SkipPod(char* footerEnd);
    SK_API static void RunDtorsOnBlock(char* footerEnd);
    SK_API static char* NextBlock(char* footerEnd);
    ...
    SK_API void installFooter(FooterAction* releaser, uint32_t padding);

    SK_API void ensureSpace(uint32_t size, uint32_t alignment);
    ...
    SK_API char* allocObjectWithFooter(uint32_t sizeIncludingFooter, uint32_t alignment);
...
public:
    SK_API SkArenaAllocWithReset(char* block, size_t blockSize, size_t firstHeapAllocation);
