...
public:
    ...
    private:
        ...
        SK_API Block(Block* prev, int allocationSize);
    ...
    SK_API SkBlockAllocator(GrowthPolicy policy, size_t blockIncrementBytes,
     ...
     */
    SK_API size_t totalUsableSpace() const;
    /**
     ...
     */
    SK_API size_t totalSpaceInUse() const;
     ...
     */
    SK_API Block* findOwningBlock(const void* ptr);
     ...
     */
    SK_API void releaseBlock(Block* block);
     ...
     */
    SK_API void stealHeapBlocks(SkBlockAllocator* other);
...
private:
    ...
    SK_API void addBlock(int minSize, int maxSize);
...
SkBlockAllocator::ByteRange SkBlockAllocator::allocate(size_t size) {
    ...
    if (end > fTail->fSize) {
        this->addBlock(iSize + kBlockOverhead, static_cast<int>( MaxBlockSize<Align, Padding>() ));
