...
public:
    struct Key {
         ...
         */
        SK_API void init(void* nameSpace, uint64_t sharedID, size_t dataSize);
     ...
     */
    SK_API static bool Find(const Key& key, FindVisitor, void* context);
    SK_API static void Add(Rec*, void* payload = nullptr);

    typedef void (*Visitor)(const Rec&, void* context);
    // Call the visitor for every Rec in the cache.
    SK_API static void VisitAll(Visitor, void* context);

    SK_API static size_t GetTotalBytesUsed();
    SK_API static size_t GetTotalByteLimit();
    SK_API static size_t SetTotalByteLimit(size_t newLimit);

    SK_API static size_t SetSingleAllocationByteLimit( size_t );
    SK_API static size_t GetSingleAllocationByteLimit( );
    SK_API static size_t GetEffectiveSingleAllocationByteLimit();

    SK_API static void PurgeAll();
    SK_API static void CheckMessages();

    SK_API static void TestDumpMemoryStatistics();

    /** Dump memory usage statistics of every Rec in the cache using the
        SkTraceMemoryDump interface.
     */
    SK_API static void DumpMemoryStatistics(SkTraceMemoryDump* dump);

    /**
     *  Returns the DiscardableFactory used by the global cache, or nullptr.
     */
    SK_API static DiscardableFactory GetDiscardableFactory();

    SK_API static SkCachedData* NewCachedData(size_t bytes);

    SK_API static void PostPurgeSharedID(uint64_t sharedID);

    /**
     *  Call SkDebugf() with diagnostic information about the state of the cache
     */
    SK_API static void Dump();
     ...
     */
    SK_API SkResourceCache(DiscardableFactory);
     ...
     */
    SK_API explicit SkResourceCache(size_t byteLimit);
    SK_API ~SkResourceCache();
     ...
     *      false : Rec is "stale" -- the cache will purge it.
     */
    SK_API bool find(const Key&, FindVisitor, void* context);
    SK_API void add(Rec*, void* payload = nullptr);
    SK_API void visitAll(Visitor, void* context);
     ...
     */
    SK_API size_t setSingleAllocationByteLimit(size_t maximumAllocationSize);
    SK_API size_t getSingleAllocationByteLimit() const;
    // returns the logical single allocation size (pinning against the budget when the cache
    // is not backed by discardable memory.
    SK_API size_t getEffectiveSingleAllocationByteLimit() const;
     ...
     *  this new limit.
     */
    SK_API size_t setTotalByteLimit(size_t newLimit);

    SK_API void purgeSharedID(uint64_t sharedID);
    ...
    SK_API SkCachedData* newCachedData(size_t bytes);
...
private:
    ...
    SK_API void checkMessages();
    SK_API void purgeAsNeeded(bool forcePurge = false);

    // linklist management
    SK_API void moveToHead(Rec*);
    SK_API void addToHead(Rec*);
    SK_API void release(Rec*);
    SK_API void remove(Rec*);

    SK_API void init();    // called by constructors
