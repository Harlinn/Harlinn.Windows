...
public:
    SK_API SkBaseDevice(const SkImageInfo&, const SkSurfaceProps&);
    ...
    SK_API bool writePixels(const SkPixmap&, int x, int y);
     ...
     */
    SK_API bool accessPixels(SkPixmap* pmap);
     ...
     *  On failure, returns false and ignores the pixmap parameter.
     */
    SK_API bool peekPixels(SkPixmap*);
     ...
     */
    SK_API SkIPoint getOrigin() const;
    /**
     ...
     */
    SK_API bool isPixelAlignedToGlobal() const;
    /**
     ...
     */
    SK_API SkMatrix getRelativeTransform(const SkBaseDevice&) const;
    ...
    SK_API bool getLocalToMarker(uint32_t, SkM44* localToMarker) const override;
    ...
    SK_API void setGlobalCTM(const SkM44& ctm);
    ...
    SK_API void drawGlyphRunList(const SkGlyphRunList& glyphRunList, const SkPaint& paint);
...
protected:
    ...
    SK_API virtual void drawArc(const SkRect& oval, SkScalar startAngle,
                         SkScalar sweepAngle, bool useCenter, const SkPaint& paint);
    virtual void drawRRect(const SkRRect& rr,
                           const SkPaint& paint) = 0;

    // Default impl calls drawPath()
    SK_API virtual void drawDRRect(const SkRRect& outer,
    ...
    SK_API virtual void drawImageLattice(const SkImage*, const SkCanvas::Lattice&,
                                  const SkRect& dst, SkFilterMode, const SkPaint&);

    virtual void drawVertices(const SkVertices*, SkBlendMode, const SkPaint&) = 0;
    SK_API virtual void drawShadow(const SkPath&, const SkDrawShadowRec&);

    // default implementation calls drawVertices
    SK_API virtual void drawPatch(const SkPoint cubics[12], const SkColor colors[4],
                           const SkPoint texCoords[4], SkBlendMode, const SkPaint& paint);

    // default implementation calls drawVertices
    SK_API virtual void drawAtlas(const SkRSXform[], const SkRect[], const SkColor[], int count,
    ...
    // or uses clipPath().
    SK_API virtual void drawEdgeAAQuad(const SkRect& rect, const SkPoint clip[4],
    ...
    SK_API virtual void drawEdgeAAImageSet(const SkCanvas::ImageSetEntry[], int count,
                                    const SkPoint dstClips[], const SkMatrix preViewMatrices[],
                                    const SkSamplingOptions&, const SkPaint&,
                                    SkCanvas::SrcRectConstraint);

    SK_API virtual void drawDrawable(SkDrawable*, const SkMatrix*, SkCanvas*);
     ...
     */
    SK_API virtual void drawDevice(SkBaseDevice*, const SkSamplingOptions&, const SkPaint&);
     ...
     */
    SK_API virtual void drawSpecial(SkSpecialImage*, const SkMatrix& localToDevice,
                             const SkSamplingOptions&, const SkPaint&);
     ...
     */
    SK_API virtual void drawFilteredImage(const skif::Mapping& mapping, SkSpecialImage* src,
                                   const SkImageFilter*, const SkSamplingOptions&, const SkPaint&);

    SK_API virtual sk_sp<SkSpecialImage> makeSpecial(const SkBitmap&);
    SK_API virtual sk_sp<SkSpecialImage> makeSpecial(const SkImage*);

    // Get a view of the entire device's current contents as an image.
    SK_API sk_sp<SkSpecialImage> snapSpecial();
    ...
    SK_API virtual sk_sp<SkSpecialImage> snapSpecial(const SkIRect& subset, bool forceCopy = false);

    virtual void setImmutable() {}

    SK_API bool readPixels(const SkPixmap&, int x, int y);

    SK_API virtual sk_sp<SkSurface> makeSurface(const SkImageInfo&, const SkSurfaceProps&);
     ...
     */
    SK_API virtual bool onReadPixels(const SkPixmap&, int x, int y);
     ...
     */
    SK_API virtual bool onWritePixels(const SkPixmap&, int x, int y);
...
private:
    ...
    SK_API void simplifyGlyphRunRSXFormAndRedraw(const SkGlyphRunList& glyphRunList, const SkPaint& paint);
    ...
    SK_API bool SK_WARN_UNUSED_RESULT setDeviceCoordinateSystem(const SkM44& deviceToGlobal,
...
protected:
    ...
    SK_API void onSave() override;
    SK_API void onRestore() override;
    SK_API void onClipRect(const SkRect& rect, SkClipOp op, bool aa) override;
    SK_API void onClipRRect(const SkRRect& rrect, SkClipOp op, bool aa) override;
    SK_API void onClipPath(const SkPath& path, SkClipOp op, bool aa) override;
    SK_API void onClipRegion(const SkRegion& globalRgn, SkClipOp op) override;
    SK_API void onClipShader(sk_sp<SkShader> shader) override;
    SK_API void onReplaceClip(const SkIRect& rect) override;
    ...
    SK_API ClipType onGetClipType() const override;
