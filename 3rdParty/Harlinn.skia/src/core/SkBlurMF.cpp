...
static uint8_t eval_V(float top, int y, const uint8_t* integral, int integralSize, float sixSigma) {
    ...
    return static_cast<uint8_t>( integral[lower] * (1.0f - frac) + integral[lower + 1] * frac );
}
...
static std::unique_ptr<GrFragmentProcessor> find_or_create_rrect_blur_mask_fp(
    ...
    auto m = SkMatrix::Scale( static_cast<SkScalar>( dimensions.width() ), static_cast<SkScalar>( dimensions.height() ));
...
bool SkBlurMaskFilterImpl::canFilterMaskGPU(const GrStyledShape& shape,
    ...
    if (maskRect) {
        ...
        SkIRect clipRect = clipBounds.makeOutset(static_cast<int32_t>( sigma3 ), static_cast<int32_t>( sigma3 ));
        SkIRect srcRect = devSpaceShapeBounds.makeOutset( static_cast<int32_t>( sigma3 ), static_cast<int32_t>( sigma3 ));
