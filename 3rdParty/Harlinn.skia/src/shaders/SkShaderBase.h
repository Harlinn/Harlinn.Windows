...
public:
    SK_API ~SkShaderBase() override;

    SK_API sk_sp<SkShader> makeInvertAlpha() const;
    SK_API sk_sp<SkShader> makeWithCTM(const SkMatrix&) const;  // owns its own ctm
    ...
    struct ContextRec {
        ...
        SK_API bool isLegacyCompatible(SkColorSpace* shadersColorSpace) const;
    };

    class Context : public ::SkNoncopyable {
    public:
        SK_API Context(const SkShaderBase& shader, const ContextRec&);

        SK_API virtual ~Context();
     ...
     */
    SK_API Context* makeContext(const ContextRec&, SkArenaAlloc*) const;
     ...
     */
    SK_API virtual std::unique_ptr<GrFragmentProcessor> asFragmentProcessor(const GrFPArgs&) const;
#endif
     ...
     */
    SK_API bool asLuminanceColor(SkColor*) const;

    // If this returns false, then we draw nothing (do not fall back to shader context)
    SK_API SK_WARN_UNUSED_RESULT
    bool appendStages(const SkStageRec&) const;

    SK_API bool SK_WARN_UNUSED_RESULT computeTotalInverse(const SkMatrix& ctm,
    ...
    //
    SK_API SkTCopyOnFirstWrite<SkMatrix> totalLocalMatrix(const SkMatrix* preLocalMatrix) const;
     ...
     */
    SK_API virtual sk_sp<SkShader> makeAsALocalMatrixShader(SkMatrix* localMatrix) const;

    SK_API SkUpdatableShader* updatableShader(SkArenaAlloc* alloc) const;
    SK_API virtual SkUpdatableShader* onUpdatableShader(SkArenaAlloc* alloc) const;
...
protected:
    SK_API SkShaderBase(const SkMatrix* localMatrix = nullptr);
    ...
    SK_API virtual bool onAppendStages(const SkStageRec&) const;

    virtual SkStageUpdater* onAppendUpdatableStages(const SkStageRec&) const { return nullptr; }

protected:
    SK_API static skvm::Coord ApplyMatrix(skvm::Builder*, const SkMatrix&, skvm::Coord, skvm::Uniforms*);
