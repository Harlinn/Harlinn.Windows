...
public:
     ...
     */
    SK_API static sk_sp<SkAnimatedImage> Make(std::unique_ptr<SkAndroidCodec>,
    ...
    SK_API ~SkAnimatedImage() override;
     ...
     */
    SK_API sk_sp<SkImage> getCurrentFrame();
     ...
     */
    SK_API void setRepetitionCount(int count);
...
protected:
    SK_API SkRect onGetBounds() override;
    SK_API void onDraw(SkCanvas*) override;
...
private:
    struct Frame {
        ...
        SK_API Frame();
        SK_API bool init(const SkImageInfo& info, OnInit);
        SK_API bool copyTo(Frame*) const;
    };
    ...
    SK_API SkAnimatedImage(std::unique_ptr<SkAndroidCodec>, const SkImageInfo& requestedInfo,
            SkIRect cropRect, sk_sp<SkPicture> postProcess);

    SK_API int computeNextFrame(int current, bool* animationEnded);
    SK_API double finish();
     ...
     */
    SK_API sk_sp<SkImage> getCurrentFrameSimple();
