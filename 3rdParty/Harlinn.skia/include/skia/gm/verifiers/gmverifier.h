...
private:
    ...
    SK_API VerifierResult(Code code, const SkString& msg);
};
...
public:
    ...
    SK_API bool needsGoldImage() const;
     ...
     */
    SK_API VerifierResult verify(const SkBitmap& gold, const SkBitmap& actual);
     ...
     * @return Ok if the verification passed, or an error if not.
     */
    SK_API VerifierResult verify(const SkBitmap& actual);

    /** Renders the GM using the "golden" configuration. This is common across all GMs/verifiers. */
    SK_API static SkBitmap RenderGoldBmp(skiagm::GM* gm, const SkColorInfo& colorInfo);
     ...
     */
    SK_API static SkColorInfo VerifierColorInfo();
...
public:
    ...
    SK_API explicit VerifierList(GM* gm);

    /** Adds a verifier to the list of verifiers. */
    SK_API void add(std::unique_ptr<GMVerifier> verifier);
     ...
     * image as input.
     */
    SK_API VerifierResult verifyAll(const SkColorInfo& colorInfo, const SkBitmap& actual);
