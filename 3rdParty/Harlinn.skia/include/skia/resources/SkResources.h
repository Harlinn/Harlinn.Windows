...
#include <skia/utils/SkAnimCodecPlayer.h>
...
class SK_API MultiFrameImageAsset final : public ImageAsset {
public:
    /**
 ...
 */
class SK_API ExternalTrackAsset : public SkRefCnt {
public:
    /**
...
public:
    SK_API static sk_sp<FileResourceProvider> Make(SkString base_dir, bool predecode = false);

    SK_API sk_sp<SkData> load(const char resource_path[], const char resource_name[]) const override;

    SK_API sk_sp<ImageAsset> loadImageAsset(const char[], const char[], const char[]) const override;
...
class SK_API ResourceProviderProxyBase : public ResourceProvider {
protected:
...
private:
    SK_API explicit CachingResourceProvider(sk_sp<ResourceProvider>);
...
public:
    SK_API static sk_sp<DataURIResourceProviderProxy> Make(sk_sp<ResourceProvider> rp,
