...
public:
     ...
     */
    SK_API static bool Encode(SkWStream* dst, const SkPixmap& src, const Options& options);
     ...
     */
    SK_API static std::unique_ptr<SkEncoder> Make(SkWStream* dst, const SkPixmap& src,
                                           const Options& options);

    SK_API ~SkJpegEncoder() override;

protected:
    SK_API bool onEncodeRows(int numRows) override;

private:
    SK_API SkJpegEncoder(std::unique_ptr<SkJpegEncoderMgr>, const SkPixmap& src);
