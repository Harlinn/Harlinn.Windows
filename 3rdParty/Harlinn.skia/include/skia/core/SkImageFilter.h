...
public:
     ...
     * may be incorrect.
     */
    SK_API SkIRect filterBounds(const SkIRect& src, const SkMatrix& ctm,
     ...
     */
    SK_API bool isColorFilterNode(SkColorFilter** filterPtr) const;
     ...
     *  way.
     */
    SK_API bool asAColorFilter(SkColorFilter** filterPtr) const;

    /**
     *  Returns the number of inputs this filter will accept (some inputs can be NULL).
     */
    SK_API int countInputs() const;

    /**
     *  Returns the input filter at a given index, or NULL if no input is connected.  The indices
     *  used are filter-specific.
     */
    SK_API const SkImageFilter* getInput(int i) const;

    // Default impl returns union of all input bounds.
    SK_API virtual SkRect computeFastBounds(const SkRect& bounds) const;
     ...
     *  else return null.
     */
    SK_API sk_sp<SkImageFilter> makeWithLocalMatrix(const SkMatrix& matrix) const;
