...
public:
    ...
    public:
        ...
        SK_API SupportedDataTypes(const GrImageContext&);
         ...
         */
        SK_API void enableDataType(DataType, int numChannels);
     ...
     */
    SK_API static std::tuple<int, DataType> NumChannelsAndDataType(SkColorType);
     ...
     */
    SK_API SkYUVAPixmapInfo(const SkYUVAInfo&,
     ...
     */
    SK_API SkYUVAPixmapInfo(const SkYUVAInfo&, DataType, const size_t rowBytes[kMaxPlanes]);

    SK_API SkYUVAPixmapInfo(const SkYUVAPixmapInfo&) = default;

    SK_API SkYUVAPixmapInfo& operator=(const SkYUVAPixmapInfo&) = default;

    SK_API bool operator==(const SkYUVAPixmapInfo&) const;
     ...
     */
    SK_API size_t computeTotalBytes(size_t planeSizes[kMaxPlanes] = nullptr) const;
     ...
     */
    SK_API bool initPixmapsFromSingleAllocation(void* memory, SkPixmap pixmaps[kMaxPlanes]) const;
    ...
    SK_API bool isSupported(const SupportedDataTypes&) const;
...
public:
    ...
    SK_API static SkColorType RecommendedRGBAColorType(DataType);

    /** Allocate space for pixmaps' pixels in the SkYUVAPixmaps. */
    SK_API static SkYUVAPixmaps Allocate(const SkYUVAPixmapInfo& yuvaPixmapInfo);

    /**
     * Use storage in SkData as backing store for pixmaps' pixels. SkData is retained by the
     * SkYUVAPixmaps.
     */
    SK_API static SkYUVAPixmaps FromData(const SkYUVAPixmapInfo&, sk_sp<SkData>);

    /**
     * Makes a deep copy of the src SkYUVAPixmaps. The returned SkYUVAPixmaps owns its planes'
     * backing stores.
     */
    SK_API static SkYUVAPixmaps MakeCopy(const SkYUVAPixmaps& src);
     ...
     */
    SK_API static SkYUVAPixmaps FromExternalMemory(const SkYUVAPixmapInfo&, void* memory);
     ...
     */
    SK_API static SkYUVAPixmaps FromExternalPixmaps(const SkYUVAInfo&, const SkPixmap[kMaxPlanes]);
    ...
    SK_API SkYUVAPixmapInfo pixmapsInfo() const;
     ...
     */
    SK_API SkYUVAInfo::YUVALocations toYUVALocations() const;

    /** Does this SkPixmaps own the backing store of the planes? */
    bool ownsStorage() const { return SkToBool(fData); }

private:
    SK_API SkYUVAPixmaps(const SkYUVAPixmapInfo&, sk_sp<SkData>);
    SK_API SkYUVAPixmaps(const SkYUVAInfo&, DataType, const SkPixmap[kMaxPlanes]);
