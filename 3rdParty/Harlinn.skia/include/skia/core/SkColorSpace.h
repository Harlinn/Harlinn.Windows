...
public:
     ...
     */
    SK_API static sk_sp<SkColorSpace> MakeSRGB();

    /**
     *  Colorspace with the sRGB primaries, but a linear (1.0) gamma.
     */
    SK_API static sk_sp<SkColorSpace> MakeSRGBLinear();

    /**
     *  Create an SkColorSpace from a transfer function and a row-major 3x3 transformation to XYZ.
     */
    SK_API static sk_sp<SkColorSpace> MakeRGB(const skcms_TransferFunction& transferFn,
                                       const skcms_Matrix3x3& toXYZ);

    /**
     *  Create an SkColorSpace from a parsed (skcms) ICC profile.
     */
    SK_API static sk_sp<SkColorSpace> Make(const skcms_ICCProfile&);

    /**
     *  Convert this color space to an skcms ICC profile struct.
     */
    SK_API void toProfile(skcms_ICCProfile*) const;

    /**
     *  Returns true if the color space gamma is near enough to be approximated as sRGB.
     */
    SK_API bool gammaCloseToSRGB() const;

    /**
     *  Returns true if the color space gamma is linear.
     */
    SK_API bool gammaIsLinear() const;
     ...
     */
    SK_API bool isNumericalTransferFn(skcms_TransferFunction* fn) const;

    /**
     *  Returns true and sets |toXYZD50| if the color gamut can be described as a matrix.
     *  Returns false otherwise.
     */
    SK_API bool toXYZD50(skcms_Matrix3x3* toXYZD50) const;
     ...
     *  linear sRGB.
     */
    SK_API sk_sp<SkColorSpace> makeLinearGamma() const;
     ...
     *  sRGB.
     */
    SK_API sk_sp<SkColorSpace> makeSRGBGamma() const;
     ...
     */
    SK_API sk_sp<SkColorSpace> makeColorSpin() const;
     ...
     */
    SK_API size_t writeToMemory(void* memory) const;

    SK_API static sk_sp<SkColorSpace> Deserialize(const void* data, size_t length);

    /**
     *  If both are null, we return true.  If one is null and the other is not, we return false.
     *  If both are non-null, we do a deeper compare.
     */
    SK_API static bool Equals(const SkColorSpace*, const SkColorSpace*);

    SK_API void       transferFn(float gabcdef[7]) const;  // DEPRECATED: Remove when webview usage is gone
    SK_API void       transferFn(skcms_TransferFunction* fn) const;
    SK_API void    invTransferFn(skcms_TransferFunction* fn) const;
    SK_API void gamutTransformTo(const SkColorSpace* dst, skcms_Matrix3x3* src_to_dst) const;
...
private:
    ...
    SK_API SkColorSpace(const skcms_TransferFunction& transferFn, const skcms_Matrix3x3& toXYZ);

    SK_API void computeLazyDstFields() const;
