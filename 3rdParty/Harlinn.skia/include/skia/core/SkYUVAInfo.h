...
public:
    ...
    SK_API static std::tuple<int, int> SubsamplingFactors(Subsampling);
     ...
     */
    SK_API static std::tuple<int, int> PlaneSubsamplingFactors(PlaneConfig, Subsampling, int planeIdx);
     ...
     * dimensions).
     */
    SK_API static int PlaneDimensions(SkISize imageDimensions,
                               PlaneConfig,
                               Subsampling,
                               SkEncodedOrigin,
    ...
    SK_API static constexpr int NumPlanes(PlaneConfig);

    /**
     * Number of Y, U, V, A channels in the ith plane for a given PlaneConfig (or 0 if i is
     * invalid).
     */
    SK_API static constexpr int NumChannelsInPlane(PlaneConfig, int i);
     ...
     */
    SK_API static YUVALocations GetYUVALocations(PlaneConfig, const uint32_t* planeChannelFlags);

    /** Does the PlaneConfig have alpha values? */
    SK_API static bool HasAlpha(PlaneConfig);
     ...
     */
    SK_API SkYUVAInfo(SkISize dimensions,
               PlaneConfig,
               Subsampling,
               SkYUVColorSpace,
     ...
     */
    SK_API size_t computeTotalBytes(const size_t rowBytes[kMaxPlanes],
     ...
     * -1).
     */
    SK_API YUVALocations toYUVALocations(const uint32_t* channelFlags) const;
     ...
     * SkYUVAInfo.
     */
    SK_API SkYUVAInfo makeSubsampling(SkYUVAInfo::Subsampling) const;
     ...
     */
    SK_API SkYUVAInfo makeDimensions(SkISize) const;

    SK_API bool operator==(const SkYUVAInfo& that) const;
...
inline constexpr int SkYUVAInfo::NumPlanes(PlaneConfig planeConfig) {
    switch (planeConfig) {
...
inline constexpr int SkYUVAInfo::NumChannelsInPlane(PlaneConfig config, int i) {
    switch (config) {
