...
class SkPathBuilder {
public:
    SK_API SkPathBuilder();
    SK_API SkPathBuilder(SkPathFillType);
    SK_API SkPathBuilder(const SkPath&);
    SK_API SkPathBuilder(const SkPathBuilder&) = default;
    SK_API ~SkPathBuilder();

    SK_API SkPathBuilder& operator=(const SkPath&);
    SK_API SkPathBuilder& operator=(const SkPathBuilder&) = default;

    SkPathFillType fillType() const { return fFillType; }
    SK_API SkRect computeBounds() const;

    SK_API SkPath snapshot() const;  // the builder is unchanged after returning this path
    SK_API SkPath detach();    // the builder is reset to empty after returning this path
    ...
    SK_API SkPathBuilder& conicTo(SkPoint pt1, SkPoint pt2, SkScalar w);
    ...
    SK_API SkPathBuilder& cubicTo(SkPoint pt1, SkPoint pt2, SkPoint pt3);
    ...
    SK_API SkPathBuilder& close();

    // Append a series of lineTo(...)
    SK_API SkPathBuilder& polylineTo(const SkPoint pts[], int count);
    ...
    */
    SK_API SkPathBuilder& arcTo(const SkRect& oval, SkScalar startAngleDeg, SkScalar sweepAngleDeg,
                         bool forceMoveTo);
    ...
    */
    SK_API SkPathBuilder& arcTo(SkPoint p1, SkPoint p2, SkScalar radius);
    ...
    /** Appends arc to SkPath. Arc is implemented by one or more conic weighted to describe
        ...
        @return             reference to SkPath
    */
    SK_API SkPathBuilder& arcTo(SkPoint r, SkScalar xAxisRotate, ArcSize largeArc, SkPathDirection sweep,
                         SkPoint xy);
    ...
    */
    SK_API SkPathBuilder& addArc(const SkRect& oval, SkScalar startAngleDeg, SkScalar sweepAngleDeg);

    // Add a new contour

    SK_API SkPathBuilder& addRect(const SkRect&, SkPathDirection, unsigned startIndex);
    SK_API SkPathBuilder& addOval(const SkRect&, SkPathDirection, unsigned startIndex);
    SK_API SkPathBuilder& addRRect(const SkRRect&, SkPathDirection, unsigned startIndex);
    ...
    SK_API SkPathBuilder& addCircle(SkScalar center_x, SkScalar center_y, SkScalar radius,
                             SkPathDirection dir = SkPathDirection::kCW);

    SK_API SkPathBuilder& addPolygon(const SkPoint pts[], int count, bool isClosed);
    ...
    SK_API SkPathBuilder& addPath(const SkPath&);

    // Performance hint, to reserve extra storage for subsequent calls to lineTo, quadTo, etc.

    SK_API void incReserve(int extraPtCount, int extraVerbCount);
    void incReserve(int extraPtCount) {
        this->incReserve(extraPtCount, extraPtCount);
    }

    SK_API SkPathBuilder& offset(SkScalar dx, SkScalar dy);
...
private:
    ...
    SK_API SkPath make(sk_sp<SkPathRef>) const;

    SK_API SkPathBuilder& privateReverseAddPath(const SkPath&);
