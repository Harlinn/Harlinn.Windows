...
public:
     ...
     *
     */
    SK_API static sk_sp<SkPathEffect> MakeSum(sk_sp<SkPathEffect> first, sk_sp<SkPathEffect> second);
     ...
     */
    SK_API static sk_sp<SkPathEffect> MakeCompose(sk_sp<SkPathEffect> outer, sk_sp<SkPathEffect> inner);
    ...
    SK_API DashType asADash(DashInfo* info) const;
     ...
     */
    SK_API bool filterPath(SkPath* dst, const SkPath& src, SkStrokeRec*, const SkRect* cullR) const;

    /** Version of filterPath that can be called when the CTM is known. */
    SK_API bool filterPath(SkPath* dst, const SkPath& src, SkStrokeRec*, const SkRect* cullR,
                    const SkMatrix& ctm) const;

    /** True if this path effect requires a valid CTM */
    SK_API bool needsCTM() const;

    SK_API static sk_sp<SkPathEffect> Deserialize(const void* data, size_t size,
