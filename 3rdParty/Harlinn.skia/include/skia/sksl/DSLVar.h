...
public:
     ...
     */
    SK_API DSLVarBase(DSLType type, skstd::string_view name, DSLExpression initialValue, PositionInfo pos);

    SK_API DSLVarBase(DSLType type, DSLExpression initialValue, PositionInfo pos);

    SK_API DSLVarBase(const DSLModifiers& modifiers, DSLType type, skstd::string_view name,
               DSLExpression initialValue, PositionInfo pos);

    SK_API DSLVarBase(const DSLModifiers& modifiers, DSLType type, DSLExpression initialValue,
               PositionInfo pos);

    DSLVarBase(DSLVarBase&&) = default;

    SK_API virtual ~DSLVarBase();
...
protected:
    SK_API DSLPossibleExpression assign(DSLExpression other);

    SK_API void swap(DSLVarBase& other);
...
public:
    ...
    SK_API VariableStorage storage() const override;

    SK_API void swap(DSLVar& other);
...
public:
    ...
    SK_API DSLGlobalVar(const char* name);

    DSLGlobalVar(DSLGlobalVar&&) = default;

    SK_API VariableStorage storage() const override;

    SK_API void swap(DSLGlobalVar& other);
     ...
     */
    SK_API DSLExpression eval(DSLExpression x, PositionInfo pos = PositionInfo::Capture());

    /**
     * Implements the following method call:
     *     half4 blender::eval(half4 src, half4 dst);
     */
    SK_API DSLExpression eval(DSLExpression x, DSLExpression y,
            PositionInfo pos = PositionInfo::Capture());

private:
    SK_API DSLExpression eval(ExpressionArray args, PositionInfo pos);

    SK_API std::unique_ptr<SkSL::Expression> methodCall(skstd::string_view methodName, PositionInfo pos);
...
public:
    ...
    SK_API DSLParameter(DSLParameter&&) = default;

    SK_API VariableStorage storage() const override;

    SK_API void swap(DSLParameter& other);
