...
class SkRemotableFontIdentitySet : public SkRefCnt {
public:
    SK_API SkRemotableFontIdentitySet(int count, SkFontIdentity** data);

    int count() const { return fCount; }
    const SkFontIdentity& at(int index) const { return fData[index]; }

    SK_API static SkRemotableFontIdentitySet* NewEmpty();

private:
    SkRemotableFontIdentitySet() : fCount(0), fData() { }

    SK_API friend SkRemotableFontIdentitySet* sk_remotable_font_identity_set_new();
...
class SkRemotableFontMgr : public SkRefCnt {
public:
    /**
