...
public:
    SK_API static sk_sp<Text> Make(sk_sp<SkTypeface> tf, const SkString& text);
    SK_API ~Text() override;
...
private:
    SK_API Text(sk_sp<SkTypeface>, const SkString&);

    SK_API SkPoint alignedPosition(SkScalar advance) const;
...
public:
    SK_API static sk_sp<TextBlob> Make(sk_sp<SkTextBlob> = nullptr);
    SK_API ~TextBlob() override;
...
private:
    SK_API explicit TextBlob(sk_sp<SkTextBlob>);
