...
public:
     ...
     */
    SK_API static sk_sp<GrDirectContext> MakeGL(sk_sp<const GrGLInterface>, const GrContextOptions&);
    SK_API static sk_sp<GrDirectContext> MakeGL(sk_sp<const GrGLInterface>);
    SK_API static sk_sp<GrDirectContext> MakeGL(const GrContextOptions&);
    SK_API static sk_sp<GrDirectContext> MakeGL();
#endif
     ...
     */
    SK_API static sk_sp<GrDirectContext> MakeVulkan(const GrVkBackendContext&, const GrContextOptions&);
    SK_API static sk_sp<GrDirectContext> MakeVulkan(const GrVkBackendContext&);
#endif
     ...
     */
    SK_API static sk_sp<GrDirectContext> MakeDirect3D(const GrD3DBackendContext&, const GrContextOptions&);
    SK_API static sk_sp<GrDirectContext> MakeDirect3D(const GrD3DBackendContext&);
#endif

#ifdef SK_DAWN
    SK_API static sk_sp<GrDirectContext> MakeDawn(const wgpu::Device&,
                                           const GrContextOptions&);
    SK_API static sk_sp<GrDirectContext> MakeDawn(const wgpu::Device&);
#endif

    SK_API static sk_sp<GrDirectContext> MakeMock(const GrMockOptions*, const GrContextOptions&);
    SK_API static sk_sp<GrDirectContext> MakeMock(const GrMockOptions*);

    SK_API ~GrDirectContext() override;
     ...
     */
    SK_API void resetContext(uint32_t state = kAll_GrBackendState);
     ...
     */
    SK_API void resetGLTextureBindings();
     ...
     */
    SK_API void abandonContext() override;
     ...
     * context.
     */
    SK_API bool abandoned() override;
     ...
     */
    SK_API void releaseResourcesAndAbandonContext();
     ...
     */
    SK_API void getResourceCacheLimits(int* maxResources, size_t* maxResourceBytes) const;

    /**
     *  Return the current GPU resource cache limit in bytes.
     */
    SK_API size_t getResourceCacheLimit() const;
     ...
     *                          in the cache.
     */
    SK_API void getResourceCacheUsage(int* resourceCount, size_t* resourceBytes) const;

    /**
     *  Gets the number of bytes in the cache consumed by purgeable (e.g. unlocked) resources.
     */
    SK_API size_t getResourceCachePurgeableBytes() const;
     ...
     */
    SK_API void setResourceCacheLimits(int maxResources, size_t maxResourceBytes);
     ...
     *                          that can be held in the cache.
     */
    SK_API void setResourceCacheLimit(size_t maxResourceBytes);
     ...
     */
    SK_API void performDeferredCleanup(std::chrono::milliseconds msNotUsed,
     ...
     *                               resource types.
     */
    SK_API void purgeUnlockedResources(size_t bytesToPurge, bool preferScratchResources);
     ...
     */
    SK_API void purgeUnlockedResources(bool scratchResourcesOnly);
     ...
     * flush calls.
     */
    SK_API bool wait(int numSemaphores, const GrBackendSemaphore* waitSemaphores,
     ...
     */
    SK_API GrSemaphoresSubmitted flush(const GrFlushInfo& info);
     ...
     * submitted work.
     */
    SK_API bool submit(bool syncCpu = false);

    /**
     * Checks whether any asynchronous work is complete and if so calls related callbacks.
     */
    SK_API void checkAsyncWorkCompletion();

    /** Enumerates all cached GPU resources and dumps their memory to traceMemoryDump. */
    // Chrome is using this!
    SK_API void dumpMemoryStatistics(SkTraceMemoryDump* traceMemoryDump) const;

    SK_API bool supportsDistanceFieldText() const;

    SK_API void storeVkPipelineCacheData();
     ...
     */
      ...
      */
    SK_API GrBackendTexture createBackendTexture(int width, int height,
    ...
    SK_API GrBackendTexture createBackendTexture(int width, int height,
      ...
      *      VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL
      */
    SK_API GrBackendTexture createBackendTexture(const SkPixmap srcData[],
                                           int numLevels,
                                           GrSurfaceOrigin,
                                           GrRenderable,
                                           GrProtected,
     ...
     *      VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL
     */
    SK_API bool updateBackendTexture(const GrBackendTexture&,
                              const SkColor4f& color,
     ...
     *      VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL
     */
    SK_API GrBackendTexture createCompressedBackendTexture(int width, int height,
                                                    const GrBackendFormat&,
                                                    const SkColor4f& color,
                                                    GrMipmapped,
                                                    GrProtected = GrProtected::kNo,
     ...
     *      VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL
     */
    SK_API bool updateCompressedBackendTexture(const GrBackendTexture&,
                                        const SkColor4f& color,
                                        GrGpuFinishedProc finishedProc,
                                        GrGpuFinishedContext finishedContext);
    ...
    SK_API void deleteBackendTexture(GrBackendTexture);
    ...
    SK_API bool precompileShader(const SkData& key, const SkData& data);
    ...
    public:
        SK_API static GrDirectContext::DirectContextID Next();
    ...
    SK_API GrDirectContextPriv priv();
    SK_API const GrDirectContextPriv priv() const;  // NOLINT(readability-const-return-type)

protected:
    SK_API GrDirectContext(GrBackendApi backend, const GrContextOptions& options);

    SK_API bool init() override;

    GrAtlasManager* onGetAtlasManager() { return fAtlasManager.get(); }
    SK_API skgpu::v1::SmallPathAtlasMgr* onGetSmallPathAtlasMgr();
...
private:
    ...
    SK_API void syncAllOutstandingGpuWork(bool shouldExecuteWhileAbandoned);
