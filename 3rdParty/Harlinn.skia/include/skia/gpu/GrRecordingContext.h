...
#include <skia/private/GrImageContext.h>
...
public:
    SK_API ~GrRecordingContext() override;
    ...
    class Arenas {
    public:
        SK_API Arenas(SkArenaAlloc*, GrSubRunAllocator*);
...
protected:
    ...
    class OwnedArenas {
    public:
        SK_API OwnedArenas(bool ddlRecording);
        SK_API ~OwnedArenas();

        SK_API Arenas get();

        SK_API OwnedArenas& operator=(OwnedArenas&&);
    ...
    SK_API GrRecordingContext(sk_sp<GrContextThreadSafeProxy>, bool ddlRecording);

    SK_API bool init() override;

    SK_API void abandonContext() override;

    SK_API GrDrawingManager* drawingManager();
    ...
    SK_API void destroyDrawingManager();
    ...
    SK_API OwnedArenas&& detachArenas();
    ...
    struct ProgramData {
        SK_API ProgramData(std::unique_ptr<const GrProgramDesc>, const GrProgramInfo*);
        SK_API ProgramData(ProgramData&&);                     // for SkTArray
        SK_API ProgramData(const ProgramData&) = delete;
        SK_API ~ProgramData();
    ...
    SK_API GrTextBlobCache* getTextBlobCache();
    SK_API const GrTextBlobCache* getTextBlobCache() const;

    SK_API GrThreadSafeCache* threadSafeCache();
    SK_API const GrThreadSafeCache* threadSafeCache() const;
...
#if GR_TEST_UTILS
        SK_API void dump(SkString* out) const;
        SK_API void dumpKeyValuePairs(SkTArray<SkString>* keys, SkTArray<double>* values) const;
#endif
    ...
    struct DMSAAStats {
        ...
        SK_API void merge(const DMSAAStats&);
