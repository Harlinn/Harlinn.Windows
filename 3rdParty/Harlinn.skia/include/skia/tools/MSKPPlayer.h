...
public:
    ...
    SK_API static std::unique_ptr<MSKPPlayer> Make(SkStreamSeekable* stream);
    ...
    SK_API SkISize frameDimensions(int i) const;
     ...
     */
    SK_API bool playFrame(SkCanvas* canvas, int i);

    /** Destroys any cached offscreen layers. */
    SK_API void resetLayers();
     ...
     */
    SK_API void allocateLayers(SkCanvas*);
     ...
     */
    SK_API std::vector<int> layerIDs(int frame = -1) const;
     ...
     */
    SK_API sk_sp<SkImage> layerSnapshot(int layerID) const;
...
private:
    ...
    SK_API static sk_sp<SkSurface> MakeSurfaceForLayer(const LayerCmds&, SkCanvas* rootCanvas);

    SK_API void collectReferencedLayers(const LayerCmds& layer, std::vector<int>*) const;
