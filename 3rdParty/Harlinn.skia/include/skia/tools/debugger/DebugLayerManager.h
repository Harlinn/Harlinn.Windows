...
#include <skia/tools/debugger/DebugCanvas.h>
...
public:
    ...
    SK_API void storeSkPicture(int nodeId, int frame, sk_sp<SkPicture> picture, SkIRect dirty);

    // Set's the command playback head for a given picture/draw event.
    SK_API void setCommand(int nodeId, int frame, int command);

    SK_API void drawLayerEventTo(SkSurface*, const int nodeId, const int frame);
    ...
    SK_API sk_sp<SkImage> getLayerAsImage(const int nodeId, const int frame);
    ...
    SK_API DrawEventSummary event(int nodeId, int frame) const;
    ...
    SK_API std::vector<LayerSummary> summarizeLayers(int frame) const;

    // Return the list of node ids which have DrawEvents on the given frame
    SK_API std::vector<int> listNodesForFrame(int frame) const;
    // Return the list of frames on which the given node had DrawEvents.
    SK_API std::vector<int> listFramesForNode(int nodeId) const;

    // asks the DebugCanvas of the indicated draw event to serialize it's commands as JSON.
    SK_API void toJSON(SkJSONWriter&, UrlDataManager&, SkCanvas*, int nodeId, int frame);

    // return a pointer to the debugcanvas of a given draw event.
    SK_API DebugCanvas* getEventDebugCanvas(int nodeid, int frame);

    // forwards the provided setting to all debugcanvases.
    SK_API void setOverdrawViz(bool overdrawViz);
    SK_API void setClipVizColor(SkColor clipVizColor);
    SK_API void setDrawGpuOpBounds(bool drawGpuOpBounds);
