...
#include <skia/core/SkPromiseImageTexture.h>
...
public:
    ...
    SK_API ~PromiseImageCallbackContext() override;

    const GrBackendFormat& backendFormat() const { return fBackendFormat; }

    SK_API void setBackendTexture(const GrBackendTexture& backendTexture);

    SK_API void destroyBackendTexture();
...
public:
    ...
    // images.
    SK_API sk_sp<SkPicture> recreateSKP(GrDirectContext*, SkPicture*);

    SK_API void uploadAllToGPU(SkTaskGroup*, GrDirectContext*);
    SK_API void deleteAllFromGPU(SkTaskGroup*, GrDirectContext*);
...
private:
    SK_API void createCallbackContexts(GrDirectContext*);
    // reinflate a deflated SKP, replacing all the indices with promise images.
    SK_API sk_sp<SkPicture> reinflateSKP(sk_sp<GrContextThreadSafeProxy>, SkData* deflatedSKP);
    ...
    public:
        SK_API PromiseImageInfo(int index, uint32_t originalUniqueID, const SkImageInfo& ii);
        SK_API PromiseImageInfo(PromiseImageInfo&& other);
        SK_API ~PromiseImageInfo();
        ...
        SK_API std::unique_ptr<SkPixmap[]> normalMipLevels() const;
        SK_API int numMipLevels() const;
        ...
        SK_API void setMipLevels(const SkBitmap& baseLevel, std::unique_ptr<SkMipmap> mipLevels);
    ...
    SK_API static void CreateBETexturesForPromiseImage(GrDirectContext*, PromiseImageInfo*);
    SK_API static void DeleteBETexturesForPromiseImage(PromiseImageInfo*);

    SK_API static sk_sp<SkImage> CreatePromiseImages(const void* rawData, size_t length, void* ctxIn);
    ...
    SK_API void uploadImage(GrDirectContext*, PromiseImageInfo*);

    // returns -1 if not found
    SK_API int findImage(SkImage* image) const;

    // returns -1 on failure
    SK_API int addImage(SkImage* image);

    // returns -1 on failure
    SK_API int findOrDefineImage(SkImage* image);
