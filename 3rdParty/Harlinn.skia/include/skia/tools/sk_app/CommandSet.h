...
public:
    ...
    SK_API void attach(Window* window);
    SK_API bool onKey(skui::Key key, skui::InputState state, skui::ModifierKey modifiers);
    SK_API bool onChar(SkUnichar, skui::ModifierKey modifiers);
    SK_API bool onSoftkey(const SkString& softkey);

    SK_API void addCommand(SkUnichar c, const char* group, const char* description,
                    std::function<void(void)> function);
    SK_API void addCommand(skui::Key k, const char* keyName, const char* group, const char* description,
                    std::function<void(void)> function);

    SK_API void drawHelp(SkCanvas* canvas);

    SK_API std::vector<SkString> getCommandsAsSoftkeys() const;
...
private:
    ...
    SK_API static bool compareCommandKey(const Command& first, const Command& second);
    SK_API static bool compareCommandGroup(const Command& first, const Command& second);
