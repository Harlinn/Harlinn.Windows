...
public:
    ...
    SK_API explicit GrContextFactory(const GrContextOptions& opts);
    SK_API GrContextFactory();

    SK_API ~GrContextFactory();

    SK_API void destroyContexts();
    SK_API void abandonContexts();
    SK_API void releaseResourcesAndAbandonContexts();

    /**
     * Get a context initialized with a type of GL context. It also makes the GL context current.
     */
    SK_API ContextInfo getContextInfo(ContextType type, ContextOverrides = ContextOverrides::kNone);
     ...
     */
    SK_API ContextInfo getSharedContextInfo(GrDirectContext* shareContext, uint32_t shareIndex = 0);

    /**
     * Get a GrContext initialized with a type of GL context. It also makes the GL context current.
     */
    SK_API GrDirectContext* get(ContextType type, ContextOverrides overrides = ContextOverrides::kNone);
    const GrContextOptions& getGlobalOptions() const { return fGlobalOptions; }

private:
    SK_API ContextInfo getContextInfoInternal(ContextType type, ContextOverrides overrides,
