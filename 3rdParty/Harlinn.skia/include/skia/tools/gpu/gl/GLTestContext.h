...
public:
    ...
    SK_API void testAbandon() override;

    /** Wait until all GPU work is finished. */
    SK_API void finish() override;

    SK_API void overrideVersion(const char* version, const char* shadingLanguageVersion);
    ...
    SK_API sk_sp<GrDirectContext> makeContext(const GrContextOptions& options) override;

protected:
    SK_API GLTestContext();

    /*
     * Methods that subclasses must call from their constructors and destructors.
     */
    SK_API void init(sk_sp<const GrGLInterface>);

    SK_API void teardown() override;
 ...
 */
SK_API GLTestContext* CreatePlatformGLTestContext(GrGLStandard forcedGpuAPI,
