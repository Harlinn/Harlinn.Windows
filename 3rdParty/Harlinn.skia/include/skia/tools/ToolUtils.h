...
SK_API const char* alphatype_name (SkAlphaType);
SK_API const char* colortype_name (SkColorType);
SK_API const char* colortype_depth(SkColorType);  // like colortype_name, but channel order agnostic
SK_API const char* tilemode_name(SkTileMode);

/**
 * Map opaque colors from 8888 to 565.
 */
SK_API SkColor color_to_565(SkColor color);

/* Return a color emoji typeface with planets to scale if available. */
SK_API sk_sp<SkTypeface> planet_typeface();

/** Return a color emoji typeface if available. */
SK_API sk_sp<SkTypeface> emoji_typeface();

/** Sample text for the emoji_typeface font. */
SK_API const char* emoji_sample_text();

/** A simple SkUserTypeface for testing. */
SK_API sk_sp<SkTypeface> sample_user_typeface();

/**
 * Returns a platform-independent text renderer.
 */
SK_API sk_sp<SkTypeface> create_portable_typeface(const char* name, SkFontStyle style);

static inline sk_sp<SkTypeface> create_portable_typeface() {
    return create_portable_typeface(nullptr, SkFontStyle());
}

SK_API void get_text_path(const SkFont&,
                   const void* text,
                   size_t      length,
                   SkTextEncoding,
                   SkPath*,
 ...
 */
SK_API bool equal_pixels(const SkPixmap&, const SkPixmap&);
SK_API bool equal_pixels(const SkBitmap&, const SkBitmap&);
SK_API bool equal_pixels(const SkImage* a, const SkImage* b);

/** Returns a newly created CheckerboardShader. */
SK_API sk_sp<SkShader> create_checkerboard_shader(SkColor c1, SkColor c2, int size);

/** Draw a checkerboard pattern in the current canvas, restricted to
    the current clip, using SkXfermode::kSrc_Mode. */
SK_API void draw_checkerboard(SkCanvas* canvas, SkColor color1, SkColor color2, int checkSize);

/** Make it easier to create a bitmap-based checkerboard */
SK_API SkBitmap create_checkerboard_bitmap(int w, int h, SkColor c1, SkColor c2, int checkSize);

SK_API sk_sp<SkImage> create_checkerboard_image(int w, int h, SkColor c1, SkColor c2, int checkSize);
...
SK_API SkBitmap create_string_bitmap(int w, int h, SkColor c, int x, int y, int textSize, const char* str);
SK_API sk_sp<SkImage> create_string_image(int w, int h, SkColor c, int x, int y, int textSize, const char* str);

// If the canvas does't make a surface (e.g. recording), make a raster surface
SK_API sk_sp<SkSurface> makeSurface(SkCanvas*, const SkImageInfo&, const SkSurfaceProps* = nullptr);

// A helper for inserting a drawtext call into a SkTextBlobBuilder
SK_API void add_to_text_blob_w_len(SkTextBlobBuilder*,
                            const char* text,
                            size_t      len,
                            SkTextEncoding,
                            const SkFont&,
...
SK_API void add_to_text_blob(SkTextBlobBuilder*, const char* text, const SkFont&, SkScalar x, SkScalar y);
...
SK_API SkPath make_star(const SkRect& bounds, int numPts = 5, int step = 2);

SK_API void create_hemi_normal_map(SkBitmap* bm, const SkIRect& dst);

SK_API void create_frustum_normal_map(SkBitmap* bm, const SkIRect& dst);

SK_API void create_tetra_normal_map(SkBitmap* bm, const SkIRect& dst);
...
SK_API bool copy_to(SkBitmap* dst, SkColorType dstCT, const SkBitmap& src);
SK_API void copy_to_g8(SkBitmap* dst, const SkBitmap& src);
...
SK_API void sniff_paths(const char filepath[], std::function<PathSniffCallback>);
