...
SK_API uint32_t GrResourceKeyHash(const uint32_t* data, size_t size);
...
public:
    ...
    protected:
        Builder(GrResourceKey* key, uint32_t domain, int data32Count) : fKey(key) {
            ...
            key->fKey[kDomainAndSize_MetaDataIdx] = domain | static_cast<uint32_t>(size << 16);
        }
...
private:
    ...
    static constexpr uint32_t kMetaDataCnt = kLastMetaDataIdx + 1;
...
public:
    ...
    SK_API static ResourceType GenerateResourceType();
...
public:
    ...
    SK_API static Domain GenerateDomain();
