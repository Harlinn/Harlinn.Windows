...
#include <skia/private/GrContext_Base.h>
#include <skia/private/GrSingleOwner.h>
...
public:
    SK_API ~GrImageContext() override;

    // Provides access to functions that aren't part of the public API.
    SK_API GrImageContextPriv priv();
    SK_API const GrImageContextPriv priv() const;  // NOLINT(readability-const-return-type)

protected:
    friend class GrImageContextPriv; // for hidden functions

    SK_API GrImageContext(sk_sp<GrContextThreadSafeProxy>);
...
private:
    ...
    SK_API static sk_sp<GrImageContext> MakeForPromiseImage(sk_sp<GrContextThreadSafeProxy>);
