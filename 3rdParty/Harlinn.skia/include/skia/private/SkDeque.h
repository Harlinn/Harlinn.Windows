...
public:
     ...
     */
    SK_API explicit SkDeque(size_t elemSize, int allocCount = 1);
    SK_API SkDeque(size_t elemSize, void* storage, size_t storageSize, int allocCount = 1);
    SK_API ~SkDeque();
     ...
     * for the new element
     */
    SK_API void* push_front();
    SK_API void* push_back();

    SK_API void pop_front();
    SK_API void pop_back();
...
public:
    ...
    public:
        ...
        SK_API Iter(const SkDeque& d, IterStart startLoc);
        SK_API void* next();
        SK_API void* prev();

        SK_API void reset(const SkDeque& d, IterStart startLoc);
...
private:
    ...
    SK_API Block*  allocateBlock(int allocCount);
    SK_API void    freeBlock(Block* block);
     ...
     */
    SK_API int  numBlocksAllocated() const;
