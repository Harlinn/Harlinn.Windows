...
class SkPathRef final : public SkNVRefCnt<SkPathRef> {
public:
    ...
    class Editor {
    public:
        SK_API Editor(sk_sp<SkPathRef>* pathRef,
    ...
    public:
        SK_API Iter();
        SK_API Iter(const SkPathRef&);

        SK_API void setPathRef(const SkPathRef&);
        ...
        */
        SK_API uint8_t next(SkPoint pts[4]);
        SK_API uint8_t peek() const;
...
public:
     ...
     */
    SK_API static SkPathRef* CreateEmpty();
    ...
    */
    ...
    SK_API SkRRect getRRect() const;

    /**
     * Transforms a path ref by a matrix, allocating a new one only if necessary.
     */
    SK_API static void CreateTransformedCopy(sk_sp<SkPathRef>* dst,
                                      const SkPathRef& src,
  ...
  //  static SkPathRef* CreateFromBuffer(SkRBuffer* buffer);
     ...
     * only if necessary.
     */
    SK_API static void Rewind(sk_sp<SkPathRef>* pathRef);
    ...
    SK_API bool operator== (const SkPathRef& ref) const;

    /**
     * Writes the path points and verbs to a buffer.
     */
    SK_API void writeToBuffer(SkWBuffer* buffer) const;

    /**
     * Gets the number of bytes that would be written in writeBuffer()
     */
    SK_API uint32_t writeSize() const;

    SK_API void interpolate(const SkPathRef& ending, SkScalar weight, SkPathRef* out) const;
    ...
    SK_API void addGenIDChangeListener(sk_sp<SkIDChangeListener>);   // Threadsafe.
    SK_API int genIDChangeListenerCount();                           // Threadsafe

    SK_API bool dataMatchesVerbs() const;
    SK_API bool isValid() const;
    SkDEBUGCODE( SK_API void validate() const { SkASSERT(this->isValid()); } )
...
private:
    ...
    SK_API void copy(const SkPathRef& ref, int additionalReserveVerbs, int additionalReservePoints);
     ...
     */
    SK_API SkPoint* growForRepeatedVerb(int /*SkPath::Verb*/ verb, int numVbs, SkScalar** weights);
     ...
     * uninitialized.
     */
    SK_API SkPoint* growForVerb(int /*SkPath::Verb*/ verb, SkScalar weight);
     ...
     * Returns pointers to the uninitialized points and conic weights data.
     */
    SK_API std::tuple<SkPoint*, SkScalar*> growForVerbsInPath(const SkPathRef& path);
     ...
     */
    SK_API friend SkPathRef* sk_create_empty_pathref();
