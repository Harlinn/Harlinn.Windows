...
#include "skia/svg/SkSVGAttribute.h"
...
public:
    SK_API ~SkSVGNode() override;

    SkSVGTag tag() const { return fTag; }

    SK_API virtual void appendChild(sk_sp<SkSVGNode>) = 0;

    SK_API void render(const SkSVGRenderContext&) const;
    SK_API bool asPaint(const SkSVGRenderContext&, SkPaint*) const;
    SK_API SkPath asPath(const SkSVGRenderContext&) const;
    SK_API SkRect objectBoundingBox(const SkSVGRenderContext&) const;

    SK_API void setAttribute(SkSVGAttribute, const SkSVGValue&);
    SK_API bool setAttribute(const char* attributeName, const char* attributeValue);

    // TODO: consolidate with existing setAttribute
    SK_API virtual bool parseAndSetAttribute(const char* name, const char* value);
...
protected:
    SK_API SkSVGNode(SkSVGTag);

    SK_API static SkMatrix ComputeViewboxMatrix(const SkRect&, const SkRect&, SkSVGPreserveAspectRatio);
    ...
    SK_API virtual bool onPrepareToRender(SkSVGRenderContext*) const;
