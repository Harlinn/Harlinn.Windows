...
public:
    SK_API static std::unique_ptr<SkShaper> MakePrimitive();
    #ifdef SK_SHAPER_HARFBUZZ_AVAILABLE
    SK_API static std::unique_ptr<SkShaper> MakeShaperDrivenWrapper(sk_sp<SkFontMgr> = nullptr);
    SK_API static std::unique_ptr<SkShaper> MakeShapeThenWrap(sk_sp<SkFontMgr> = nullptr);
    SK_API static std::unique_ptr<SkShaper> MakeShapeDontWrapOrReorder(sk_sp<SkFontMgr> = nullptr);
    SK_API static void PurgeHarfBuzzCache();
    #endif
    #ifdef SK_SHAPER_CORETEXT_AVAILABLE
    SK_API static std::unique_ptr<SkShaper> MakeCoreText();
    #endif

    SK_API static std::unique_ptr<SkShaper> Make(sk_sp<SkFontMgr> = nullptr);
    SK_API static void PurgeCaches();

    SK_API SkShaper();
    SK_API virtual ~SkShaper();
...
public:
    SK_API static std::unique_ptr<FontRunIterator>
    MakeFontMgrRunIterator(const char* utf8, size_t utf8Bytes,
                           const SkFont& font, sk_sp<SkFontMgr> fallback);
    SK_API static std::unique_ptr<SkShaper::FontRunIterator>
    MakeFontMgrRunIterator(const char* utf8, size_t utf8Bytes,
                           const SkFont& font, sk_sp<SkFontMgr> fallback,
    ...
    SK_API static std::unique_ptr<BiDiRunIterator>
    ...
    SK_API static std::unique_ptr<ScriptRunIterator>
    ...
    SK_API static std::unique_ptr<LanguageRunIterator>
...
public:
    SkTextBlobBuilderRunHandler(const char* utf8Text, SkPoint offset)
        : fUtf8Text(utf8Text)
        , fOffset(offset) {}
    SK_API sk_sp<SkTextBlob> makeBlob();
    SkPoint endPoint() { return fOffset; }

    SK_API void beginLine() override;
    SK_API void runInfo(const RunInfo&) override;
    SK_API void commitRunInfo() override;
    SK_API Buffer runBuffer(const RunInfo&) override;
    SK_API void commitRunBuffer(const RunInfo&) override;
    SK_API void commitLine() override;
