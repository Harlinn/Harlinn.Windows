...
#include "skia/particles/SkParticleData.h"
...
public:
    ...
    SK_API void visitFields(SkFieldVisitor* v);

    // Load/compute cached resources
    SK_API void prepare(const skresources::ResourceProvider*);
...
public:
    SK_API SkParticleEffect(sk_sp<SkParticleEffectParams> params);

    // Start playing this effect, specifying initial values for the emitter's properties
    SK_API void start(double now, bool looping, SkPoint position, SkVector heading, float scale,
    ...
    SK_API const SkSL::UniformInfo* uniformInfo() const;
    ...
    SK_API bool setUniform(const char* name, const float* val, int count);

    SK_API static void RegisterParticleTypes();

private:
    SK_API void setCapacity(int capacity);
    SK_API void updateStorage();

    // Helpers to break down update
    SK_API void advanceTime(double now);
