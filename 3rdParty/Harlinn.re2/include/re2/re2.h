...
class RE2 {
 public:
  ...
  HRE2_EXPORT int ProgramSize() const;
  HRE2_EXPORT int ReverseProgramSize() const;

  // If histogram is not null, outputs the program fanout
  // as a histogram bucketed by powers of 2.
  // Returns the number of the largest non-empty bucket.
  HRE2_EXPORT int ProgramFanout(std::vector<int>* histogram) const;
  HRE2_EXPORT int ReverseProgramFanout(std::vector<int>* histogram) const;
 ...
 public:
  ...
  HRE2_EXPORT bool PossibleMatchRange(std::string* min, std::string* max,
                          int maxlen) const;
  ...
  HRE2_EXPORT const std::map<std::string, int>& NamedCapturingGroups() const;

  // Return a map from capturing indices to names.
  // The map has no entries for unnamed groups.
  // Only valid until the re is deleted.
  HRE2_EXPORT const std::map<int, std::string>& CapturingGroupNames() const;
