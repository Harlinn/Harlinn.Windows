...
class IMkvWriter {
 ...
 protected:
  WEBM_EXPORT IMkvWriter();
  WEBM_EXPORT virtual ~IMkvWriter();
...
WEBM_EXPORT bool WriteEbmlHeader(IMkvWriter* writer, uint64_t doc_type_version,
...
WEBM_EXPORT bool WriteEbmlHeader(IMkvWriter* writer);

// Copies in Chunk from source to destination between the given byte positions
WEBM_EXPORT bool ChunkedCopy(mkvparser::IMkvReader* source, IMkvWriter* dst, int64_t start,
                 int64_t size);
...
class Frame {
 public:
  ...
  WEBM_EXPORT bool CopyFrom(const Frame& frame);

  // Copies |frame| data into |frame_|. Returns true on success.
  WEBM_EXPORT bool Init(const uint8_t* frame, uint64_t length);

  // Copies |additional| data into |additional_|. Returns true on success.
  WEBM_EXPORT bool AddAdditionalData(const uint8_t* additional, uint64_t length,
                         uint64_t add_id);

  // Returns true if the frame has valid parameters.
  WEBM_EXPORT bool IsValid() const;

  // Returns true if the frame can be written as a SimpleBlock based on current
  // parameters.
  WEBM_EXPORT bool CanBeSimpleBlock() const;
  ...
  WEBM_EXPORT void set_duration(uint64_t duration);
  ...
  WEBM_EXPORT void set_reference_block_timestamp(int64_t reference_block_timestamp);
...
class CuePoint {
 public:
  ...
  WEBM_EXPORT uint64_t Size() const;

  // Output the CuePoint element to the writer. Returns true on success.
  WEBM_EXPORT bool Write(IMkvWriter* writer) const;
 ...
 private:
  ...
  WEBM_EXPORT uint64_t PayloadSize() const;
...
class Cues {
 public:
  ...
  WEBM_EXPORT bool AddCue(CuePoint* cue);

  // Returns the cue point by index. Returns NULL if there is no cue point
  // match.
  WEBM_EXPORT CuePoint* GetCueByIndex(int32_t index) const;

  // Returns the total size of the Cues element
  WEBM_EXPORT uint64_t Size();
...
class ContentEncAESSettings {
 public:
  ...
  WEBM_EXPORT ContentEncAESSettings();
...
class ContentEncoding {
 public:
  ...
  WEBM_EXPORT bool SetEncryptionID(const uint8_t* id, uint64_t length);
 ...
 private:
  ...
  WEBM_EXPORT uint64_t EncodingSize(uint64_t compresion_size,
                        uint64_t encryption_size) const;

  // Returns the size in bytes for the encryption elements.
  WEBM_EXPORT uint64_t EncryptionSize() const;
...
class PrimaryChromaticity {
 public:
  ...
  WEBM_EXPORT uint64_t PrimaryChromaticitySize(libwebm::MkvId x_id,
                                   libwebm::MkvId y_id) const;
  WEBM_EXPORT bool Valid() const;
  WEBM_EXPORT bool Write(IMkvWriter* writer, libwebm::MkvId x_id,
             libwebm::MkvId y_id) const;
...
class MasteringMetadata {
 public:
  ...
  WEBM_EXPORT uint64_t MasteringMetadataSize() const;
  WEBM_EXPORT bool Valid() const;
  WEBM_EXPORT bool Write(IMkvWriter* writer) const;

  // Copies non-null chromaticity.
  WEBM_EXPORT bool SetChromaticity(const PrimaryChromaticity* r,
...
class Colour {
 public:
  ...
  WEBM_EXPORT uint64_t ColourSize() const;
  WEBM_EXPORT bool Valid() const;
  WEBM_EXPORT bool Write(IMkvWriter* writer) const;

  // Deep copies |mastering_metadata|.
  WEBM_EXPORT bool SetMasteringMetadata(const MasteringMetadata& mastering_metadata);
...
class Projection {
 public:
  ...
  WEBM_EXPORT uint64_t ProjectionSize() const;
  WEBM_EXPORT bool Write(IMkvWriter* writer) const;

  WEBM_EXPORT bool SetProjectionPrivate(const uint8_t* private_data,
...
class Track {
 public:
  ...
  WEBM_EXPORT bool SetCodecPrivate(const uint8_t* codec_private, uint64_t length);

  WEBM_EXPORT void set_codec_id(const char* codec_id);
  const char* codec_id() const { return codec_id_; }
  const uint8_t* codec_private() const { return codec_private_; }
  WEBM_EXPORT void set_language(const char* language);
  ...
  WEBM_EXPORT void set_name(const char* name);
...
class VideoTrack : public Track {
 public:
  ...
  // The |seed| parameter is used to synthesize a UID for the track.
  WEBM_EXPORT explicit VideoTrack(unsigned int* seed);
  ...
  WEBM_EXPORT bool SetStereoMode(uint64_t stereo_mode);

  // Sets the video's alpha mode. Returns true on success.
  WEBM_EXPORT bool SetAlphaMode(uint64_t alpha_mode);
  ...
  WEBM_EXPORT void set_colour_space(const char* colour_space);
  const char* colour_space() const { return colour_space_; }

  Colour* colour() { return colour_; }

  // Deep copies |colour|.
  WEBM_EXPORT bool SetColour(const Colour& colour);

  Projection* projection() { return projection_; }

  // Deep copies |projection|.
  WEBM_EXPORT bool SetProjection(const Projection& projection);

 private:
  // Returns the size in bytes of the Video element.
  WEBM_EXPORT uint64_t VideoPayloadSize() const;
...
class AudioTrack : public Track {
 public:
  // The |seed| parameter is used to synthesize a UID for the track.
  WEBM_EXPORT explicit AudioTrack(unsigned int* seed);
  WEBM_EXPORT virtual ~AudioTrack();
...
class Tracks {
 public:
  ...
  WEBM_EXPORT static const char kOpusCodecId[];
  WEBM_EXPORT static const char kVorbisCodecId[];
  WEBM_EXPORT static const char kAv1CodecId[];
  WEBM_EXPORT static const char kVp8CodecId[];
  WEBM_EXPORT static const char kVp9CodecId[];
  WEBM_EXPORT static const char kWebVttCaptionsId[];
  WEBM_EXPORT static const char kWebVttDescriptionsId[];
  WEBM_EXPORT static const char kWebVttMetadataId[];
  WEBM_EXPORT static const char kWebVttSubtitlesId[];
  ...
  WEBM_EXPORT bool AddTrack(Track* track, int32_t number);

  // Returns the track by index. Returns NULL if there is no track match.
  WEBM_EXPORT const Track* GetTrackByIndex(uint32_t idx) const;

  // Search the Tracks and return the track that matches |tn|. Returns NULL
  // if there is no track match.
  WEBM_EXPORT Track* GetTrackByNumber(uint64_t track_number) const;

  // Returns true if the track number is an audio track.
  WEBM_EXPORT bool TrackIsAudio(uint64_t track_number) const;

  // Returns true if the track number is a video track.
  WEBM_EXPORT bool TrackIsVideo(uint64_t track_number) const;
...
class Chapter {
 public:
  ...
  // MKV is pending.
  WEBM_EXPORT bool set_id(const char* id);
  ...
  WEBM_EXPORT void set_time(const Segment& segment, uint64_t start_time_ns,
  ...
  WEBM_EXPORT bool add_string(const char* title, const char* language, const char* country);
 ...
 private:
  ...
  // error.
  WEBM_EXPORT uint64_t WriteAtom(IMkvWriter* writer) const;
...
class Chapters {
 public:
  ...
  WEBM_EXPORT Chapter* AddChapter(unsigned int* seed);

  // Returns the number of chapters that have been added.
  WEBM_EXPORT int Count() const;
 ...
 private:
  ...
  WEBM_EXPORT bool ExpandChaptersArray();
  ...
  WEBM_EXPORT uint64_t WriteEdition(IMkvWriter* writer) const;
...
class Tag {
 public:
  WEBM_EXPORT bool add_simple_tag(const char* tag_name, const char* tag_string);
 ...
 private:
  ...
  class SimpleTag {
   public:
    ...
    // error.
    WEBM_EXPORT bool set_tag_name(const char* tag_name);

    // Copies the language to the |tag_string_| member.  Returns false
    // on error.
    WEBM_EXPORT bool set_tag_string(const char* tag_string);
    ...
    // success, 0 if error.
    WEBM_EXPORT uint64_t Write(IMkvWriter* writer) const;
  ...
  WEBM_EXPORT void ShallowCopy(Tag* dst) const;
...
class Cluster {
 public:
  ...
  WEBM_EXPORT Cluster(uint64_t timecode, int64_t cues_pos, uint64_t timecode_scale,
          bool write_last_frame_with_duration = false,
          bool fixed_size_timecode = false);
  WEBM_EXPORT ~Cluster();

  WEBM_EXPORT bool Init(IMkvWriter* ptr_writer);

  // Adds a frame to be output in the file. The frame is written out through
  // |writer_| if successful. Returns true on success.
  WEBM_EXPORT bool AddFrame(const Frame* frame);
  ...
  WEBM_EXPORT bool AddFrame(const uint8_t* data, uint64_t length, uint64_t track_number,
  ...
  //                 frame, expressed in timecode units.
  //   is_key:       Flag telling whether or not this frame is a key frame.
  WEBM_EXPORT bool AddFrameWithAdditional(const uint8_t* data, uint64_t length,
  ...
  //                 frame, expressed in timecode units.
  //   is_key:       Flag telling whether or not this frame is a key frame.
  WEBM_EXPORT bool AddFrameWithDiscardPadding(const uint8_t* data, uint64_t length,
  ...
  WEBM_EXPORT bool AddMetadata(const uint8_t* data, uint64_t length, uint64_t track_number,
                   uint64_t timecode, uint64_t duration);

  // Increments the size of the cluster's data in bytes.
  WEBM_EXPORT void AddPayloadSize(uint64_t size);
  ...
  WEBM_EXPORT bool Finalize(bool set_last_frame_duration, uint64_t duration);
  ...
  WEBM_EXPORT int64_t GetRelativeTimecode(int64_t abs_timecode) const;
...
class SeekHead {
 ...
 private:
  ...
  WEBM_EXPORT uint64_t MaxEntrySize() const;
...
class Segment {
 public:
  ...
  // track number.
  WEBM_EXPORT Track* AddTrack(int32_t number);
  ...
  // the track number.
  WEBM_EXPORT uint64_t AddAudioTrack(int32_t sample_rate, int32_t channels, int32_t number);
  ...
  WEBM_EXPORT Chapter* AddChapter();
  ...
  WEBM_EXPORT bool AddCuePoint(uint64_t timestamp, uint64_t track);
  ...
  // Inputs:
  //   frame: frame object
  WEBM_EXPORT bool AddGenericFrame(const Frame* frame);

  // Adds a VP8 video track to the segment. Returns the number of the track on
  // success, 0 on error. |number| is the number to use for the video track.
  // |number| must be >= 0. If |number| == 0 then the muxer will decide on
  // the track number.
  WEBM_EXPORT uint64_t AddVideoTrack(int32_t width, int32_t height, int32_t number);
  ...
  WEBM_EXPORT bool CopyAndMoveCuesBeforeClusters(mkvparser::IMkvReader* reader,
                                     IMkvWriter* writer);
  ...
  WEBM_EXPORT bool CuesTrack(uint64_t track_number);

  // This will force the muxer to create a new Cluster when the next frame is
  // added.
  WEBM_EXPORT void ForceNewClusterOnNextFrame();
  ...
  WEBM_EXPORT void OutputCues(bool output_cues);

  // Toggles whether to write the last frame in each Cluster with Duration.
  WEBM_EXPORT void AccurateClusterDuration(bool accurate_cluster_duration);

  // Toggles whether to write the Cluster Timecode using exactly 8 bytes.
  WEBM_EXPORT void UseFixedSizeClusterTimecode(bool fixed_size_cluster_timecode);
  ...
  WEBM_EXPORT bool SetChunking(bool chunking, const char* filename);
  ...
  WEBM_EXPORT bool DocTypeIsWebm() const;
 ...
 private:
  ...
  // Cues elements.
  WEBM_EXPORT bool CheckHeaderInfo();

  // Sets |doc_type_version_| based on the current element requirements.
  WEBM_EXPORT void UpdateDocTypeVersion();
  ...
  // on success.
  WEBM_EXPORT bool UpdateChunkName(const char* ext, char** name) const;
  ...
  WEBM_EXPORT int TestFrame(uint64_t track_num, uint64_t timestamp_ns, bool key) const;

  // Create a new cluster, using the earlier of the first enqueued
  // frame, or the indicated time. Returns true on success.
  WEBM_EXPORT bool MakeNewCluster(uint64_t timestamp_ns);
  ...
  WEBM_EXPORT bool DoNewClusterProcessing(uint64_t track_num, uint64_t timestamp_ns,
                              bool key);

  // Adjusts Cue Point values (to place Cues before Clusters) so that they
  // reflect the correct offsets.
  WEBM_EXPORT void MoveCuesBeforeClusters();
  ...
  WEBM_EXPORT void MoveCuesBeforeClustersHelper(uint64_t diff, int index,
                                    uint64_t* cue_size);
