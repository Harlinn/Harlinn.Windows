...
class LineReader : protected Reader {
 public:
  ...
  WEBM_EXPORT int GetLine(std::string* line);

 protected:
  WEBM_EXPORT virtual ~LineReader();
...
struct Time {
  ...
  WEBM_EXPORT bool operator==(const Time& rhs) const;
  WEBM_EXPORT bool operator<(const Time& rhs) const;
  WEBM_EXPORT bool operator>(const Time& rhs) const;
  WEBM_EXPORT bool operator<=(const Time& rhs) const;
  WEBM_EXPORT bool operator>=(const Time& rhs) const;

  WEBM_EXPORT presentation_t presentation() const;
  WEBM_EXPORT Time& presentation(presentation_t);

  WEBM_EXPORT Time& operator+=(presentation_t);
  WEBM_EXPORT Time operator+(presentation_t) const;

  WEBM_EXPORT Time& operator-=(presentation_t);
  WEBM_EXPORT presentation_t operator-(const Time&) const;
};
...
class Parser : private LineReader {
 ...
 private:
  ...
  //
  WEBM_EXPORT static int ParseTimingsLine(std::string* line,
  ...
  //
  WEBM_EXPORT static int ParseTime(const std::string& line, std::string::size_type* off,
                       Time* time);

  // Parse the cue settings from the timings line, starting at the
  // given offset.  Returns negative if error, 0 on success.
  //
  WEBM_EXPORT static int ParseSettings(const std::string& line, std::string::size_type off,
                           Cue::settings_t* settings);
  ...
  //
  WEBM_EXPORT static int ParseNumber(const std::string& line, std::string::size_type* off);
