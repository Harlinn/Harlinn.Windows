...
WEBM_EXPORT std::string GetTempFileName();

// Returns size of file specified by |file_name|, or 0 upon failure.
WEBM_EXPORT uint64_t GetFileSize(const std::string& file_name);

// Gets the contents file_name as a string. Returns false on error.
WEBM_EXPORT bool GetFileContents(const std::string& file_name, std::string* contents);

// Manages life of temporary file specified at time of construction. Deletes
// file upon destruction.
class TempFileDeleter {
 public:
  WEBM_EXPORT TempFileDeleter();
