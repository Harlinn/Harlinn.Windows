...
// the converted value.
WEBM_EXPORT std::int64_t NanosecondsTo90KhzTicks(std::int64_t nanoseconds);
WEBM_EXPORT std::int64_t Khz90TicksToNanoseconds(std::int64_t khz90_ticks);
...
WEBM_EXPORT bool ParseVP9SuperFrameIndex(const std::uint8_t* frame,
                             std::size_t frame_length, Ranges* frame_ranges,
                             bool* error);

// Writes |val| to |fileptr| and returns true upon success.
WEBM_EXPORT bool WriteUint8(std::uint8_t val, std::FILE* fileptr);

// Reads 2 bytes from |buf| and returns them as a uint16_t. Returns 0 when |buf|
// is a nullptr.
WEBM_EXPORT std::uint16_t ReadUint16(const std::uint8_t* buf);
