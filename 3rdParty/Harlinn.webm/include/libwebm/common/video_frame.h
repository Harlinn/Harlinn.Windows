...
class VideoFrame {
 public:
  ...
  struct Buffer {
    ...
    WEBM_EXPORT bool Init(std::size_t new_length);
  ...
  VideoFrame(bool keyframe, std::int64_t pts_in_nanoseconds, Codec vpx_codec)
      : keyframe_(keyframe),
        ...
        codec_(vpx_codec) {}
  WEBM_EXPORT bool Init(std::size_t length);
  WEBM_EXPORT bool Init(std::size_t length, std::int64_t nano_pts, Codec codec);
  ...
  WEBM_EXPORT bool SetBufferLength(std::size_t length);
