...
class VpxPesParser {
 public:
  ...
  struct BcmvHeader {
    ...
    // after parsing input.
    WEBM_EXPORT explicit BcmvHeader(std::uint32_t len);

    WEBM_EXPORT bool operator==(const BcmvHeader& other) const;
  ...
  WEBM_EXPORT bool Open(const std::string& pes_file_path);
  ...
  WEBM_EXPORT bool ParseNextPacket(PesHeader* header, VideoFrame* frame);
  ...
  //     0     0      1     X           Y
  WEBM_EXPORT bool VerifyPacketStartCode() const;
  WEBM_EXPORT bool ReadStreamId(std::uint8_t* stream_id) const;
  WEBM_EXPORT bool ReadPacketLength(std::uint16_t* packet_length) const;
 ...
 private:
  ...
  WEBM_EXPORT bool FindStartCode(std::size_t origin, std::size_t* offset) const;
  ...
  WEBM_EXPORT bool IsPayloadFragmented(const PesHeader& header) const;
  ...
  WEBM_EXPORT bool AccumulateFragmentedPayload(std::size_t pes_packet_length,
  ...
  WEBM_EXPORT bool RemoveStartCodeEmulationPreventionBytes(
