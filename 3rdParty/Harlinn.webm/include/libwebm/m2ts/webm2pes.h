...
struct PesOptionalHeader {
  ...
  WEBM_EXPORT void SetPtsBits(std::int64_t pts_90khz);

  // Writes fields to |buffer| and returns true. Returns false when write or
  // field value validation fails.
  WEBM_EXPORT bool Write(bool write_pts, PacketDataBuffer* buffer) const;
};
...
struct BCMVHeader {
  ...
  WEBM_EXPORT bool Write(PacketDataBuffer* buffer) const;
...
class Webm2Pes {
 public:
  ...
  // to report failure.
  WEBM_EXPORT bool ConvertToFile();
  ...
  // an error occurs.
  WEBM_EXPORT bool ConvertToPacketReceiver();

  // Writes |vpx_frame| out as PES packet[s] and stores output in |packet_data|.
  // Returns true for success, false for failure.
  WEBM_EXPORT static bool WritePesPacket(const VideoFrame& frame,
                             PacketDataBuffer* packet_data);

  uint64_t bytes_written() const { return bytes_written_; }

 private:
  WEBM_EXPORT bool InitWebmParser();
  WEBM_EXPORT bool ReadVideoFrame(const mkvparser::Block::Frame& mkvparser_frame,
                      VideoFrame* frame);
...
WEBM_EXPORT bool CopyAndEscapeStartCodes(const std::uint8_t* raw_input,
