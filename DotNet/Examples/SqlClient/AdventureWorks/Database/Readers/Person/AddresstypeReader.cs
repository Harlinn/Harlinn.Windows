...
public class AddresstypeReader : IDisposable
{
    ...
    public Types.Person.AddresstypeDataType ToDataObject()
    {
        return new Types.Person.AddresstypeDataType(Addresstypeid,
            Name,
            Rowguid,
            Modifieddate);
    }

    public List<Types.Person.AddresstypeDataType> ToList()
    {
        var list = new List<Types.Person.AddresstypeDataType>();
        while (Read())
        {
