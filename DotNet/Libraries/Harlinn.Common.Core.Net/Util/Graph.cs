...
namespace Harlinn.Common.Core.Net.Util
{
    // Do topological sort like Entity Framework Core, using
    // a slightly modified version that does not pull in
    ...
    public class Graph<TVertex, TEdge> where TVertex : notnull
    {
        ...
        public Graph(IComparer<TVertex> secondarySortComparer)
        {
            _secondarySortComparer = secondarySortComparer;
        }

        public Graph(Comparison<TVertex> secondarySortComparer)
        ...
        public IEnumerable<TVertex> GetOutgoingNeighbors(TVertex from) => _successorMap.TryGetValue(from, out var successorSet) ? successorSet.Keys : Enumerable.Empty<TVertex>();

        public IEnumerable<TVertex> GetIncomingNeighbors(TVertex to) => _predecessorMap.TryGetValue(to, out var predecessors) ? predecessors.Keys : Enumerable.Empty<TVertex>();
            ...
            Func<string, string>? formatException = null)
        {
            ...
            Assert(batches.Count < 2, "TopologicalSortCore did batching but withBatching was false");
                ...
                if (vertexesProcessed < _vertices.Count)
                {
                           ...
                           && canBreakEdges != null)
                    {
                        ...
                        if (canBreakEdges(incomingNeighbor, candidateVertex, GetEdges(incomingNeighbor, candidateVertex)))
                        {
                            ...
                            Assert(removed, "Candidate vertex not found in successor map");
                            removed = _predecessorMap[candidateVertex].Remove(incomingNeighbor);
                            Assert(removed, "Incoming neighbor not found in predecessor map");
