...
{
    public class ComputedColumnsReader : DataReaderWrapper
    {
        ...
        public const int DefinitionOrdinal = 36;
        public const int UsesDatabaseCollationOrdinal = 37;
        public const int IsPersistedOrdinal = 38;
        public const int TypeNameOrdinal = 39;

        public ComputedColumnsReader([DisallowNull] ILoggerFactory loggerFactory, [DisallowNull] SqlDataReader sqlDataReader, bool ownsReader = true)
            : base(loggerFactory, sqlDataReader, ownsReader)
        {
        }

        public ComputedColumnsReader([DisallowNull] SqlDataReader sqlDataReader, bool ownsReader = true)
            : base(sqlDataReader, ownsReader)
        {
        }

        public ComputedColumnsReader([DisallowNull] ILogger logger, [DisallowNull] SqlDataReader sqlDataReader, bool ownsReader = true)
        ...
        public Types.ComputedColumn GetComputedColumn()
        {
            ...
            return new Types.ComputedColumn(objectId, name, columnId, systemTypeId, userTypeId, maxLength, precision, scale, collationName, isNullable, isAnsiPadded, isRowguidcol, isIdentity, isComputed, isFilestream, isReplicated, isNonSqlSubscribed, isMergePublished, isDtsReplicated, isXmlDocument, xmlCollectionId, defaultObjectId, ruleObjectId, isSparse, isColumnSet, generatedAlwaysType, generatedAlwaysTypeDesc, encryptionType, encryptionTypeDesc, encryptionAlgorithmName, columnEncryptionKeyId, columnEncryptionKeyDatabaseName, isHidden, isMasked, graphType, graphTypeDesc, definition, usesDatabaseCollation, isPersisted, typeName);
        }

        public IReadOnlyList<Types.ComputedColumn> GetComputedColumns()
        {
            var columns = new List<Types.ComputedColumn>();
            while (Read())
            {
                columns.Add(GetComputedColumn());
            }
            return columns;
        }
    }
