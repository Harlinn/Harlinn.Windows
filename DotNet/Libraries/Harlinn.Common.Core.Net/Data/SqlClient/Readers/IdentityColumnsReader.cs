...
{
    public class IdentityColumnsReader : DataReaderWrapper
    {
        ...
        public const int SeedValueOrdinal = 36;
        public const int IncrementValueOrdinal = 37;
        public const int LastValueOrdinal = 38;
        public const int IsNotForReplicationOrdinal = 39;
        public const int TypeNameOrdinal = 40;

        public IdentityColumnsReader([DisallowNull] ILoggerFactory loggerFactory, [DisallowNull] SqlDataReader sqlDataReader, bool ownsReader = true)
            : base(loggerFactory, sqlDataReader, ownsReader)
        {
        }

        public IdentityColumnsReader([DisallowNull] SqlDataReader sqlDataReader, bool ownsReader = true)
            : base(sqlDataReader, ownsReader)
        {
        }

        public IdentityColumnsReader([DisallowNull] ILogger logger, [DisallowNull] SqlDataReader sqlDataReader, bool ownsReader = true)
        ...
        {
            return base.GetValue(SeedValueOrdinal);
        }

        public object GetIncrementValue()
        {
            return base.GetValue(IncrementValueOrdinal);
        }

        public object? GetLastValue()
        {
            if (base.IsDBNull(LastValueOrdinal))
            {
                return null!;
            }
            return base.GetValue(LastValueOrdinal);
        }
        ...
        public Types.IdentityColumn GetIdentityColumn()
        {
            ...
            return new Types.IdentityColumn(objectId, name, columnId, systemTypeId, userTypeId, maxLength, precision, scale, collationName, isNullable, isAnsiPadded, isRowguidcol, isIdentity, isComputed, isFilestream, isReplicated, isNonSqlSubscribed, isMergePublished, isDtsReplicated, isXmlDocument, xmlCollectionId, defaultObjectId, ruleObjectId, isSparse, isColumnSet, generatedAlwaysType, generatedAlwaysTypeDesc, encryptionType, encryptionTypeDesc, encryptionAlgorithmName, columnEncryptionKeyId, columnEncryptionKeyDatabaseName, isHidden, isMasked, graphType, graphTypeDesc, seedValue, incrementValue, lastValue, isNotForReplication, typeName);
        }

        public IReadOnlyList<Types.IdentityColumn> GetIdentityColumns()
        {
            var columns = new List<Types.IdentityColumn>();
            while (Read())
            {
                columns.Add(GetIdentityColumn());
            }
            return columns;
        }
    }
