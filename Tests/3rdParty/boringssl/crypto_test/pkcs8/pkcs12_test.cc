...
static const char8_t kUnicodePassword[] = u8"Hello, 世界";
...
TEST(PKCS12Test, TestUnicode) {
  ...
  TestImpl("Unicode", StringToBytes(data), reinterpret_cast<const char *>( kUnicodePassword ), nullptr);
}
...
TEST(PKCS12Test, RoundTrip) {
  ...
  // Test some Unicode.
  const auto* pw1 = u8"Hello, 世界!";
  TestRoundTrip(kPassword, reinterpret_cast<const char*>( pw1 ),
                bssl::Span<const uint8_t>(kTestKey),
                bssl::Span<const uint8_t>(kTestCert),
                {bssl::Span<const uint8_t>(kTestCert2)}, 0, 0, 0, 0);
  TestRoundTrip( reinterpret_cast<const char*>( kUnicodePassword ), nullptr /* no name */,
                bssl::Span<const uint8_t>(kTestKey),
                bssl::Span<const uint8_t>(kTestCert),
                {bssl::Span<const uint8_t>(kTestCert2)}, 0, 0, 0, 0);
