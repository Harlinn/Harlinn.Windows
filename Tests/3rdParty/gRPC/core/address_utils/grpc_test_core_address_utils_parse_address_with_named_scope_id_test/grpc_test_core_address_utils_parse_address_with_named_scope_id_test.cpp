...
struct sockaddr_in6 resolve_with_gettaddrinfo( const char* uri_text )
{
    ...
    struct sockaddr_in6 out = *reinterpret_cast<struct sockaddr_in6*>( result->ai_addr );
    // Cleanup
    freeaddrinfo( result );
    return out;
}
...
int main( int argc, char** argv )
{
    ...
    struct sockaddr_in6 result_from_getaddrinfo = resolve_with_gettaddrinfo( target.c_str( ) );
    // Run the test
    gpr_log( GPR_DEBUG,
        "Run test_grpc_parse_ipv6_parity_with_getaddrinfo with target: %s", target.c_str( ) );
