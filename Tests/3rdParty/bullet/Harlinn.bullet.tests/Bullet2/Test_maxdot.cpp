...
int Test_maxdot(void)
{
	...
	for (size = 1; size <= MAX_SIZE; size++)
	{
		...
		for (position = 0; position < size; position++)
		{
			...
			if (test < 0 || test >= size)
			{
				vlog("Error @ %zu: index out of bounds! *%ld vs %ld \n", size, correct, test);
				continue;
			}
			if (correct != test)
			{
				vlog("Error @ %zu: index misreported! *%ld vs %ld  (*%f, %f)\n", size, correct, test,
...
BOOST_FIXTURE_TEST_SUITE( TestSuite_maxdot, LocalFixture )

// --run_test=TestSuite_maxdot/RunTest
BOOST_AUTO_TEST_CASE( RunTest )
{
	auto result = Test_maxdot( );
