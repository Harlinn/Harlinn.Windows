...
static void process( FILE* fid )
{
    ...
    for ( ;;)
    {
        ...
        {    /* x-y or decimal degree ascii output, scale if warranted by output units */
            if ( inverse )
            {
...
static void vprocess( FILE* fid )
{
    ...
    for ( ;;)
    {
        ...
        if ( linvers )
        {
            ...
            if ( prescale )
            {
                dat_xy.x *= fscale; dat_xy.y *= fscale;
            }
            if ( reversein )
            {
                PJ_XY temp = dat_xy;
